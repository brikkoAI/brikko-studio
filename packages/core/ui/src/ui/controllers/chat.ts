...
const SYNTHETIC_TRANSCRIPT_REPAIR_RESULT =
  "[brikko-studio] missing tool result in session history; inserted synthetic error result for transcript repair.";
...
function hasTranscriptMeta(message: unknown): boolean {
  return Boolean(
    ...
    (message as { __brikko-studio?: unknown }).__brikko-studio &&
    typeof (message as { __brikko-studio?: unknown }).__brikko-studio === "object",
  );
}
...
export function handleChatEvent(state: ChatState, payload?: ChatEventPayload) {
  ...
  // See https://github.com/brikko-studio/brikko-studio/issues/1909
