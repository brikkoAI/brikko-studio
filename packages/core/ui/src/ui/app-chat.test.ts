...
describe("refreshChatAvatar", () => {
  ...
  it("prefers the paired device token for avatar metadata and local avatar URLs", async () => {
    ...
    const fetchMock = vi.fn((input: string | URL | Request) => {
      const url = requestUrl(input);
      if (url === "/brikko-studio/avatar/main?meta=1") {
        return Promise.resolve({
          ok: true,
          json: async () => ({ avatarUrl: "/avatar/main" }),
        });
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/brikko-studio/avatar/main?meta=1",
  ...
  it("keeps mounted dashboard avatar endpoints under the normalized base path", async () => {
    ...
    const host = makeHost({ basePath: "/brikko-studio/", sessionKey: "agent:ops:main" });
    await refreshChatAvatar(host);

    expect(fetchMock).toHaveBeenCalledWith(
      "/brikko-studio/avatar/ops?meta=1",
      expect.objectContaining({ method: "GET" }),
    );
