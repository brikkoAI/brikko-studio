...
function resolveRequiredHomeDir(
  ...
  const explicitHome = normalizeHomeValue(env.BRIKKO_STUDIO_HOME);
...
export function resolveStateDir(
  ...
  if (env.BRIKKO_STUDIO_TEST_FAST === "1" || fs.existsSync(nextDir)) {
    return nextDir;
  }
...
function resolveDefaultAgentWorkspaceDir(env: NodeJS.ProcessEnv = process.env): string {
  ...
  const profile = env.BRIKKO_STUDIO_PROFILE?.trim();
  if (profile && normalizeLowercaseStringOrEmpty(profile) !== "default") {
    return path.join(home, ".brikko-studio", `workspace-${profile}`);
  }
