...
<title>Brikko Studio Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.brikko-studioCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.brikko-studioCanvasA2UIAction &&
        typeof window.brikko-studioCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.brikko-studioSendUserAction === "function";
  ...
  window.addEventListener("brikko-studio:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Brikko Studio node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.brikko-studioSendUserAction === "function"
        ? window.brikko-studioSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.BRIKKO_STUDIO_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.BRIKKO_STUDIO_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Brikko Studio Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
