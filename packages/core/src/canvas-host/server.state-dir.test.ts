...
describe("canvas host state dir defaults", () => {
  ...
  it("uses BRIKKO_STUDIO_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("brikko-studio-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Brikko Studio Canvas");
      } finally {
        await handler.close();
      }
    });
