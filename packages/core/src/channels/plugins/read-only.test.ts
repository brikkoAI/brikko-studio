...
vi.mock("../../plugins/plugin-module-loader-cache.js", async (importOriginal) => {
  ...
  function listCandidatePluginDirs(params: LoaderParams): string[] {
    ...
    const workspaceExtensionsDir = params.workspaceDir
      ? path.join(params.workspaceDir, ".brikko-studio", "extensions")
      : undefined;
  ...
  function loadBrikko StudioPlugins(params: LoaderParams) {
    ...
    const channelSetups = listCandidatePluginDirs(params).flatMap((pluginDir) => {
      ...
      const brikko-studio = isRecord(packageJson) ? packageJson.brikko-studio : undefined;
      const setupEntry = isRecord(brikko-studio) ? brikko-studio.setupEntry : undefined;
...
function writeExternalSetupChannelPlugin(
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: `@example/brikko-studio-${pluginId}`,
        version: "1.0.0",
        brikko-studio: {
...
function writeBundledSetupChannelPlugin(
  ...
  process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR = bundledRoot;
...
describe("listReadOnlyChannelPluginsForConfig", () => {
  it("keeps built plugin loader candidates inside the installed package dist root", () => {
    const packageRoot = path.join(makeTempDir(), "node_modules", "brikko-studio");
  ...
  it("discovers trusted external channel plugins from the default agent workspace", () => {
    ...
    const pluginDir = path.join(workspaceDir, ".brikko-studio", "extensions", "external-chat-plugin");
    fs.mkdirSync(pluginDir, { recursive: true });
