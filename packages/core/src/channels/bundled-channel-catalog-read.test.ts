...
const originalTrustBundledPluginsDir = process.env.BRIKKO_STUDIO_TEST_TRUST_BUNDLED_PLUGINS_DIR;
...
afterEach(() => {
  ...
  if (originalTrustBundledPluginsDir === undefined) {
    ...
    process.env.BRIKKO_STUDIO_TEST_TRUST_BUNDLED_PLUGINS_DIR = originalTrustBundledPluginsDir;
  }
...
function useBundledPluginsDir(extensionsRoot: string | undefined): void {
  if (extensionsRoot) {
    process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR = extensionsRoot;
    process.env.BRIKKO_STUDIO_TEST_TRUST_BUNDLED_PLUGINS_DIR = "1";
  } else {
    delete process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR;
  }
...
function seedRoot(prefix: string): string {
  ...
  writeJsonFile(path.join(root, "package.json"), { name: "brikko-studio" });
...
function seedChannelPkg(
  ...
  writeJsonFile(path.join(pluginDir, "brikko-studio.plugin.json"), {
    id: opts.id,
...
describe("listBundledChannelCatalogEntries", () => {
  ...
  it("falls back to dist/channel-catalog.json when the resolver returns undefined", () => {
    // BRIKKO_STUDIO_DISABLE_BUNDLED_PLUGINS, missing bundled tree, or an unresolvable
  ...
  it("falls back to dist/channel-catalog.json when the resolved dir has no plugin package.jsons", () => {
    // A stale staged dir or an BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR override pointing at
