...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when BRIKKO_STUDIO_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".brikko-studio", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when BRIKKO_STUDIO_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", BRIKKO_STUDIO_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".brikko-studio-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers BRIKKO_STUDIO_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        BRIKKO_STUDIO_PROFILE: "rescue",
        BRIKKO_STUDIO_STATE_DIR: "C:\\State\\brikko-studio",
      },
      expected: path.join("C:\\State\\brikko-studio", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", BRIKKO_STUDIO_PROFILE: "default" },
      expected: path.join("/home/test", ".brikko-studio", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\brikko-studio",
          environment: {
            NODE_ENV: "production",
            BRIKKO_STUDIO_PORT: "18789",
          },
          environmentValueSources: {
            NODE_ENV: "inline",
            BRIKKO_STUDIO_PORT: "inline",
          },
          sourcePath: resolveTaskScriptPath(env),
        });
      },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\brikko-studio\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\brikko-studio\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from BRIKKO_STUDIO_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ BRIKKO_STUDIO_STATE_DIR: path.join(tmpDir, "custom-state") }),
