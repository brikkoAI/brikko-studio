...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds BRIKKO_STUDIO_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `brikko-studio gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service BRIKKO_STUDIO_GATEWAY_TOKEN does not match gateway.auth.token in brikko-studio.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.BRIKKO_STUDIO_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.BRIKKO_STUDIO_GATEWAY_TOKEN);
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `brikko-studio gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
