...
describe("resolveGatewayService", () => {
  ...
  it("guards mutating service adapters when config was written by a newer Brikko Studio", async () => {
    ...
    const envSnapshot = captureEnv(["HOME", "BRIKKO_STUDIO_STATE_DIR", "BRIKKO_STUDIO_CONFIG_PATH"]);
    try {
...
describe("readGatewayServiceState", () => {
  it("tracks installed, loaded, and running separately", async () => {
    ...
    const state = await readGatewayServiceState(service, {
      env: { BRIKKO_STUDIO_GATEWAY_PORT: "1" },
    });

    expect(state.installed).toBe(true);
    expect(state.loaded).toBe(true);
    expect(state.running).toBe(true);
    expect(state.env.BRIKKO_STUDIO_GATEWAY_PORT).toBe("18789");
  });
});
...
describe("startGatewayService", () => {
  ...
  it("requests repair before start when the loaded service points at temporary install paths", async () => {
    const service = createService({
      readCommand: vi.fn(async () => ({
        programArguments: [
          "/private/tmp/brikko-studio-ai-install-cli-pr118/tools/node/bin/node",
          "/tmp/brikko-studio-ai-install-cli-pr118/lib/node_modules/brikko-studio/dist/index.js",
          "gateway",
        ],
        environment: {},
      })),
