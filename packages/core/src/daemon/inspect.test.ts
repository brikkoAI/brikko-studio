...
[Service]
ExecStart=/usr/bin/node /home/brikko-studio/.npm-global/lib/node_modules/brikko-studio/dist/entry.js gateway --port 18789
Restart=always
Environment=BRIKKO_STUDIO_SERVICE_MARKER=brikko-studio
Environment=BRIKKO_STUDIO_SERVICE_KIND=gateway
Environment=BRIKKO_STUDIO_SERVICE_VERSION=2026.3.8
...
[Service]
ExecStart=/usr/bin/node /opt/brikko-studio-worker/dist/index.js worker
`;
...
describe("detectMarkerLineWithGateway", () => {
  it("returns null for brikko-studio-test.service (brikko-studio only in description, no gateway on same line)", () => {
    expect(detectMarkerLineWithGateway(TEST_SERVICE_CONTENTS)).toBeNull();
  });

  it("returns brikko-studio for the canonical gateway unit (ExecStart has both brikko-studio and gateway)", () => {
    expect(detectMarkerLineWithGateway(GATEWAY_SERVICE_CONTENTS)).toBe("brikko-studio");
  });
  ...
  it("handles line continuations — marker and gateway split across physical lines", () => {
    const contents = `[Service]\nExecStart=/usr/bin/node /opt/brikko-studio/dist/entry.js \\\n  gateway --port 18789\n`;
    expect(detectMarkerLineWithGateway(contents)).toBe("brikko-studio");
  });
  ...
  it("ignores non-gateway ExecStart commands that only pass gateway-named options", () => {
    const contents = `[Service]\nExecStart=/usr/bin/brikko-studio-helper --gateway-url http://127.0.0.1:18789 sync\n`;
...
describe("findExtraGatewayServices (linux / scanSystemdDir) — real filesystem", () => {
  ...
  it.skipIf(!isLinux)("does not report brikko-studio-test.service as a gateway service", async () => {
    const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-test-"));
    const systemdDir = path.join(tmpHome, ".config", "systemd", "user");
    try {
      await fs.mkdir(systemdDir, { recursive: true });
      await fs.writeFile(path.join(systemdDir, "brikko-studio-test.service"), TEST_SERVICE_CONTENTS);
  ...
  it.skipIf(!isLinux)(
    "does not report the canonical brikko-studio-gateway.service as an extra service",
    async () => {
      const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-test-"));
      const systemdDir = path.join(tmpHome, ".config", "systemd", "user");
      try {
        await fs.mkdir(systemdDir, { recursive: true });
        await fs.writeFile(
          path.join(systemdDir, "brikko-studio-gateway.service"),
  ...
  it.skipIf(!isLinux)(
    ...
    async () => {
      ...
      try {
        await fs.mkdir(systemdDir, { recursive: true });
        await fs.writeFile(
          path.join(systemdDir, "brikko-studio-companion.service"),
  ...
  it.skipIf(!isLinux)(
    "reports custom-named gateway units that execute brikko-studio gateway",
    async () => {
      const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-test-"));
      const systemdDir = path.join(tmpHome, ".config", "systemd", "user");
      const unitPath = path.join(systemdDir, "custom-brikko-studio.service");
      try {
        await fs.mkdir(systemdDir, { recursive: true });
        await fs.writeFile(unitPath, CUSTOM_BRIKKO_STUDIO_GATEWAY_CONTENTS);
        const result = await findExtraGatewayServices({ HOME: tmpHome });
        expect(result).toEqual([
          {
            platform: "linux",
            label: "custom-brikko-studio.service",
            detail: `unit: ${unitPath}`,
            scope: "user",
...
<key>KeepAlive</key><dict><key>OtherJobEnabled</key><dict><key>ai.brikko-studio.gateway</key><true/></dict></dict>
<key>ProgramArguments</key><array><string>/usr/local/bin/brikko-studio-helper</string><string>sync</string></array>
</dict></plist>`,
      );
      const result = await findExtraGatewayServices({ HOME: tmpHome });
      expect(result).toEqual([]);
...
<key>ProgramArguments</key><array><string>/usr/local/bin/brikko-studio-helper</string><string>--gateway-url</string><string>http://127.0.0.1:18789</string><string>sync</string></array>
</dict></plist>`,
      );
      const result = await findExtraGatewayServices({ HOME: tmpHome });
      expect(result).toEqual([]);
  ...
  it("reports custom LaunchAgents that execute brikko-studio gateway", async () => {
    const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-test-"));
    const launchdDir = path.join(tmpHome, "Library", "LaunchAgents");
    const plistPath = path.join(launchdDir, "com.example.brikko-studio-gateway.plist");
    try {
      await fs.mkdir(launchdDir, { recursive: true });
      await fs.writeFile(
        plistPath,
        `<?xml version="1.0" encoding="UTF-8"?>
<plist version="1.0"><dict>
<key>Label</key><string>com.example.brikko-studio-gateway</string>
<key>ProgramArguments</key><array><string>/usr/local/bin/brikko-studio</string><string>gateway</string><string>--port</string><string>18888</string></array>
</dict></plist>`,
      );
      const result = await findExtraGatewayServices({ HOME: tmpHome });
      expect(result).toEqual([
        {
          platform: "darwin",
          label: "com.example.brikko-studio-gateway",
...
describe("findExtraGatewayServices (win32)", () => {
  ...
  it("collects only non-brikko-studio marker tasks from schtasks output", async () => {
    execSchtasksMock.mockResolvedValueOnce({
      code: 0,
      stdout: [
