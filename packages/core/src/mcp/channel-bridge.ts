...
export class Brikko StudioChannelBridge {
  ...
  constructor(
    private readonly cfg: Brikko StudioConfig,
  ...
  async start(): Promise<void> {
    ...
    this.gateway = new GatewayClientCtor({
      ...
      clientDisplayName: "Brikko Studio MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`brikko-studio mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `brikko-studio mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
