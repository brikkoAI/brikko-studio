...
export const BRIKKO_STUDIO_NODE_OPTIONS_READY = "BRIKKO_STUDIO_NODE_OPTIONS_READY";
export const BRIKKO_STUDIO_NODE_EXTRA_CA_CERTS_READY = "BRIKKO_STUDIO_NODE_EXTRA_CA_CERTS_READY";
...
export function buildCliRespawnPlan(
  ...
  if (shouldSkipRespawnForArgv(argv) || isTruthyEnvValue(env.BRIKKO_STUDIO_NO_RESPAWN)) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[BRIKKO_STUDIO_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[BRIKKO_STUDIO_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !isTruthyEnvValue(env[BRIKKO_STUDIO_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[BRIKKO_STUDIO_NODE_OPTIONS_READY] = "1";
