...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    ...
    if (result.success) {
      expect(result.data.$schema).toBe("https://brikko-studio.ai/config.json");
    }
  });
  ...
  it("preserves $schema through validateConfigObject round-trip", () => {
    ...
    if (res.ok) {
      expect(res.config.$schema).toBe("https://brikko-studio.ai/config.json");
    }
  });
});
...
describe("config identity/materialization regressions", () => {
  it("keeps explicit responsePrefix and group mention patterns", () => {
    ...
    if (res.ok) {
      ...
      expect(res.config.agents?.list?.[0]?.groupChat?.mentionPatterns).toEqual(["@brikko-studio"]);
    }
  });
...
describe("config strict validation", () => {
  ...
  it("rejects resolved-only gateway.bind aliases as invalid schema values, not legacy", async () => {
    await withTempHome(async (home) => {
      ...
      const prev = process.env.BRIKKO_STUDIO_BIND;
      process.env.BRIKKO_STUDIO_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.BRIKKO_STUDIO_BIND;
        } else {
          process.env.BRIKKO_STUDIO_BIND = prev;
        }
      }
    });
  });
