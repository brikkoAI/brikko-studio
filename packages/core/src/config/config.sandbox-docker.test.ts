...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("brikko-studio-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "brikko-studio-sandbox-browser-global" },
      agentBrowser: { network: "brikko-studio-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("brikko-studio-sandbox-browser-agent");
  });
