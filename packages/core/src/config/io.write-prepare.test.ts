...
describe("config io write prepare", () => {
      ...
      resolvePersistCandidateForWrite({
        ...
        sourceConfig: {
          plugins: {
            entries: {},
            installs: {
              "brikko-studio-web-search": {
                source: "npm",
                spec: "@ollama/brikko-studio-web-search",
                installPath: "/tmp/brikko-studio-web-search",
                resolvedName: "@ollama/brikko-studio-web-search",
        ...
        nextConfig: {
          plugins: {
            ...
            installs: {
              "brikko-studio-web-search": {
                source: "npm",
                spec: "@ollama/brikko-studio-web-search@0.2.2",
                installPath: "/tmp/brikko-studio-web-search",
                resolvedName: "@ollama/brikko-studio-web-search",
                resolvedVersion: "0.2.2",
              },
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("brikko-studio config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('brikko-studio config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/brikko-studio.json",
        expectedConfigPath: "/tmp/brikko-studio.json",
  ...
  it("rejects writes that would flatten a root include", () => {
    const sourceConfig = {
      $schema: "https://brikko-studio.ai/config-from-include.json",
      gateway: { mode: "local" },
    };
