  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.brikko-studio/logs/brikko-studio.log" },
        hooks: {
          path: "~/.brikko-studio/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.brikko-studio/telegram.token",
              },
            },
          },
          whatsapp: {
            accounts: {
              personal: {
                authDir: "~/.brikko-studio/credentials/wa-personal",
              },
            },
          },
          imessage: {
          ...
          list: [
            {
              id: "main",
              workspace: "~/ws-agent",
              agentDir: "~/.brikko-studio/agents/main",
              identity: {
                name: "~not-a-path",
              },
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".brikko-studio", "logs", "brikko-studio.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".brikko-studio", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".brikko-studio", "telegram.token"),
      );
      expect(cfg.channels?.whatsapp?.accounts?.personal?.authDir).toBe(
        path.join(home, ".brikko-studio", "credentials", "wa-personal"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".brikko-studio", "agents", "main"));
