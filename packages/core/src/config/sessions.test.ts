...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ BRIKKO_STUDIO_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-brikko-studio",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-brikko-studio#general");
  });
  ...
  it("derives session transcripts dir from BRIKKO_STUDIO_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { BRIKKO_STUDIO_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when BRIKKO_STUDIO_STATE_DIR differs from stored paths", () => {
