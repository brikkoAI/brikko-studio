...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "brikko-studio-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  const createFastConfigIO = (home: string) =>
    createConfigIO({
      env: { BRIKKO_STUDIO_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      homedir: () => home,
      logger: silentLogger,
    });
  ...
  it("logs health-state write failures through public config reads", async () => {
    await withSuiteHome(async (home) => {
      ...
      const healthPath = path.join(home, ".brikko-studio", "logs", "config-health.json");
  ...
  it("migrates shipped plugin install config records into the plugin index", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".brikko-studio", "brikko-studio.json");
      const pluginDir = path.join(home, ".brikko-studio", "plugins", "demo");
      const manifestPath = path.join(pluginDir, "brikko-studio.plugin.json");
      ...
      try {
        ...
        await expect(
          readPersistedInstalledPluginIndex({
            stateDir: path.join(home, ".brikko-studio"),
          }),
  ...
  it("migrates shipped plugin install config records even when the manifest is missing", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".brikko-studio", "brikko-studio.json");
      const pluginDir = path.join(home, ".brikko-studio", "plugins", "missing");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
  ...
  it("keeps shipped plugin install config records when index migration fails", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".brikko-studio", "brikko-studio.json");
      const unwritableStatePath = path.join(home, ".brikko-studio");
  ...
  it("keeps writes inside an BRIKKO_STUDIO_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".brikko-studio", "brikko-studio.json");
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "brikko-studio.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "brikko-studio.json"), "utf-8"),
      ) as {
  ...
  it("suppresses overwrite audit output when skipOutputLogs is set", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          VITEST: "true",
          BRIKKO_STUDIO_TEST_CONFIG_OVERWRITE_LOG: "1",
  ...
  it("rejects destructive internal writes before replacing the config", async () => {
    await withSuiteHome(async (home) => {
      ...
      const original = {
        ...
        agents: { list: [{ id: "main", default: true, workspace: "/tmp/brikko-studio-main" }] },
  ...
  it("rejects root-include partial writes instead of flattening the root config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".brikko-studio", "brikko-studio.json");
      const includePath = path.join(home, ".brikko-studio", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ $schema: "https://brikko-studio.ai/config-from-include.json" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/brikko-studio-test-required-plugin",
          source: "/tmp/brikko-studio-test-required-plugin/index.ts",
          manifestPath: "/tmp/brikko-studio-test-required-plugin/brikko-studio.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
    ...
    await withSuiteHome(async (home) => {
      ...
      try {
        ...
        if (previousConfigPath === undefined) {
          ...
          process.env.BRIKKO_STUDIO_CONFIG_PATH = previousConfigPath;
        }
      }
    });
  });
  ...
  it("notifies in-process reloaders with resolved source config when persisted env refs are restored", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".brikko-studio", "brikko-studio.json");
      const previousConfigPath = process.env.BRIKKO_STUDIO_CONFIG_PATH;
      const previousGatewayToken = process.env.BRIKKO_STUDIO_GATEWAY_TOKEN;
      process.env.BRIKKO_STUDIO_CONFIG_PATH = configPath;
      process.env.BRIKKO_STUDIO_GATEWAY_TOKEN = "gateway-token-runtime";
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
          {
            gateway: {
              mode: "local",
              auth: { mode: "token", token: "${BRIKKO_STUDIO_GATEWAY_TOKEN}" },
            },
      ...
      try {
        ...
        if (previousGatewayToken === undefined) {
          ...
          process.env.BRIKKO_STUDIO_GATEWAY_TOKEN = previousGatewayToken;
        }
      }
    });
  });
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/brikko-studio-test-demo",
          source: "/tmp/brikko-studio-test-demo/index.ts",
          manifestPath: "/tmp/brikko-studio-test-demo/brikko-studio.plugin.json",
          configSchema: {
            type: "object",
            properties: {
    ...
    await withSuiteHome(async (home) => {
      ...
      try {
        ...
        const postWriteSnapshot = await createConfigIO({
          env: { BRIKKO_STUDIO_CONFIG_PATH: configPath, VITEST: "true" } as NodeJS.ProcessEnv,
          homedir: () => home,
          logger: silentLogger,
  ...
  it("preserves authored tilde paths when runtime-shaped writes hand back absolute paths", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            logging: { file: "~/brikko-studio-upgrade-survivor/gateway.jsonl" },
          },
          null,
          2,
        )}\n`,
      ...
      await io.writeConfigFile(
        {
          logging: {
            file: path.join(home, "brikko-studio-upgrade-survivor", "gateway.jsonl"),
            level: "debug",
          },
        },
