  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        BRIKKO_STUDIO_PLUGIN_CATALOG_PATHS: undefined,
        BRIKKO_STUDIO_MPM_CATALOG_PATHS: undefined,
        BRIKKO_STUDIO_LOAD_SHELL_ENV: undefined,
        BRIKKO_STUDIO_DEFER_SHELL_ENV_FALLBACK: undefined,
        BRIKKO_STUDIO_SHELL_ENV_TIMEOUT_MS: undefined,
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.BRIKKO_STUDIO_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected BRIKKO_STUDIO_STATE_DIR or explicit stateDir for .env test setup");
  }
