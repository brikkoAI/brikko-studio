...
describe("applyPluginAutoEnable channels", () => {
  it("uses env-scoped catalog metadata for preferOver auto-enable decisions", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@brikko-studio/env-secondary",
            brikko-studio: {
              channel: {
                id: "env-secondary",
                label: "Env Secondary",
              ...
              install: {
                npmSpec: "@brikko-studio/env-secondary",
              },
            },
          },
        ],
  ...
  it("memoizes external catalog preferOver lookups within one auto-enable pass", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@brikko-studio/env-primary",
            brikko-studio: {
              channel: {
                id: "env-primary",
                label: "Env Primary",
              ...
              install: {
                npmSpec: "@brikko-studio/env-primary",
              },
            },
          },
          {
  ...
  describe("third-party channel plugins (pluginId ≠ channelId)", () => {
    ...
    it("prefers an external plugin that declares preferOver for a bundled channel", () => {
      ...
      expect(result.config.plugins?.entries?.["brikko-studio-modern-chat"]?.enabled).toBe(true);
    ...
    it("falls back to the bundled channel when the preferred external plugin is disabled", () => {
      const result = applyPluginAutoEnable({
        config: {
          channels: { "legacy-bundled-chat": { token: "legacy" } },
          plugins: { entries: { "brikko-studio-modern-chat": { enabled: false } } },
        },
        env: makeIsolatedEnv(),
        manifestRegistry: makeRegistry([
          {
      ...
      expect(result.config.plugins?.entries?.["brikko-studio-modern-chat"]?.enabled).toBe(false);
    ...
    it("does not auto-disable a lower-priority channel plugin that was explicitly selected", () => {
      ...
      expect(result.config.plugins?.entries?.["brikko-studio-qqbot"]?.enabled).toBe(true);
