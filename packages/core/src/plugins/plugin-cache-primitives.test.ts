...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as Brikko StudioConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as Brikko StudioConfig;
    const load = vi.fn(async (config?: Brikko StudioConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: Brikko StudioConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
