...
function createWebSearchEnv(overrides?: Partial<NodeJS.ProcessEnv>) {
  return {
    BRIKKO_STUDIO_HOME: "/tmp/brikko-studio-home",
    ...overrides,
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/brikko-studio.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/brikko-studio.plugin.json",
        channels: [],
        providers: [],
        cliBackends: [],
        syntheticAuthRefs: [],
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  it("resolves current config contents when config changes in place", () => {
    ...
    const env = createWebSearchEnv({ BRIKKO_STUDIO_HOME: "/tmp/brikko-studio-home-a" });
  ...
  it("resolves current env contents when env changes in place", () => {
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.BRIKKO_STUDIO_HOME = "/tmp/brikko-studio-home-b";
      },
      expectedLoaderCalls: 2,
    });
  });
