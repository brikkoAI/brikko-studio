...
function isTrustedSourceLinkedOfficialPackage(pkg: NonNullable<ClawHubPackageDetail["package"]>) {
  ...
  return (
    ...
    (sourceRepo === "brikko-studio/brikko-studio" ||
      sourceRepo === "github.com/brikko-studio/brikko-studio" ||
      sourceRepo === "https://github.com/brikko-studio/brikko-studio")
  );
}
...
function validateClawHubPluginPackage(params: {
  ...
  if (pkg.family === "skill") {
    return buildClawHubInstallFailure(
      `"${pkg.name}" is a skill. Use "brikko-studio skills install ${pkg.name}" instead.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this Brikko Studio runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires Brikko Studio >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
    ...
    return {
      ...
      clawhub: {
        ...
        clawhubUrl:
          ...
          normalizeOptionalString(process.env.BRIKKO_STUDIO_CLAWHUB_URL) ||
          "https://clawhub.ai",
