...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "brikko-studio-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.BRIKKO_STUDIO_DISABLE_BUNDLED_PLUGINS;
...
export function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "brikko-studio.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.BRIKKO_STUDIO_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.BRIKKO_STUDIO_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}
