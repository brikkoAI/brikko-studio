...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".brikko-studio", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "BRIKKO_STUDIO_HOME", "BRIKKO_STUDIO_STATE_DIR"]);
    try {
      const homeDir = await createTempDir("brikko-studio-bundle-commands-home-");
      const workspaceDir = await createTempDir("brikko-studio-bundle-commands-workspace-");
