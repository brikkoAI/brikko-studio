...
> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-marketplace-"));
...
> {
  ...
  try {
    ...
    try {
      ...
      tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-marketplace-download-"));
