  ...
  if (/unsupported_country_region_territory/i.test(message)) {
    return createOpenAICodexOAuthError(
      ...
      [
        ...
        "If you normally use a proxy, verify HTTPS_PROXY, HTTP_PROXY, or ALL_PROXY is set for the BrikkoStudio process and then retry `brikko-studio models auth login --provider openai-codex`.",
      ].join(" "),
      error,
    );
  }
