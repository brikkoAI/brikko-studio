...
const PLUGIN_SDK_PACKAGE_NAMES = ["brikko-studio/plugin-sdk", "@brikko-studio/plugin-sdk"] as const;
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.BRIKKO_STUDIO_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.prototype.hasOwnProperty.call(aliasMap, `brikko-studio/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  const result: Record<string, string> = {
    ...(extensionApiAlias
      ? { "brikko-studio/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
    ...(pluginSdkAlias
      ? Object.fromEntries(
