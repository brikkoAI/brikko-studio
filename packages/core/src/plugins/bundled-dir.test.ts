...
const originalBundledDir = process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR;
const originalDisableBundledPlugins = process.env.BRIKKO_STUDIO_DISABLE_BUNDLED_PLUGINS;
...
function createBrikko StudioRoot(params: {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: "brikko-studio" }, null, 2)}\n`,
    "utf8",
  );
  return repoRoot;
}
...
function seedBundledPluginTree(rootDir: string, relativeDir: string, pluginId = "discord") {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: `@brikko-studio/${pluginId}` }, null, 2)}\n`,
    "utf8",
  );
  fs.writeFileSync(
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
  if (params.disableBundledPlugins === undefined) {
    delete process.env.BRIKKO_STUDIO_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.BRIKKO_STUDIO_DISABLE_BUNDLED_PLUGINS = params.disableBundledPlugins;
  }
...
function expectResolvedBundledDirFromRoot(params: {
  ...
  expectResolvedBundledDir({
    ...
    argv1: params.argv1 ?? path.join(params.repoRoot, "brikko-studio.mjs"),
...
afterEach(() => {
  ...
  if (originalBundledDir === undefined) {
    ...
    process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR = originalBundledDir;
  }
  if (originalDisableBundledPlugins === undefined) {
    delete process.env.BRIKKO_STUDIO_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.BRIKKO_STUDIO_DISABLE_BUNDLED_PLUGINS = originalDisableBundledPlugins;
  }
...
describe("resolveBundledPluginsDir", () => {
  it.each([
    [
      ...
      {
        prefix: "brikko-studio-bundled-dir-runtime-",
    ...
    [
      ...
      {
        prefix: "brikko-studio-bundled-dir-dist-",
        hasDistExtensions: true,
      },
      {
        expectedRelativeDir: path.join("dist", "extensions"),
    ...
    [
      ...
      {
        prefix: "brikko-studio-bundled-dir-git-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "brikko-studio-bundled-dir-vitest-",
        hasExtensions: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
      },
    ...
    [
      ...
      {
        prefix: "brikko-studio-bundled-dir-tsx-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "brikko-studio-bundled-dir-git-",
        hasExtensions: true,
        hasSrc: true,
        hasGitCheckout: true,
        hasPnpmWorkspace: true,
  ...
  it("reports missing pnpm workspace deps for source checkouts", () => {
    ...
    process.argv[1] = path.join(repoRoot, "brikko-studio.mjs");
  ...
  it("separates tilde override cache entries by BRIKKO_STUDIO_HOME", () => {
    const homeA = makeRepoRoot("brikko-studio-bundled-dir-home-a-");
    const homeB = makeRepoRoot("brikko-studio-bundled-dir-home-b-");
    ...
    const bundledA = resolveBundledPluginsDir({ ...envBase, BRIKKO_STUDIO_HOME: homeA });
    const bundledB = resolveBundledPluginsDir({ ...envBase, BRIKKO_STUDIO_HOME: homeB });
  ...
  it("ignores an existing override under an argv1-derived fake package root", () => {
    ...
    process.argv[1] = path.join(installedRoot, "brikko-studio.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR = path.join(installedRoot, "dist", "extensions");
    delete process.env.BRIKKO_STUDIO_DISABLE_BUNDLED_PLUGINS;
  ...
  it("does not let VITEST relax existing override trust checks", () => {
    const overrideRoot = makeRepoRoot("brikko-studio-bundled-dir-vitest-override-reject-");
    ...
    process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
    delete process.env.BRIKKO_STUDIO_TEST_TRUST_BUNDLED_PLUGINS_DIR;
    delete process.env.BRIKKO_STUDIO_DISABLE_BUNDLED_PLUGINS;
  ...
  it("falls back from a missing override instead of returning an untrusted future path", () => {
    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("brikko-studio-bundled-dir-missing-cwd-"));
    process.argv[1] = "/usr/bin/env";
    process.execArgv.length = 0;
    delete process.env.VITEST;
    const missingOverride = path.join(
      makeRepoRoot("brikko-studio-bundled-dir-missing-override-"),
      "extensions",
    );
    process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR = missingOverride;
    delete process.env.BRIKKO_STUDIO_DISABLE_BUNDLED_PLUGINS;
  ...
  it("falls back to argv root when an existing rejected override is unrelated", () => {
    ...
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
    const overrideRoot = makeRepoRoot("brikko-studio-bundled-dir-rejected-override-");
    seedBundledPluginTree(overrideRoot, "extensions", "memory-core");

    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("brikko-studio-bundled-dir-rejected-cwd-"));
    process.argv[1] = path.join(installedRoot, "brikko-studio.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
  ...
  it.each([
    {
      ...
      createScenario: () => {
        ...
        return {
          installedRoot,
          cwd: cwdRepoRoot,
          argv1: path.join(installedRoot, "brikko-studio.mjs"),
        };
      },
    },
    {
