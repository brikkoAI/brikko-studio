...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/brikko-studio-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "brikko-studio.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    BRIKKO_STUDIO_HOME: path.join(homeDir, "home"),
    BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
