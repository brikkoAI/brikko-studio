...
import type { ResolvedTtsConfig, SpeechProviderPlugin } from "brikko-studio/plugin-sdk/speech-core";
import { withEnv, withEnvAsync } from "brikko-studio/plugin-sdk/test-env";
import { beforeEach, describe, expect, it, vi } from "vitest";
import { resolveWorkspacePackagePublicModuleUrl } from "../../plugin-sdk/test-helpers/public-surface-loader.js";

type TtsRuntimeModule = typeof import("brikko-studio/plugin-sdk/tts-runtime");
type TtsCoreModule = typeof import("brikko-studio/plugin-sdk/speech-core");
...
const speechCoreRuntimeApiModuleId = resolveWorkspacePackagePublicModuleUrl({
  packageName: "@brikko-studio/speech-core",
...
async function loadTtsCore(): Promise<TtsCoreModule> {
  ttsCorePromise ??= import("brikko-studio/plugin-sdk/speech-core");
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.BRIKKO_STUDIO_TTS_PREFS;
      process.env.BRIKKO_STUDIO_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.BRIKKO_STUDIO_TTS_PREFS = prevPrefs;
      }
    };
