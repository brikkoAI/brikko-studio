...
describe("stageBundledPluginRuntime", () => {
  it("stages bundled dist plugins as runtime wrappers without linking plugin node_modules", () => {
    const repoRoot = makeRepoRoot("brikko-studio-stage-bundled-runtime-");
    ...
    expect(
      fs
        .lstatSync(
          path.join(repoRoot, "dist", "extensions", "node_modules", "brikko-studio", "plugin-sdk"),
        )
        .isSymbolicLink(),
    ).toBe(false);
    expect(
      fs.readFileSync(
        path.join(repoRoot, "dist", "extensions", "node_modules", "brikko-studio", "package.json"),
        "utf8",
      ),
    ...
    expect(fs.existsSync(path.join(runtimePluginDir, "node_modules", "brikko-studio"))).toBe(false);
  });

  it("keeps extension-local plugin-sdk wrappers resolving canonical dist chunks", async () => {
    const repoRoot = makeRepoRoot("brikko-studio-stage-bundled-runtime-sdk-wrapper-");
  ...
  it("writes wrappers that forward plugin entry imports into canonical dist files", async () => {
    const repoRoot = makeRepoRoot("brikko-studio-stage-bundled-runtime-chunks-");
    createDistPluginDir(repoRoot, "diffs");
    setupRepoFiles(repoRoot, {
  ...
  it("stages root runtime sidecars that bundled plugin boundaries resolve directly", () => {
    const repoRoot = makeRepoRoot("brikko-studio-stage-bundled-runtime-sidecars-");
  ...
  it("keeps plugin command registration on the canonical dist graph when loaded from dist-runtime", async () => {
    const repoRoot = makeRepoRoot("brikko-studio-stage-bundled-runtime-commands-");
    ...
    fs.writeFileSync(
      ...
      [
        "const registry = globalThis.__brikko-studioTestPluginCommands ??= new Map();",
  ...
  it("copies package metadata files but symlinks other non-js plugin artifacts into the runtime overlay", () => {
    const repoRoot = makeRepoRoot("brikko-studio-stage-bundled-runtime-assets-");
    createDistPluginDir(repoRoot, "diffs");
    setupRepoFiles(repoRoot, {
      [bundledDistPluginFile("diffs", "package.json")]: JSON.stringify(
        { name: "@brikko-studio/diffs", brikko-studio: { extensions: ["./index.js"] } },
        null,
        2,
      ),
      [bundledDistPluginFile("diffs", "brikko-studio.plugin.json")]: "{}\n",
    ...
    expectRuntimeArtifactText({
      repoRoot,
      pluginId: "diffs",
      relativePath: "brikko-studio.plugin.json",
      expectedText: "{}\n",
      symbolicLink: false,
    });
    expectRuntimeArtifactText({
  ...
  it("copies bundled plugin skill trees into the runtime overlay", () => {
    const repoRoot = makeRepoRoot("brikko-studio-stage-bundled-runtime-skills-");
  ...
  it("preserves package metadata needed for bundled plugin discovery from dist-runtime", () => {
    const repoRoot = makeRepoRoot("brikko-studio-stage-bundled-runtime-discovery-");
    ...
    setupRepoFiles(repoRoot, {
      ...
      [bundledDistPluginFile("demo", "brikko-studio.plugin.json")]: JSON.stringify(
        {
          id: "demo",
          channels: ["demo"],
  ...
  it("removes stale runtime plugin directories that are no longer in dist", () => {
    const repoRoot = makeRepoRoot("brikko-studio-stage-bundled-runtime-stale-");
  ...
  it("removes dist-runtime when the built bundled plugin tree is absent", () => {
    const repoRoot = makeRepoRoot("brikko-studio-stage-bundled-runtime-missing-");
  ...
  it("tolerates EEXIST when an identical runtime symlink is materialized concurrently", () => {
    const repoRoot = makeRepoRoot("brikko-studio-stage-bundled-runtime-eexist-");
    createDistPluginDir(repoRoot, "feishu");
    setupRepoFiles(repoRoot, {
      [bundledDistPluginFile("feishu", "index.js")]: "export default {}\n",
  ...
  it.each(["EACCES", "ENOSYS"] as const)(
    ...
    (code) => {
      const repoRoot = makeRepoRoot("brikko-studio-stage-bundled-runtime-win-copy-");
      createDistPluginDir(repoRoot, "feishu");
      setupRepoFiles(repoRoot, {
        [bundledDistPluginFile("feishu", "index.js")]: "export default {}\n",
        [bundledDistPluginFile("feishu", "assets/fixture.txt")]: "# Feishu Doc\n",
