...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-plugin-prefer-over-"));
...
describe("plugin loader preferOver activation", () => {
  it("loads the preferred external channel plugin without the replaced bundled plugin tools", () => {
    ...
    const env = {
      BRIKKO_STUDIO_STATE_DIR: makeTempDir(),
    ...
    expect(autoEnabled.config.plugins?.entries?.["brikko-studio-qqbot"]?.enabled).toBe(true);
    expect(autoEnabled.config.plugins?.entries?.qqbot?.enabled).toBe(false);
    expect(registry.plugins.find((plugin) => plugin.id === "brikko-studio-qqbot")?.status).toBe(
      "loaded",
    );
    expect(registry.plugins.find((plugin) => plugin.id === "qqbot")?.status).toBe("disabled");
    expect(registry.tools.map((tool) => tool.pluginId)).toEqual(["brikko-studio-qqbot"]);
    ...
    const registry = loadBrikko StudioPlugins({
      ...
      config: {
        ...
        plugins: {
          ...
          entries: {
            qqbot: { enabled: true },
            "brikko-studio-qqbot": { enabled: true },
          },
        },
      },
      env,
