...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before BrikkoStudio consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether BrikkoStudio should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume BrikkoStudio's bundle
   ...
   *
   * Use this when the backend should consume a canonical BrikkoStudio auth profile
   ...
   * `profile-only` treats the selected BrikkoStudio auth profile as the sole auth
  ...
  prepareExecution?: (
    ...
    | undefined;
  /**
   * Whether this CLI backend can expose native tools outside BrikkoStudio's tool
