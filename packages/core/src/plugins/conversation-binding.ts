...
const APPROVALS_PATH = "~/.brikko-studio/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "brikko-studio-app-server:thread:",
  "brikko-studio-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("brikko-studio.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("brikko-studio.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Brikko Studio instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
