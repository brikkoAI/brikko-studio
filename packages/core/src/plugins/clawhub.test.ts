...
async function createClawHubArchive(entries: Record<string, string>) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-clawhub-archive-"));
  tempDirs.push(dir);
...
describe("installPluginFromClawHub", () => {
  ...
  beforeEach(() => {
    ...
    installPluginFromArchiveMock.mockResolvedValue({
      ok: true,
      pluginId: "demo",
      targetDir: "/tmp/brikko-studio/plugins/demo",
      version: "2026.3.22",
    });
  });
  ...
  it("marks official source-linked Brikko Studio packages as trusted for install scanning", async () => {
    fetchClawHubPackageDetailMock.mockResolvedValueOnce({
      package: {
        ...
        verification: {
          tier: "source-linked",
          sourceRepo: "brikko-studio/brikko-studio",
        },
      },
    });
  ...
  it("falls back to strict files[] verification when sha256hash is missing", async () => {
    const archive = await createClawHubArchive({
      "brikko-studio.plugin.json": '{"id":"demo"}',
    ...
    fetchClawHubPackageVersionMock.mockResolvedValueOnce({
      version: {
        ...
        files: [
          ...
          {
            path: "brikko-studio.plugin.json",
            size: 13,
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      'ClawHub package "demo@2026.3.22" is missing sha256hash; falling back to files[] verification. Validated files: dist/index.js, brikko-studio.plugin.json. Validated generated metadata files present in archive: _meta.json (JSON parse plus slug/version match only).',
    );
  });
  ...
  it("validates _meta.json against canonical package and resolved version metadata", async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      'ClawHub package "demo@2026.3.22" is missing sha256hash; falling back to files[] verification. Validated files: brikko-studio.plugin.json. Validated generated metadata files present in archive: _meta.json (JSON parse plus slug/version match only).',
    );
  });
  ...
  it("omits the skipped-files suffix when no generated extras are present", async () => {
    ...
    expect(result).toMatchObject({ ok: true, pluginId: "demo" });
    expect(logger.warn).toHaveBeenCalledWith(
      'ClawHub package "demo@2026.3.22" is missing sha256hash; falling back to files[] verification. Validated files: brikko-studio.plugin.json.',
    );
  });
  ...
  it("rejects fallback verification when _meta.json exceeds the per-file size limit", async () => {
    ...
    const listedFileEntry = {
      name: "brikko-studio.plugin.json",
      dir: false,
    ...
    const loadAsyncSpy = vi.spyOn(JSZip, "loadAsync").mockResolvedValueOnce({
      files: {
        ...
        "brikko-studio.plugin.json": listedFileEntry,
      },
    } as unknown as JSZip);
    fetchClawHubPackageVersionMock.mockResolvedValueOnce({
      version: {
  ...
  it("rejects fallback verification when a file hash drifts from files[] metadata", async () => {
    ...
    expect(result).toMatchObject({
      ok: false,
      code: CLAWHUB_INSTALL_ERROR_CODE.ARCHIVE_INTEGRITY_MISMATCH,
      error: `ClawHub archive contents do not match files[] metadata for "demo@2026.3.22": expected brikko-studio.plugin.json to hash to ${"1".repeat(64)}, got ${sha256Hex('{"id":"demo"}')}.`,
    });
    expect(installPluginFromArchiveMock).not.toHaveBeenCalled();
  });
  ...
  it("rejects fallback metadata with leading or trailing path whitespace", async () => {
    ...
    expect(result).toMatchObject({
      ...
      code: CLAWHUB_INSTALL_ERROR_CODE.MISSING_ARCHIVE_INTEGRITY,
      error:
        'ClawHub version metadata for "demo@2026.3.22" has an invalid files[0].path (path "brikko-studio.plugin.json " has leading or trailing whitespace).',
    });
    expect(downloadClawHubPackageArchiveMock).not.toHaveBeenCalled();
  });
  ...
  it("rejects fallback verification when the archive includes a whitespace-suffixed file path", async () => {
    ...
    expect(result).toMatchObject({
      ...
      code: CLAWHUB_INSTALL_ERROR_CODE.ARCHIVE_INTEGRITY_MISMATCH,
      error:
        'ClawHub archive contents do not match files[] metadata for "demo@2026.3.22": invalid package file path "brikko-studio.plugin.json " (path "brikko-studio.plugin.json " has leading or trailing whitespace).',
    });
    expect(installPluginFromArchiveMock).not.toHaveBeenCalled();
  });
  ...
  it("rejects fallback metadata with duplicate files[] paths", async () => {
    ...
    expect(result).toMatchObject({
      ...
      code: CLAWHUB_INSTALL_ERROR_CODE.MISSING_ARCHIVE_INTEGRITY,
      error:
        'ClawHub version metadata for "demo@2026.3.22" has duplicate files[] path "brikko-studio.plugin.json".',
    });
    expect(downloadClawHubPackageArchiveMock).not.toHaveBeenCalled();
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expected: {
        ...
        error: '"calendar" is a skill. Use "brikko-studio skills install calendar" instead.',
      },
    },
    {
