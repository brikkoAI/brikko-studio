...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "brikko-studio-version-" });
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "brikko-studio", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "brikko-studio", version: "2.3.4" });
  ...
  it("ignores non-brikko-studio package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers BRIKKO_STUDIO_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        BRIKKO_STUDIO_VERSION: "9.9.9",
        BRIKKO_STUDIO_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale BRIKKO_STUDIO_VERSION for compatibility checks", () => {
    const previous = process.env.BRIKKO_STUDIO_VERSION;
    const previousService = process.env.BRIKKO_STUDIO_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.BRIKKO_STUDIO_VERSION = "2026.3.25";
      process.env.BRIKKO_STUDIO_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      process.env.BRIKKO_STUDIO_VERSION = previous;
      process.env.BRIKKO_STUDIO_SERVICE_VERSION = previousService;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        ...
        BRIKKO_STUDIO_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        BRIKKO_STUDIO_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        BRIKKO_STUDIO_VERSION: "2026.3.99",
        BRIKKO_STUDIO_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
  ...
  it("prefers runtime VERSION over service/package markers and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        ...
        BRIKKO_STUDIO_SERVICE_VERSION: "  2.0.0  ",
