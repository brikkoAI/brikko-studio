...
async function writeWizardConfigFile(config: Brikko StudioConfig): Promise<Brikko StudioConfig> {
...
export async function runSetupWizard(
  ...
  await prompter.intro("Brikko Studio setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readSetupConfigFileSnapshot();
  let baseConfig: Brikko StudioConfig = snapshot.valid
    ? snapshot.exists
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.brikko-studio.ai/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("brikko-studio doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("brikko-studio doctor")}`,
        `Inspect: ${formatCliCommand("brikko-studio plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("brikko-studio configure")}.`;
  ...
  let localGatewayToken = process.env.BRIKKO_STUDIO_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.BRIKKO_STUDIO_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: Brikko StudioConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
  if (opts.skipBootstrap) {
    nextConfig = applySkipBootstrapConfig(nextConfig);
  }
