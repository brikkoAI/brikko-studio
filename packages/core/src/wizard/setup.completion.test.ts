...
function createDeps() {
  const deps: NonNullable<Parameters<typeof setupWizardShellCompletion>[0]["deps"]> = {
    ...
    checkShellCompletionStatus: vi.fn(async (_binName: string) => ({
      ...
      cacheExists: false,
      cachePath: "/tmp/brikko-studio.zsh",
...
describe("setupWizardShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("brikko-studio");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "brikko-studio");
