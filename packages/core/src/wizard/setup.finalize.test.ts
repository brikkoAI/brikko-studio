...
const resolveExistingKey = vi.hoisted(() =>
  vi.fn<(config: BrikkoStudioConfig, provider: string) => string | undefined>(() => undefined),
);
const hasExistingKey = vi.hoisted(() =>
  vi.fn<(config: BrikkoStudioConfig, provider: string) => boolean>(() => false),
);
...
const listConfiguredWebSearchProviders = vi.hoisted(() =>
  vi.fn<(params?: { config?: BrikkoStudioConfig }) => PluginWebSearchProviderEntry[]>(() => []),
);
...
function createEnabledFirecrawlSearchConfig(): BrikkoStudioConfig {
  return {
    tools: {
      web: {
        search: {
