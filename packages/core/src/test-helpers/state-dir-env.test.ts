...
type EnvSnapshot = {
  brikko-studio?: string;
};

function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    brikko-studio: process.env.BRIKKO_STUDIO_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.BRIKKO_STUDIO_STATE_DIR).toBe(snapshot.brikko-studio);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips BRIKKO_STUDIO_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/brikko-studio-state-dir-test");
    expect(process.env.BRIKKO_STUDIO_STATE_DIR).toBe("/tmp/brikko-studio-state-dir-test");
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("brikko-studio-state-dir-env-", async ({ tempRoot, stateDir }) => {
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("brikko-studio-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;
