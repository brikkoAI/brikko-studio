...
describe("shouldAttemptTtsPayload", () => {
  ...
  beforeAll(() => {
    root = mkdtempSync(path.join(tmpdir(), "brikko-studio-tts-config-"));
  });
  ...
  beforeEach(() => {
    originalPrefsPath = process.env.BRIKKO_STUDIO_TTS_PREFS;
    dir = path.join(root, `case-${caseId++}`);
    mkdirSync(dir, { recursive: true });
    prefsPath = path.join(dir, "tts.json");
    process.env.BRIKKO_STUDIO_TTS_PREFS = prefsPath;
  });

  afterEach(() => {
    if (originalPrefsPath === undefined) {
      delete process.env.BRIKKO_STUDIO_TTS_PREFS;
    } else {
      process.env.BRIKKO_STUDIO_TTS_PREFS = originalPrefsPath;
    }
  });
