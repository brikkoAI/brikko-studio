...
const SETUP_TOKEN_MODEL = process.env.BRIKKO_STUDIO_LIVE_SETUP_TOKEN_MODEL?.trim() ?? "";
...
async function resolveTokenSource(): Promise<TokenSource> {
  ...
  if (explicitToken) {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-setup-token-"));
  ...
  if (SETUP_TOKEN_RAW && SETUP_TOKEN_RAW !== "1" && SETUP_TOKEN_RAW !== "auto") {
    throw new Error(
      "BRIKKO_STUDIO_LIVE_SETUP_TOKEN did not look like a setup-token. Use BRIKKO_STUDIO_LIVE_SETUP_TOKEN_VALUE for raw tokens.",
    );
  }

  if (candidates.length === 0) {
    throw new Error(
      "No Anthropics setup-token profiles found. Set BRIKKO_STUDIO_LIVE_SETUP_TOKEN_VALUE or BRIKKO_STUDIO_LIVE_SETUP_TOKEN_PROFILE.",
    );
  }
