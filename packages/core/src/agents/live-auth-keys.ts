...
const GOOGLE_LIVE_SINGLE_KEY = "BRIKKO_STUDIO_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "BRIKKO_STUDIO_LIVE_ANTHROPIC_KEY",
    listVar: "BRIKKO_STUDIO_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "BRIKKO_STUDIO_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `BRIKKO_STUDIO_LIVE_${base}_KEY`;
