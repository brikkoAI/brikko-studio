...
type CoreToolDefinition = {
  ...
  includeInBrikko StudioGroup?: boolean;
};
...
const CORE_TOOL_DEFINITIONS: CoreToolDefinition[] = [
  ...
  },
  {
    ...
    sectionId: "runtime",
    profiles: ["coding"],
    includeInBrikko StudioGroup: true,
  },
  {
    id: "web_search",
    label: "web_search",
...
function buildCoreToolGroupMap() {
  ...
  const brikko-studioTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInBrikko StudioGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:brikko-studio": brikko-studioTools,
