...
type CoreToolDefinition = {
  ...
  includeInBrikkoStudioGroup?: boolean;
};
...
const CORE_TOOL_DEFINITIONS: CoreToolDefinition[] = [
  ...
  },
  {
    ...
    sectionId: "runtime",
    profiles: ["coding"],
    includeInBrikkoStudioGroup: true,
  },
  {
    id: "web_search",
    label: "web_search",
...
function buildCoreToolGroupMap() {
  ...
  const brikko-studioTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInBrikkoStudioGroup).map(
    (tool) => tool.id,
  );
  return {
