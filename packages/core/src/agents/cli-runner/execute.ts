...
const CLI_BACKEND_PRESERVE_ENV = "BRIKKO_STUDIO_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.BRIKKO_STUDIO_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.BRIKKO_STUDIO_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.BRIKKO_STUDIO_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.BRIKKO_STUDIO_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.BRIKKO_STUDIO_MCP_MESSAGE_CHANNEL || "<empty>"}`,
    `senderIsOwner=${childEnv.BRIKKO_STUDIO_MCP_SENDER_IS_OWNER || "<empty>"}`,
  ].join(" ");
}
    ...
    return await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        if (logOutputText) {
          ...
          if (
            env.BRIKKO_STUDIO_MCP_TOKEN ||
            env.BRIKKO_STUDIO_MCP_SESSION_KEY ||
            env.BRIKKO_STUDIO_MCP_SENDER_IS_OWNER
          ) {
