...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          BRIKKO_STUDIO_MCP_TOKEN:
            params.senderIsOwner === true
              ? mcpLoopbackRuntime.ownerToken
              : mcpLoopbackRuntime.nonOwnerToken,
          BRIKKO_STUDIO_MCP_AGENT_ID: sessionAgentId ?? "",
          BRIKKO_STUDIO_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          BRIKKO_STUDIO_MCP_SESSION_KEY: params.sessionKey ?? "",
          BRIKKO_STUDIO_MCP_MESSAGE_CHANNEL: params.messageChannel ?? params.messageProvider ?? "",
        }
      : undefined,
    warn: (message) => cliBackendLog.warn(message),
  });
