...
describe("loadCliSessionHistoryMessages", () => {
  ...
  it("reads the canonical session transcript instead of an arbitrary external path", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-cli-state-"));
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-cli-outside-"));
  ...
  it("honors custom session store roots when resolving hook history transcripts", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-cli-state-"));
    const customStoreDir = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-cli-store-"));
    vi.stubEnv("BRIKKO_STUDIO_STATE_DIR", stateDir);
