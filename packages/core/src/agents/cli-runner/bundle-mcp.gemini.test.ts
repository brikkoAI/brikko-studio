...
describe("prepareCliBundleMcpConfig gemini", () => {
  it("writes Gemini system settings for bundle MCP servers", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/brikko-studio-bundle-mcp-gemini",
    ...
    expect(prepared.env?.BRIKKO_STUDIO_MCP_TOKEN).toBe("loopback-token-123");
    ...
    expect(raw.mcp?.allowed).toEqual(["brikko-studio"]);
    expect(raw.mcpServers?.brikko-studio?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.brikko-studio?.headers?.Authorization).toBe("Bearer loopback-token-123");
