...
describe("ensureAuthProfileStore", () => {
  ...
  function restoreAgentDirEnv(params: {
    ...
    restoreEnvValue("BRIKKO_STUDIO_AGENT_DIR", params.previousAgentDir);
  ...
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-auth-merge-"));
    ...
    try {
      ...
      process.env.BRIKKO_STUDIO_AGENT_DIR = mainDir;
  ...
  it("uses the main agent's newer OAuth profile when an agent still has a stale default profile", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-auth-drift-"));
    const previousAgentDir = process.env.BRIKKO_STUDIO_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    try {
      const mainDir = path.join(root, "main-agent");
  ...
  it("keeps a newer agent replacement credential while repairing stale default references", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-auth-drift-newer-agent-"));
    const previousAgentDir = process.env.BRIKKO_STUDIO_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    try {
      const mainDir = path.join(root, "main-agent");
  ...
  it("preserves a valid main default OAuth profile while replacing a stale agent override", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-auth-drift-base-default-"));
    const previousAgentDir = process.env.BRIKKO_STUDIO_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    try {
      const mainDir = path.join(root, "main-agent");
  ...
  it("keeps a stale default OAuth profile when the main profile belongs to a different identity", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-auth-drift-mismatch-"));
    const previousAgentDir = process.env.BRIKKO_STUDIO_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    try {
      const mainDir = path.join(root, "main-agent");
  ...
  it.each([
    ...
    ({ name, profile, expected }) => {
      withTempAgentDir("brikko-studio-auth-alias-", (agentDir) => {
        const storeData = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it("normalizes mode/apiKey aliases while migrating legacy auth.json", () => {
    withTempAgentDir("brikko-studio-auth-legacy-alias-", (agentDir) => {
      fs.writeFileSync(
  ...
  it("does not load legacy flat auth-profiles.json entries at runtime", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-auth-flat-profiles-"));
    try {
  ...
  it("merges legacy oauth.json into auth-profiles.json", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-oauth-migrate-"));
    ...
    try {
      ...
      process.env.BRIKKO_STUDIO_STATE_DIR = root;
      ...
      clearRuntimeAuthProfileStoreSnapshots();
      restoreEnvValue("BRIKKO_STUDIO_STATE_DIR", previousStateDir);
      restoreAgentDirEnv({ previousAgentDir, previousPiAgentDir });
      fs.rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("exposes provider-managed runtime auth without persisting copied tokens", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-external-auth-"));
    const previousAgentDir = process.env.BRIKKO_STUDIO_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    try {
      const agentDir = path.join(root, "agent");
  ...
  it("does not write inherited auth stores during secrets runtime reads", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-secrets-runtime-"));
    const previousStateDir = process.env.BRIKKO_STUDIO_STATE_DIR;
    try {
  ...
  it("does not clone inherited auth stores during normal agent reads", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-auth-read-through-"));
    const previousStateDir = process.env.BRIKKO_STUDIO_STATE_DIR;
    try {
      const stateDir = path.join(root, ".brikko-studio");
      const mainAgentDir = path.join(stateDir, "agents", "main", "agent");
  ...
  it("logs one warning with aggregated reasons for rejected auth-profiles entries", () => {
    ...
    try {
      withTempAgentDir("brikko-studio-auth-invalid-", (agentDir) => {
        const invalidStore = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it.each([
    {
      ...
      prefix: "brikko-studio-nonstr-key-ref-",
    ...
    },
    {
      ...
      prefix: "brikko-studio-nonstr-key-num-",
      profileId: "openai:default",
      profile: {
        type: "api_key",
        provider: "openai",
    ...
    },
    {
      ...
      prefix: "brikko-studio-nonstr-key-dup-",
      profileId: "openai:default",
      profile: {
        type: "api_key",
        provider: "openai",
    ...
    },
    {
      ...
      prefix: "brikko-studio-nonstr-key-malformed-ref-",
      profileId: "openai:default",
      profile: {
        type: "api_key",
        provider: "openai",
    ...
    },
    {
      ...
      prefix: "brikko-studio-nonstr-token-ref-",
    ...
    },
    {
      ...
      prefix: "brikko-studio-nonstr-token-num-",
      profileId: "anthropic:default",
      profile: {
        type: "token",
        provider: "anthropic",
    ...
    },
    {
      ...
      prefix: "brikko-studio-str-token-",
      profileId: "anthropic:default",
      profile: {
        type: "token",
        provider: "anthropic",
