...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    brikko-studio: process.env.BRIKKO_STUDIO_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.brikko-studio === undefined) {
      delete process.env.BRIKKO_STUDIO_GATEWAY_TOKEN;
    } else {
      process.env.BRIKKO_STUDIO_GATEWAY_TOKEN = envSnapshot.brikko-studio;
    }
  });
  ...
  it("uses BRIKKO_STUDIO_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.BRIKKO_STUDIO_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
