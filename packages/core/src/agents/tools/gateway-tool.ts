...
import { isBrikkoStudioOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as BrikkoStudioConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as BrikkoStudioConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isBrikkoStudioOwnerOnlyCoreToolName("gateway"),
    description:
