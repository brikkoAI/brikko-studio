...
import { isBrikko StudioOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `brikko-studio security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as Brikko StudioConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as Brikko StudioConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isBrikko StudioOwnerOnlyCoreToolName("gateway"),
    description:
