...
describeCacheLive("pi embedded runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.BRIKKO_STUDIO_CACHE_TRACE,
      file: process.env.BRIKKO_STUDIO_CACHE_TRACE_FILE,
      messages: process.env.BRIKKO_STUDIO_CACHE_TRACE_MESSAGES,
      prompt: process.env.BRIKKO_STUDIO_CACHE_TRACE_PROMPT,
      system: process.env.BRIKKO_STUDIO_CACHE_TRACE_SYSTEM,
    };
    process.env.BRIKKO_STUDIO_CACHE_TRACE = "1";
    process.env.BRIKKO_STUDIO_CACHE_TRACE_FILE = liveCacheTraceFile;
    process.env.BRIKKO_STUDIO_CACHE_TRACE_MESSAGES = "0";
    process.env.BRIKKO_STUDIO_CACHE_TRACE_PROMPT = "0";
    process.env.BRIKKO_STUDIO_CACHE_TRACE_SYSTEM = "0";
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      ...
      restore("BRIKKO_STUDIO_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("BRIKKO_STUDIO_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("BRIKKO_STUDIO_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("BRIKKO_STUDIO_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("BRIKKO_STUDIO_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "BRIKKO_STUDIO_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "BRIKKO_STUDIO_LIVE_ANTHROPIC_CACHE_MODEL",
