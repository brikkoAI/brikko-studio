...
async function createHeartbeatAgentsWorkspace() {
  const workspaceDir = await makeTempWorkspace("brikko-studio-bootstrap-");
...
describe("hasCompletedBootstrapTurn", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(await fs.realpath("/tmp"), "brikko-studio-bootstrap-turn-"));
  });
