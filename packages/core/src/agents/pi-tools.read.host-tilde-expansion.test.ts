...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("brikko-studio-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("brikko-studio-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("brikko-studio-tilde-test-mkdir-");
  ...
  it("ignores BRIKKO_STUDIO_HOME for write operations", async () => {
    const brikko-studioHome = await createTempDir("brikko-studio-home-override-", os.tmpdir());
    const dir = await createTempDir("brikko-studio-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("BRIKKO_STUDIO_HOME", brikko-studioHome);

    createHostWorkspaceWriteTool(brikko-studioHome, { workspaceOnly: false });
    ...
    await expect(fs.access(path.join(brikko-studioHome, path.basename(testFile)))).rejects.toBeDefined();
  });

  it("ignores BRIKKO_STUDIO_HOME for mkdir operations", async () => {
    const brikko-studioHome = await createTempDir("brikko-studio-home-override-", os.tmpdir());
    const dir = await createTempDir("brikko-studio-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expect(fs.access(path.join(brikko-studioHome, path.basename(newDir)))).rejects.toBeDefined();
  });

  it("ignores BRIKKO_STUDIO_HOME for readFile operations", async () => {
    const brikko-studioHome = await createTempDir("brikko-studio-home-override-", os.tmpdir());
    const dir = await createTempDir("brikko-studio-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(brikko-studioHome, { workspaceOnly: false });
    const content = await mocks.editOps!.readFile(toTildePath(testFile));
  ...
  it("ignores BRIKKO_STUDIO_HOME for access operations", async () => {
    const brikko-studioHome = await createTempDir("brikko-studio-home-override-", os.tmpdir());
    const dir = await createTempDir("brikko-studio-tilde-test-edit-");
