...
describe("resolveOAuthRefreshLockPath", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-auth-lock-path-"));
...
describe("resolveOAuthRefreshLockPath fuzz", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-auth-lock-path-fuzz-"));
    process.env.BRIKKO_STUDIO_STATE_DIR = stateDir;
  });
