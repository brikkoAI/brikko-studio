...
describe("compactSkillPaths", () => {
  ...
  it("replaces home directory prefix with ~ in skill locations", () => {
    ...
    const skillDir = path.join(home, ".brikko-studio-test-skills", "test-skill");
  ...
  it("preserves paths outside home directory", () => {
    const outsideHome = path.join(path.parse(os.homedir()).root, "brikko-studio-external-skills");
