...
describe("normalizeMessagesForLlmBoundary", () => {
  ...
  it("keeps runtime-context transcript entries out of the LLM boundary", () => {
    ...
    expect(output).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ customType: "brikko-studio.runtime-context" })]),
    );
  ...
  it("includes resolved auth profile fields for context-engine afterTurn compaction", () => {
    ...
    const legacy = buildAfterTurnRuntimeContext({
      attempt: {
        ...
        agentAccountId: "acct-1",
        authProfileId: "openai:p1",
        config: { plugins: { slots: { contextEngine: "lossless-claw" } } } as Brikko StudioConfig,
        skillsSnapshot: undefined,
        senderIsOwner: true,
        provider: "openai-codex",
        modelId: "gpt-5.4",
