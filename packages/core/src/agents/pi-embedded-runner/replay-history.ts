...
// `provider:"brikko-studio"` assistant entries written by the channel-delivery
...
const TRANSCRIPT_ONLY_BRIKKO_STUDIO_MODELS = new Set<string>(["delivery-mirror", "gateway-injected"]);
...
function isTranscriptOnlyOpenclawAssistant(message: AgentMessage): boolean {
  ...
  return (
    ...
    typeof model === "string" &&
    TRANSCRIPT_ONLY_BRIKKO_STUDIO_MODELS.has(model)
  );
}
