...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-managed-media-"));
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside brikko-studio tmp root",
      media: path.join(os.tmpdir(), "outside-brikko-studio", "passwd"),
      expected: /sandbox/i,
    },
    {
