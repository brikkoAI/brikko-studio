...
function resolveAttachmentLimits(config: Brikko StudioConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".brikko-studio", "attachments");
  const relDir = path.posix.join(".brikko-studio", "attachments", attachmentId);
