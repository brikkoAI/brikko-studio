...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "brikko-studio-auth-suite-mock");
...
describe("runWithModelFallback", () => {
  it("skips auth store bootstrap when no auth profile sources exist", async () => {
    ...
    const result = await runWithModelFallback({
      ...
      provider: "openai",
      model: "gpt-4.1-mini",
      agentDir: "/tmp/brikko-studio-no-auth-profiles",
      run,
    });

    expect(result.result).toBe("ok");
    expect(authSourceCheckMock.hasAnyAuthProfileStoreSource).toHaveBeenCalledWith(
      "/tmp/brikko-studio-no-auth-profiles",
    );
  ...
  it("sanitizes model identifiers in model_not_found warnings", async () => {
    const warnLogs = createWarnLogCapture("brikko-studio-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
