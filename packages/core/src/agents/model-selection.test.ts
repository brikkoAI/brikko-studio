...
function resolveAnthropicOpusThinking(cfg: BrikkoStudioConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: BrikkoStudioConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveConfiguredRefForTest(cfg: Partial<BrikkoStudioConfig>) {
...
describe("model-selection", () => {
  ...
  describe("isCliProvider", () => {
    ...
    it("returns true for setup-registered cli backends", () => {
      expect(isCliProvider("claude-cli", {} as BrikkoStudioConfig)).toBe(true);
    });

    it("returns false for provider ids", () => {
      expect(isCliProvider("example-cli", {} as BrikkoStudioConfig)).toBe(false);
    });
  });
  ...
  describe("buildModelAliasIndex", () => {
    it("should build alias index from config", () => {
      const cfg: Partial<BrikkoStudioConfig> = {
        agents: {
          defaults: {
            models: {
