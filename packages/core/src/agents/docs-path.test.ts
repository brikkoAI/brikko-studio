...
import {
  resolveBrikkoStudioDocsPath,
  resolveBrikkoStudioReferencePaths,
  resolveBrikkoStudioSourcePath,
} from "./docs-path.js";
...
describe("resolveBrikkoStudioDocsPath", () => {
  it("uses the workspace docs directory when it has canonical docs metadata", async () => {
    ...
    await expect(resolveBrikkoStudioDocsPath({ workspaceDir: root })).resolves.toBe(
      path.join(root, "docs"),
    );
  });
  ...
  it("finds bundled package docs from a nested package path", async () => {
    ...
    await expect(resolveBrikkoStudioDocsPath({ cwd: nested })).resolves.toBe(path.join(root, "docs"));
  });
  ...
  it("does not accept incomplete template-only docs directories", async () => {
    ...
    await expect(resolveBrikkoStudioDocsPath({ cwd: root })).resolves.toBeNull();
  });
});

describe("resolveBrikkoStudioSourcePath", () => {
  it("returns the package root only for git checkouts", async () => {
    const root = await makePackageRoot("brikko-studio-source-git-");
    await fs.mkdir(path.join(root, ".git"));

    await expect(resolveBrikkoStudioSourcePath({ cwd: root })).resolves.toBe(root);
  });

  it("omits source path for npm-style package installs", async () => {
    const root = await makePackageRoot("brikko-studio-source-npm-");

    await expect(resolveBrikkoStudioSourcePath({ cwd: root })).resolves.toBeNull();
  });
});

describe("resolveBrikkoStudioReferencePaths", () => {
  it("returns docs and local source together for git checkouts", async () => {
    ...
    await expect(resolveBrikkoStudioReferencePaths({ cwd: root })).resolves.toEqual({
