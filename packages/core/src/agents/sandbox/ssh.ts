...
export async function createSshSandboxSessionFromConfigText(params: {
  ...
  const configDir = await fs.mkdtemp(path.join(resolveSshTmpRoot(), "brikko-studio-sandbox-ssh-"));
...
export async function createSshSandboxSessionFromSettings(
  ...
  try {
    ...
    const hostAlias = "brikko-studio-sandbox";
    const configPath = path.join(configDir, "config");
    const lines = [
      `Host ${hostAlias}`,
...
function resolveSshTmpRoot(): string {
  return path.resolve(resolvePreferredBrikko StudioTmpDir() ?? os.tmpdir());
}
