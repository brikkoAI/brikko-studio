      ...
      defaults: {
        sandbox: {
          ...
          ssh: {
            ...
            command: "ssh",
            workspaceRoot: "/remote/brikko-studio",
...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "brikko-studio-test-ssh-config"),
    host: "brikko-studio-sandbox",
  };
}
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "brikko-studio-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "brikko-studio-ssh-worker-abcd1234",
    ...
    expect(sshMocks.runSshSandboxCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        remoteCommand: expect.stringContaining("/remote/brikko-studio/brikko-studio-ssh-agent-worker"),
      }),
    );
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const backend = await createSshSandboxBackend({
      ...
      cfg: {
        ...
        browser: {
          enabled: false,
          image: "brikko-studio-browser",
          containerPrefix: "brikko-studio-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/brikko-studio/brikko-studio-ssh-agent-worker");
