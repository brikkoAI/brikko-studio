...
export async function ensureDockerImage(image: string) {
  ...
  if (image === DEFAULT_SANDBOX_IMAGE) {
    throw new Error(
      `Sandbox image not found: ${image}. Build it with scripts/sandbox-setup.sh before enabling Docker sandboxing. The default image includes python3 for sandbox write/edit helpers; Brikko Studio will not substitute plain debian:bookworm-slim.`,
    );
  }
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "brikko-studio.sandbox=1");
  args.push("--label", `brikko-studio.sessionKey=${params.scopeKey}`);
  args.push("--label", `brikko-studio.createdAtMs=${createdAtMs}`);
  args.push("--label", `brikko-studio.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `brikko-studio.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markBrikko StudioExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "brikko-studio.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`brikko-studio sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`brikko-studio sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("brikko-studio sandbox recreate --all");
}
