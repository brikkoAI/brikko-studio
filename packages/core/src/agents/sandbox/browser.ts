...
import {
  ...
  DEFAULT_BRIKKO_STUDIO_BROWSER_PROFILE_NAME,
  resolveProfile,
...
const CDP_SOURCE_RANGE_ENV_KEY = "BRIKKO_STUDIO_BROWSER_CDP_SOURCE_RANGE";
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_BRIKKO_STUDIO_BROWSER_PROFILE_NAME,
    extraArgs: [],
    tabCleanup: {
      enabled: true,
      idleMinutes: 120,
...
export async function ensureSandboxBrowser(params: {
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "brikko-studio.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `brikko-studio sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `brikko-studio sandbox recreate --browser --agent ${agentId}`;
          }
          return "brikko-studio sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: {
        "brikko-studio.sandboxBrowser": "1",
        "brikko-studio.browserConfigEpoch": SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
      },
    ...
    args.push("-e", `BRIKKO_STUDIO_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `BRIKKO_STUDIO_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `BRIKKO_STUDIO_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push(
      "-e",
      `BRIKKO_STUDIO_BROWSER_AUTO_START_TIMEOUT_MS=${params.cfg.browser.autoStartTimeoutMs}`,
    );
    if (effectiveCdpSourceRange) {
      args.push("-e", `${CDP_SOURCE_RANGE_ENV_KEY}=${effectiveCdpSourceRange}`);
    }
    args.push("-e", `BRIKKO_STUDIO_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `BRIKKO_STUDIO_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
    args.push("-e", "BRIKKO_STUDIO_BROWSER_NO_SANDBOX=1");
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_BRIKKO_STUDIO_BROWSER_PROFILE_NAME)
    : null;
