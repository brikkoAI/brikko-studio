...
describe("provider attribution", () => {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { BRIKKO_STUDIO_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
    ...
    expect(resolveProviderAttributionHeaders("openai", { BRIKKO_STUDIO_VERSION: "2026.3.22" })).toEqual({
      originator: "brikko-studio",
      version: "2026.3.22",
      "User-Agent": "brikko-studio/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { BRIKKO_STUDIO_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ BRIKKO_STUDIO_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
