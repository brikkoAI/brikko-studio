...
describe("createOpenAIWebSocketStreamFn", () => {
  ...
  it("keeps native turn metadata stable across websocket retries and increments attempt", async () => {
    ...
    expect(firstPayload.metadata?.brikko-studio_session_id).toBe("sess-turn-metadata-retry");
    expect(firstPayload.metadata?.brikko-studio_transport).toBe("websocket");
    expect(firstPayload.metadata?.brikko-studio_turn_id).toBeTruthy();
    expect(secondPayload.metadata?.brikko-studio_turn_id).toBe(firstPayload.metadata?.brikko-studio_turn_id);
    expect(firstPayload.metadata?.brikko-studio_turn_attempt).toBe("1");
    expect(secondPayload.metadata?.brikko-studio_turn_attempt).toBe("2");
  });
  ...
  it("does not attach native OpenAI session headers or metadata for custom responses endpoints", async () => {
    ...
    expect(payload.metadata?.brikko-studio_session_id).toBeUndefined();
    expect(payload.metadata?.brikko-studio_transport).toBeUndefined();
  ...
  it("pools cleanly released sessions behind the explicit pool flag", async () => {
    ...
    try {
      releaseWsSession("registry-test", {
        allowPool: true,
        env: {
          BRIKKO_STUDIO_OPENAI_WS_POOL: "1",
          BRIKKO_STUDIO_OPENAI_WS_SESSION_POOL_IDLE_MS: "1000",
