...
const TARGET_MODEL_REFS = parseTargetModelRefs(
  process.env.BRIKKO_STUDIO_LIVE_TOOL_REPLAY_REPAIR_MODELS ?? DEFAULT_TARGET_MODEL_REFS,
);
  ...
  return (raw ?? "")
    ...
    .map((ref) => {
      ...
      if (!provider?.trim() || !modelId) {
        throw new Error(
          `Invalid BRIKKO_STUDIO_LIVE_TOOL_REPLAY_REPAIR_MODELS entry: ${JSON.stringify(ref)}`,
        );
      }
