...
describe("resolveBundledSkillsDir", () => {
  ...
  it("returns BRIKKO_STUDIO_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-bundled-override-"));
    process.env.BRIKKO_STUDIO_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.BRIKKO_STUDIO_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "brikko-studio" }));
