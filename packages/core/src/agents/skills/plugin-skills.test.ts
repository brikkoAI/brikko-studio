    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "brikko-studio.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "brikko-studio.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "brikko-studio.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("brikko-studio-");
  const acpxRoot = await tempDirs.make("brikko-studio-acpx-plugin-");
  const helperRoot = await tempDirs.make("brikko-studio-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("brikko-studio-");
  const pluginRoot = await tempDirs.make("brikko-studio-plugin-");
  const outsideDir = await tempDirs.make("brikko-studio-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("brikko-studio-");
    const pluginRoot = await tempDirs.make("brikko-studio-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("brikko-studio-");
    const pluginRoot = await tempDirs.make("brikko-studio-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
