...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "brikko-studio-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "brikko-studio-bundled") {
    return {
...
function loadSkillEntries(
  ...
  const loadSkills = (params: { dir: string; source: string }): LoadedSkillRecord[] => {
    ...
    // skill directories (e.g. ~/.brikko-studio/skills/coze/koze-retrieval/SKILL.md).
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "brikko-studio-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "brikko-studio-managed",
  });
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`brikko-studio skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`brikko-studio skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
