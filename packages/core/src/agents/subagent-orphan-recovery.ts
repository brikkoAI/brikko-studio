 ...
 *
 * @see https://github.com/brikko-studio/brikko-studio/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /brikko-studio\.json|brikko-studio gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumeResult = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify brikko-studio.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
