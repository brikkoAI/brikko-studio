...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("brikko-studio-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("brikko-studio-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("brikko-studio-sbx-");
