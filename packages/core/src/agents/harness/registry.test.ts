...
const originalRuntime = process.env.BRIKKO_STUDIO_AGENT_RUNTIME;

afterEach(() => {
  clearAgentHarnesses();
  if (originalRuntime == null) {
    delete process.env.BRIKKO_STUDIO_AGENT_RUNTIME;
  } else {
    process.env.BRIKKO_STUDIO_AGENT_RUNTIME = originalRuntime;
  }
});
...
describe("agent harness registry", () => {
  ...
  it("keeps model-specific harnesses behind plugin registration in auto mode", () => {
    process.env.BRIKKO_STUDIO_AGENT_RUNTIME = "auto";
  ...
  it("honors explicit PI mode", () => {
    process.env.BRIKKO_STUDIO_AGENT_RUNTIME = "pi";
    registerAgentHarness(makeHarness("plugin-harness", { priority: 200 }), {
      ownerPluginId: "plugin-a",
    });
  ...
  it("honors explicit plugin harness mode when the plugin harness is registered", () => {
    process.env.BRIKKO_STUDIO_AGENT_RUNTIME = "custom";
