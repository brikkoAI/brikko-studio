...
describe("runCliAgent bundle MCP e2e", () => {
  it(
    ...
    async () => {
      ...
      const envSnapshot = captureEnv([
        ...
        "BRIKKO_STUDIO_DISABLE_PERSISTED_PLUGIN_REGISTRY",
      ]);
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-cli-bundle-mcp-"));
      process.env.HOME = tempHome;
      process.env.USERPROFILE = tempHome;
      delete process.env.BRIKKO_STUDIO_HOME;
      delete process.env.BRIKKO_STUDIO_STATE_DIR;
      process.env.BRIKKO_STUDIO_DISABLE_PERSISTED_PLUGIN_REGISTRY = "1";
      ...
      const pluginRoot = path.join(tempHome, ".brikko-studio", "extensions", "bundle-probe");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-cli-live-cleanup-"));
      process.env.HOME = tempHome;
      process.env.USERPROFILE = tempHome;
      delete process.env.BRIKKO_STUDIO_HOME;
      delete process.env.BRIKKO_STUDIO_STATE_DIR;
