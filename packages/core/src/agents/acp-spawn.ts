...
function resolveHeartbeatConfigForAgent(params: {
  cfg: BrikkoStudioConfig;
  agentId: string;
}): NonNullable<NonNullable<BrikkoStudioConfig["agents"]>["defaults"]>["heartbeat"] {
  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an BrikkoStudio config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for BrikkoStudio config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: BrikkoStudioConfig, agentId: string): boolean {
