...
vi.mock("../model-selection.js", () => ({
  isCliProvider: (provider: string, cfg?: Brikko StudioConfig) =>
...
async function withTempSessionStore<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-session-store-"));
  try {
...
describe("updateSessionStoreAfterAgentRun", () => {
  ...
  it("persists claude-cli session bindings when the backend is configured", async () => {
    await withTempSessionStore(async ({ storePath }) => {
      ...
      const sessionId = "test-brikko-studio-session";
      const sessionStore: Record<string, SessionEntry> = {
        [sessionKey]: {
          sessionId,
          updatedAt: 1,
  ...
  it("persists cleared Claude CLI bindings through session-store merge", async () => {
    await withTempSessionStore(async ({ storePath }) => {
      ...
      const entry: SessionEntry = {
        sessionId: "brikko-studio-session-1",
        updatedAt: 1,
        cliSessionBindings: {
          "claude-cli": {
