...
metadata: {"brikko-studio":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
function loadTestWorkspaceSkillEntries(workspaceDir: string): SkillEntry[] {
  const skills = loadSkillsFromDirSafe({
    ...
    source: "brikko-studio-workspace",
  }).skills;
...
const workspaceSuite = createFixtureSuite("brikko-studio-skills-install-");
...
async function withWorkspaceCase(
  ...
  const envSnapshot = captureEnv(["BRIKKO_STUDIO_STATE_DIR"]);
  try {
...
describe("installSkill code safety scanning", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("BRIKKO_STUDIO_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["BRIKKO_STUDIO_STATE_DIR", "BRIKKO_STUDIO_CONFIG_PATH"]);
    try {
      process.env.BRIKKO_STUDIO_STATE_DIR = "/tmp/untrusted-state";
      process.env.BRIKKO_STUDIO_CONFIG_PATH = "/tmp/untrusted-config/brikko-studio.json";
      ...
      ).toBe("/Users/tester/.brikko-studio");
    } finally {
      envSnapshot.restore();
    }
  });
  ...
  it("surfaces plugin scanner findings from before_install", async () => {
    ...
    await withWorkspaceCase(async ({ workspaceDir }) => {
      ...
      expect(handler.mock.calls[0]?.[0]).toMatchObject({
        ...
        targetType: "skill",
        origin: "brikko-studio-workspace",
