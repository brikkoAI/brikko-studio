...
vi.mock("./cli/container-target.js", () => ({
  ...
  resolveCliContainerTarget: (argv: string[], env: NodeJS.ProcessEnv = process.env) =>
    argv.includes("--container") ? "demo" : (env.BRIKKO_STUDIO_CONTAINER ?? null),
}));
...
describe("entry root version fast path", () => {
  it("prints version output and skips host handling when container-targeted", async () => {
    ...
    expect(
      tryHandleRootVersionFastPath(["node", "brikko-studio", "--version"], {
        output,
        exit,
        resolveVersion,
      }),
    ...
    exit.mockClear();
    expect(
      tryHandleRootVersionFastPath(["node", "brikko-studio", "--container", "demo", "--version"], {
        output,
        exit,
        resolveVersion,
      }),
    ...
    expect(
      tryHandleRootVersionFastPath(["node", "brikko-studio", "--version"], {
        env: { BRIKKO_STUDIO_CONTAINER: "demo" },
        output,
        exit,
        resolveVersion,
      }),
