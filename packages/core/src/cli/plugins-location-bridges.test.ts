    ...
    plugins: [
      {
        ...
        startup: {},
        packageInstall: {
          clawhubSpec: `clawhub:@brikko-studio/${pluginId}`,
...
describe("listPersistedBundledPluginLocationBridges", () => {
  ...
  it("keeps persisted bundled relocations npm-first for launch", async () => {
    readPersistedInstalledPluginIndexMock.mockResolvedValue(
      makeIndex({
        ...
        manifestPath: "/app/dist/extensions/diagnostics-otel/brikko-studio.plugin.json",
        manifestHash: "hash",
        ...
        packageInstall: {
          defaultChoice: "clawhub",
          clawhub: {
            spec: "clawhub:@brikko-studio/diagnostics-otel",
  ...
  it("does not create a relocation bridge without persisted or official install metadata", async () => {
    readPersistedInstalledPluginIndexMock.mockResolvedValue(
      makeIndex({
        pluginId: "local-only",
        manifestPath: "/app/dist/extensions/local-only/brikko-studio.plugin.json",
        manifestHash: "hash",
