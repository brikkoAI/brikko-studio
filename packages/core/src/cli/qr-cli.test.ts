...
describe("registerQrCli", () => {
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("brikko-studio devices approve <requestId>");
  });
  ...
  it("uses BRIKKO_STUDIO_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("BRIKKO_STUDIO_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),
