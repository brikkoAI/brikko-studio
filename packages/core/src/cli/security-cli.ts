...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["brikko-studio security audit", "Run a local security audit."],
          [
            "brikko-studio security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["brikko-studio security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "brikko-studio security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["brikko-studio security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["brikko-studio security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.brikko-studio.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("brikko-studio security audit --fix")}`));
        if (!fixResult) {
