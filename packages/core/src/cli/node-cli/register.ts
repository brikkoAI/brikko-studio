    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "brikko-studio node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["brikko-studio node status", "Check node host service status."],
          ["brikko-studio node install", "Install the node host service."],
          ["brikko-studio node start", "Start the installed node host service."],
          ["brikko-studio node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.brikko-studio.ai/cli/node")}\n`,
    );
