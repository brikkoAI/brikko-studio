...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.brikko-studio.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Brikko Studio hooks")
    ...
    .option("--hook-url <url>", "Brikko Studio hook URL")
    .option("--hook-token <token>", "Brikko Studio hook token")
