...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["brikko-studio status", "Show channel health + session summary."],
          ["brikko-studio status --all", "Full diagnosis (read-only)."],
          ["brikko-studio status --json", "Machine-readable output."],
          ["brikko-studio status --usage", "Show model provider usage/quota snapshots."],
          [
            "brikko-studio status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["brikko-studio status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.brikko-studio.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.brikko-studio.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["brikko-studio sessions", "List all sessions."],
          ["brikko-studio sessions --agent work", "List sessions for one agent."],
          ["brikko-studio sessions --all-agents", "Aggregate sessions across agents."],
          ["brikko-studio sessions --active 120", "Only last 2 hours."],
          ["brikko-studio sessions --json", "Machine-readable output."],
          ["brikko-studio sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.brikko-studio.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["brikko-studio sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "brikko-studio sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["brikko-studio sessions cleanup --enforce", "Apply maintenance now."],
          ["brikko-studio sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["brikko-studio sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "brikko-studio sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .brikko-studio/trajectory-exports")
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["brikko-studio commitments", "List pending inferred follow-ups."],
          ["brikko-studio commitments --all", "List all inferred follow-ups."],
          ["brikko-studio commitments --agent work", "List one agent's inferred follow-ups."],
          ["brikko-studio commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
