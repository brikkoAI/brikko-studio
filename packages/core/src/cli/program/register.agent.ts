...
${formatHelpExamples([
  ['brikko-studio agent --to +15555550123 --message "status update"', "Start a new session."],
  ['brikko-studio agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['brikko-studio agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.brikko-studio.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.brikko-studio.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "brikko-studio agents set-identity --identity-file ~/.brikko-studio/workspace/IDENTITY.md --agent main",
