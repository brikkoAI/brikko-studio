  ...
  return {
    ...
    mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: BrikkoStudioConfig) => void }) => {
      ...
      return {
        ...
        previousHash: "hash-1",
        snapshot: { path: "/tmp/brikko-studio.json" },
        nextConfig: draft,
        result: undefined,
      };
    }),
...
describe("exec-policy CLI", () => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    mocks.readConfigFileSnapshot.mockImplementationOnce(async () => ({
      path: "/tmp/brikko-studio.json\u001B[2J\nforged",
      hash: "config-hash-1",
      config: mocks.getConfig(),
    }));
    ...
    expect(output).toContain("/tmp/brikko-studio.json");
    ...
    expect(output).not.toContain("/tmp/brikko-studio.json\nforged");
