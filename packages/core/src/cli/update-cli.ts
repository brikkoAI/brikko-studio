...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["brikko-studio update", "Update a source checkout (git)"],
        ["brikko-studio update --channel beta", "Switch to beta channel (git + npm)"],
        ["brikko-studio update --channel dev", "Switch to dev channel (git + npm)"],
        ["brikko-studio update --tag beta", "One-off update to a dist-tag or version"],
        ["brikko-studio update --tag main", "One-off package install from GitHub main"],
        ["brikko-studio update --dry-run", "Preview actions without changing anything"],
        ["brikko-studio update --no-restart", "Update without restarting the service"],
        ["brikko-studio update --json", "Output result as JSON"],
        ["brikko-studio update --yes", "Non-interactive (accept downgrade prompts)"],
        ["brikko-studio update wizard", "Interactive update wizard"],
        ["brikko-studio --update", "Shorthand for brikko-studio update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run brikko-studio update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.brikko-studio.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["brikko-studio update status", "Show channel + version status."],
          ["brikko-studio update status --json", "JSON output."],
          ["brikko-studio update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.brikko-studio.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
