...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    BRIKKO_STUDIO_STATE_DIR: "/tmp/brikko-studio-daemon",
    BRIKKO_STUDIO_CONFIG_PATH: "/tmp/brikko-studio-daemon/brikko-studio.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.BRIKKO_STUDIO_STATE_DIR ?? "/tmp/brikko-studio-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.BRIKKO_STUDIO_CONFIG_PATH ?? `${stateDir}/brikko-studio.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/brikko-studio-daemon/");
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.BRIKKO_STUDIO_STATE_DIR = "/tmp/brikko-studio-cli";
    process.env.BRIKKO_STUDIO_CONFIG_PATH = "/tmp/brikko-studio-cli/brikko-studio.json";
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(callGatewayStatusProbe).toHaveBeenCalledWith(
      expect.objectContaining({
        requireRpc: true,
        configPath: "/tmp/brikko-studio-daemon/brikko-studio.json",
      }),
    );
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/brikko-studio-cli/brikko-studio.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.BRIKKO_STUDIO_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.BRIKKO_STUDIO_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("uses the fast config path for plain same-file status reads", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-status-config-"));
    const configPath = path.join(tmp, "brikko-studio.json");
    await fs.writeFile(
      configPath,
      JSON.stringify({
        gateway: {
    ...
    process.env.BRIKKO_STUDIO_STATE_DIR = tmp;
    process.env.BRIKKO_STUDIO_CONFIG_PATH = configPath;
    serviceReadCommand.mockResolvedValueOnce({
      programArguments: ["/bin/node", "cli", "gateway", "--port", "19001"],
      environment: {
        BRIKKO_STUDIO_STATE_DIR: tmp,
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.BRIKKO_STUDIO_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection when probe fails", async () => {
    ...
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "brikko-studio-gateway" }],
        hints: [],
      },
      healthy: false,
