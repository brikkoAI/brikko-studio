  ...
  it("treats a gateway listener child pid as healthy ownership", async () => {
    const snapshot = await inspectGatewayRestartWithSnapshot({
      ...
      portUsage: {
        port: 18789,
        status: "busy",
        listeners: [{ pid: 7001, ppid: 7000, commandLine: "brikko-studio-gateway" }],
        hints: [],
      },
    });
  ...
  it("treats a busy port as healthy when runtime status lags but the probe succeeds", async () => {
    ...
    const snapshot = await inspectGatewayRestartWithSnapshot({
      ...
      portUsage: {
        port: 18789,
        status: "busy",
        listeners: [{ pid: 9100, commandLine: "brikko-studio-gateway" }],
        hints: [],
      },
    });
  ...
  it("requires the expected gateway version when provided", async () => {
    ...
    const snapshot = await inspectGatewayRestartWithSnapshot({
      ...
      portUsage: {
        port: 18789,
        status: "busy",
        listeners: [{ pid: 8000, commandLine: "brikko-studio-gateway" }],
        hints: [],
      },
    });
