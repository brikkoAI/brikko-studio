...
export function registerDnsCli(program: Command) {
  const dns = program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.brikko-studio.ai/cli/dns")}\n`,
    );
  ...
  dns
    ...
    .option("--domain <domain>", "Wide-area discovery domain (e.g. brikko-studio.internal)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(
        theme.heading(
          "Recommended config ($BRIKKO_STUDIO_CONFIG_PATH, default ~/.brikko-studio/brikko-studio.json):",
        ),
      );
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by brikko-studio dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in the active Brikko Studio config ($BRIKKO_STUDIO_CONFIG_PATH, default ~/.brikko-studio/brikko-studio.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
