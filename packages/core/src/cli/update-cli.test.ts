...
vi.mock("../utils.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveConfigDir: () => "/tmp/brikko-studio-config",
  };
});
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/brikko-studio-update-tests";
  let fixtureCount = 0;
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("brikko-studio-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("brikko-studio-updated-root");
  ...
  beforeEach(() => {
    ...
    serviceReadCommand.mockImplementation(async () =>
      (await serviceLoaded()) ? { programArguments: ["brikko-studio", "gateway", "run"] } : null,
    );
    ...
    prepareRestartScript.mockResolvedValue("/tmp/brikko-studio-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "brikko-studio-gateway" }],
      hints: [],
    });
  ...
  it("bounds completion cache refresh during update follow-up", async () => {
    const root = createCaseDir("brikko-studio-completion-timeout");
    ...
    expect(spawnSync).toHaveBeenCalledWith(
      expect.any(String),
      [path.join(root, "brikko-studio.mjs"), "completion", "--write-state"],
      expect.objectContaining({
        env: expect.objectContaining({
          BRIKKO_STUDIO_COMPLETION_SKIP_PLUGIN_COMMANDS: "1",
        }),
        timeout: 30_000,
      }),
    );
  ...
  it("logs friendly hint with manual refresh command when completion cache write times out", async () => {
    const root = createCaseDir("brikko-studio-completion-timeout-msg");
    pathExists.mockResolvedValue(true);
    ...
    expect(logs.some((line) => line.includes("brikko-studio completion --write-state"))).toBe(true);
    ...
    expect(spawn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          ...
          BRIKKO_STUDIO_UPDATE_POST_CORE: "1",
          BRIKKO_STUDIO_UPDATE_POST_CORE_CHANNEL: "dev",
        }),
      }),
    );
  ...
  it("does not carry gateway service markers into the post-core update process", async () => {
    ...
    await withEnvAsync(
      {
        BRIKKO_STUDIO_SERVICE_MARKER: "brikko-studio",
        BRIKKO_STUDIO_SERVICE_KIND: "gateway",
      },
      async () => {
    ...
    expect(spawnEnv?.BRIKKO_STUDIO_SERVICE_MARKER).toBeUndefined();
    expect(spawnEnv?.BRIKKO_STUDIO_SERVICE_KIND).toBeUndefined();
  });
    ...
    expect(spawn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          BRIKKO_STUDIO_UPDATE_POST_CORE: "1",
          BRIKKO_STUDIO_UPDATE_POST_CORE_CHANNEL: "dev",
          BRIKKO_STUDIO_UPDATE_POST_CORE_REQUESTED_CHANNEL: "dev",
        }),
      }),
    );
  ...
  it("keeps downgrade post-update work in the current process", async () => {
    const downgradedRoot = createCaseDir("brikko-studio-downgraded-root");
    setupUpdatedRootRefresh({
  ...
  it("post-core resume mode skips the core update and only runs post-update tasks", async () => {
    await withEnvAsync(
      {
        BRIKKO_STUDIO_UPDATE_POST_CORE: "1",
        BRIKKO_STUDIO_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
  ...
  it("uses a fail-closed integrity policy for post-core plugin updates", async () => {
    ...
    await expect(
      onIntegrityDrift({
        pluginId: "demo",
        spec: "@brikko-studio/demo@1.0.0",
        resolvedSpec: "@brikko-studio/demo@1.0.0",
          ...
          outcomes: [
            {
              ...
              message:
                proceed === false
                  ? "Failed to update demo: aborted: npm package integrity drift detected for @brikko-studio/demo@1.0.0"
  ...
  it("preserves fresh-process plugin failure details in parent json output", async () => {
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      queueMicrotask(async () => {
        const resultPath = env?.BRIKKO_STUDIO_UPDATE_POST_CORE_RESULT_PATH;
        if (resultPath) {
          await fs.writeFile(
            resultPath,
            JSON.stringify({
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "brikko-studio", "update", "status", "--json"]);
  ...
  it.each([
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "brikko-studio@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
  ...
  it("warns but still runs package updates when disk space looks low", async () => {
    ...
    expectPackageInstallSpec("brikko-studio@latest");
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
    expect(
      vi
  ...
  it("allows package updates from inherited gateway service env when the managed gateway is not running", async () => {
    mockPackageInstallStatus(createCaseDir("brikko-studio-update"));
  ...
  it("refuses package updates from inherited gateway service env when --no-restart leaves the gateway running", async () => {
    ...
    serviceReadCommand.mockResolvedValue({
      programArguments: ["brikko-studio", "gateway", "run"],
      environment: {
        BRIKKO_STUDIO_SERVICE_MARKER: "brikko-studio",
        BRIKKO_STUDIO_SERVICE_KIND: "gateway",
      },
  ...
  it("refuses package updates from inside the active gateway process tree", async () => {
    ...
    expect(errors.join("\n")).toContain(
      "brikko-studio update detected it is running inside the gateway process tree.",
    );
  ...
  it("blocks package updates when the target requires a newer Node runtime", async () => {
    ...
    expect(errors.join("\n")).toContain("Node ");
    expect(errors.join("\n")).toContain(
      "Bare `npm i -g brikko-studio` can silently install an older compatible release.",
    );
  });
  ...
  it.each([
    {
      ...
      expectedSpec: "brikko-studio@next",
    },
    {
      name: "main shorthand",
      run: async () => {
      ...
      expectedSpec: "github:brikko-studio/brikko-studio#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("brikko-studio-update"));
        await updateCommand({ yes: true, tag: "github:brikko-studio/brikko-studio#main" });
      },
      expectedSpec: "github:brikko-studio/brikko-studio#main",
    },
    {
      name: "BRIKKO_STUDIO_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("brikko-studio-update"));
        await withEnvAsync(
          { BRIKKO_STUDIO_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/brikko-studio-next.tgz" },
          async () => {
            await updateCommand({ yes: true, tag: "latest" });
          },
        );
      },
      expectedSpec: "http://10.211.55.2:8138/brikko-studio-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "brikko-studio");
    mockPackageInstallStatus(tempDir);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "brikko-studio", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("stops package post-update work when staged npm install verification fails", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-update-staged-fail-"));
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "brikko-studio", version: "2026.4.20" }),
      "utf-8",
    );
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (
        ...
        const stageRoot = path.join(stagePrefix, "lib", "node_modules", "brikko-studio");
        await fs.mkdir(path.join(stageRoot, "dist"), { recursive: true });
        await fs.writeFile(
          path.join(stageRoot, "package.json"),
          JSON.stringify({ name: "brikko-studio", version: "2026.4.25" }),
          "utf-8",
        );
  ...
  it("marks package post-update doctor as update-in-progress", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-update-package-"));
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "brikko-studio");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "brikko-studio", version: "2026.4.21" }),
      "utf-8",
    );
  ...
  it("stops a running managed gateway before package replacement", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-update-stop-service-"));
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "brikko-studio");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
  ...
  it("refreshes package installs even when the current version already matches the target", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-update-current-"));
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "brikko-studio");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "brikko-studio", version: "2026.4.23" }),
      "utf-8",
    );
    await fs.writeFile(entryPath, "export {};\n", "utf-8");
    for (const relativePath of TEST_BUNDLED_RUNTIME_SIDECAR_PATHS) {
  ...
  it("retries package updates without optional deps when npm global update fails", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-update-optional-"));
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "brikko-studio");
    mockPackageInstallStatus(pkgRoot);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "brikko-studio", version: "1.0.0" }),
      "utf-8",
    );
    ...
    const installCall = vi
      ...
      .mock.calls.find(
        ([argv]) =>
          ...
          argv[1] === "i" &&
          argv[2] === "-g" &&
          argv.includes("brikko-studio@latest"),
      );
  ...
  it("persists channel and runs post-update work after switching from package to git", async () => {
    const tempDir = createCaseDir("brikko-studio-update");
    const gitRoot = path.join(tempDir, "..", "brikko-studio");
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    expect(logs.join("\n")).toContain(
      "Commit, stash, or discard the local changes, then rerun `brikko-studio update`.",
    );
  ...
  it("fails a JSON package update when fallback restart leaves the old gateway running", async () => {
    const updatedRoot = createCaseDir("brikko-studio-updated-root");
    ...
    },
    {
        ...
        await withEnvAsync(
          {
            ...
            BRIKKO_STUDIO_CONFIG_PATH: "./config/brikko-studio.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          cwd: root,
          env: expect.objectContaining({
            BRIKKO_STUDIO_STATE_DIR: path.resolve("./state"),
            BRIKKO_STUDIO_CONFIG_PATH: path.resolve("./config/brikko-studio.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            BRIKKO_STUDIO_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ BRIKKO_STUDIO_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.BRIKKO_STUDIO_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("brikko-studio-update-wizard");
    await withEnvAsync({ BRIKKO_STUDIO_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/brikko-studio as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    await withEnvAsync({ BRIKKO_STUDIO_GIT_DIR: undefined }, async () => {
      expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "brikko-studio"));
    });
