...
describe("persistPluginInstall", () => {
  ...
  it("adds installed plugins to restrictive allowlists before enabling", async () => {
    ...
    enablePluginInConfig.mockImplementation((...args: unknown[]) => {
      const [cfg, pluginId] = args as [Brikko StudioConfig, string];
  ...
  it("warns when an installed npm plugin remains shadowed by a config-selected source", async () => {
    ...
    const next = await persistPluginInstall({
      ...
      install: {
        ...
        installPath: "/tmp/brikko-studio/npm/node_modules/@brikko-studio/discord/index.ts",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "active config source: /tmp/brikko-studio-upstream/extensions/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain(
      "installed npm source: /tmp/brikko-studio/npm/node_modules/@brikko-studio/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain("brikko-studio plugins doctor");
  });
  ...
  it("does not warn when the config-selected source is inside the npm install path", async () => {
      ...
      plugins: [
        {
          id: "discord",
          origin: "config",
          source: "/tmp/brikko-studio/npm/node_modules/@brikko-studio/discord/dist/index.js",
          status: "loaded",
        },
      ],
      diagnostics: [],
    ...
    await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@brikko-studio/discord",
        installPath: "/tmp/brikko-studio/npm/node_modules/@brikko-studio/discord",
      },
    });
