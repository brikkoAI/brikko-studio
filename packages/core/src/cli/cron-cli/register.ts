...
export function registerCronCli(program: Command) {
  const cron = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/cron", "docs.brikko-studio.ai/cli/cron")}\n${theme.muted("Upgrade tip:")} run \`brikko-studio doctor --fix\` to normalize legacy cron job storage.\n`,
    );
