...
export function shouldUseRootHelpFastPath(
  ...
  const invocation = resolveCliArgvInvocation(argv);
  return (
    env.BRIKKO_STUDIO_DISABLE_CLI_STARTUP_HELP_FAST_PATH !== "1" &&
...
export function shouldUseBrowserHelpFastPath(
  ...
  if (env.BRIKKO_STUDIO_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1") {
    return false;
  }
  const invocation = resolveCliArgvInvocation(argv);
  return (
...
export function resolveMissingPluginCommandMessage(
  ...
  if (parentPluginId) {
    ...
    if (config?.plugins?.entries?.[parentPluginId]?.enabled === false) {
      return (
        `The \`brikko-studio ${normalizedPluginId}\` command is unavailable because ` +
    ...
    if (commandAlias.kind === "runtime-slash") {
      const cliHint = commandAlias.cliCommand
        ? `Use \`brikko-studio ${commandAlias.cliCommand}\` for related CLI operations, or `
        : "Use ";
      return (
