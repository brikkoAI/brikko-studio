...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "brikko-studio", "gateway"] },
    { argv: ["node", "brikko-studio", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "brikko-studio", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: ["node", "brikko-studio", "--profile", "server", "gateway", "run", "--allow-unconfigured"],
    },
  ...
  it.each([
    ...
    { argv: ["node", "brikko-studio", "gateway", "status"] },
    { argv: ["node", "brikko-studio", "gateway", "call", "health"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "brikko-studio.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("brikko-studio.internal");
    } finally {
      log.mockRestore();
    }
  });
