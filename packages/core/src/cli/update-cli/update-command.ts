...
const POST_CORE_UPDATE_ENV = "BRIKKO_STUDIO_UPDATE_POST_CORE";
const POST_CORE_UPDATE_CHANNEL_ENV = "BRIKKO_STUDIO_UPDATE_POST_CORE_CHANNEL";
const POST_CORE_UPDATE_REQUESTED_CHANNEL_ENV = "BRIKKO_STUDIO_UPDATE_POST_CORE_REQUESTED_CHANNEL";
const POST_CORE_UPDATE_RESULT_PATH_ENV = "BRIKKO_STUDIO_UPDATE_POST_CORE_RESULT_PATH";
const UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE_ENV =
  "BRIKKO_STUDIO_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE";
...
function formatGatewayAncestryBlockMessage(pid: number): string {
  return `brikko-studio update detected it is running inside the gateway process tree.
...
function isRunningInsideGatewayService(
  ...
  if (env.BRIKKO_STUDIO_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER) {
    return false;
  }
  const serviceKind = env.BRIKKO_STUDIO_SERVICE_KIND?.trim();
...
async function resolvePackageRuntimePreflightError(params: {
  ...
  return [
    `Node ${process.versions.node ?? "unknown"} is too old for brikko-studio@${targetLabel}.`,
    `The requested package requires ${status.nodeEngine}.`,
    "Upgrade Node to 22.14+ or Node 24, then rerun `brikko-studio update`.",
    "Bare `npm i -g brikko-studio` can silently install an older compatible release.",
    "After upgrading Node, use `npm i -g brikko-studio@latest`.",
  ].join("\n");
}
...
function stripGatewayServiceMarkerEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  delete resolvedEnv.BRIKKO_STUDIO_SERVICE_MARKER;
  delete resolvedEnv.BRIKKO_STUDIO_SERVICE_KIND;
  return resolvedEnv;
}
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("brikko-studio completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
async function maybeRestartService(params: {
  ...
  const verifyRestartedGateway = async (expectedGatewayVersion: string | undefined) => {
    ...
    const diagnosticLines = [
      ...
      `Run \`${replaceCliName(formatCliCommand("brikko-studio gateway status --deep"), CLI_NAME)}\` for details.`,
    ];
    if (params.opts.json) {
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("brikko-studio gateway restart"), CLI_NAME)}`,
          ),
        );
      }
      if (isPackageManagerUpdateMode(params.result.mode)) {
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("brikko-studio doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("brikko-studio gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("brikko-studio gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
async function continuePostCoreUpdateInFreshProcess(params: {
  ...
  const resultDir =
    params.opts.json === true
      ? await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-update-post-core-"))
      : null;
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  const devTargetRef =
    channel === "dev" ? process.env.BRIKKO_STUDIO_UPDATE_DEV_TARGET_REF?.trim() || undefined : undefined;
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "dirty") {
      ...
      defaultRuntime.log(
        theme.muted("Commit, stash, or discard the local changes, then rerun `brikko-studio update`."),
      );
    }
    if (result.reason === "not-git-install") {
      ...
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g brikko-studio@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g brikko-studio@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
