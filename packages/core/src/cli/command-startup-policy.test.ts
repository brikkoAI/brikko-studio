...
describe("command-startup-policy", () => {
  ...
  it("matches plugin preload policy", () => {
    ...
    expect(
      shouldLoadPluginsForCommandPath({
        argv: ["node", "brikko-studio", "agent", "--json"],
        commandPath: ["agent"],
        jsonOutputMode: true,
      }),
    ).toBe(false);
    expect(
      shouldLoadPluginsForCommandPath({
        argv: ["node", "brikko-studio", "agent", "--json", "--local"],
        commandPath: ["agent"],
        jsonOutputMode: true,
      }),
    ).toBe(true);
  ...
  it("uses process env banner suppression when startup env is omitted", () => {
    const originalHideBanner = process.env.BRIKKO_STUDIO_HIDE_BANNER;
    try {
