...
describe("plugins search command", () => {
  ...
  it("searches ClawHub code and bundle plugin families", async () => {
      ...
      .mockResolvedValueOnce([
        {
          score: 10,
          package: {
            name: "brikko-studio-calendar-bundle",
    ...
    expect(mocks.logs.join("\n")).toContain("brikko-studio-calendar");
    expect(mocks.logs.join("\n")).toContain(
      "Install: brikko-studio plugins install clawhub:brikko-studio-calendar",
    );
  });
