...
const logPathTracker = createSuiteLogPathTracker("brikko-studio-test-env-log-level-");

describe("BRIKKO_STUDIO_LOG_LEVEL", () => {
  ...
  beforeEach(() => {
    originalEnv = process.env.BRIKKO_STUDIO_LOG_LEVEL;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.BRIKKO_STUDIO_LOG_LEVEL = originalEnv;
    }
    loggingState.invalidEnvLogLevelValue = null;
    resetLogger();
    setLoggerOverride(null);
  ...
  it("applies a valid env override to both file and console levels", () => {
    ...
    process.env.BRIKKO_STUDIO_LOG_LEVEL = "debug";
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.BRIKKO_STUDIO_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("BRIKKO_STUDIO_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid BRIKKO_STUDIO_LOG_LEVEL="nope"');
  });
});
