...
const SUPPORT_EXPORT_PREFIX = "brikko-studio-diagnostics-";
...
function readStabilityBundle(
  ...
  if (target === false) {
    return { status: "missing", dir: "$BRIKKO_STUDIO_STATE_DIR/logs/stability" };
  }
...
function renderSummary(params: {
  ...
  return [
    ...
    "- `logs/brikko-studio-sanitized.jsonl`: sanitized log summaries and metadata",
