...
describe("diagnostic support export", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-support-export-"));
  ...
  it("writes a shareable zip without raw chats, webhook bodies, or secrets", async () => {
    ...
    const logTail: LogTailPayload = {
      file: path.join(tempDir, "logs", "brikko-studio.log"),
      cursor: 200,
      size: 200,
      truncated: false,
      reset: false,
    ...
    const result = await writeDiagnosticSupportExport({
      env: {
        ...process.env,
        HOME: tempDir,
        BRIKKO_STUDIO_STATE_DIR: tempDir,
      },
      stateDir: tempDir,
      outputPath,
      ...
      readStatusSnapshot: async () => ({
        service: {
          loaded: true,
          command: {
            programArguments: ["brikko-studio", "gateway", "run", "--token", fakeToken],
            environment: {
              HOME: tempDir,
              BRIKKO_STUDIO_GATEWAY_TOKEN: fakeToken,
            },
          },
        },
        gateway: {
    ...
    expect(Object.keys(entries).toSorted()).toEqual([
      ...
      "logs/brikko-studio-sanitized.jsonl",
      "manifest.json",
    ...
    expect(combined).toContain("$BRIKKO_STUDIO_STATE_DIR");
    ...
    const sanitizedLogs = entries["logs/brikko-studio-sanitized.jsonl"];
    ...
    expect(status.data?.service?.command?.environment?.BRIKKO_STUDIO_GATEWAY_TOKEN).toBe("<redacted>");
  ...
  it("redacts Windows USERPROFILE paths when HOME is unset", () => {
    ...
    const stateDir = `${userProfile}\\AppData\\Roaming\\brikko-studio`;
    const redaction = {
      env: {
    ...
    expect(redactSupportString(`${stateDir}\\logs\\gateway.log`, redaction)).toBe(
      "$BRIKKO_STUDIO_STATE_DIR\\logs\\gateway.log",
    );
    expect(
    ...
    const status = sanitizeSupportSnapshotValue(
      {
        service: {
          command: {
            programArguments: [
              "node",
              `${userProfile}\\brikko-studio\\dist\\index.js`,
              "--config",
              `${stateDir}\\brikko-studio.json`,
            ],
            sourcePath: "c:\\users\\support-user\\AppData\\Local\\brikko-studio\\gateway-service.json",
          },
        },
      },
      redaction,
    ...
    expect(serialized).toContain("~\\\\brikko-studio\\\\dist\\\\index.js");
    expect(serialized).toContain("$BRIKKO_STUDIO_STATE_DIR\\\\brikko-studio.json");
    expect(serialized).toContain("~\\\\AppData\\\\Local\\\\brikko-studio\\\\gateway-service.json");
  });
  ...
  it("keeps writing when log tail collection fails", async () => {
    ...
    await writeDiagnosticSupportExport({
      ...
      readLogTail: async () => {
        throw new Error(`log tail failed at ${tempDir}/brikko-studio.log with token ${fakeToken}`);
      },
    });

    const entries = await readZipTextEntries(outputPath);
    expect(Object.keys(entries).toSorted()).toContain("logs/brikko-studio-sanitized.jsonl");
