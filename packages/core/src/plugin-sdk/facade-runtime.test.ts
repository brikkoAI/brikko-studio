...
const originalBundledPluginsDir = process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR;
...
function writePluginPackageJson(pluginDir: string, name = "demo"): void {
  writeJsonFile(path.join(pluginDir, "package.json"), {
    name: `@brikko-studio/plugin-${name}`,
    version: "0.0.0",
    type: "module",
  });
}
...
function useBundledPluginDirOverrideForTest(dir: string): void {
  process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR = dir;
...
afterEach(() => {
  ...
  if (originalBundledPluginsDir === undefined) {
    ...
    process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR = originalBundledPluginsDir;
  }
...
describe("plugin-sdk facade runtime", () => {
  it("honors trusted bundled plugin dir overrides", () => {
    const overrideA = createBundledPluginDir("brikko-studio-facade-runtime-a-", "override-a");
    const overrideB = createBundledPluginDir("brikko-studio-facade-runtime-b-", "override-b");
  ...
  it("falls back to package source surfaces when an override dir is partial", () => {
    const overrideDir = createTrustedBundledFixtureRoot("brikko-studio-facade-runtime-empty-");
  ...
  it("returns the same object identity on repeated calls (sentinel consistency)", () => {
    const dir = createBundledPluginDir("brikko-studio-facade-identity-", "identity-check");
  ...
  it("breaks circular facade re-entry during module evaluation", () => {
    const dir = createBundledPluginDir("brikko-studio-facade-circular-", "circular-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("back-fills the sentinel before post-load facade tracking re-enters", () => {
    const dir = createBundledPluginDir("brikko-studio-facade-post-load-", "post-load-ok");
    const location = {
      modulePath: path.join(dir, "demo", "api.js"),
      boundaryRoot: dir,
    };
  ...
  it("clears the cache on load failure so retries re-execute", () => {
    const dir = createThrowingPluginDir("brikko-studio-facade-throw-");
    useBundledPluginDirOverrideForTest(dir);
  ...
  it("allows runtime-api facade loads when the bundled plugin is explicitly enabled", () => {
    const dir = createTempDirSync("brikko-studio-facade-runtime-enabled-");
  ...
  it("resolves a globally-installed plugin whose rootDir basename matches the dirName", () => {
    const lineDir = createTempDirSync("brikko-studio-facade-global-line-");
    ...
    fs.writeFileSync(
      path.join(lineDir, "brikko-studio.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
  ...
  it("resolves a globally-installed plugin with an encoded scoped rootDir basename", () => {
    const encodedDir = createTempDirSync("brikko-studio-facade-encoded-line-");
    ...
    fs.writeFileSync(
      path.join(encodedDir, "brikko-studio.plugin.json"),
      JSON.stringify({
        id: "line",
        channels: ["line"],
        configSchema: { type: "object", additionalProperties: false, properties: {} },
  ...
  it("prefers the source runtime snapshot for facade activation checks", () => {
    const dir = createTempDirSync("brikko-studio-facade-source-snapshot-");
    ...
    fs.writeFileSync(
      path.join(dir, "demo", "brikko-studio.plugin.json"),
      JSON.stringify({
        id: "demo",
      }),
      "utf8",
