...
describe("defineBundledChannelEntry", () => {
  it("runs tool registrations without channel sidecar hydration during tool discovery", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-bundled-entry-tools-"));
  ...
  it("loads runtime sidecars during discovery registration", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-bundled-entry-runtime-"));
    tempDirs.push(tempRoot);
    const runtimeMarker = path.join(tempRoot, "runtime-loaded");
...
async function expectBuiltArtifactNodeRequireFastPath(
  ...
  vi.stubEnv("BRIKKO_STUDIO_PLUGIN_LOAD_PROFILE", "1");
  ...
  try {
    ...
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-channel-entry-contract-"));
    tempDirs.push(tempRoot);
...
describe("loadBundledEntryExportSync", () => {
  ...
  it("normalizes Windows absolute sidecar paths before module loads them", async () => {
    ...
    vi.doMock("../infra/boundary-file-read.js", () => ({
      openBoundaryFileSync: () => ({
        ok: true,
        path: "C:\\Users\\alice\\brikko-studio\\dist\\extensions\\feishu\\helper.ts",
    ...
    try {
      ...
      expect(
        channelEntryContract.loadBundledEntryExportSync<number>(
          "file:///C:/Users/alice/brikko-studio/dist/extensions/feishu/index.js",
          {
            specifier: "./helper.ts",
            exportName: "load",
          },
      ...
      expect(jitiLoad).toHaveBeenCalledWith(
        "file:///C:/Users/alice/brikko-studio/dist/extensions/feishu/helper.ts",
      );
    } finally {
      platformSpy.mockRestore();
  ...
  it("can disable source-tree fallback for dist bundled entry checks", () => {
    ...
    fs.writeFileSync(path.join(tempRoot, "package.json"), '{"name":"brikko-studio"}\n', "utf8");
    const pluginRoot = path.join(tempRoot, "dist", "extensions", "telegram");
    ...
    vi.stubEnv("BRIKKO_STUDIO_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK", "1");
