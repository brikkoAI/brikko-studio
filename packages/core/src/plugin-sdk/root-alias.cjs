...
const pluginSdkPackageNames = ["brikko-studio/plugin-sdk", "@brikko-studio/plugin-sdk"];
...
const shouldPreferSourceGraph =
  ...
  (process.env.NODE_ENV !== "production" ||
    ...
    process.env.BRIKKO_STUDIO_PLUGIN_SDK_SOURCE_IN_TESTS === "1");
...
function onDiagnosticEvent(listener) {
  ...
  if (!diagnosticEvents || typeof diagnosticEvents.onDiagnosticEvent !== "function") {
    throw new Error("brikko-studio/plugin-sdk root alias could not resolve onDiagnosticEvent");
  }
...
function listPluginSdkExportedSubpaths() {
  ...
  const cacheKey = `${packageRoot}::privateQa=${process.env.BRIKKO_STUDIO_ENABLE_PRIVATE_QA_CLI === "1" ? "1" : "0"}`;
...
function listPrivateLocalOnlyPluginSdkSubpaths() {
  if (process.env.BRIKKO_STUDIO_ENABLE_PRIVATE_QA_CLI !== "1") {
    return [];
  }
  try {
...
function getModuleLoader(tryNative) {
  ...
  const moduleLoader = createJiti(__filename, {
    ...
    // so local plugins do not create a second transpiled Brikko Studio core graph.
