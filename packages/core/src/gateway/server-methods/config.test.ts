...
describe("resolveConfigOpenCommand", () => {
  it("uses open on macOS", () => {
    expect(resolveConfigOpenCommand("/tmp/brikko-studio.json", "darwin")).toEqual({
      command: "open",
      args: ["/tmp/brikko-studio.json"],
    });
  });

  it("uses xdg-open on Linux", () => {
    expect(resolveConfigOpenCommand("/tmp/brikko-studio.json", "linux")).toEqual({
      command: "xdg-open",
      args: ["/tmp/brikko-studio.json"],
    });
  });
...
describe("config.openFile", () => {
  ...
  it("opens the configured file without shell interpolation", async () => {
    process.env.BRIKKO_STUDIO_CONFIG_PATH = "/tmp/config $(touch pwned).json";
  ...
  it("returns a generic error and logs details when the opener fails", async () => {
    process.env.BRIKKO_STUDIO_CONFIG_PATH = "/tmp/config.json";
    execFileMock.mockImplementation((...args: unknown[]) => {
