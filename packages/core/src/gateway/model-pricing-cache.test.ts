...
const pluginManifestRegistryMocks = vi.hoisted(() => ({
  ...
  listBrikkoStudioPluginManifestMetadata: vi.fn(),
}));
...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    listBrikkoStudioPluginManifestMetadata: (
      params?: Parameters<typeof actual.listBrikkoStudioPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listBrikkoStudioPluginManifestMetadata(params);
      return actual.listBrikkoStudioPluginManifestMetadata(params);
    },
  };
});
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listBrikkoStudioPluginManifestMetadata.mockClear();
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as BrikkoStudioConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(pluginManifestRegistryMocks.listBrikkoStudioPluginManifestMetadata).not.toHaveBeenCalled();
