...
import { clearActivatedPluginRuntimeState, loadBrikko StudioPlugins } from "../plugins/loader.js";
...
const FALLBACK_GATEWAY_CONTEXT_STATE_KEY: unique symbol = Symbol.for(
  "brikko-studio.fallbackGatewayContextState",
);
...
const PLUGIN_SUBAGENT_POLICY_STATE_KEY: unique symbol = Symbol.for(
  "brikko-studio.pluginSubagentOverridePolicyState",
);
...
export function setPluginSubagentOverridePolicies(cfg: Brikko StudioConfig): void {
  ...
  if (!policy?.allowModelOverride) {
    return {
      ...
      reason:
        ...
        "See https://docs.brikko-studio.ai/tools/plugin#runtime-helpers and search for: " +
...
export function loadGatewayPlugins(params: {
  ...
  const pluginRegistry = loadBrikko StudioPlugins({
    config: resolvedConfig,
