  ...
  it("passes session, account, and message channel headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-brikko-studio-account-id": "work",
        "x-brikko-studio-message-channel": "telegram",
      },
    ...
    const sendToolsList = async (senderIsOwner: "true" | "false") =>
      await sendRaw({
        ...
        headers: {
          ...
          "x-brikko-studio-message-channel": "matrix",
        },
        body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
      });
  ...
  it("ignores spoofed owner headers when the bearer token is non-owner scoped", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-session-key": "agent:main:matrix:dm:test",
        "x-brikko-studio-message-channel": "matrix",
        "x-brikko-studio-sender-is-owner": "true",
      },
      body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
    });
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.brikko-studio?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.brikko-studio?.headers?.Authorization).toBe(
      "Bearer ${BRIKKO_STUDIO_MCP_TOKEN}",
    );
    expect(config.mcpServers?.brikko-studio?.headers?.["x-brikko-studio-message-channel"]).toBe(
      "${BRIKKO_STUDIO_MCP_MESSAGE_CHANNEL}",
    );
    expect(config.mcpServers?.brikko-studio?.headers?.["x-brikko-studio-sender-is-owner"]).toBeUndefined();
  });
});
