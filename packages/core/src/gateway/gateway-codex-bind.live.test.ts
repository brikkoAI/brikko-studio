...
const CODEX_BIND_LIVE = isTruthyEnvValue(process.env.BRIKKO_STUDIO_LIVE_CODEX_BIND);
...
async function writePluginBindingApproval(params: {
  ...
  const brikko-studioDir = path.join(params.homeDir, ".brikko-studio");
  await fs.mkdir(brikko-studioDir, { recursive: true });
  await fs.writeFile(
    path.join(brikko-studioDir, "plugin-binding-approvals.json"),
    `${JSON.stringify(
      {
        version: 1,
        approvals: [
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        gatewayToken: process.env.BRIKKO_STUDIO_GATEWAY_TOKEN,
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-live-codex-bind-"));
      const tempHome = path.join(tempRoot, "home");
      const stateDir = path.join(tempRoot, "state");
      const workspace = path.join(tempRoot, "workspace");
      const configPath = path.join(tempRoot, "brikko-studio.json");
      ...
      const bindModel =
        process.env.BRIKKO_STUDIO_LIVE_CODEX_BIND_MODEL?.trim() || DEFAULT_CODEX_BIND_MODEL;
      ...
      try {
        ...
        restoreEnvVar("BRIKKO_STUDIO_CONFIG_PATH", previous.configPath);
        restoreEnvVar("BRIKKO_STUDIO_GATEWAY_TOKEN", previous.gatewayToken);
        restoreEnvVar("HOME", previous.home);
        restoreEnvVar("BRIKKO_STUDIO_SKIP_CANVAS_HOST", previous.skipCanvas);
        restoreEnvVar("BRIKKO_STUDIO_SKIP_CHANNELS", previous.skipChannels);
        restoreEnvVar("BRIKKO_STUDIO_SKIP_CRON", previous.skipCron);
        restoreEnvVar("BRIKKO_STUDIO_SKIP_GMAIL_WATCHER", previous.skipGmail);
        restoreEnvVar("BRIKKO_STUDIO_STATE_DIR", previous.stateDir);
      }
    },
