...
vi.mock("../infra/brikko-studio-root.js", async (importOriginal) => {
...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    prevSkipChannels = process.env.BRIKKO_STUDIO_SKIP_CHANNELS;
    prevSkipGmail = process.env.BRIKKO_STUDIO_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.BRIKKO_STUDIO_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.BRIKKO_STUDIO_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.BRIKKO_STUDIO_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.BRIKKO_STUDIO_SKIP_GMAIL_WATCHER;
    } else {
      process.env.BRIKKO_STUDIO_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.BRIKKO_STUDIO_SKIP_PROVIDERS;
    } else {
      process.env.BRIKKO_STUDIO_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    ...
    if (!configPath) {
      throw new Error("BRIKKO_STUDIO_CONFIG_PATH is not set");
    }
  ...
  async function withNonMinimalGatewayServer(
    ...
    return await withEnvAsync({ BRIKKO_STUDIO_TEST_MINIMAL_GATEWAY: undefined }, async () =>
  ...
  it("keeps last-known-good auth snapshot active when gateway auth token exec reload fails", async () => {
    ...
    if (!stateDir) {
      throw new Error("BRIKKO_STUDIO_STATE_DIR is not set");
    }
...
`,
    ...
    const previousGatewayTokenEnv = process.env.BRIKKO_STUDIO_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.BRIKKO_STUDIO_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
