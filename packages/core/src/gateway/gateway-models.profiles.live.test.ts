...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.BRIKKO_STUDIO_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.BRIKKO_STUDIO_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.BRIKKO_STUDIO_LIVE_GATEWAY_SMOKE);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.BRIKKO_STUDIO_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_MODEL_TIMEOUT_MS = resolveGatewayLiveModelTimeoutMs();
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.BRIKKO_STUDIO_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.BRIKKO_STUDIO_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["BRIKKO_STUDIO_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.BRIKKO_STUDIO_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.BRIKKO_STUDIO_LIVE_GATEWAY_MODELS?.trim();
  const useExplicitModels = Boolean(rawModels) && rawModels !== "modern" && rawModels !== "all";
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.BRIKKO_STUDIO_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.BRIKKO_STUDIO_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.BRIKKO_STUDIO_LIVE_MODEL_TIMEOUT_MS,
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.BRIKKO_STUDIO_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.BRIKKO_STUDIO_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.BRIKKO_STUDIO_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("BRIKKO_STUDIO_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("BRIKKO_STUDIO_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("BRIKKO_STUDIO_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.BRIKKO_STUDIO_LIVE_GATEWAY_MODELS;
    delete process.env.BRIKKO_STUDIO_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.BRIKKO_STUDIO_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.BRIKKO_STUDIO_LIVE_GATEWAY_MODELS = "openai/gpt-5.5,anthropic/claude-opus-4-6";
    delete process.env.BRIKKO_STUDIO_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.BRIKKO_STUDIO_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.BRIKKO_STUDIO_LIVE_GATEWAY_MAX_MODELS = "2";
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    configPath: process.env.BRIKKO_STUDIO_CONFIG_PATH,
    token: process.env.BRIKKO_STUDIO_GATEWAY_TOKEN,
    skipChannels: process.env.BRIKKO_STUDIO_SKIP_CHANNELS,
    skipGmail: process.env.BRIKKO_STUDIO_SKIP_GMAIL_WATCHER,
    skipCron: process.env.BRIKKO_STUDIO_SKIP_CRON,
    skipCanvas: process.env.BRIKKO_STUDIO_SKIP_CANVAS_HOST,
    disableBonjour: process.env.BRIKKO_STUDIO_DISABLE_BONJOUR,
    logLevel: process.env.BRIKKO_STUDIO_LOG_LEVEL,
    agentDir: process.env.BRIKKO_STUDIO_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.BRIKKO_STUDIO_STATE_DIR,
  };
  ...
  if (QUIET_LIVE_LOGS) {
    process.env.BRIKKO_STUDIO_DISABLE_BONJOUR = "1";
    process.env.BRIKKO_STUDIO_LOG_LEVEL = "silent";
  }
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-live-state-"));
  process.env.BRIKKO_STUDIO_STATE_DIR = tempStateDir;
  ...
  process.env.BRIKKO_STUDIO_AGENT_DIR = tempAgentDir;
  process.env.PI_CODING_AGENT_DIR = tempAgentDir;

  const workspaceDir = resolveAgentWorkspaceDir(params.cfg, agentId);
  await fs.mkdir(workspaceDir, { recursive: true });
  await fs.mkdir(path.join(workspaceDir, ".brikko-studio"), { recursive: true });
  await fs.writeFile(
    path.join(workspaceDir, ".brikko-studio", "workspace-state.json"),
    `${JSON.stringify(
      {
        version: 1,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-live-"));
  const tempConfigPath = path.join(tempDir, "brikko-studio.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.BRIKKO_STUDIO_CONFIG_PATH = tempConfigPath;
  ...
  try {
    ...
    process.env.BRIKKO_STUDIO_CONFIG_PATH = previous.configPath;
    process.env.BRIKKO_STUDIO_GATEWAY_TOKEN = previous.token;
    process.env.BRIKKO_STUDIO_SKIP_CHANNELS = previous.skipChannels;
    process.env.BRIKKO_STUDIO_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.BRIKKO_STUDIO_SKIP_CRON = previous.skipCron;
    process.env.BRIKKO_STUDIO_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.BRIKKO_STUDIO_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.BRIKKO_STUDIO_LOG_LEVEL = previous.logLevel;
    process.env.BRIKKO_STUDIO_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.BRIKKO_STUDIO_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via BRIKKO_STUDIO_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.brikko-studio-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
