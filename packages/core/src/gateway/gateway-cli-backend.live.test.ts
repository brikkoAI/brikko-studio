...
const CLI_LIVE = isTruthyEnvValue(process.env.BRIKKO_STUDIO_LIVE_CLI_BACKEND);
const CLI_RESUME = isTruthyEnvValue(process.env.BRIKKO_STUDIO_LIVE_CLI_BACKEND_RESUME_PROBE);
const CLI_DEBUG = isTruthyEnvValue(process.env.BRIKKO_STUDIO_LIVE_CLI_BACKEND_DEBUG);
const CLI_CI_SAFE_CODEX_CONFIG = isTruthyEnvValue(
  process.env.BRIKKO_STUDIO_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG,
);
const CLI_MCP_SCHEMA_PROBE = isTruthyEnvValue(
  process.env.BRIKKO_STUDIO_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE,
);
...
const CLI_BACKEND_REQUEST_TIMEOUT_MS = parsePositiveIntegerEnv(
  "BRIKKO_STUDIO_LIVE_CLI_BACKEND_REQUEST_TIMEOUT_MS",
  15 * 60_000,
);
  ...
  it(
    ...
    async () => {
      const preservedEnv = new Set(
        parseJsonStringArray(
          "BRIKKO_STUDIO_LIVE_CLI_BACKEND_PRESERVE_ENV",
          process.env.BRIKKO_STUDIO_LIVE_CLI_BACKEND_PRESERVE_ENV,
        ) ?? [],
      );
      ...
      const rawModel = process.env.BRIKKO_STUDIO_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
      ...
      const cliCommand = process.env.BRIKKO_STUDIO_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
      if (!cliCommand) {
        throw new Error(
          `BRIKKO_STUDIO_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
        );
      }
      ...
      const cliClearEnv =
        parseJsonStringArray(
          "BRIKKO_STUDIO_LIVE_CLI_BACKEND_CLEAR_ENV",
          process.env.BRIKKO_STUDIO_LIVE_CLI_BACKEND_CLEAR_ENV,
        ) ??
      ...
      const cliImageArg =
        process.env.BRIKKO_STUDIO_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || providerDefaults?.imageArg;
      const cliImageMode =
        parseImageMode(process.env.BRIKKO_STUDIO_LIVE_CLI_BACKEND_IMAGE_MODE) ??
        providerDefaults?.imageMode;
      if (cliImageMode && !cliImageArg) {
        throw new Error(
          "BRIKKO_STUDIO_LIVE_CLI_BACKEND_IMAGE_MODE requires BRIKKO_STUDIO_LIVE_CLI_BACKEND_IMAGE_ARG.",
        );
      }

      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-live-cli-"));
      ...
      const disableMcpConfig = process.env.BRIKKO_STUDIO_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
