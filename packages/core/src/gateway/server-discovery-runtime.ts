...
function resolveDiscoveryAdvertiseTimeoutMs(env: NodeJS.ProcessEnv): number {
  const raw = env.BRIKKO_STUDIO_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS?.trim();
  if (!raw) {
...
export async function startGatewayDiscovery(params: {
  ...
  const localDiscoveryEnabled =
    mdnsMode !== "off" &&
    !isTruthyEnvValue(process.env.BRIKKO_STUDIO_DISABLE_BONJOUR) &&
  ...
  const tailnetDns = needsTailnetDns
    ...
    : undefined;
  const sshPortEnv = mdnsMinimal ? undefined : process.env.BRIKKO_STUDIO_SSH_PORT?.trim();
