...
const previousToolTimeoutMs = process.env.BRIKKO_STUDIO_VOICECLAW_REALTIME_TOOL_TIMEOUT_MS;
const previousMaxConcurrentTools = process.env.BRIKKO_STUDIO_VOICECLAW_REALTIME_MAX_CONCURRENT_TOOLS;

afterEach(() => {
  restoreEnv("BRIKKO_STUDIO_VOICECLAW_REALTIME_TOOL_TIMEOUT_MS", previousToolTimeoutMs);
  restoreEnv("BRIKKO_STUDIO_VOICECLAW_REALTIME_MAX_CONCURRENT_TOOLS", previousMaxConcurrentTools);
});
...
describe("VoiceClawRealtimeToolRuntime", () => {
  ...
  it("does not turn non-cooperative cancellations into timeout injections", async () => {
    process.env.BRIKKO_STUDIO_VOICECLAW_REALTIME_TOOL_TIMEOUT_MS = "10";
    const runtime = new VoiceClawRealtimeToolRuntime([
  ...
  it("frees the concurrency slot after a non-cooperative tool times out", async () => {
    process.env.BRIKKO_STUDIO_VOICECLAW_REALTIME_TOOL_TIMEOUT_MS = "10";
    process.env.BRIKKO_STUDIO_VOICECLAW_REALTIME_MAX_CONCURRENT_TOOLS = "1";
    const runtime = new VoiceClawRealtimeToolRuntime([
      makeTool("stuck", async () => await new Promise<never>(() => {})),
