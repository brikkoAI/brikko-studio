...
export class VoiceClawRealtimeSession {
  ...
  private readonly gatewayConfig: BrikkoStudioConfig;
  ...
  private async startSession(config: VoiceClawSessionConfigEvent): Promise<void> {
    ...
    if (!authResult.ok) {
      this.send({ type: "error", message: "BrikkoStudio gateway authentication failed", code: 401 });
    ...
    if (config.brainAgent !== "none" && this.auth.mode === "none" && !localDirect) {
      this.send({
        type: "error",
        message: "BrikkoStudio real-time brain requires gateway auth for non-local connections",
        code: 403,
      });
    ...
    if (config.brainAgent !== "none" && !senderIsOwner) {
      this.send({
        type: "error",
        message: "BrikkoStudio real-time brain requires owner-equivalent gateway auth",
        code: 403,
      });
