 ...
 *
 * Implements the OpenResponses `/v1/responses` endpoint for Brikko Studio Gateway.
 *
...
function createResponseSessionScope(params: {
  ...
  return normalizeResponseSessionScope({
    ...
    requestedSessionKey: getHeader(params.req, "x-brikko-studio-session-key"),
  });
}
      ...
      const content =
        Array.isArray(payloads) && payloads.length > 0
          ? payloads
              ...
              .filter(Boolean)
              .join("\n\n")
          : "No response from Brikko Studio.";
  ...
  unsubscribe = onAgentEvent((evt) => {
    ...
    if (evt.stream === "lifecycle") {
      ...
      if (phase === "end" || phase === "error") {
        const finalText = accumulatedText || "No response from Brikko Studio.";
