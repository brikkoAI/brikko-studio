  ...
  return [
    ...
    JSON.stringify({
      ...
      message: {
        role: "user",
        content:
          'Sender (untrusted metadata):\n```json\n{"label":"brikko-studio-control-ui"}\n```\n\n[Thu 2026-03-26 16:29 GMT] hi',
      },
    }),
    JSON.stringify({
      type: "assistant",
...
async function withClaudeProjectsDir<T>(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-claude-history-"));
...
describe("cli session history", () => {
  ...
  it("augments chat history when a session has a claude-cli binding", async () => {
    await withClaudeProjectsDir(async ({ homeDir, sessionId }) => {
      ...
      expect(messages[0]).toMatchObject({
        role: "user",
        __brikko-studio: { cliSessionId: sessionId },
      });
    });
  });
  ...
  it("augments anthropic-routed chat history when a Claude CLI binding has local messages", async () => {
    await withClaudeProjectsDir(async ({ homeDir, sessionId }) => {
      ...
      expect(messages).toContainEqual(
        expect.objectContaining({
          role: "user",
          __brikko-studio: expect.objectContaining({ cliSessionId: sessionId }),
        }),
      );
    });
  });
...
describe("readClaudeCliFallbackSeed", () => {
  ...
  beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-fallback-seed-"));
