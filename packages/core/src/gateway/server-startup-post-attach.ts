...
const SKIP_STARTUP_MODEL_PREWARM_ENV = "BRIKKO_STUDIO_SKIP_STARTUP_MODEL_PREWARM";
...
function resolveRestartSentinelPathFast(env: NodeJS.ProcessEnv = process.env): string {
  ...
  const resolveHome = () => {
    const explicitHome = normalizePathEnv(env.BRIKKO_STUDIO_HOME);
    if (explicitHome) {
  ...
  const override = normalizePathEnv(env.BRIKKO_STUDIO_STATE_DIR);
  if (override) {
    return path.join(resolveUserPath(override), RESTART_SENTINEL_FILENAME);
  }
  const home = resolveHome();
  const newStateDir = path.join(home, ".brikko-studio");
  if (env.BRIKKO_STUDIO_TEST_FAST === "1" || fs.existsSync(newStateDir)) {
  ...
  await measureStartup(params.startupTrace, "sidecars.channels", async () => {
    if (!skipChannels) {
      ...
      params.logChannels.info(
        "skipping channel start (BRIKKO_STUDIO_SKIP_CHANNELS=1 or BRIKKO_STUDIO_SKIP_PROVIDERS=1)",
      );
    }
  });
