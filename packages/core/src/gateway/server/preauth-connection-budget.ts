...
const UNKNOWN_CLIENT_IP_BUDGET_KEY = "__brikko-studio_unknown_client_ip__";

function getMaxPreauthConnectionsPerIpFromEnv(env: NodeJS.ProcessEnv = process.env): number {
  const configured =
    env.BRIKKO_STUDIO_MAX_PREAUTH_CONNECTIONS_PER_IP ||
    (env.VITEST && env.BRIKKO_STUDIO_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP);
  if (!configured) {
