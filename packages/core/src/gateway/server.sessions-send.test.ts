...
const { createBrikko StudioTools } = await import("../agents/brikko-studio-tools.js");
...
type SessionSendTool = ReturnType<typeof createBrikko StudioTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createBrikko StudioTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["BRIKKO_STUDIO_GATEWAY_PORT", "BRIKKO_STUDIO_GATEWAY_TOKEN"]);
  ...
  process.env.BRIKKO_STUDIO_GATEWAY_PORT = String(gatewayPort);
  process.env.BRIKKO_STUDIO_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("BRIKKO_STUDIO_CONFIG_PATH missing in gateway test environment");
      }
