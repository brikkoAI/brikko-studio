...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : normalizeOptionalString(process.env.BRIKKO_STUDIO_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.BRIKKO_STUDIO_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set BRIKKO_STUDIO_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: brikko-studio doctor --fix",
