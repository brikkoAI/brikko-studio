...
function createGatewayStartupTrace() {
  const logEnabled = isTruthyEnvValue(process.env.BRIKKO_STUDIO_GATEWAY_STARTUP_TRACE);
  let timelineConfig: Brikko StudioConfig | undefined;
  ...
  const eventLoopTimelineEnabled = () =>
    ...
    isTruthyEnvValue(process.env.BRIKKO_STUDIO_DIAGNOSTICS_EVENT_LOOP);
  ...
  return {
    setConfig(config: Brikko StudioConfig) {
...
export type GatewayServerOptions = {
   ...
   * reparsing brikko-studio.json during server startup.
   */
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    isVitestRuntimeEnv() && process.env.BRIKKO_STUDIO_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.BRIKKO_STUDIO_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "BRIKKO_STUDIO_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "BRIKKO_STUDIO_RAW_STREAM_PATH",
  ...
  let cfgAtStart: Brikko StudioConfig;
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `brikko-studio config set gateway.auth.mode token` and `brikko-studio config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: Brikko StudioConfig) =>
  ...
  try {
    ...
    const gatewayRequestContext = createGatewayRequestContext({
      ...
      clients,
      enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: Brikko StudioConfig) => {
        enforceSharedGatewaySessionGenerationForConfigWrite({
