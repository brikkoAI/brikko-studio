...
const loadBrikkoStudioPlugins = vi.hoisted(() => vi.fn());
...
function getLastPluginLoadLogger(): {
  ...
  const call = loadBrikkoStudioPlugins.mock.calls.at(-1)?.[0] as
    | {
        logger?: {
          info: (message: string) => void;
          warn: (message: string) => void;
...
async function createSubagentRuntime(
  ...
  loadBrikkoStudioPlugins.mockReturnValue(createRegistry([]));
...
beforeEach(() => {
  ...
  loadBrikkoStudioPlugins.mockReset();
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadBrikkoStudioPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("pins the initial startup channel registry against later active-registry churn", async () => {
    ...
    loadBrikkoStudioPlugins.mockReturnValue(startupRegistry);
  ...
  test("treats an empty startup scope as no plugin load instead of an unscoped load", async () => {
    ...
    expect(loadBrikkoStudioPlugins).not.toHaveBeenCalled();
  ...
  test("runs registry hook before priming configured bindings", async () => {
    ...
    loadBrikkoStudioPlugins.mockReturnValue(pluginRegistry);
