...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "brikko-studio-test-empty-bundled-plugins");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".brikko-studio", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["BRIKKO_STUDIO_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  const workspaceDir = path.join(tempHome, "brikko-studio");
  await fs.mkdir(workspaceDir, { recursive: true });
  process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "brikko-studio-gw-mock-home-",
        minimalGateway: true,
      });
      ...
      const configDir = path.join(tempHome, ".brikko-studio");
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "brikko-studio-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-wizard-home-"));
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-minimal-gateway-home-"));
      const configPath = path.join(tempHome, ".brikko-studio", "brikko-studio.json");
      const bundledPluginsDir = path.join(tempHome, "brikko-studio-test-no-bundled-extensions");
      process.env.HOME = tempHome;
      process.env.BRIKKO_STUDIO_STATE_DIR = path.join(tempHome, ".brikko-studio");
      process.env.BRIKKO_STUDIO_CONFIG_PATH = configPath;
      process.env.BRIKKO_STUDIO_SKIP_CHANNELS = "1";
      ...
      process.env.BRIKKO_STUDIO_SKIP_BROWSER_CONTROL_SERVER = "1";
      process.env.BRIKKO_STUDIO_SKIP_PROVIDERS = "1";
      process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
      process.env.BRIKKO_STUDIO_TEST_MINIMAL_GATEWAY = "1";
