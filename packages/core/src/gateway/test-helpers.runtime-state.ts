...
const GATEWAY_TEST_CONFIG_ROOT_KEY = Symbol.for("brikko-studio.gatewayTestHelpers.configRoot");
...
const gatewayTestHoisted = vi.hoisted(() => {
  const key = Symbol.for("brikko-studio.gatewayTestHelpers.hoisted");
...
export const testConfigRoot = resolveGlobalSingleton(GATEWAY_TEST_CONFIG_ROOT_KEY, () => ({
  value: path.join(os.tmpdir(), `brikko-studio-gateway-test-${process.pid}-${crypto.randomUUID()}`),
}));

export function setTestConfigRoot(root: string): void {
  testConfigRoot.value = root;
  process.env.BRIKKO_STUDIO_CONFIG_PATH = path.join(root, "brikko-studio.json");
}
