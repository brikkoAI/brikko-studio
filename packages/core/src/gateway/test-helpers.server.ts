...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.BRIKKO_STUDIO_CONFIG_PATH) {
    configPaths.add(process.env.BRIKKO_STUDIO_CONFIG_PATH);
  }
  if (process.env.BRIKKO_STUDIO_STATE_DIR) {
    configPaths.add(path.join(process.env.BRIKKO_STUDIO_STATE_DIR, "brikko-studio.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "brikko-studio-test-no-bundled-extensions")
    : "brikko-studio-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  const stateDir = process.env.BRIKKO_STUDIO_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".brikko-studio-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".brikko-studio-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".brikko-studio-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>brikko-studio-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__brikko-studioTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__brikko-studioTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["BRIKKO_STUDIO_GATEWAY_TOKEN"]);
  const prev = process.env.BRIKKO_STUDIO_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.BRIKKO_STUDIO_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.BRIKKO_STUDIO_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
