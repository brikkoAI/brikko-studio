...
beforeAll(async () => {
  sharedTempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-sessions-config-"));
...
async function writeUnresolvedAuthProfileTokenRef(missingEnvVar: string) {
  ...
  const authStorePath = path.join(resolveBrikko StudioAgentDir(), AUTH_PROFILE_FILENAME);
...
describe("gateway config methods", () => {
  it("rejects config.set when SecretRef resolution fails", async () => {
    const missingEnvVar = `BRIKKO_STUDIO_MISSING_SECRETREF_${Date.now()}`;
    delete process.env[missingEnvVar];
    const current = await rpcReq<{
      hash?: string;
  ...
  it("does not reject config.set for unresolved auth-profile refs outside submitted config", async () => {
    const missingEnvVar = `BRIKKO_STUDIO_MISSING_AUTH_PROFILE_REF_${Date.now()}`;
  ...
  it("rejects config.patch when merged SecretRefs cannot resolve", async () => {
    const missingEnvVar = `BRIKKO_STUDIO_MISSING_SECRETREF_PATCH_${Date.now()}`;
    delete process.env[missingEnvVar];
...
describe("gateway config.apply", () => {
  it("rejects config.apply when SecretRef resolution fails", async () => {
    const missingEnvVar = `BRIKKO_STUDIO_MISSING_SECRETREF_APPLY_${Date.now()}`;
    delete process.env[missingEnvVar];
    const current = await rpcReq<{
      hash?: string;
      raw?: string | null;
  ...
  it("does not reject config.apply for unresolved auth-profile refs outside submitted config", async () => {
    const missingEnvVar = `BRIKKO_STUDIO_MISSING_AUTH_PROFILE_REF_APPLY_${Date.now()}`;
    await writeUnresolvedAuthProfileTokenRef(missingEnvVar);
