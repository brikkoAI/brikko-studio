...
export function resolveHooksConfig(cfg: Brikko StudioConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: Brikko StudioConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-brikko-studio-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-brikko-studio-idempotency-key"]) ||
