...
function loadGatewayConfig(): Brikko StudioConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: Brikko StudioConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallTimeout(
  ...
  const hasEnvHandshakeTimeout =
    Boolean(process.env.BRIKKO_STUDIO_HANDSHAKE_TIMEOUT_MS) ||
    Boolean(process.env.VITEST && process.env.BRIKKO_STUDIO_TEST_HANDSHAKE_TIMEOUT_MS);
...
function resolveGatewayCallContext(opts: CallGatewayBaseOptions): ResolvedGatewayCallContext {
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.BRIKKO_STUDIO_GATEWAY_URL);
  ...
  const config = opts.config ?? (canSkipConfigLoad ? ({} as Brikko StudioConfig) : loadGatewayConfig());
...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `brikko-studio doctor` for diagnostics.";
  }
  return message;
}
