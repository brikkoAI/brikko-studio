...
const DEFAULT_ENV = {
  ...
  BRIKKO_STUDIO_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when BRIKKO_STUDIO env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when BRIKKO_STUDIO env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when BRIKKO_STUDIO env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
