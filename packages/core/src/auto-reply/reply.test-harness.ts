...
type HomeEnvSnapshot = {
  ...
  BRIKKO_STUDIO_STATE_DIR: string | undefined;
  BRIKKO_STUDIO_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    BRIKKO_STUDIO_AGENT_DIR: process.env.BRIKKO_STUDIO_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".brikko-studio", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.BRIKKO_STUDIO_STATE_DIR = path.join(home, ".brikko-studio");
    process.env.BRIKKO_STUDIO_AGENT_DIR = path.join(home, ".brikko-studio", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".brikko-studio", "agent");
  ...
  return withFastReplyConfig({
    agents: {
      defaults: {
        ...
        workspace: path.join(home, "brikko-studio"),
      },
    },
    channels: {
      whatsapp: {
