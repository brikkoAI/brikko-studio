...
function buildDiagnosticsParams(
  ...
  return {
    cfg: { commands: { text: true } } as Brikko StudioConfig,
    ctx: {
      Provider: "whatsapp",
      Surface: "whatsapp",
      CommandSource: "text",
      ...
      return (
        options.execResult ?? {
          ...
          details: {
            ...
            command: "brikko-studio gateway diagnostics export --json",
            cwd: "/tmp",
          },
        }
      );
...
describe("diagnostics command", () => {
  it("requests Gateway diagnostics approval without a duplicate pending chat reply", async () => {
    ...
    ).toContain("https://docs.brikko-studio.ai/gateway/diagnostics");
    ...
    expect(command).not.toBe("brikko-studio gateway diagnostics export --json");
  });
  ...
  it("falls back to a visible reply when approval cannot be queued", async () => {
    ...
    expect(result?.reply?.text).toContain("https://docs.brikko-studio.ai/gateway/diagnostics");
  ...
  it("routes group diagnostics confirmations privately", async () => {
    const commandHandler = vi.fn(async () => ({
      text: [
        "Codex diagnostics sent to OpenAI servers:",
        "- channel whatsapp, Brikko Studio session session-1, Codex thread codex-thread-1",
      ].join("\n"),
    }));
    registerHostTrustedReservedCommandForTest({
      name: "codex",
