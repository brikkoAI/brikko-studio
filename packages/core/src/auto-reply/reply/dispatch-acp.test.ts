...
const policyMocks = vi.hoisted(() => ({
  resolveAcpDispatchPolicyError: vi.fn<(cfg: Brikko StudioConfig) => AcpRuntimeError | null>(() => null),
  resolveAcpAgentPolicyError: vi.fn<(cfg: Brikko StudioConfig, agent: string) => AcpRuntimeError | null>(
    () => null,
  ),
}));
...
const sessionMetaMocks = vi.hoisted(() => ({
  readAcpSessionEntry: vi.fn<
    (params: { sessionKey: string; cfg?: Brikko StudioConfig }) => AcpSessionStoreEntry | null
  >(() => null),
}));
...
vi.mock("../../acp/policy.js", () => ({
  resolveAcpDispatchPolicyError: (cfg: Brikko StudioConfig) =>
    policyMocks.resolveAcpDispatchPolicyError(cfg),
  resolveAcpAgentPolicyError: (cfg: Brikko StudioConfig, agent: string) =>
...
function createAcpConfigWithVisibleToolTags(): Brikko StudioConfig {
    ...
    sessionMetaMocks.readAcpSessionEntry.mockImplementation(
      (params: { sessionKey: string; cfg?: Brikko StudioConfig }) =>
        params.sessionKey === canonicalSessionKey
          ? {
              cfg: params.cfg ?? createAcpTestConfig(),
              storePath: "/tmp/brikko-studio-session-store.json",
              sessionKey: canonicalSessionKey,
