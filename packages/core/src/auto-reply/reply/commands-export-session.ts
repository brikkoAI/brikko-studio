...
function replaceHtmlPlaceholder(template: string, name: string, value: string): string {
  ...
  const placeholder = new RegExp(
    `(<(?:script|style)\\b(?=[^>]*\\bdata-brikko-studio-export-placeholder="${name}")[^>]*>)(</(?:script|style)>)`,
  );
  const next = template.replace(
    placeholder,
    (_match: string, openTag: string, closeTag: string) => {
      replaced = true;
      const finalOpenTag = openTag.replace(/\sdata-brikko-studio-export-placeholder="[^"]*"/, "");
...
export async function buildExportSessionReply(params: HandleCommandsParams): Promise<ReplyPayload> {
  ...
  const defaultFileName = `brikko-studio-session-${entry.sessionId.slice(0, 8)}-${timestamp}.html`;
