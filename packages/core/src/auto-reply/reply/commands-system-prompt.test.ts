...
const { createBrikkoStudioCodingToolsMock } = vi.hoisted(() => ({
  createBrikkoStudioCodingToolsMock: vi.fn(() => []),
}));
...
vi.mock("../../agents/pi-tools.js", () => ({
  createBrikkoStudioCodingTools: createBrikkoStudioCodingToolsMock,
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    createBrikkoStudioCodingToolsMock.mockClear();
    createBrikkoStudioCodingToolsMock.mockReturnValue([]);
  });

  it("opts command tool builds into gateway subagent binding", async () => {
    await resolveCommandsSystemPromptBundle(makeParams());

    expect(vi.mocked(createBrikkoStudioCodingTools)).toHaveBeenCalledWith(
