...
function createRemoteStageParams(home: string): {
  ...
  return {
    ...
    workspaceDir: join(home, "brikko-studio"),
    sessionKey,
    remoteCacheDir: join(home, ".brikko-studio", "media", "remote-cache", slugifySessionKey(sessionKey)),
  };
}
...
describe("stageSandboxMedia scp remote paths", () => {
  it("rejects remote attachment filenames with shell metacharacters before spawning scp", async () => {
    await withSandboxMediaTempHome("brikko-studio-triggers-", async (home) => {
