...
describe("secrets runtime snapshot auth integration", () => {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("brikko-studio-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".brikko-studio", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".brikko-studio", "agents", "ops", "agent");
