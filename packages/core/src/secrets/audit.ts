...
import { resolveStateDir, type Brikko StudioConfig } from "../config/config.js";
...
export async function runSecretsAudit(
  ...
  const config = snapshot.valid ? snapshot.config : ({} as Brikko StudioConfig);
  let resolution = {
    refsChecked: 0,
    skippedExecRefs: 0,
