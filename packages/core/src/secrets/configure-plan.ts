...
function configureCandidateSortKey(candidate: ConfigureCandidate): string {
  ...
  return `brikko-studio:${candidate.path}`;
}
...
export function buildConfigureCandidatesForScope(params: {
  ...
  const brikko-studioCandidates = discoverConfigSecretTargets(params.config)
      ...
      return Object.assign(
        {
          ...
          label: entry.path,
          configFile: `brikko-studio.json` as const,
  ...
  return [...brikko-studioCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "brikko-studio secrets configure",
