...
const createTempDir = () => tempDirs.make("brikko-studio-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets BRIKKO_STUDIO_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "brikko-studio",
    });

    expect(env.BRIKKO_STUDIO_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("brikko-studio");
  });

  it("overrides pre-existing BRIKKO_STUDIO_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      BRIKKO_STUDIO_SHELL: "wrong",
    });
    expect(env.BRIKKO_STUDIO_SHELL).toBe("acp-client");
  });
  ...
  it("preserves BRIKKO_STUDIO_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        BRIKKO_STUDIO_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["BRIKKO_STUDIO_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Brikko Studio bridge", () => {
    ...
    expect(env.BRIKKO_STUDIO_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.BRIKKO_STUDIO_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "brikko-studio",
        serverArgs: ["acp"],
        defaultServerCommand: "brikko-studio",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("BRIKKO_STUDIO_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "brikko-studio", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "brikko-studio", "dist", "entry.js");
    const shimPath = path.join(dir, "brikko-studio.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\brikko-studio\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/brikko-studio-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/brikko-studio-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/brikko-studio-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
