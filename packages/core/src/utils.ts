...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".brikko-studio");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.BRIKKO_STUDIO_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$BRIKKO_STUDIO_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via BRIKKO_STUDIO_STATE_DIR.
