...
describe("gateway-watch tmux wrapper", () => {
  it("derives stable session names from profile and port", () => {
    expect(resolveGatewayWatchTmuxSessionName({ args: ["gateway", "--force"], env: {} })).toBe(
      "brikko-studio-gateway-watch-main",
    );
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["gateway", "--force", "--port", "19001"],
        env: { BRIKKO_STUDIO_PROFILE: "Dev Profile" },
      }),
    ).toBe("brikko-studio-gateway-watch-dev-profile-19001");
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["--dev", "gateway", "--port=18789"],
        env: {},
      }),
    ).toBe("brikko-studio-gateway-watch-dev");
  });
  ...
  it("builds a login-shell command that runs the raw watcher in the repo", () => {
    const command = buildGatewayWatchTmuxCommand({
      ...
      cwd: "/repo with spaces/brikko-studio",
      env: {
        BRIKKO_STUDIO_GATEWAY_PORT: "19001",
        BRIKKO_STUDIO_PROFILE: "Dev Profile",
        SHELL: "/bin/zsh",
      },
      nodePath: "/opt/node",
      sessionName: "brikko-studio-gateway-watch-main",
    });

    expect(command).toContain("exec '/bin/zsh' -lc");
    expect(command).toContain("/repo with spaces/brikko-studio");
    expect(command).toContain("'BRIKKO_STUDIO_GATEWAY_WATCH_TMUX_CHILD=1'");
    expect(command).toContain("'BRIKKO_STUDIO_GATEWAY_WATCH_SESSION=brikko-studio-gateway-watch-main'");
    expect(command).toContain("'\\''-u'\\'' '\\''NO_COLOR'\\''");
    expect(command).toContain("'FORCE_COLOR=1'");
    expect(command).toContain("'BRIKKO_STUDIO_GATEWAY_PORT=19001'");
    expect(command).toContain("'BRIKKO_STUDIO_PROFILE=Dev Profile'");
  ...
  it("consumes benchmark flags and passes the CPU profile dir to the watched child", () => {
    ...
    expect(command).toContain("'BRIKKO_STUDIO_RUN_NODE_CPU_PROF_DIR=.artifacts/gateway-watch-profiles'");
  ...
  it("creates a detached tmux session when none exists", () => {
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      1,
      "tmux",
      ["has-session", "-t", "brikko-studio-gateway-watch-main"],
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      [
        ...
        "-t",
        "brikko-studio-gateway-watch-main",
        "@brikko-studio.gateway_watch.cwd",
        "/repo",
      ],
      expect.objectContaining({ encoding: "utf8" }),
    );
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      [
        ...
        "-t",
        "brikko-studio-gateway-watch-main",
        "BRIKKO_STUDIO_GATEWAY_WATCH_CWD",
        "/repo",
      ],
      expect.objectContaining({ encoding: "utf8" }),
    );
    expect(stderr.chunks.join("")).toContain(
      "gateway:watch started in tmux session brikko-studio-gateway-watch-main",
    );
    expect(stdout.chunks.join("")).toContain("tmux attach -t brikko-studio-gateway-watch-main");
    expect(stdout.chunks.join("")).toContain(
      "tmux show-options -v -t brikko-studio-gateway-watch-main @brikko-studio.gateway_watch.cwd",
    );
  });
  ...
  it("auto-attaches in an interactive terminal after creating a session", () => {
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      5,
      "tmux",
      ["attach-session", "-t", "brikko-studio-gateway-watch-main"],
  ...
  it("switches tmux clients instead of nesting attach when already inside tmux", () => {
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      5,
      "tmux",
      ["switch-client", "-t", "brikko-studio-gateway-watch-main"],
      expect.objectContaining({ stdio: "inherit" }),
    );
  });
  ...
  it("respawns the existing tmux pane on repeated runs", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force", "--port=19001"],
      cwd: "/repo",
      env: { BRIKKO_STUDIO_PROFILE: "dev", SHELL: "/bin/zsh" },
      nodePath: "/node",
      spawnSync,
      stderr: stderr.stream,
      stdout: stdout.stream,
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      [
        ...
        "-k",
        "-t",
        "brikko-studio-gateway-watch-dev-19001",
        "-c",
        "/repo",
        expect.stringContaining("scripts/watch-node.mjs"),
      ],
    ...
    expect(stderr.chunks.join("")).toContain(
      "gateway:watch restarted in tmux session brikko-studio-gateway-watch-dev-19001",
    );
  });
  ...
  it("recreates a stale session when its active pane target is missing", () => {
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      3,
      "tmux",
      ["kill-session", "-t", "brikko-studio-gateway-watch-main"],
      expect.objectContaining({ encoding: "utf8" }),
    );
    expect(spawnSync).toHaveBeenNthCalledWith(
      4,
  ...
  it("runs the raw foreground watcher when tmux mode is disabled", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force"],
      cwd: "/repo",
      env: { BRIKKO_STUDIO_GATEWAY_WATCH_TMUX: "0" },
      nodePath: "/node",
      spawnSync,
    });
