...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.BRIKKO_STUDIO_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*brikko-studio-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "brikko-studio";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "brikko-studio");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "brikko-studio-gw");
  ...
  records.push(`_brikko-studio-gw._tcp IN PTR ${instanceLabel}._brikko-studio-gw._tcp`);
  records.push(`${instanceLabel}._brikko-studio-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._brikko-studio-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; brikko-studio-content-hash: ${contentHash}\n${contentBody}`;
}
