...
describe("isGatewayArgv", () => {
  ...
  it("matches known entrypoints across slash and case variants", () => {
    expect(isGatewayArgv(["NODE", "C:\\Brikko Studio\\DIST\\ENTRY.JS", "gateway"])).toBe(true);
    expect(isGatewayArgv(["bun", "/srv/brikko-studio/scripts/run-node.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["node", "/srv/brikko-studio/brikko-studio.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/brikko-studio/src/entry.ts", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/brikko-studio/src/index.ts", "gateway"])).toBe(true);
  });

  it("matches the brikko-studio executable but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv(["C:\\bin\\brikko-studio.cmd", "gateway"])).toBe(true);
    expect(isGatewayArgv(["/usr/local/bin/brikko-studio-gateway", "gateway"])).toBe(false);
    expect(
      isGatewayArgv(["/usr/local/bin/brikko-studio-gateway", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
    expect(
      isGatewayArgv(["C:\\bin\\brikko-studio-gateway.EXE", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
  });
  ...
  it("rejects unknown gateway argv even when the token is present", () => {
    expect(isGatewayArgv(["node", "/srv/brikko-studio/custom.js", "gateway"])).toBe(false);
