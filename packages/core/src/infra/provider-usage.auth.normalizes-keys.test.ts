...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "brikko-studio-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".brikko-studio");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".brikko-studio", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      BRIKKO_STUDIO_STATE_DIR: path.join(home, ".brikko-studio"),
      ...env,
    };
  ...
  async function writeConfig(home: string, config: Record<string, unknown>) {
    ...
    await fs.writeFile(
      path.join(stateDir, "brikko-studio.json"),
