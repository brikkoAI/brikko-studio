const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "BRIKKO_STUDIO_LAUNCHD_LABEL"],
  systemd: ["BRIKKO_STUDIO_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["BRIKKO_STUDIO_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "BRIKKO_STUDIO_SERVICE_MARKER",
  "BRIKKO_STUDIO_SERVICE_KIND",
] as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.BRIKKO_STUDIO_SERVICE_MARKER?.trim();
