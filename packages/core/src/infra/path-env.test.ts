...
describe("ensureBrikkoStudioCliOnPath", () => {
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/brikko-studio-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "brikko-studio");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function resetBootstrapEnv(pathValue = "/usr/bin") {
    ...
    delete process.env.BRIKKO_STUDIO_PATH_BOOTSTRAPPED;
    delete process.env.BRIKKO_STUDIO_ALLOW_PROJECT_LOCAL_BIN;
  ...
  it("prepends the bundled app bin dir when a sibling brikko-studio exists", () => {
  ...
  it("keeps the current runtime directory ahead of system PATH hardening", () => {
    const tmp = abs("/tmp/brikko-studio-path/case-runtime-dir");
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "brikko-studio");
      ...
      if (envValue === undefined) {
        ...
        process.env.BRIKKO_STUDIO_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        const tmp = abs("/tmp/brikko-studio-path/case-linuxbrew");
