...
const TEMP_DIR_PREFIX = "brikko-studio-install-source-utils-";
...
async function expectPackFallsBackToDetectedArchive(params: {
  ...
  const cwd = await createTempDir("brikko-studio-install-source-utils-");
  const archivePath = path.join(cwd, "brikko-studio-plugin-1.2.3.tgz");
  ...
  const result = await packNpmSpecToArchive({
    spec: "brikko-studio-plugin@1.2.3",
    timeoutMs: 5000,
    cwd,
  });
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("brikko-studio-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
  ...
  it.each([
    {
      ...
      path: async () => "/tmp/does-not-exist-brikko-studio-archive.tgz",
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "brikko-studio-plugin@1.2.3",
          name: "brikko-studio-plugin",
          version: "1.2.3",
          filename: "brikko-studio-plugin-1.2.3.tgz",
    ...
    const result = await runPack("brikko-studio-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "brikko-studio-plugin",
        version: "1.2.3",
        resolvedSpec: "brikko-studio-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "brikko-studio-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "brikko-studio-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\nbrikko-studio-plugin-1.2.3.tgz\n",
    });
  ...
  it.each([
    ...
    },
    {
      ...
      stdout:
        'npm notice using cache\n[{"id":"brikko-studio-plugin@1.2.3","name":"brikko-studio-plugin","version":"1.2.3","integrity":"sha512-test-integrity","shasum":"abc123"}]\n',
      expectedMetadata: {
        name: "brikko-studio-plugin",
        version: "1.2.3",
        resolvedSpec: "brikko-studio-plugin@1.2.3",
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr: "npm error code E404\nnpm error 404  '@brikko-studio/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@brikko-studio/whatsapp", cwd);
    expectPackError(result, [
      "Package not found on npm",
      "@brikko-studio/whatsapp",
      "docs.brikko-studio.ai/tools/plugin",
    ]);
  });
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("brikko-studio-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "brikko-studio-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        "npm notice creating package\n" +
        JSON.stringify([
          {
            id: "@brikko-studio/plugin-demo@2.0.0",
            filename: "brikko-studio-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@brikko-studio/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "brikko-studio-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@brikko-studio/plugin-demo@2.0.0",
      },
    });
  });
