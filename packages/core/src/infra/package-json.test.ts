...
describe("package-json helpers", () => {
  it("reads package version and trims package name", async () => {
    await withTempDir({ prefix: "brikko-studio-package-json-" }, async (root) => {
      await fs.writeFile(
        path.join(root, "package.json"),
        JSON.stringify({ version: " 1.2.3 ", name: "  @brikko-studio/demo  " }),
        "utf8",
      );
      ...
      await expectPackageMeta({
        ...
        expectedName: "@brikko-studio/demo",
      });
    });
  });
  ...
  it.each([
    ...
    },
    {
      ...
      writePackageJson: async (root: string) => {
        await fs.writeFile(
          path.join(root, "package.json"),
          JSON.stringify({ version: "   ", name: "@brikko-studio/demo" }),
          "utf8",
        );
      },
      expectedVersion: null,
