...
function candidateBinDirs(opts: EnsureBrikkoStudioPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `brikko-studio` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "brikko-studio");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.BRIKKO_STUDIO_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "brikko-studio"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `brikko-studio`
