...
describe("package dist inventory", () => {
  it("tracks missing and stale dist files", async () => {
    await withTempDir({ prefix: "brikko-studio-dist-inventory-" }, async (packageRoot) => {
  ...
  it("keeps npm-omitted dist artifacts out of the inventory", async () => {
    await withTempDir({ prefix: "brikko-studio-dist-inventory-pack-" }, async (packageRoot) => {
  ...
  it("keeps transient plugin dependency trees out of the inventory", async () => {
    await withTempDir({ prefix: "brikko-studio-dist-inventory-plugin-deps-" }, async (packageRoot) => {
  ...
  it("omits packaged extension node_modules while keeping extension runtime files", async () => {
    await withTempDir(
      { prefix: "brikko-studio-dist-inventory-extension-node-modules-" },
      async (packageRoot) => {
  ...
  it("keeps publishable externalized bundled plugin dist trees out of the inventory", async () => {
    await withTempDir({ prefix: "brikko-studio-dist-inventory-externalized-" }, async (packageRoot) => {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          name: "@brikko-studio/external-chat",
          brikko-studio: {
            release: {
      ...
      await fs.writeFile(
        bundledPackageJson,
        JSON.stringify({
          name: "@brikko-studio/bundled-chat",
          brikko-studio: {},
        }),
        "utf8",
      );
  ...
  it("keeps publishable core-package runtime plugin dist trees in the inventory", async () => {
    await withTempDir({ prefix: "brikko-studio-dist-inventory-core-runtime-" }, async (packageRoot) => {
  ...
  it("reports runtime-created install staging dirs during installed dist verification", async () => {
    await withTempDir({ prefix: "brikko-studio-dist-inventory-stage-" }, async (packageRoot) => {
      ...
      const suffixedStageFile = path.join(
        ...
        "extensions",
        "browser",
        ".brikko-studio-install-stage-AbC123",
        "node_modules",
        "playwright-core",
        "package.json",
      );
      ...
      await expect(collectPackageDistInventoryErrors(packageRoot)).resolves.toEqual([
        ...
        "unexpected packaged dist file dist/extensions/browser/.brikko-studio-install-stage-AbC123/node_modules/playwright-core/package.json",
      ]);
    });
  });
  ...
  it("matches install-stage paths case-insensitively across path segments", () => {
    expect(
      isLegacyPluginDependencyInstallStagePath(
        "dist/extensions/brave/.brikko-studio-install-stage/node_modules/typebox/package.json",
      ),
    ).toBe(true);
    expect(
      isLegacyPluginDependencyInstallStagePath(
        "dist/Extensions/browser/.BRIKKO_STUDIO-INSTALL-STAGE-AbC123/node_modules/playwright-core/package.json",
      ),
    ).toBe(true);
    expect(
      isLegacyPluginDependencyInstallStagePath(
    ...
    expect(
      isLegacyPluginDependencyInstallStagePath(
        "dist/extensions/browser/.brikko-studio-runtime-deps-copy-AbC123/package.json",
      ),
    ).toBe(false);
    expect(
      isLegacyPluginDependencyInstallStagePath("dist/extensions/.brikko-studio-install-stage"),
    ).toBe(false);
  });

  it("rejects pre-populated install-stage debris at publish time", async () => {
    await withTempDir({ prefix: "brikko-studio-dist-inventory-stage-publish-" }, async (packageRoot) => {
      ...
      await expect(collectLegacyPluginDependencyStagingDebrisPaths(packageRoot)).resolves.toEqual([
        "dist/extensions/browser/.brikko-studio-install-stage-AbC123",
  ...
  it("rejects mixed-case install-stage debris on case-sensitive release builders", async () => {
    await withTempDir(
      { prefix: "brikko-studio-dist-inventory-stage-extensions-case-" },
      async (packageRoot) => {
    ...
    await withTempDir(
      { prefix: "brikko-studio-dist-inventory-stage-root-case-" },
      async (packageRoot) => {
        const mixedCaseStage = path.join(
          packageRoot,
          "Dist",
          "Extensions",
          "browser",
          ".BRIKKO_STUDIO-INSTALL-STAGE-AbC123",
          "package.json",
        );
        await fs.mkdir(path.dirname(mixedCaseStage), { recursive: true });
        await fs.writeFile(mixedCaseStage, "{}", "utf8");
        ...
        await expect(collectLegacyPluginDependencyStagingDebrisPaths(packageRoot)).resolves.toEqual(
          ["Dist/Extensions/browser/.BRIKKO_STUDIO-INSTALL-STAGE-AbC123"],
        );
        await expect(writePackageDistInventory(packageRoot)).rejects.toThrow(
          /unexpected legacy plugin dependency staging debris/,
        );
  ...
  it("treats a missing dist/extensions tree as no staging debris", async () => {
    await withTempDir({ prefix: "brikko-studio-dist-inventory-no-extensions-" }, async (packageRoot) => {
  ...
  it("fails closed when the inventory is missing", async () => {
    await withTempDir({ prefix: "brikko-studio-dist-inventory-missing-" }, async (packageRoot) => {
      await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
      await expect(collectPackageDistInventoryErrors(packageRoot)).resolves.toEqual([
  ...
  it("rejects symlinked dist entries", async () => {
    await withTempDir({ prefix: "brikko-studio-dist-inventory-symlink-" }, async (packageRoot) => {
