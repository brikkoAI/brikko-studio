...
vi.mock("./brikko-studio-root.js", async () => {
...
describe("update-startup", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "brikko-studio-update-check-suite-" });
  let tempDir: string;
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv([
      "BRIKKO_STUDIO_NO_AUTO_UPDATE",
      "BRIKKO_STUDIO_STATE_DIR",
      "NODE_ENV",
      "VITEST",
    ]);
    process.env.BRIKKO_STUDIO_STATE_DIR = tempDir;
  ...
  it("honors BRIKKO_STUDIO_NO_AUTO_UPDATE for configured auto-updates", async () => {
    mockPackageUpdateStatus("beta", "2.0.0-beta.1");
    process.env.BRIKKO_STUDIO_NO_AUTO_UPDATE = "1";
    const log = { info: vi.fn() };
    const runAutoUpdate = createAutoUpdateSuccessMock();
    ...
    expect(runAutoUpdate).not.toHaveBeenCalled();
    expect(log.info).toHaveBeenCalledWith(
      "auto-update disabled by BRIKKO_STUDIO_NO_AUTO_UPDATE",
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/brikko-studio/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      [
        process.execPath,
        "/opt/brikko-studio/dist/entry.js",
        "update",
        "--yes",
        "--channel",
        "beta",
