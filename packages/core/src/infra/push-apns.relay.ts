...
const GATEWAY_DEVICE_ID_HEADER = "x-brikko-studio-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-brikko-studio-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-brikko-studio-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.BRIKKO_STUDIO_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "BRIKKO_STUDIO_APNS_RELAY_BASE_URL"
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.BRIKKO_STUDIO_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require BRIKKO_STUDIO_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.BRIKKO_STUDIO_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
