...
const BRIKKO_STUDIO_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "brikko-studio.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.BRIKKO_STUDIO_DIAGNOSTICS)) &&
    typeof env.BRIKKO_STUDIO_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.BRIKKO_STUDIO_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: BRIKKO_STUDIO_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.BRIKKO_STUDIO_DIAGNOSTICS_RUN_ID ? { runId: env.BRIKKO_STUDIO_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.BRIKKO_STUDIO_DIAGNOSTICS_ENV ? { envName: env.BRIKKO_STUDIO_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.BRIKKO_STUDIO_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
