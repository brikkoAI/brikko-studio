...
describe("logAcceptedEnvOption", () => {
  it("logs accepted env options once with redaction and formatting", async () => {
    ...
    withEnv(
      {
        ...
        BRIKKO_STUDIO_TEST_ENV: "  line one\nline two  ",
      },
      () => {
        logAcceptedEnvOption({
          key: "BRIKKO_STUDIO_TEST_ENV",
    ...
    expect(loggerMocks.info).toHaveBeenCalledWith(
      "env: BRIKKO_STUDIO_TEST_ENV=<redacted> (test option)",
    );
  });
    ...
    withEnv(
      {
        VITEST: "1",
        NODE_ENV: "development",
        BRIKKO_STUDIO_BLANK_ENV: "value",
      },
      () => {
        logAcceptedEnvOption({
          key: "BRIKKO_STUDIO_BLANK_ENV",
