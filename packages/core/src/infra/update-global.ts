...
const PRIMARY_PACKAGE_NAME = "brikko-studio";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const BRIKKO_STUDIO_MAIN_PACKAGE_SPEC = "github:brikko-studio/brikko-studio#main";
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        ...
        (await pathExists(path.join(packageRoot, pluginRoot, "brikko-studio.plugin.json")))
      ) {
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.BRIKKO_STUDIO_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.BRIKKO_STUDIO_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return BRIKKO_STUDIO_MAIN_PACKAGE_SPEC;
  }
