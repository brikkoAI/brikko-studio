...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  ...
  gatewayPort: 18789,
  displayName: "Mac Studio (Brikko Studio)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "brikko-studio.internal", expected: "brikko-studio.internal." },
    { value: "brikko-studio.internal.", expected: "brikko-studio.internal." },
    { value: "  brikko-studio.internal  ", expected: "brikko-studio.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { BRIKKO_STUDIO_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { BRIKKO_STUDIO_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("brikko-studio.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "brikko-studio.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      `$ORIGIN brikko-studio.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_brikko-studio-gw._tcp IN PTR studio-london._brikko-studio-gw._tcp`,
      `studio-london._brikko-studio-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (Brikko Studio)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/brikko-studio`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN brikko-studio.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._brikko-studio-gw._tcp IN TXT`,
        `displayName=Mac Studio (Brikko Studio)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("brikko-studio.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("brikko-studio.internal."),
