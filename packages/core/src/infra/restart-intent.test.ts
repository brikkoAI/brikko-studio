...
function createIntentEnv(): NodeJS.ProcessEnv {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-restart-intent-"));
  tempDirs.push(dir);
  return {
    ...process.env,
...
function intentPath(env: NodeJS.ProcessEnv): string {
  return path.join(env.BRIKKO_STUDIO_STATE_DIR ?? "", "gateway-restart-intent.json");
}
...
describe("gateway restart intent", () => {
  ...
  it("does not follow an existing intent-path symlink when writing", () => {
    const env = createIntentEnv();
    const targetPath = path.join(env.BRIKKO_STUDIO_STATE_DIR ?? "", "attacker-target.txt");
