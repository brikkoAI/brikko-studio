...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses brikko-studio-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "brikko-studio-gateway" },
          { pid: process.pid, cmd: "brikko-studio-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes ancestor pids so a sidecar cannot kill its parent gateway — regression for #68451", () => {
      // Regression: brikko-studio-weixin sidecar (child of the gateway) invoked
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: parentGatewayPid, cmd: "brikko-studio-gateway" },
          { pid: unrelatedStalePid, cmd: "brikko-studio-gateway" },
        ]),
        stderr: "",
      });
      ...
      () => {
        ...
        mockReadFileSync.mockImplementation((path: unknown): string => {
          if (path === `/proc/${directParentPid}/status`) {
            return `Name:\tbrikko-studio-gateway\nPid:\t${directParentPid}\nPPid:\t${grandparentPid}\n`;
          }
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: directParentPid, cmd: "brikko-studio-gateway" },
            { pid: grandparentPid, cmd: "brikko-studio-gateway" },
            { pid: benignStalePid, cmd: "brikko-studio-gateway" },
          ]),
          stderr: "",
        });
    ...
    it("excludes PID 1 when the direct parent gateway is the container entrypoint — container topology", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: 1, cmd: "brikko-studio-gateway" },
          { pid: benignStalePid, cmd: "brikko-studio-gateway" },
        ]),
        stderr: "",
      });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: pluginHostPid, cmd: "brikko-studio-gateway" },
            { pid: gatewayGrandparentPid, cmd: "brikko-studio-gateway" },
          ]),
          stderr: "",
        });
    ...
    it("excludes pids whose command does not include 'brikko-studio'", () => {
    ...
    it("returns verified gateway pids from Windows helpers on win32", () => {
      ...
      try {
        ...
        mockReadWindowsProcessArgs.mockReturnValue(["brikko-studio", "gateway"]);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple brikko-studio pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "brikko-studio-gateway" },
          { pid: pid2, cmd: "brikko-studio-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-brikko-studio pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks brikko-studio processes — non-brikko-studio listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty brikko-studio stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty brikko-studio stdout must not be treated as port-free.
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "brikko-studio-gateway" }]),
          stderr: "",
        };
      });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'brikko-studio'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("brikko-studio")
      // inside the mid-loop flush: a non-brikko-studio cmd between two entries must not
      // be pushed, but the following brikko-studio entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-brikko-studio entry first, then brikko-studio entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid brikko-studio entry must still be found.
      const stdout = `p0\ncbrikko-studio-gateway\np${stalePid}\ncbrikko-studio-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero brikko-studio pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-brikko-studio stdout (line 145)", () => {
    it("treats status 1 + non-brikko-studio stdout as port-free (not an brikko-studio process)", () => {
      // status 1 + non-empty stdout where no brikko-studio pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill brikko-studio pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-brikko-studio output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no brikko-studio pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
