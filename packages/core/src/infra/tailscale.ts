...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.BRIKKO_STUDIO_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Brikko Studio. You can keep running the web gateway without it: \`${formatCliCommand("brikko-studio gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
