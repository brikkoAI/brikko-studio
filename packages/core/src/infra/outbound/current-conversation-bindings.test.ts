...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.BRIKKO_STUDIO_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-current-bindings-"));
    process.env.BRIKKO_STUDIO_STATE_DIR = testStateDir;
