...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: BrikkoStudioConfig | null = null;
  ...
  get(key: string, cfg: BrikkoStudioConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: BrikkoStudioConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: BrikkoStudioConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: BrikkoStudioConfig): void {
