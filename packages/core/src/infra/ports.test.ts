...
describe("ports helpers", () => {
  ...
  it("prints an Brikko Studio-specific hint when port details look like another Brikko Studio instance", async () => {
    ...
    await handlePortError(
      new PortInUseError(18789, "node dist/index.js brikko-studio gateway"),
      18789,
      "gateway start",
      runtime,
    ).catch(() => {});
...
describeUnix("inspectPortUsage", () => {
  ...
  it("falls back to ss when lsof is unavailable", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/brikko-studio/dist/index.js gateway --port 18789\n",
            stderr: "",
            code: 0,
          };
        }
    ...
    try {
      ...
      expect(result.listeners[0]?.commandLine).toContain("brikko-studio");
...
describe("inspectPortUsage on Windows", () => {
  it("uses PowerShell process command lines to classify Brikko Studio listeners", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "powershell") {
        return {
          stdout:
            '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\me\\AppData\\Roaming\\npm\\node_modules\\brikko-studio\\dist\\index.js gateway run\r\n',
          stderr: "",
          code: 0,
        };
      }
  ...
  it("falls back to wmic when PowerShell cannot read the command line", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "wmic") {
        return {
          stdout: "CommandLine=node.exe C:\\brikko-studio\\dist\\index.js gateway run\r\n",
          stderr: "",
          code: 0,
        };
      }
