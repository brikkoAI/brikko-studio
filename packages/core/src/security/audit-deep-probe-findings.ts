...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("brikko-studio status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("brikko-studio security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set BRIKKO_STUDIO_GATEWAY_TOKEN/BRIKKO_STUDIO_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("brikko-studio security audit --deep")}".`,
    });
  }
  return findings;
}
