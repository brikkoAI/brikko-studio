...
type CollectDangerousConfigFlags = (cfg: Brikko StudioConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.BRIKKO_STUDIO_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.BRIKKO_STUDIO_GATEWAY_PASSWORD);
