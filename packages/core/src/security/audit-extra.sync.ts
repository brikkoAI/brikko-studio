...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep BRIKKO_STUDIO_STATE_DIR on a local-only volume and re-run "${formatCliCommand("brikko-studio security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: Brikko StudioConfig): SecurityAuditFinding[] {
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer BRIKKO_STUDIO_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const brikko-studioGatewayToken =
    typeof env.BRIKKO_STUDIO_GATEWAY_TOKEN === "string" && env.BRIKKO_STUDIO_GATEWAY_TOKEN.trim()
      ? env.BRIKKO_STUDIO_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-brikko-studio-session-key for per-request session routing. ` +
