...
const BRIKKO_STUDIO_CLI_WRAPPER_PATH = fileURLToPath(new URL("../../brikko-studio.mjs", import.meta.url));
const BRIKKO_STUDIO_RUN_NODE_SCRIPT_PATH = fileURLToPath(
  new URL("../../scripts/run-node.mjs", import.meta.url),
);
const BRIKKO_STUDIO_DIST_ENTRY_JS_PATH = fileURLToPath(new URL("../../dist/entry.js", import.meta.url));
const BRIKKO_STUDIO_DIST_ENTRY_MJS_PATH = fileURLToPath(
...
export function resolveLocalAuthCliInvocation(params?: {
  ...
  const hasDistEntry =
    params?.hasDistEntry ??
    (existsSync(BRIKKO_STUDIO_DIST_ENTRY_JS_PATH) || existsSync(BRIKKO_STUDIO_DIST_ENTRY_MJS_PATH));
  const hasRunNodeScript = params?.hasRunNodeScript ?? existsSync(BRIKKO_STUDIO_RUN_NODE_SCRIPT_PATH);
  const command = params?.execPath ?? process.execPath;
  const wrapperPath = params?.wrapperPath ?? BRIKKO_STUDIO_CLI_WRAPPER_PATH;
  const runNodePath = params?.runNodePath ?? BRIKKO_STUDIO_RUN_NODE_SCRIPT_PATH;
...
export function resolveLocalAuthSpawnCwd(params: { args: string[]; defaultCwd?: string }): string {
  ...
  if (entryBase === "brikko-studio.mjs") {
...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: "pairing required: run brikko-studio devices list",
      pairingHint:
        "Pairing required. Run `brikko-studio devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export async function runTui(opts: RunTuiOptions): Promise<TuiResult> {
  ...
  const previousConsoleSubsystemFilter = isLocalMode
    ...
    : null;
  if (isLocalMode) {
    setConsoleSubsystemFilter(["__brikko-studio_tui_quiet__"]);
  }
  ...
  const updateHeader = () => {
    ...
    const title = opts.title ?? "brikko-studio tui";
    header.setText(
      theme.header(
