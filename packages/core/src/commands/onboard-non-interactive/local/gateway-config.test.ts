...
describe("applyNonInteractiveGatewayConfig token resolution chain", () => {
  const originalEnvToken = process.env.BRIKKO_STUDIO_GATEWAY_TOKEN;
  ...
  afterEach(() => {
    if (originalEnvToken === undefined) {
      ...
      process.env.BRIKKO_STUDIO_GATEWAY_TOKEN = originalEnvToken;
    }
  ...
  it("prefers existing plaintext token over ambient BRIKKO_STUDIO_GATEWAY_TOKEN on re-onboard", () => {
    // A stale shell/launchd BRIKKO_STUDIO_GATEWAY_TOKEN must not rotate a
    // persisted token — that would break already-paired clients.
    process.env.BRIKKO_STUDIO_GATEWAY_TOKEN = "stale-env-token";
    const nextConfig = createTokenConfig("existing-user-token");
  ...
  it("uses BRIKKO_STUDIO_GATEWAY_TOKEN to fill an empty config on first-run", () => {
  ...
  it("preserves an existing SecretRef even when ambient BRIKKO_STUDIO_GATEWAY_TOKEN is set", () => {
  ...
  it("overrides an existing SecretRef when --gateway-token-ref-env is provided", () => {
    const newRefId = "BRIKKO_STUDIO_GATEWAY_TOKEN_NEW_REF";
