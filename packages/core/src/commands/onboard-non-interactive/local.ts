        ...
        hints:
          daemonInstall.skippedReason === "systemd-user-unavailable"
            ? [
                ...
                "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `brikko-studio gateway run` or `brikko-studio agent --local`.",
              ]
            : [`Run \`${formatCliCommand("brikko-studio gateway status --deep")}\` for more detail.`],
      });
      runtime.exit(1);
      return;
    }
      ...
      logNonInteractiveOnboardingFailure({
        ...
        hints: !opts.installDaemon
          ? [
              ...
              `Fix: start \`${formatCliCommand("brikko-studio gateway run")}\`, re-run with \`--install-daemon\`, or use \`--skip-health\`.`,
