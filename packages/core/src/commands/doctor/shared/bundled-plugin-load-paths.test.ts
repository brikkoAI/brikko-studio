...
import { bundledDistPluginRootAt, bundledPluginRootAt } from "brikko-studio/plugin-sdk/test-fixtures";
...
function bundled(pluginId: string, localPath: string): BundledPluginSource {
  return {
    pluginId,
    localPath,
    npmSpec: `@brikko-studio/${pluginId}`,
  };
}
...
describe("bundled plugin load path repair", () => {
  beforeEach(() => {
    const packageRoot = "/app/node_modules/brikko-studio";
  ...
  it("ignores bundled plugins that already resolve to source extensions", () => {
    const sourcePath = path.resolve("repo", "brikko-studio", "extensions", "feishu");
    vi.spyOn(bundledSources, "resolveBundledPluginSources").mockReturnValue(
