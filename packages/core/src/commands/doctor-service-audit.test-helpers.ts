...
export function readEmbeddedGatewayTokenForTest(
  ...
  return isEnvironmentFileOnlySource(command?.environmentValueSources?.BRIKKO_STUDIO_GATEWAY_TOKEN)
    ? undefined
    : normalizeOptionalString(command?.environment?.BRIKKO_STUDIO_GATEWAY_TOKEN);
}
