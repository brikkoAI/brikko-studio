...
import {
  BRIKKO_STUDIO_WRAPPER_ENV_KEY,
  resolveGatewayProgramArguments,
  resolveBrikko StudioWrapperPath,
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperPath = await resolveBrikko StudioWrapperPath(
    params.wrapperPath ?? params.env[BRIKKO_STUDIO_WRAPPER_ENV_KEY],
  );
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [BRIKKO_STUDIO_WRAPPER_ENV_KEY]: wrapperPath }
    : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.BRIKKO_STUDIO_PROFILE)
        : undefined,
    platform,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("brikko-studio gateway install")}\` after fixing the error.`;
}
