...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-gateway-health-auth-"));
  try {
...
describe("resolveGatewayHealthProbeToken", () => {
  ...
  it("does not fall back to stale BRIKKO_STUDIO_GATEWAY_TOKEN when a SecretRef is unresolved", async () => {
    await withTempDir(async (dir) => {
      process.env.BRIKKO_STUDIO_GATEWAY_TOKEN = "stale-env-token";
  ...
  it("resolves password auth for the local onboarding health probe", async () => {
    process.env.BRIKKO_STUDIO_GATEWAY_TOKEN = "stale-env-token";
    process.env.BRIKKO_STUDIO_GATEWAY_PASSWORD = "resolved-password"; // pragma: allowlist secret
