...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (!loaded) {
    ...
    if (params.cfg.gateway?.mode !== "remote") {
      ...
      if (!install) {
        note(
          `Run ${formatCliCommand("brikko-studio gateway install")} when you want to install the gateway service.`,
          "Gateway",
        );
      }
      if (install) {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.BRIKKO_STUDIO_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("brikko-studio gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
