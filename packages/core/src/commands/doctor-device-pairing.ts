...
function resolvePendingPairingIssue(
  ...
  const approveCommand = formatCliArgs(["brikko-studio", "devices", "approve", pending.requestId]);
  const inspectCommand = formatCliArgs(["brikko-studio", "devices", "list"]);
  if (!paired) {
    return {
      kind: "first-time",
      pending,
  ...
  if (paired.publicKey !== pending.publicKey) {
    return {
      ...
      approveCommand,
      inspectCommand,
      removeCommand: formatCliArgs(["brikko-studio", "devices", "remove", pending.deviceId]),
    };
  }
...
function formatPairingStoreReadIssue(error: JsonFileReadError): string {
  ...
  return `- Device pairing store ${error.filePath} ${problem}. Brikko Studio refused to treat it as empty to avoid overwriting approved pairings. Fix the JSON or file permissions, or move it aside and re-pair devices.`;
}
