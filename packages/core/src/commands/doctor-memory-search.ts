...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("brikko-studio doctor --fix")} or ${formatCliCommand("brikko-studio memory status --fix")}`
    : `Verify: ${formatCliCommand("brikko-studio memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: Brikko StudioConfig): Promise<void> {
  try {
 ...
 * Runs as part of `brikko-studio doctor` — config-only checks where possible;
...
export async function noteMemorySearchHealth(
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("brikko-studio config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("brikko-studio config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("brikko-studio memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`brikko-studio config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    if (isKeyOptionalMemoryProvider(resolved.provider)) {
      ...
      // not warn. A skipped probe means the user ran `brikko-studio doctor` without
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("brikko-studio configure --section model")}`,
        `- To disable: ${formatCliCommand("brikko-studio config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("brikko-studio memory status --deep")}`,
      ].join("\n"),
      "Memory search",
