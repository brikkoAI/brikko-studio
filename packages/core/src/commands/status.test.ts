...
beforeAll(() => {
  envSnapshot = captureEnv(["BRIKKO_STUDIO_PROFILE"]);
  process.env.BRIKKO_STUDIO_PROFILE = "isolated";
});
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/brikko-studio/pnpm-lock.yaml",
        markerPath: "/tmp/brikko-studio/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.BRIKKO_STUDIO_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.brikko-studio.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.brikko-studio.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.BRIKKO_STUDIO_GATEWAY_TOKEN?.trim();
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("brikko-studio status --all") ||
          line.includes("brikko-studio --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("brikko-studio --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("BRIKKO_STUDIO_GATEWAY_TOKEN", "abcd1234", async () => {
