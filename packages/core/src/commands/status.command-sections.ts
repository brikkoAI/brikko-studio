...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("brikko-studio security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("brikko-studio security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusFooterLines(params: {
  ...
  return [
    ...
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("brikko-studio status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("brikko-studio logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("brikko-studio node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("brikko-studio status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("brikko-studio gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`brikko-studio devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("brikko-studio devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("brikko-studio devices list")}`),
  ];
}
