...
vi.mock("../config/sessions/transcript-resolve.runtime.js", () => {
  ...
  const resolveSessionFile = (sessionId: string, agentId: string, sessionsDir?: string): string =>
    joinPath(sessionsDir ?? ".brikko-studio", "agents", agentId, "sessions", `${sessionId}.jsonl`);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "brikko-studio-agent-", skipSessionCleanup: true });
}

function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<Brikko StudioConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<Brikko StudioConfig["channels"]>["telegram"]>>,
...
describe("agentCommand", () => {
  ...
  it("uses no-tools plain prompt mode for one-shot model runs", async () => {
    await withTempHome(async (home) => {
      ...
      await agentCommand(
        {
          message: "Reply with exactly BRIKKO_STUDIO-MODEL-OK",
          agentId: "main",
          model: "openrouter/auto",
          modelRun: true,
          promptMode: "none",
    ...
    await withTempHome(async (home) => {
      ...
      expect(callArgs).toEqual(
        expect.objectContaining({
          provider: "openrouter",
          model: "openrouter/auto",
          prompt: "Reply with exactly BRIKKO_STUDIO-MODEL-OK",
          modelRun: true,
          promptMode: "none",
          disableTools: true,
        }),
