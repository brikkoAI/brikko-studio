...
export async function setupSkills(
  ...
  let next: Brikko StudioConfig = cfg;
  if (installable.length > 0) {
    ...
    for (const name of selected) {
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("brikko-studio doctor")}\` to review skills + requirements.`,
      );
      runtime.log("Docs: https://docs.brikko-studio.ai/skills");
    }
  }
