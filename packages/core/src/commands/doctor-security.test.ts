...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    ...
    pluginRegistry.list = [];
    prevToken = process.env.BRIKKO_STUDIO_GATEWAY_TOKEN;
    prevPassword = process.env.BRIKKO_STUDIO_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    ...
    if (prevPassword === undefined) {
      ...
      process.env.BRIKKO_STUDIO_GATEWAY_PASSWORD = prevPassword;
    }
  ...
  async function withExecApprovalsFile(
    ...
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-doctor-security-"));
    process.env.HOME = home;
    await fs.mkdir(path.join(home, ".brikko-studio"), { recursive: true });
    await fs.writeFile(
      path.join(home, ".brikko-studio", "exec-approvals.json"),
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as Brikko StudioConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.BRIKKO_STUDIO_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as Brikko StudioConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as Brikko StudioConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats unset bind as loopback for host-side doctor checks", async () => {
    const cfg = { gateway: {} } as Brikko StudioConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).toContain("No channel security warnings detected");
    expect(message).not.toContain("Gateway bound");
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as Brikko StudioConfig;
    await noteSecurityWarnings(cfg);
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("brikko-studio approvals get --gateway");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    await noteSecurityWarnings({} as Brikko StudioConfig);
    ...
    expect(message).toContain("Run: brikko-studio security audit --deep");
  });
