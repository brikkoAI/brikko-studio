...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-brikko-studio-backup";
...
describe("backupVerifyCommand", () => {
  ...
  it("verifies a valid backup archive", async () => {
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-backup-verify-out-"));
    try {
      ...
      const payloadArchivePath = `${archiveRoot}/payload/posix/tmp/.brikko-studio/state.txt`;
      await fs.writeFile(
        manifestPath,
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-brikko-studio-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.brikko-studio",
            archivePath: `${rootName}/payload/posix/tmp/.brikko-studio`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("rejects unsafe archive paths", async () => {
    for (const { tempPrefix, archivePath, error } of [
      {
        tempPrefix: "brikko-studio-backup-traversal-",
        archivePath: `${TEST_ARCHIVE_ROOT}/payload/../escaped.txt`,
        error: /path traversal segments/i,
      },
      {
        tempPrefix: "brikko-studio-backup-backslash-",
        archivePath: `${TEST_ARCHIVE_ROOT}/payload\\..\\escaped.txt`,
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    ...
    try {
      ...
      const stateArchivePath = `${archiveRoot}/payload/posix/tmp/.brikko-studio/state.txt`;
  ...
  it("rejects duplicate manifest and payload entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.brikko-studio/payload.txt`;
    for (const options of [
      {
        tempPrefix: "brikko-studio-backup-duplicate-manifest-",
        payloads: [{ fileName: "payload.txt", contents: "payload\n" }],
        buildTarEntries: ({
          manifestPath,
          payloadPaths,
      ...
      {
        tempPrefix: "brikko-studio-backup-duplicate-payload-",
        payloads: [
