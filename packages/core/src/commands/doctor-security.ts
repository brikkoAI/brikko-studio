  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("brikko-studio approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
export async function noteSecurityWarnings(cfg: Brikko StudioConfig) {
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      "  Host exec gating still comes from ~/.brikko-studio/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("brikko-studio approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.brikko-studio.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("brikko-studio configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("brikko-studio config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("brikko-studio doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("brikko-studio config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('brikko-studio config set session.dmScope "per-channel-peer"') +
