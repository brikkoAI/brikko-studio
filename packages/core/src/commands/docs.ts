...
const SEARCH_TOOL = "https://docs.brikko-studio.ai/mcp.SearchBrikko Studio";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.brikko-studio.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('brikko-studio docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.brikko-studio.ai/");
      runtime.log(`Search: ${formatCliCommand('brikko-studio docs "your query"')}`);
    }
    return;
  }
