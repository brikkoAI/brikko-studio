...
import { bundledPluginRoot, bundledPluginRootAt } from "brikko-studio/plugin-sdk/test-fixtures";
...
const bundledChatNpmSpec = "@brikko-studio/bundled-chat@1.2.3";
  ...
  resolveBundledPluginSources.mockReturnValue(
    new Map([
      [
        ...
        {
          pluginId: "bundled-chat",
          localPath: bundledPluginRootAt("/opt/brikko-studio", "bundled-chat"),
          npmSpec: bundledChatNpmSpec,
        },
      ],
    ]),
...
function createManifestRecord(
  ...
  return {
    ...
    hooks: [],
    origin: "bundled",
    rootDir: `/tmp/brikko-studio-test/${id}`,
    source: `/tmp/brikko-studio-test/${id}/index.ts`,
    manifestPath: `/tmp/brikko-studio-test/${id}/brikko-studio.plugin.json`,
    ...rest,
  };
}
...
describe("ensureChannelSetupPluginInstalled", () => {
  ...
  it("installs npm channel plugins into the active profile extensions dir", async () => {
    ...
    const profileStateDir = "/tmp/brikko-studio-ledger-channel";
    process.env.BRIKKO_STUDIO_STATE_DIR = profileStateDir;
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "bundled-chat",
    ...
    expect(select).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        options: expect.arrayContaining([
          expect.objectContaining({
            value: "local",
            hint: bundledPluginRootAt("/opt/brikko-studio", "bundled-chat"),
          }),
        ]),
      }),
    );
  ...
  it("offers ClawHub as the first-class install source for channel catalog entries", async () => {
    ...
    await ensureChannelSetupPluginInstalled({
      ...
      entry: {
        ...
        install: {
          clawhubSpec: "clawhub:brikko-studio/clawhub-chat@2026.5.2",
    ...
    expect(select).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        options: [
          expect.objectContaining({
            value: "clawhub",
            label: "Download from ClawHub (clawhub:brikko-studio/clawhub-chat@2026.5.2)",
          }),
          expect.objectContaining({
            value: "skip",
          }),
  ...
  it("skips the install prompt when autoConfirmSingleSource is set and only npm is available", async () => {
    ...
    const npmOnlyEntry: ChannelPluginCatalogEntry = {
      ...
      install: {
        npmSpec: "@brikko-studio/wecom@2026.4.23",
      },
    };
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "wecom-brikko-studio-plugin",
      installPath: "/tmp/wecom-brikko-studio-plugin",
    });
    vi.mocked(fs.existsSync).mockReturnValue(false);
    resolveBundledPluginSources.mockReturnValue(new Map());
    ...
    expect(select).not.toHaveBeenCalled();
    expect(result.installed).toBe(true);
    expect(result.pluginId).toBe("wecom-brikko-studio-plugin");
  });
