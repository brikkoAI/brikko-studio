...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".brikko-studio", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: Brikko StudioConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [
    ["BRIKKO_STUDIO_GATEWAY_TOKEN", await getenv("BRIKKO_STUDIO_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["BRIKKO_STUDIO_GATEWAY_PASSWORD", await getenv("BRIKKO_STUDIO_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "BRIKKO_STUDIO_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = normalizeOptionalString(env.BRIKKO_STUDIO_NO_RESPAWN) ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- BRIKKO_STUDIO_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
