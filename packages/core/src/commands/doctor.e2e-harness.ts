...
vi.mock("../config/config.js", async () => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/brikko-studio.json",
    createConfigIO,
...
vi.mock("brikko-studio/plugin-sdk/provider-stream-shared", () => ({
...
beforeEach(() => {
  ...
  originalUpdateInProgress = process.env.BRIKKO_STUDIO_UPDATE_IN_PROGRESS;
  process.env.BRIKKO_STUDIO_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-doctor-state-"));
