...
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  const updateInProgress = isTruthyEnvValue(process.env.BRIKKO_STUDIO_UPDATE_IN_PROGRESS);
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("brikko-studio update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
