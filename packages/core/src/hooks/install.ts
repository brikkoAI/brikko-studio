...
async function ensureBrikkoStudioHooks(manifest: HookPackageManifest) {
  ...
  if (list.length === 0) {
    throw new Error("package.json brikko-studio.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(
  ...
  for (const entry of hookEntries) {
    ...
    if (!runtime.isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `brikko-studio.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `brikko-studio.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromArchive(
  ...
  return await runtime.withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "brikko-studio-hook-",
    timeoutMs,
    logger,
...
export async function installHooksFromNpmSpec(params: {
  ...
  return await runtime.installFromValidatedNpmSpecArchive({
    tempDirPrefix: "brikko-studio-hook-pack-",
    spec,
    timeoutMs,
