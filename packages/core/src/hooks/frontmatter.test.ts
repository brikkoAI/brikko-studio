...
`;
    ...
    expect(parsed.brikko-studio.emoji).toBe("💾");
    expect(parsed.brikko-studio.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.brikko-studio.emoji).toBe("📝");
    expect(parsed.brikko-studio.events).toEqual(["command"]);
    expect(parsed.brikko-studio.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.brikko-studio.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"brikko-studio": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"brikko-studio": {"events": ["test"]}}');
  });
...
describe("resolveBrikko StudioMetadata", () => {
  ...
  it("returns undefined when brikko-studio key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    ...
    expect(result?.install?.[1].package).toBe("@brikko-studio/hook");
  });
...
`;
    ...
    expect(brikko-studio?.requires?.config).toEqual(["workspace.dir"]);
    expect(brikko-studio?.install?.[0].kind).toBe("bundled");
  });
