...
This directory contains hooks that ship with Brikko Studio. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.brikko-studio/workspace`)
...
homepage: https://docs.brikko-studio.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.brikko-studio/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.brikko-studio/brikko-studio.json`:
...
2. Restart gateway: `pkill -9 -f 'brikko-studio.*gateway' && pnpm brikko-studio gateway`
3. Enable the hook: `brikko-studio hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.brikko-studio.ai/automation/hooks
