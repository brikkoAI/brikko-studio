...
homepage: https://docs.brikko-studio.ai/automation/hooks#command-logger
metadata:
  {
    "brikko-studio":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.brikko-studio/logs/commands.log`
...
`~/.brikko-studio/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.brikko-studio/logs/commands.log ~/.brikko-studio/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/brikko-studio`:
   ```
   /home/username/.brikko-studio/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
