...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "brikko-studio-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.BRIKKO_STUDIO_BUNDLED_HOOKS_DIR;
    process.env.BRIKKO_STUDIO_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.BRIKKO_STUDIO_BUNDLED_HOOKS_DIR;
    } else {
      process.env.BRIKKO_STUDIO_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".brikko-studio", "extensions", "sample-bundle");
    ...
    await fsp.writeFile(
      ...
      [
        ...
        'metadata: {"brikko-studio":{"events":["command:new"]}}',
        "---",
        "",
        "# Bundle hook",
        "",
  ...
  function createConfig(enabled: boolean): Brikko StudioConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("brikko-studio-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Brikko Studio hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".brikko-studio", "extensions", "claude-bundle");
