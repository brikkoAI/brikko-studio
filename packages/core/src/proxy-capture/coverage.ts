...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[brikko-studio proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[brikko-studio proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`brikko-studio proxy coverage\` for details.`,
  );
}
