...
import { runRealtimeSttLiveTest } from "brikko-studio/plugin-sdk/provider-test-contracts";
...
const LIVE_MODEL_ID = process.env.BRIKKO_STUDIO_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.5";
const LIVE_IMAGE_MODEL = process.env.BRIKKO_STUDIO_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const LIVE_VISION_MODEL = process.env.BRIKKO_STUDIO_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-4.1-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.BRIKKO_STUDIO_LIVE_TEST === "1";
