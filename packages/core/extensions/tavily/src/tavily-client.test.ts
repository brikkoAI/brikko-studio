...
vi.mock("brikko-studio/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: brikko-studio", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "brikko-studio" });
  });

  it("runTavilyExtract sends X-Client-Source: brikko-studio", async () => {
