...
} from "brikko-studio/plugin-sdk/markdown-table-runtime";
import { requireRuntimeConfig } from "brikko-studio/plugin-sdk/plugin-config-runtime";
import { normalizePollInput, type PollInput } from "brikko-studio/plugin-sdk/poll-runtime";
import { createSubsystemLogger, getChildLogger } from "brikko-studio/plugin-sdk/runtime-env";
import {
...
function requireOutboundActiveWebListener(params: { cfg: BrikkoStudioConfig; accountId?: string }): {
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`brikko-studio channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
