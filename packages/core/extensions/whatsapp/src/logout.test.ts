...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "brikko-studio-test-web-logout-"));
    previousOAuthDir = process.env.BRIKKO_STUDIO_OAUTH_DIR;
    process.env.BRIKKO_STUDIO_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  afterAll(async () => {
    if (previousOAuthDir === undefined) {
      ...
      process.env.BRIKKO_STUDIO_OAUTH_DIR = previousOAuthDir;
    }
  ...
  it("does not delete through symlinked auth dirs inside the Brikko Studio auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the Brikko Studio auth root", async () => {
