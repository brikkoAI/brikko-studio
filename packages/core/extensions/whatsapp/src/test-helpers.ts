...
import { formatEnvelopeTimestamp } from "brikko-studio/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("brikko-studio:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("brikko-studio:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[brikko-studio]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/brikko-studio-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("brikko-studio:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("brikko-studio:lastSocket")];
  if (typeof getter === "function") {
