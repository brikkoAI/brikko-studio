...
import { mockPinnedHostnameResolution } from "brikko-studio/plugin-sdk/test-env";
import { optimizeImageToPng } from "brikko-studio/plugin-sdk/web-media";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredBrikko StudioTmpDir(), "brikko-studio-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub BRIKKO_STUDIO_STATE_DIR.
    // Also keep it outside the Brikko Studio temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["BRIKKO_STUDIO_STATE_DIR"]);
    process.env.BRIKKO_STUDIO_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "brikko-studio-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredBrikko StudioTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default Brikko Studio state workspace and sandbox roots", async () => {
  ...
  it("rejects default Brikko Studio state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
