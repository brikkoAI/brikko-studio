...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredBrikkoStudioTmpDir(), "brikko-studio-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    ...
    // Also keep it outside the BrikkoStudio temp root so default localRoots doesn't accidentally make all state readable.
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredBrikkoStudioTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default BrikkoStudio state workspace and sandbox roots", async () => {
  ...
  it("rejects default BrikkoStudio state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
