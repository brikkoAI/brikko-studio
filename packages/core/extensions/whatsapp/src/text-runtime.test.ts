...
describe("jidToE164", () => {
  it("maps @lid using reverse mapping file", async () => {
    await withTempDir("brikko-studio-state-", async (stateDir) => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("brikko-studio-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("brikko-studio-lid-a-", async (first) => {
      await withTempDir("brikko-studio-lid-b-", (second) => {
