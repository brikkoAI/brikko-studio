...
import { resetLogger, setLoggerOverride } from "brikko-studio/plugin-sdk/runtime-env";
...
function mockCredsJsonSpies(readContents: string) {
  const credsSuffix = path.join("/tmp", "brikko-studio-oauth", "whatsapp", "default", "creds.json");
...
describe("web session", () => {
  ...
  it("creates WA socket with QR handler", async () => {
    const authDir = createTempAuthDir("brikko-studio-wa-creds-test");
  ...
  it("serializes creds.update saves to avoid overlapping writes", async () => {
    ...
    const authDir = createTempAuthDir("brikko-studio-wa-queue");
    const openMock = mockFsOpenForCredsWrites({
  ...
  it("lets different authDir queues flush independently", async () => {
    ...
    const authDirA = createTempAuthDir("brikko-studio-wa-a");
    const authDirB = createTempAuthDir("brikko-studio-wa-b");
    const onError = vi.fn();
  ...
  it("writes creds.json atomically via temp file and rename", async () => {
    ...
    await writeCredsJsonAtomically("/tmp/brikko-studio-oauth/whatsapp/default", {
    ...
    expect(String(renameArgs[1] ?? "")).toContain(
      path.join("/tmp", "brikko-studio-oauth", "whatsapp", "default", "creds.json"),
    );
  ...
  it("keeps the previous creds.json valid if the atomic rename fails", async () => {
    const authDir = createTempAuthDir("brikko-studio-wa-creds-atomic");
