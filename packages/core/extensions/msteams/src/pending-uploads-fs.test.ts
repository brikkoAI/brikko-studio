...
async function makeTempStateDir(): Promise<string> {
  const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "brikko-studio-msteams-pending-"));
  createdTempDirs.push(dir);
  return dir;
}

function makeEnv(stateDir: string): NodeJS.ProcessEnv {
  return { ...process.env, BRIKKO_STUDIO_STATE_DIR: stateDir };
}
...
describe("prepareFileConsentActivityFs end-to-end", () => {
  ...
  it("writes the pending upload to the fs store with the same id as the card", async () => {
    ...
    const originalEnv = process.env.BRIKKO_STUDIO_STATE_DIR;
    ...
    try {
      ...
      if (originalEnv === undefined) {
        ...
        process.env.BRIKKO_STUDIO_STATE_DIR = originalEnv;
      }
    }
  });
});
