...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.BRIKKO_STUDIO_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    const started = await startAdvertiser({
      ...
      cliPath: "/opt/homebrew/bin/brikko-studio",
      minimal: false,
    });

    expect(createService).toHaveBeenCalledTimes(1);
    const [gatewayCall] = createService.mock.calls as Array<[Record<string, unknown>]>;
    expect(gatewayCall?.[0]?.type).toBe("brikko-studio-gw");
    ...
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.cliPath).toBe(
      "/opt/homebrew/bin/brikko-studio",
    );
  ...
  it("honors truthy BRIKKO_STUDIO_DISABLE_BONJOUR values", async () => {
    enableAdvertiserUnitMode();
    process.env.BRIKKO_STUDIO_DISABLE_BONJOUR = "true";
  ...
  it("honors explicit Bonjour opt-in inside detected containers", async () => {
    enableAdvertiserUnitMode();
    process.env.BRIKKO_STUDIO_DISABLE_BONJOUR = "0";
    vi.spyOn(fs, "existsSync").mockImplementation((filePath) => String(filePath) === "/.dockerenv");
  ...
  it("suppresses ciao self-probe retry console noise while advertising", async () => {
    ...
    try {
      ...
      console.log(
        "[test._brikko-studio-gw._tcp.local.] failed probing with reason: Error: Can't probe for a service which is announced already. Received announcing for service test._brikko-studio-gw._tcp.local.. Trying again in 2 seconds!",
      );
  ...
  it("falls back to brikko-studio when system hostname is invalid for DNS", async () => {
    // Allow advertiser to run in unit tests.
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.BRIKKO_STUDIO_MDNS_HOSTNAME;
    ...
    const [gatewayCall] = createService.mock.calls as Array<[ServiceCall]>;
    expect(gatewayCall?.[0]?.hostname).toBe("brikko-studio");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("brikko-studio.local");
  ...
  it("uses system hostname when BRIKKO_STUDIO_MDNS_HOSTNAME is unset", async () => {
    // Allow advertiser to run in unit tests.
    delete process.env.VITEST;
    process.env.NODE_ENV = "development";
    delete process.env.BRIKKO_STUDIO_MDNS_HOSTNAME;
