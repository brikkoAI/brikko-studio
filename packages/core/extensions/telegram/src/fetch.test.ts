import { resolveFetch } from "brikko-studio/plugin-sdk/fetch-runtime";
...
describe("resolveTelegramFetch", () => {
  ...
  it("uses BRIKKO_STUDIO_PROXY_URL as a Telegram explicit proxy when proxy env is absent", async () => {
    vi.stubEnv("BRIKKO_STUDIO_PROXY_URL", "http://127.0.0.1:7788");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
  ...
  it("preserves caller-provided custom fetch when BRIKKO_STUDIO_PROXY_URL is present", async () => {
    vi.stubEnv("BRIKKO_STUDIO_PROXY_URL", "http://127.0.0.1:7788");
    const proxyFetch = vi.fn(async () => ({ ok: true }) as Response) as unknown as typeof fetch;
  ...
  it("prefers standard proxy env over BRIKKO_STUDIO_PROXY_URL for Telegram", async () => {
    vi.stubEnv("BRIKKO_STUDIO_PROXY_URL", "http://127.0.0.1:7788");
    vi.stubEnv("https_proxy", "http://127.0.0.1:7890");
    undiciFetch.mockResolvedValue({ ok: true } as Response);
