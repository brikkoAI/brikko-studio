...
type ResolveConfiguredBindingRouteFn =
  typeof import("brikko-studio/plugin-sdk/conversation-runtime").resolveConfiguredBindingRoute;
type EnsureConfiguredBindingRouteReadyFn =
  typeof import("brikko-studio/plugin-sdk/conversation-runtime").ensureConfiguredBindingRouteReady;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("brikko-studio/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
vi.mock("brikko-studio/plugin-sdk/session-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("brikko-studio/plugin-sdk/session-store-runtime")>(
    "brikko-studio/plugin-sdk/session-store-runtime",
  );
  return {
    ...actual,
...
vi.mock("brikko-studio/plugin-sdk/command-auth-native", async () => {
  const actual = await vi.importActual<typeof import("brikko-studio/plugin-sdk/command-auth-native")>(
    "brikko-studio/plugin-sdk/command-auth-native",
  );
...
vi.mock("brikko-studio/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("brikko-studio/plugin-sdk/plugin-runtime")>(
    "brikko-studio/plugin-sdk/plugin-runtime",
  );
  return {
    ...actual,
...
describe("registerTelegramNativeCommands — session metadata", () => {
  ...
  beforeEach(() => {
    ...
    sessionMocks.resolveAndPersistSessionFile.mockClear().mockImplementation(async (params) => {
      const sessionFile =
        params.fallbackSessionFile ?? `/tmp/brikko-studio-sessions/${params.sessionId}.jsonl`;
      return {
        sessionFile,
        sessionEntry: {
          ...params.sessionEntry,
    ...
    sessionMocks.resolveStorePath.mockClear().mockReturnValue("/tmp/brikko-studio-sessions.json");
  ...
  it("uses the target session model when building native argument menus", async () => {
    ...
    expect(sessionMocks.loadSessionStore).toHaveBeenCalledWith("/tmp/brikko-studio-sessions.json");
    expect(sendMessage).toHaveBeenCalledWith(
      100,
  ...
  it("passes a persisted topic session file to plugin commands", async () => {
    sessionMocks.resolveStorePath.mockReturnValue("/tmp/brikko-studio-sessions/sessions.json");
    sessionMocks.loadSessionStore.mockReturnValue({
    ...
    const { handler } = registerAndResolveCommandHandler({
      commandName: "codex",
      cfg: { commands: { allowFrom: { telegram: ["200"] } } } as Brikko StudioConfig,
    ...
    expect(sessionMocks.resolveAndPersistSessionFile).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        storePath: "/tmp/brikko-studio-sessions/sessions.json",
        sessionsDir: "/tmp/brikko-studio-sessions",
        fallbackSessionFile: "/tmp/brikko-studio-sessions/sess-topic-topic-42.jsonl",
      }),
    );
    expect(pluginRuntimeMocks.executePluginCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        sessionKey: "agent:main:telegram:group:-1001234567890:topic:42",
        sessionId: "sess-topic",
        sessionFile: "/tmp/brikko-studio-sessions/sess-topic-topic-42.jsonl",
        messageThreadId: 42,
      }),
    );
  });
