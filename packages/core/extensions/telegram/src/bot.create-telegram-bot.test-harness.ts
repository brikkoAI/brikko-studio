...
type GetRuntimeConfigFn =
  typeof import("brikko-studio/plugin-sdk/runtime-config-snapshot").getRuntimeConfig;
type LoadSessionStoreFn =
  typeof import("brikko-studio/plugin-sdk/session-store-runtime").loadSessionStore;
type ResolveStorePathFn =
  typeof import("brikko-studio/plugin-sdk/session-store-runtime").resolveStorePath;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/brikko-studio-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
