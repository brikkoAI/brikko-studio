...
vi.mock("brikko-studio/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("brikko-studio/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("brikko-studio/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("brikko-studio/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("brikko-studio/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("brikko-studio/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/brikko-studio/synology-chat-sessions.json"),
