...
import { normalizeOptionalAccountId } from "brikko-studio/plugin-sdk/account-id";
import { getChatChannelMeta, type ChannelPlugin } from "brikko-studio/plugin-sdk/core";
import {
  formatDocsLink,
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: BRIKKO_STUDIO_TWITCH_ACCESS_TOKEN",
...
export async function configureWithEnvToken(
  ...
  const useEnv = await prompter.confirm({
    message: "Twitch env var BRIKKO_STUDIO_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId: requestedAccountId, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.BRIKKO_STUDIO_TWITCH_ACCESS_TOKEN?.trim();
