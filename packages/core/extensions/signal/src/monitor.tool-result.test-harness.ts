...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/brikko-studio-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("brikko-studio/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("brikko-studio/plugin-sdk/security-runtime")>(
    "brikko-studio/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
vi.mock("brikko-studio/plugin-sdk/system-event-runtime", async () => {
  const actual = await vi.importActual<typeof import("brikko-studio/plugin-sdk/system-event-runtime")>(
    "brikko-studio/plugin-sdk/system-event-runtime",
  );
  return {
    ...actual,
...
vi.mock("brikko-studio/plugin-sdk/transport-ready-runtime", () => ({
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("brikko-studio/plugin-sdk/reply-runtime"),
      import("brikko-studio/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    config = {
