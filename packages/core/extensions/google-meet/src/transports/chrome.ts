  ...
  if (result.code !== 0 || !outputMentionsBlackHole2ch(output)) {
    ...
    throw new Error(
      [
        ...
        "Install BlackHole 2ch and route Chrome input/output through the BrikkoStudio audio bridge.",
        hint,
      ]
        .filter(Boolean)
        .join(" "),
...
function meetStatusScript(params: {
  ...
  if (!inCall && (host === "accounts.google.com" || /use your google account|to continue to google meet|choose an account|sign in to (join|continue)/i.test(pageText))) {
    ...
    manualActionMessage = "Sign in to Google in the BrikkoStudio browser profile, then retry the Meet join.";
    ...
    manualActionMessage = "Admit the BrikkoStudio browser participant in Google Meet, then retry speech.";
  } else if (permissionNeeded) {
    manualActionReason = "meet-permission-required";
    manualActionMessage = allowMicrophone
      ? "Allow microphone/camera/speaker permissions for Meet in the BrikkoStudio browser profile, then retry."
      : "Join without microphone/camera permissions in the BrikkoStudio browser profile, then retry.";
    ...
    manualActionMessage = allowMicrophone
      ? "Meet is showing the microphone choice. Click Use microphone in the BrikkoStudio browser profile, then retry."
      : "Meet is showing the microphone choice. Choose the no-microphone option in the BrikkoStudio browser profile, then retry.";
  }
  ...
  do {
    try {
      ...
      browser = {
        ...
        manualActionMessage:
          "Open the BrikkoStudio browser profile, finish Google Meet login, admission, or permission prompts, then retry.",
        notes: [
          ...permissionNotes,
