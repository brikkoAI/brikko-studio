...
metadata: { "brikko-studio": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Brikko Studio `sessions_spawn`
- **File I/O** == Brikko Studio `read`/`write`
- **Remote fetch** == Brikko Studio `web_fetch` (or `exec` with curl when POST is required)
