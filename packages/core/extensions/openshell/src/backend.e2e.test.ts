...
import { createSandboxTestContext } from "brikko-studio/plugin-sdk/test-fixtures";
import {
...
const BRIKKO_STUDIO_OPENSHELL_E2E = process.env.BRIKKO_STUDIO_E2E_OPENSHELL === "1";
const BRIKKO_STUDIO_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const BRIKKO_STUDIO_OPENSHELL_COMMAND =
  process.env.BRIKKO_STUDIO_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "brikko-studio-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && BRIKKO_STUDIO_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: BRIKKO_STUDIO_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(BRIKKO_STUDIO_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(BRIKKO_STUDIO_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-openshell-e2e-"));
      ...
      const gatewayName = `brikko-studio-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `brikko-studio-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/brikko-studio-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: BRIKKO_STUDIO_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("brikko-studio-openshell-e2e");
