...
describe("openshell backend manager", () => {
  ...
  it("checks runtime status with config override from Brikko Studio config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "brikko-studio-session-1234",
        backendId: "openshell",
        runtimeLabel: "brikko-studio-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: expect.objectContaining({
        sandboxName: "brikko-studio-session-1234",
        config: expect.objectContaining({
          from: "custom-source",
        }),
      }),
      args: ["sandbox", "get", "brikko-studio-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "brikko-studio-session-5678",
        backendId: "openshell",
        runtimeLabel: "brikko-studio-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "brikko-studio",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: expect.objectContaining({
        sandboxName: "brikko-studio-session-5678",
        config: expect.objectContaining({
          command: "/usr/local/bin/openshell",
          gateway: "lab",
        }),
      }),
      args: ["sandbox", "delete", "brikko-studio-session-5678"],
    });
  });
});
...
describe("openshell fs bridges", () => {
  it("writes locally and syncs the file to the remote workspace", async () => {
    const workspaceDir = await makeTempDir("brikko-studio-openshell-fs-");
  ...
  it("rejects symlink-parent writes instead of escaping the local mount root", async () => {
    const workspaceDir = await makeTempDir("brikko-studio-openshell-fs-");
    const outsideDir = await makeTempDir("brikko-studio-openshell-outside-");
  ...
  it("maps agent mount paths when the sandbox workspace is read-only", async () => {
    const workspaceDir = await makeTempDir("brikko-studio-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("brikko-studio-openshell-agent-");
