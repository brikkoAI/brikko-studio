...
function makeStateDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-catchup-test-"));
...
describe("saveBlueBubblesCatchupCursor + loadBlueBubblesCatchupCursor — retry map", () => {
  ...
  it("filters malformed retry entries during load (zero, negative, non-numeric)", async () => {
    ...
    await saveBlueBubblesCatchupCursor("acct", 100);
    const stateRoot = process.env.BRIKKO_STUDIO_STATE_DIR;
    if (!stateRoot) {
      throw new Error("BRIKKO_STUDIO_STATE_DIR must be set by the test harness");
    }
