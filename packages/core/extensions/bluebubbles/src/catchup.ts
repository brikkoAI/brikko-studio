...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  // Explicit BRIKKO_STUDIO_STATE_DIR overrides take precedence (including
  // per-test mkdtemp dirs in this module's test suite).
  if (env.BRIKKO_STUDIO_STATE_DIR?.trim()) {
    return resolveStateDir(env);
  }
  // Default test isolation: per-pid tmpdir, no bleed into real ~/.brikko-studio.
  // Use resolvePreferredBrikko StudioTmpDir + string concat (mirrors
  ...
  // Canonical Brikko Studio state dir: honors `~` expansion + legacy/new
