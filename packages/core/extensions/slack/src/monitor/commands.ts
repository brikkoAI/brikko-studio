import type { SlackSlashCommandConfig } from "brikko-studio/plugin-sdk/config-types";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(
    normalizeOptionalString(raw?.name) ?? "brikko-studio",
  );
  const name = normalizedName || "brikko-studio";
  return {
