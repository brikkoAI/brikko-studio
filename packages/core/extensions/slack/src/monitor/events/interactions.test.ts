...
vi.mock("brikko-studio/plugin-sdk/system-event-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("brikko-studio/plugin-sdk/system-event-runtime")>();
  return {
    ...actual,
...
vi.mock("brikko-studio/plugin-sdk/approval-gateway-runtime", () => ({
...
describe("registerSlackInteractionEvents", () => {
  ...
  it("enqueues structured events and updates button rows", async () => {
    ...
    await handler!({
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "brikko-studio:verify" }],
            },
          ],
        },
      },
      action: {
        type: "button",
        action_id: "brikko-studio:verify",
        block_id: "verify_block",
        value: "approved",
  ...
  it("registers a matcher that accepts plugin action ids beyond the Brikko Studio prefix", () => {
    const { ctx, getActionMatcher } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });

    const matcher = getActionMatcher();
    expect(matcher).toBeTruthy();
    expect(matcher?.test("brikko-studio:verify")).toBe(true);
  ...
  it("treats Slack reply buttons as plain interaction events instead of plugin dispatch", async () => {
    ...
    await handler!({
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "brikko-studio:reply_button" }],
            },
          ],
        },
      },
      action: {
        type: "button",
        action_id: "brikko-studio:reply_button",
        block_id: "reply_actions",
        value: "codex",
    ...
    expect(enqueueSystemEventMock).toHaveBeenCalledWith(
      expect.stringContaining('"actionId":"brikko-studio:reply_button"'),
      expect.any(Object),
    );
    expect(app.client.chat.update).toHaveBeenCalledTimes(1);
  });
  ...
  it("drops modal lifecycle payloads when mismatch guard triggers", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V123",
          callback_id: "brikko-studio:deploy_form",
  ...
  it("falls back to container channel and message timestamps", async () => {
    ...
    await handler!({
      ...
      action: {
        type: "button",
        action_id: "brikko-studio:container",
          ...
          blocks: [
            {
              ...
              elements: [{ type: "multi_static_select", action_id: "brikko-studio:multi" }],
            },
          ],
        },
      },
  ...
  it("renders date/time/datetime picker selections in confirmation rows", async () => {
          ...
          blocks: [
            {
              type: "actions",
              block_id: "date_block",
              elements: [{ type: "datepicker", action_id: "brikko-studio:date" }],
            },
            {
              type: "actions",
              block_id: "time_block",
              elements: [{ type: "timepicker", action_id: "brikko-studio:time" }],
            },
            {
              type: "actions",
              block_id: "datetime_block",
              elements: [{ type: "datetimepicker", action_id: "brikko-studio:datetime" }],
            },
          ],
        },
      },
    ...
    await handler!({
      ...
      action: {
        type: "datetimepicker",
        action_id: "brikko-studio:datetime",
        block_id: "datetime_block",
  ...
  it("captures workflow button trigger metadata", async () => {
    ...
    await handler!({
      ...
      action: {
        ...
        action_id: "brikko-studio:workflow",
  ...
  it("captures modal submissions and enqueues view submission event", async () => {
    ...
    expect(payload).toMatchObject({
      ...
      actionId: "view:brikko-studio:deploy_form",
      callbackId: "brikko-studio:deploy_form",
      viewId: "V123",
      userId: "U777",
  ...
  it("keeps no-channel modal events open when allowFrom is unset", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V444",
          callback_id: "brikko-studio:routing_form",
            ...
            values: {
              ...
              email_block: {
                email_input: {
                  ...
                  value: "team@brikko-studio.ai",
                },
              },
              url_block: {
                url_input: {
                  type: "url_text_input",
                  value: "https://docs.brikko-studio.ai",
                },
              },
              richtext_block: {
                richtext_input: {
    ...
    expect(payload.inputs).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          inputKind: "email",
          inputEmail: "team@brikko-studio.ai",
        }),
        expect.objectContaining({
          actionId: "url_input",
          inputKind: "url",
          inputUrl: "https://docs.brikko-studio.ai/",
        }),
        expect.objectContaining({
  ...
  it("truncates rich text preview to keep payload summaries compact", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V555",
          callback_id: "brikko-studio:long_richtext",
  ...
  it("caps oversized interaction payloads with compact summaries", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V915",
          callback_id: "brikko-studio:oversize",
          private_metadata: JSON.stringify({
            channelId: "D915",
            channelType: "im",
            userId: "U915",
