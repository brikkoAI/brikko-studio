...
import { safeEqualSecret } from "brikko-studio/plugin-sdk/security-runtime";
import {
...
import { isTrustedProxyAddress, resolveClientIp, type Brikko StudioConfig } from "./runtime-api.js";

const INTERACTION_MAX_BODY_BYTES = 64 * 1024;
const INTERACTION_BODY_TIMEOUT_MS = 10_000;
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__brikko-studio_channel_id";
...
type InteractionCallbackConfig = Pick<Brikko StudioConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "brikko-studio-mattermost-interactions").update(botToken).digest("hex");
}
