...
} from "brikko-studio/plugin-sdk/error-runtime";
import { resolveGlobalMap } from "brikko-studio/plugin-sdk/global-singleton";
import * as memoryCoreHostRuntimeCoreModule from "brikko-studio/plugin-sdk/memory-core-host-runtime-core";
import * as runtimeConfigSnapshotModule from "brikko-studio/plugin-sdk/runtime-config-snapshot";
import * as sessionStoreRuntimeModule from "brikko-studio/plugin-sdk/session-store-runtime";
...
const DREAMS_FILE_LOCKS_KEY = Symbol.for("brikko-studio.memoryCore.dreamingNarrative.fileLocks");
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("brikko-studio:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("brikko-studio-dreaming-backfill-");
    ...
    expect(content.match(/brikko-studio:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("brikko-studio-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- brikko-studio:dreaming:diary:start -->");
    expect(content).toContain("<!-- brikko-studio:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- brikko-studio:dreaming:diary:start -->");
    const end = content.indexOf("<!-- brikko-studio:dreaming:diary:end -->");
  ...
  it("prepends diary before existing managed blocks", async () => {
    ...
    await fs.writeFile(
      dreamsPath,
      "## Light Sleep\n<!-- brikko-studio:dreaming:light:start -->\n- Candidate: test\n<!-- brikko-studio:dreaming:light:end -->\n",
      "utf-8",
    );
    await appendNarrativeEntry({
      workspaceDir,
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("brikko-studio-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
        "# Dream Diary",
        "",
        "<!-- brikko-studio:dreaming:diary:start -->",
        "---",
        "",
        ...
        "The server room smelled like rain.",
        "",
        "<!-- brikko-studio:dreaming:diary:end -->",
        "",
      ].join("\n"),
      "utf-8",
    );
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("brikko-studio-dreaming-narrative-");
    const stateDir = await createTempWorkspace("brikko-studio-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("brikko-studio-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("brikko-studio-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDir = await createTempWorkspace("brikko-studio-dreaming-detach-");
    const logger = createMockLogger();
