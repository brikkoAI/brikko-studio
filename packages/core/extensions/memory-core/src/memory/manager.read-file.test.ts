...
import { readMemoryFile } from "brikko-studio/plugin-sdk/memory-core-host-engine-storage";
...
describe("MemoryIndexManager.readFile", () => {
  ...
  beforeAll(async () => {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-mem-read-"));
