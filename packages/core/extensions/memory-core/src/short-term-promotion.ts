...
import { formatMemoryDreamingDay } from "brikko-studio/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "brikko-studio/plugin-sdk/memory-host-events";
...
const PROMOTION_MARKER_PREFIX = "brikko-studio-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*brikko-studio-memory-promotion:/i.test(snippet) ||
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*brikko-studio-memory-promotion:([^\n]+?)\s*-->/gi);
