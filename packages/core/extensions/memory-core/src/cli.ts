    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["brikko-studio memory status", "Show index and provider status."],
          [
            "brikko-studio memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["brikko-studio memory status --deep", "Probe embedding provider readiness."],
          ["brikko-studio memory index --force", "Force a full reindex."],
          ['brikko-studio memory search "meeting notes"', "Quick search using positional query."],
          [
            'brikko-studio memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `brikko-studio memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "brikko-studio memory rem-backfill --path ./memory",
          ...
          ["brikko-studio memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.brikko-studio.ai/cli/memory")}\n`,
    );
