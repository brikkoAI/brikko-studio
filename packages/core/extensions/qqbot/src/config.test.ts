...
import { type JsonSchemaObject, validateJsonSchemaValue } from "brikko-studio/plugin-sdk/config-schema";
...
describe("qqbot config", () => {
  it("accepts top-level speech overrides in the manifest schema", () => {
    const manifest = JSON.parse(
      fs.readFileSync(new URL("../brikko-studio.plugin.json", import.meta.url), "utf-8"),
  ...
  it("accepts SecretRef-backed credentials in the runtime schema", () => {
    const parsed = QQBotConfigSchema.safeParse({
      ...
      upgradeUrl: "https://docs.brikko-studio.ai/channels/qqbot",
      upgradeMode: "doc",
      accounts: {
        bot2: {
          appId: "654321",
  ...
  it("preserves top-level media and upgrade config on the default account", () => {
    ...
    expect(resolved.config.upgradeUrl).toBe("https://docs.brikko-studio.ai/channels/qqbot");
