...
function resolveQianfanPreset(cfg: Brikko StudioConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: Brikko StudioConfig) => {
...
export function applyQianfanProviderConfig(cfg: Brikko StudioConfig): Brikko StudioConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: Brikko StudioConfig): Brikko StudioConfig {
