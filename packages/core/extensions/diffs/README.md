...
Read-only diff viewer plugin for **Brikko Studio** agents.
...
Set plugin-wide defaults in `~/.brikko-studio/brikko-studio.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/brikko-studio",
        },
      },
    },
  },
...
Before:
Brikko Studio supports plugins.

After:
Brikko Studio supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/brikko-studio-diffs`).
