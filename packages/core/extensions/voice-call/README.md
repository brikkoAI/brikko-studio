...
Official Voice Call plugin for **Brikko Studio**.
...
Docs: `https://docs.brikko-studio.ai/plugins/voice-call`
Plugin system: `https://docs.brikko-studio.ai/plugin`

## Install (local dev)

### Option A: install via Brikko Studio (recommended)
...
```bash
PLUGIN_HOME=~/.brikko-studio/extensions
mkdir -p "$PLUGIN_HOME"
...
- If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `brikko-studio doctor --fix` to rewrite them.
- advanced webhook, streaming, and tunnel notes: `https://docs.brikko-studio.ai/plugins/voice-call`
...
`https://docs.brikko-studio.ai/plugins/voice-call#stale-call-reaper`

## TTS for calls

Voice Call uses the core `messages.tts` configuration for
streaming speech on calls. Override examples and provider caveats live here:
`https://docs.brikko-studio.ai/plugins/voice-call#tts-for-calls`
