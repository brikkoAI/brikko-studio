...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredBrikkoStudioTmpDir: vi.fn(() => path.join(os.tmpdir(), "brikko-studio-device-pair-tests")),
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredBrikkoStudioTmpDir: pluginApiMocks.resolvePreferredBrikkoStudioTmpDir,
...
function createApi(params?: {
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as BrikkoStudioPluginApi["runtime"],
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredBrikkoStudioTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredBrikkoStudioTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the BrikkoStudio iOS app:");
    ...
    expect(text).not.toContain("![BrikkoStudio pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the BrikkoStudio iOS app:");
