...
let tmpDirModule: typeof import("../infra/tmp-brikko-studio-dir.js");
...
describe("pw-tools-core", () => {
  ...
  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".brikko-studio-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "brikko-studio-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "brikko-studio-preferred", "downloads")}${path.sep}`;
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).toBe(
      path.resolve(path.join(path.sep, "tmp", "brikko-studio-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "brikko-studio-preferred", "downloads")}${path.sep}`),
    );
  });
