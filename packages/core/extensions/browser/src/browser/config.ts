...
export type ResolvedBrowserProfile = {
  ...
  mcpArgs?: string[];
  color: string;
  driver: "brikko-studio" | "existing-session";
  executablePath?: string;
  headless: boolean;
...
export const BRIKKO_STUDIO_BROWSER_HEADLESS_ENV = "BRIKKO_STUDIO_BROWSER_HEADLESS";
...
function normalizeHexColor(raw: string | undefined): string {
  ...
  if (!value) {
    return DEFAULT_BRIKKO_STUDIO_BROWSER_COLOR;
  }
...
function isLocalManagedProfile(profile: ResolvedBrowserProfile): boolean {
  return profile.driver === "brikko-studio" && profile.cdpIsLoopback && !profile.attachOnly;
}
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_BRIKKO_STUDIO_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_BRIKKO_STUDIO_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_BRIKKO_STUDIO_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_BRIKKO_STUDIO_BROWSER_PROFILE_NAME]
        ? DEFAULT_BRIKKO_STUDIO_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "brikko-studio";
...
export function resolveManagedBrowserHeadlessMode(
  ...
  const envHeadless = parseBooleanValue(env[BRIKKO_STUDIO_BROWSER_HEADLESS_ENV]);
...
export function getManagedBrowserMissingDisplayError(
  ...
  const sourceHint =
    mode.source === "request"
      ? "request override"
      : mode.source === "env"
        ? `${BRIKKO_STUDIO_BROWSER_HEADLESS_ENV}=0`
        : mode.source === "profile"
  ...
  return (
    ...
    `Set ${BRIKKO_STUDIO_BROWSER_HEADLESS_ENV}=1, remove the headed override, or launch under Xvfb.`
  );
}
