...
async function runBrowserLiveProbe(req: BrowserRequest, ctx: BrowserRouteContext) {
  ...
  try {
    ...
    return {
      ...
      status: "fail" as const,
      summary: String(err),
      fixHint: "Run brikko-studio browser start, then retry with brikko-studio browser doctor --deep.",
    };
  }
}
...
function parseHeadlessStartOverride(params: {
  ...
  if (
    params.profileCtx.profile.driver !== "brikko-studio" ||
    ...
    jsonError(
      params.res,
      400,
      `Headless start override is only supported for locally launched brikko-studio profiles. Profile "${params.profileCtx.profile.name}" is attach-only, remote, or existing-session.`,
    );
    return { ok: false };
  }
...
export function registerBrowserBasicRoutes(app: BrowserRouteRegistrar, ctx: BrowserRouteContext) {
  ...
  app.post(
    ...
    asyncBrowserRoute(async (req, res) => {
      ...
      if (driver && driver !== "brikko-studio" && driver !== "clawd" && driver !== "existing-session") {
        return jsonError(
          res,
          400,
          `unsupported profile driver "${driver}"; use "brikko-studio", "clawd", or "existing-session"`,
        );
      }
          ...
          await service.createProfile({
            ...
            driver:
              driver === "existing-session"
                ? "existing-session"
                : driver === "brikko-studio" || driver === "clawd"
                  ? "brikko-studio"
                  : undefined,
          }),
      });
