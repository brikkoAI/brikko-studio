...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        brikko-studio: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.BRIKKO_STUDIO_GATEWAY_PORT;
    process.env.BRIKKO_STUDIO_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.BRIKKO_STUDIO_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.BRIKKO_STUDIO_GATEWAY_PASSWORD;
