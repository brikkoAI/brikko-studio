...
describe("chrome.ts internal", () => {
  ...
  describe("buildBrikko StudioChromeLaunchArgs branches", () => {
    ...
    it("lets a request headless override beat env and profile headed settings", () => {
      const args = buildBrikko StudioChromeLaunchArgs({
        ...
        env: { BRIKKO_STUDIO_BROWSER_HEADLESS: "0" },
      });
      expect(args).toContain("--headless=new");
      expect(args).toContain("--disable-gpu");
    });
  ...
  describe("launchBrikko StudioChrome", () => {
    ...
    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "brikko-studio-launch-"));
    });
  ...
  describe("launchBrikko StudioChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "brikko-studio-decorated-"));
      try {
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_BRIKKO_STUDIO_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("logs a warning when decorateBrikko StudioProfile throws and still returns a running Chrome", async () => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".brikko-studio-profile-decorated") || s.endsWith("Preferences")) {
