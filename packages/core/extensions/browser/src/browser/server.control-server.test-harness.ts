...
const state: HarnessState = {
  ...
  cfgDefaultProfile: "brikko-studio",
  cfgProfiles: {},
...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.BRIKKO_STUDIO_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "brikko-studio",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/brikko-studio" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    brikko-studio: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "brikko-studio";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.BRIKKO_STUDIO_GATEWAY_PORT;
  process.env.BRIKKO_STUDIO_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.BRIKKO_STUDIO_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.BRIKKO_STUDIO_GATEWAY_PASSWORD;
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.BRIKKO_STUDIO_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.BRIKKO_STUDIO_GATEWAY_PASSWORD;
  } else {
    process.env.BRIKKO_STUDIO_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
