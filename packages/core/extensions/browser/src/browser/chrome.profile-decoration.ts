...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".brikko-studio-profile-decorated");
}
...
export function decorateBrikkoStudioProfile(
  ...
  const desiredName = opts?.name ?? DEFAULT_BRIKKO_STUDIO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_BRIKKO_STUDIO_BROWSER_COLOR).toUpperCase();
