...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.BRIKKO_STUDIO_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-nostr-"));
  process.env.BRIKKO_STUDIO_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.BRIKKO_STUDIO_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".brikko-studio");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.BRIKKO_STUDIO_STATE_DIR = previous;
    }
