...
- Start from `brikko-studio wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `brikko-studio wiki obsidian status` before shelling out, then prefer the dedicated helpers like `brikko-studio wiki obsidian search`, `brikko-studio wiki obsidian open`, `brikko-studio wiki obsidian command`, and `brikko-studio wiki obsidian daily`.
