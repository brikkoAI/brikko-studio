# @brikko-studio/memory-wiki

Persistent wiki compiler and Obsidian-friendly knowledge vault for **Brikko Studio**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.brikko-studio-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
- The compiled agent digests in `.brikko-studio-wiki/cache/agent-digest.json` and `.brikko-studio-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
