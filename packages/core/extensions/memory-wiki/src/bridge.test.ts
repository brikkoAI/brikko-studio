...
} from "brikko-studio/plugin-sdk/memory-host-events";
...
describe("syncMemoryWikiBridgeSources", () => {
  ...
  it("imports public memory artifacts and stays idempotent across reruns", async () => {
    ...
    const appConfig: Brikko StudioConfig = {
      agents: {
    ...
    const logLines = (await fs.readFile(path.join(vaultDir, ".brikko-studio-wiki", "log.jsonl"), "utf8"))
      .trim()
      .split("\n");
