...
export const WIKI_VAULT_DIRECTORIES = [
  ...
  "_views",
  ".brikko-studio-wiki",
  ".brikko-studio-wiki/locks",
  ".brikko-studio-wiki/cache",
] as const;
...
function buildIndexMarkdown(): string {
  return withTrailingNewline(
    replaceManagedMarkdownBlock({
      original: "# Wiki Index\n",
      heading: "## Generated",
      startMarker: "<!-- brikko-studio:wiki:index:start -->",
      endMarker: "<!-- brikko-studio:wiki:index:end -->",
...
- Use \`.brikko-studio-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
This vault is maintained by the Brikko Studio memory-wiki plugin.
...
- \`.brikko-studio-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.

## Notes
<!-- brikko-studio:human:start -->
<!-- brikko-studio:human:end -->
`);
}
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(
    path.join(rootDir, ".brikko-studio-wiki", "state.json"),
    withTrailingNewline(
      JSON.stringify(
        {
          version: 1,
  ...
  await writeFileIfMissing(path.join(rootDir, ".brikko-studio-wiki", "log.jsonl"), "", createdFiles);
