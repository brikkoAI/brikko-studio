...
const AGENT_DIGEST_PATH = ".brikko-studio-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".brikko-studio-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- brikko-studio:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- brikko-studio:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        filePath,
        title: group.heading,
        startMarker: `<!-- brikko-studio:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- brikko-studio:wiki:${group.dir}:index:end -->`,
