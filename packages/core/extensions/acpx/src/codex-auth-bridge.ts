...
const RUN_CONFIGURED_COMMAND_SENTINEL = "--brikko-studio-run-configured";
...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep Brikko Studio's isolated CODEX_HOME wrapper, but launch the plugin-local
...
if (!command) {
  console.error("[brikko-studio] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[brikko-studio] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in Brikko Studio; fallback must not float to an unpatched newer release.
...
async function prepareIsolatedCodexHome(baseDir: string): Promise<string> {
  ...
  await fs.writeFile(
    ...
    "# Generated by Brikko Studio for Codex ACP sessions.\n",
    "utf8",
  );
  return codexHome;
}
