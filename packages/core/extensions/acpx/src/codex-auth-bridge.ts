...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep BrikkoStudio's isolated CODEX_HOME wrapper, but launch the plugin-local
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in BrikkoStudio; fallback must not float to an unpatched newer release.
...
async function prepareIsolatedCodexHome(baseDir: string): Promise<string> {
  ...
  await fs.writeFile(
    ...
    "# Generated by BrikkoStudio for Codex ACP sessions.\n",
    "utf8",
  );
  return codexHome;
}
