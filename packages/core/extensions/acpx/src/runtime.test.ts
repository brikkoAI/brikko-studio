...
const DOCUMENTED_BRIKKO_STUDIO_BRIDGE_COMMAND =
  "env BRIKKO_STUDIO_HIDE_BANNER=1 BRIKKO_STUDIO_SUPPRESS_NOTES=1 brikko-studio acp --url ws://127.0.0.1:18789 --token-file ~/.brikko-studio/gateway.token --session agent:main:main";
const CODEX_ACP_COMMAND = "npx @zed-industries/codex-acp@^0.12.0";
const CODEX_ACP_WRAPPER_COMMAND = `node "/tmp/brikko-studio/acpx/codex-acp-wrapper.mjs"`;
...
function makeRuntime(
  ...
  const runtime = new AcpxRuntime({
    ...
    agentRegistry: {
      resolve: (agentName: string) => (agentName === "brikko-studio" ? "brikko-studio acp" : agentName),
      list: () => ["codex", "brikko-studio"],
    },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("injects Codex ACP startup config into the scoped registry", () => {
    ...
    expect(__testing.isCodexAcpCommand("brikko-studio acp")).toBe(false);
  });
  ...
  it("routes brikko-studio ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const bridgeEnsure = vi.spyOn(bridgeSafeDelegate, "ensureSession").mockResolvedValue({
      sessionKey: "agent:brikko-studio:acp:test",
      backend: "acpx",
  ...
  it("routes non-brikko-studio sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for brikko-studio sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:brikko-studio:acp:test",
      backend: "acpx",
      runtimeSessionName: "brikko-studio-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the brikko-studio agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "brikko-studio" ? "codex" : agentName),
        list: () => ["codex", "brikko-studio"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the brikko-studio bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "brikko-studio acp" : agentName),
        list: () => ["codex", "brikko-studio"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped brikko-studio bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "brikko-studio" ? DOCUMENTED_BRIKKO_STUDIO_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "brikko-studio"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for local node brikko-studio entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "brikko-studio" ? "env BRIKKO_STUDIO_HIDE_BANNER=1 node brikko-studio.mjs acp" : agentName,
        list: () => ["codex", "brikko-studio"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:brikko-studio:acp:test",
        agentCommand: DOCUMENTED_BRIKKO_STUDIO_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:brikko-studio:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:brikko-studio:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to brikko-studio bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
