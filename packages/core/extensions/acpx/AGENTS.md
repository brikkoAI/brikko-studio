...
The ACPX extension is a thin Brikko Studio wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `brikko-studio/acpx`, not in this extension.
...
Use this flow when Brikko Studio needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `brikko-studio/acpx` repo first.
2. In Brikko Studio, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Brikko Studio and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Brikko Studio ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Brikko Studio-specific glue, move it to `brikko-studio/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
