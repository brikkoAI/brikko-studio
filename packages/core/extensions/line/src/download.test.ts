...
vi.mock("brikko-studio/plugin-sdk/media-store", () => ({
...
describe("downloadLineMedia", () => {
  ...
  beforeEach(() => {
    ...
    saveMediaBufferMock.mockImplementation(
      async (_buffer: Buffer, contentType?: string, subdir?: string) => ({
        path: `/home/user/.brikko-studio/media/${subdir ?? "unknown"}/saved-media`,
        contentType,
      }),
    );
  });
  ...
  it("persists inbound media with the shared media store", async () => {
    ...
    expect(result).toEqual({
      path: "/home/user/.brikko-studio/media/inbound/saved-media",
