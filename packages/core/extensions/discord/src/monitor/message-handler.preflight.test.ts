...
function createThreadBinding(
  overrides?: Partial<import("brikko-studio/plugin-sdk/conversation-runtime").SessionBindingRecord>,
) {
  return {
    ...
    ...overrides,
  } satisfies import("brikko-studio/plugin-sdk/conversation-runtime").SessionBindingRecord;
}
...
async function runThreadBoundPreflight(params: {
  ...
  threadBinding: import("brikko-studio/plugin-sdk/conversation-runtime").SessionBindingRecord;
  discordConfig: DiscordConfig;
...
describe("preflightDiscordMessage", () => {
  ...
  it("preflights direct-message voice notes without mention gating", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hello brikko-studio from dm audio");
    ...
    expect(result?.preflightAudioTranscript).toBe("hello brikko-studio from dm audio");
  });
  ...
  it("canonicalizes PluralKit webhook messages to the original Discord message id", async () => {
    ...
    const result = await runGuildPreflight({
      ...
      message: createDiscordMessage({
        id: "proxy-456",
        channelId: "c1",
        content: "<@brikko-studio-bot> hello",
        webhookId: "pluralkit-webhook-1",
        author: {
          id: "webhook-author",
          bot: true,
        ...
        mentionedUsers: [{ id: "brikko-studio-bot" }],
      }),
      discordConfig: {
  ...
  it("still allows bot control commands with an explicit mention when allowBots=mentions", async () => {
    ...
    const message = createDiscordMessage({
      ...
      channelId,
      content: "<@brikko-studio-bot> /new incident room",
      mentionedUsers: [{ id: "brikko-studio-bot" }],
      author: {
        id: "relay-bot-1",
        bot: true,
  ...
  it("uses attachment content_type for guild audio preflight mention detection", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hey brikko-studio");
    ...
    expect(result).not.toBeNull();
    expect(result?.wasMentioned).toBe(true);
    expect(result?.preflightAudioTranscript).toBe("hey brikko-studio");
  });
    ...
    try {
      const result = await runGuildPreflight({
        ...
        message: createDiscordMessage({
          id: "m-binding-2",
          channelId,
          content: "hello <@brikko-studio-bot>",
          author: { id: "user-1", bot: false, username: "alice" },
          mentionedUsers: [{ id: "brikko-studio-bot" }],
        }),
        discordConfig: {} as DiscordConfig,
