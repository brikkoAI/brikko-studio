...
import { resolveUserPath } from "brikko-studio/plugin-sdk/agent-harness";
...
function parseTrajectoryEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.BRIKKO_STUDIO_TRAJECTORY?.trim().toLowerCase();
...
function resolveTrajectoryFilePath(params: {
  ...
  const dirOverride = params.env.BRIKKO_STUDIO_TRAJECTORY_DIR?.trim();
  if (dirOverride) {
