...
vi.mock("brikko-studio/plugin-sdk/agent-harness-runtime", async (importOriginal) => ({
  ...(await importOriginal<typeof import("brikko-studio/plugin-sdk/agent-harness-runtime")>()),
...
function buildCurrentCodexApprovalElicitation() {
  return {
    ...
    _meta: {
      ...
      tool_params_display: [
        { name: "repo", display_name: "Repository", value: "brikko-studio/brikko-studio" },
      ],
    },
    requestedSchema: {
      type: "object",
...
describe("Codex app-server elicitation bridge", () => {
  ...
  it("accepts current Codex MCP approval elicitations with an empty form schema", async () => {
    ...
    expect(approvalRequest.description).toContain("Repository: brikko-studio/brikko-studio");
  });
          ...
          tool_params_display: [
            {
              ...
              value: "\u001b]8;;https://evil.example\u001b\\brikko-studio/brikko-studio\u001b]8;;\u001b\\",
            },
          ],
        },
        requestedSchema: {
  ...
  it("falls back to stable names when display labels sanitize to empty", async () => {
          ...
          tool_params_display: [
            {
              name: "repo",
              display_name: "\u202e",
              value: "brikko-studio/brikko-studio",
            },
          ],
        },
        requestedSchema: {
    ...
    expect(approvalRequest.description).toContain("- repo: brikko-studio/brikko-studio");
