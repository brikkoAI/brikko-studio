...
type CodexDynamicToolsProfile = "native-first" | "brikko-studio-compat";
...
const codexDynamicToolsProfileSchema = z.enum(["native-first", "brikko-studio-compat"]);
...
export function resolveCodexAppServerRuntimeOptions(
  ...
  const envCommand = readNonEmptyString(env.BRIKKO_STUDIO_CODEX_APP_SERVER_BIN);
  ...
  const commandSource: CodexAppServerCommandSource = configCommand
    ...
    : envCommand
      ? "env"
      : "managed";
  const args = resolveArgs(config.args, env.BRIKKO_STUDIO_CODEX_APP_SERVER_ARGS);
  ...
  const policyMode =
    ...
    resolvePolicyMode(env.BRIKKO_STUDIO_CODEX_APP_SERVER_MODE) ??
    "yolo";
  ...
  return {
    ...
    approvalPolicy:
      ...
      resolveApprovalPolicy(env.BRIKKO_STUDIO_CODEX_APP_SERVER_APPROVAL_POLICY) ??
      (policyMode === "guardian" ? "on-request" : "never"),
    sandbox:
      resolveSandbox(config.sandbox) ??
      resolveSandbox(env.BRIKKO_STUDIO_CODEX_APP_SERVER_SANDBOX) ??
...
export function resolveCodexComputerUseConfig(
  ...
  const marketplaceSource =
    ...
    readNonEmptyString(env.BRIKKO_STUDIO_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE);
  const marketplacePath =
    readNonEmptyString(params.overrides?.marketplacePath) ??
    readNonEmptyString(config.marketplacePath) ??
    readNonEmptyString(env.BRIKKO_STUDIO_CODEX_COMPUTER_USE_MARKETPLACE_PATH);
  const marketplaceName =
    readNonEmptyString(params.overrides?.marketplaceName) ??
    readNonEmptyString(config.marketplaceName) ??
    readNonEmptyString(env.BRIKKO_STUDIO_CODEX_COMPUTER_USE_MARKETPLACE_NAME);
  const autoInstall =
    params.overrides?.autoInstall ??
    config.autoInstall ??
    readBooleanEnv(env.BRIKKO_STUDIO_CODEX_COMPUTER_USE_AUTO_INSTALL) ??
    false;
  const marketplaceDiscoveryTimeoutMs = normalizePositiveNumber(
    params.overrides?.marketplaceDiscoveryTimeoutMs ??
      ...
      readNumberEnv(env.BRIKKO_STUDIO_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS),
    DEFAULT_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS,
  );
  const enabled =
    params.overrides?.enabled ??
    config.enabled ??
    readBooleanEnv(env.BRIKKO_STUDIO_CODEX_COMPUTER_USE) ??
  ...
  return {
    ...
    pluginName:
      ...
      readNonEmptyString(env.BRIKKO_STUDIO_CODEX_COMPUTER_USE_PLUGIN_NAME) ??
    ...
    mcpServerName:
      ...
      readNonEmptyString(env.BRIKKO_STUDIO_CODEX_COMPUTER_USE_MCP_SERVER_NAME) ??
