...
import type { PluginCommandContext, PluginCommandResult } from "brikko-studio/plugin-sdk/plugin-entry";
...
describe("codex command", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-codex-command-"));
  });
  ...
  it("asks before sending diagnostics feedback for the attached Codex thread", async () => {
    ...
    expect(safeCodexControlRequest).toHaveBeenCalledWith(
      ...
      CODEX_CONTROL_METHODS.feedback,
      {
        ...
        includeLogs: true,
        tags: {
          source: "brikko-studio-diagnostics",
          channel: "test",
        },
      },
    );
