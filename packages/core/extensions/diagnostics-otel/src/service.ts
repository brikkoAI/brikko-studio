...
const DEFAULT_SERVICE_NAME = "brikko-studio";
const DROPPED_OTEL_ATTRIBUTE_KEYS = new Set([
  "brikko-studio.callId",
  "brikko-studio.parentSpanId",
  "brikko-studio.runId",
  "brikko-studio.sessionId",
  "brikko-studio.sessionKey",
  "brikko-studio.spanId",
  "brikko-studio.toolCallId",
  "brikko-studio.traceId",
]);
...
const PRELOADED_OTEL_SDK_ENV = "BRIKKO_STUDIO_OTEL_PRELOADED";
...
function assignModelCallSizeTimingAttrs(
  ...
  assignPositiveNumberAttr(attrs, "brikko-studio.model_call.request_bytes", evt.requestPayloadBytes);
  assignPositiveNumberAttr(attrs, "brikko-studio.model_call.response_bytes", evt.responseStreamBytes);
  assignPositiveNumberAttr(
    attrs,
    "brikko-studio.model_call.time_to_first_byte_ms",
...
function addUpstreamRequestIdSpanEvent(
  ...
  span.addEvent?.("brikko-studio.provider.request", {
    "brikko-studio.upstreamRequestIdHash": boundedHash,
  });
}
...
function assignOtelModelContentAttributes(
  ...
  if (policy.inputMessages) {
    assignOtelContentAttribute(attributes, "brikko-studio.content.input_messages", event.inputMessages);
  }
  if (policy.outputMessages) {
    assignOtelContentAttribute(
      attributes,
      "brikko-studio.content.output_messages",
      event.outputMessages,
    );
  }
  if (policy.systemPrompt) {
    assignOtelContentAttribute(attributes, "brikko-studio.content.system_prompt", event.systemPrompt);
  }
}
...
function assignOtelToolContentAttributes(
  ...
  if (policy.toolInputs) {
    assignOtelContentAttribute(attributes, "brikko-studio.content.tool_input", event.toolInput);
  }
  if (policy.toolOutputs) {
    assignOtelContentAttribute(attributes, "brikko-studio.content.tool_output", event.toolOutput);
  }
}
...
function assignOtelLogEventAttributes(
  ...
  for (const rawKey in eventAttributes) {
    ...
    assignOtelLogAttribute(attributes, `brikko-studio.${key}`, eventAttributes[rawKey]);
  }
}
...
function addTraceAttributes(
  ...
  attributes["brikko-studio.traceId"] = normalized.traceId;
  if (normalized.spanId) {
    attributes["brikko-studio.spanId"] = normalized.spanId;
  }
  if (normalized.parentSpanId) {
    attributes["brikko-studio.parentSpanId"] = normalized.parentSpanId;
  }
  if (normalized.traceFlags) {
    attributes["brikko-studio.traceFlags"] = normalized.traceFlags;
  }
}
...
export function createDiagnosticsOtelService(): BrikkoStudioPluginService {
  ...
  return {
    ...
    async start(ctx) {
      ...
      const meter = metrics.getMeter("brikko-studio");
      const tracer = trace.getTracer("brikko-studio");
      ...
      const tokensCounter = meter.createCounter("brikko-studio.tokens", {
        unit: "1",
      ...
      const costCounter = meter.createCounter("brikko-studio.cost.usd", {
        unit: "1",
        description: "Estimated model cost (USD)",
      });
      const durationHistogram = meter.createHistogram("brikko-studio.run.duration_ms", {
        unit: "ms",
        description: "Agent run duration",
      });
      const harnessDurationHistogram = meter.createHistogram("brikko-studio.harness.duration_ms", {
        unit: "ms",
        description: "Agent harness lifecycle duration",
      });
      const contextHistogram = meter.createHistogram("brikko-studio.context.tokens", {
        unit: "1",
        description: "Context window size and usage",
      });
      const webhookReceivedCounter = meter.createCounter("brikko-studio.webhook.received", {
        unit: "1",
        description: "Webhook requests received",
      });
      const webhookErrorCounter = meter.createCounter("brikko-studio.webhook.error", {
        unit: "1",
        description: "Webhook processing errors",
      });
      const webhookDurationHistogram = meter.createHistogram("brikko-studio.webhook.duration_ms", {
        unit: "ms",
        description: "Webhook processing duration",
      });
      const messageQueuedCounter = meter.createCounter("brikko-studio.message.queued", {
        unit: "1",
        description: "Messages queued for processing",
      });
      const messageProcessedCounter = meter.createCounter("brikko-studio.message.processed", {
        unit: "1",
        description: "Messages processed by outcome",
      });
      const messageDurationHistogram = meter.createHistogram("brikko-studio.message.duration_ms", {
        unit: "ms",
        description: "Message processing duration",
      });
      const messageDeliveryStartedCounter = meter.createCounter(
        "brikko-studio.message.delivery.started",
        {
          unit: "1",
      ...
      const messageDeliveryDurationHistogram = meter.createHistogram(
        "brikko-studio.message.delivery.duration_ms",
        {
          unit: "ms",
          description: "Outbound message delivery duration",
        },
      );
      const queueDepthHistogram = meter.createHistogram("brikko-studio.queue.depth", {
        unit: "1",
        description: "Queue depth on enqueue/dequeue",
      });
      const queueWaitHistogram = meter.createHistogram("brikko-studio.queue.wait_ms", {
        unit: "ms",
        description: "Queue wait time before execution",
      });
      const laneEnqueueCounter = meter.createCounter("brikko-studio.queue.lane.enqueue", {
        unit: "1",
        description: "Command queue lane enqueue events",
      });
      const laneDequeueCounter = meter.createCounter("brikko-studio.queue.lane.dequeue", {
        unit: "1",
        description: "Command queue lane dequeue events",
      });
      const sessionStateCounter = meter.createCounter("brikko-studio.session.state", {
        unit: "1",
        description: "Session state transitions",
      });
      const sessionStuckCounter = meter.createCounter("brikko-studio.session.stuck", {
        unit: "1",
        description: "Sessions stuck in processing",
      });
      const sessionStuckAgeHistogram = meter.createHistogram("brikko-studio.session.stuck_age_ms", {
        unit: "ms",
        description: "Age of stuck sessions",
      });
      const runAttemptCounter = meter.createCounter("brikko-studio.run.attempt", {
        unit: "1",
        description: "Run attempts",
      });
      const toolLoopCounter = meter.createCounter("brikko-studio.tool.loop", {
        unit: "1",
        description: "Detected repetitive tool-call loop events",
      });
      const modelCallDurationHistogram = meter.createHistogram("brikko-studio.model_call.duration_ms", {
        unit: "ms",
        description: "Model call duration",
      });
      const modelCallRequestBytesHistogram = meter.createHistogram(
        "brikko-studio.model_call.request_bytes",
        {
          unit: "By",
      ...
      const modelCallResponseBytesHistogram = meter.createHistogram(
        "brikko-studio.model_call.response_bytes",
        {
          unit: "By",
      ...
      const toolExecutionDurationHistogram = meter.createHistogram(
        "brikko-studio.tool.execution.duration_ms",
        {
          unit: "ms",
          description: "Tool execution duration",
        },
      );
      const execProcessDurationHistogram = meter.createHistogram("brikko-studio.exec.duration_ms", {
        unit: "ms",
        description: "Exec process duration",
      });
      const memoryRssHistogram = meter.createHistogram("brikko-studio.memory.rss_bytes", {
        unit: "By",
        description: "Resident set size reported by diagnostic memory samples",
      });
      const memoryHeapUsedHistogram = meter.createHistogram("brikko-studio.memory.heap_used_bytes", {
        unit: "By",
        description: "Heap used bytes reported by diagnostic memory samples",
      });
      const memoryHeapTotalHistogram = meter.createHistogram("brikko-studio.memory.heap_total_bytes", {
        unit: "By",
        description: "Heap total bytes reported by diagnostic memory samples",
      });
      const memoryExternalHistogram = meter.createHistogram("brikko-studio.memory.external_bytes", {
        unit: "By",
      ...
      const memoryArrayBuffersHistogram = meter.createHistogram(
        "brikko-studio.memory.array_buffers_bytes",
        {
          unit: "By",
          description: "ArrayBuffer bytes reported by diagnostic memory samples",
        },
      );
      const memoryPressureCounter = meter.createCounter("brikko-studio.memory.pressure", {
        unit: "1",
        description: "Diagnostic memory pressure events",
      });
      const livenessWarningCounter = meter.createCounter("brikko-studio.liveness.warning", {
        unit: "1",
        description: "Diagnostic liveness warning events",
      });
      const livenessEventLoopDelayP99Histogram = meter.createHistogram(
        "brikko-studio.liveness.event_loop_delay_p99_ms",
        {
          unit: "ms",
      ...
      const livenessEventLoopDelayMaxHistogram = meter.createHistogram(
        "brikko-studio.liveness.event_loop_delay_max_ms",
        {
          unit: "ms",
      ...
      const livenessEventLoopUtilizationHistogram = meter.createHistogram(
        "brikko-studio.liveness.event_loop_utilization",
        {
          unit: "1",
      ...
      const livenessCpuCoreRatioHistogram = meter.createHistogram(
        "brikko-studio.liveness.cpu_core_ratio",
        {
          unit: "1",
          description: "CPU core ratio reported by diagnostic liveness warnings",
        },
      );
      const telemetryExporterCounter = meter.createCounter("brikko-studio.telemetry.exporter.events", {
        unit: "1",
      ...
      if (logsEnabled) {
        ...
        const otelLogger = logProvider.getLogger("brikko-studio");
        recordLogRecord = (evt, metadata) => {
          try {
            ...
            assignOtelLogAttribute(attributes, "brikko-studio.log.level", logLevelName);
            if (evt.loggerName) {
              assignOtelLogAttribute(attributes, "brikko-studio.logger", evt.loggerName);
            }
            if (evt.loggerParents?.length) {
              assignOtelLogAttribute(
                attributes,
                "brikko-studio.logger.parents",
      ...
      const addRunAttrs = (
        ...
        if (evt.provider) {
          spanAttrs["brikko-studio.provider"] = evt.provider;
        }
        if (evt.model) {
          spanAttrs["brikko-studio.model"] = evt.model;
        }
        if (evt.channel) {
          spanAttrs["brikko-studio.channel"] = evt.channel;
        }
        if (evt.trigger) {
          spanAttrs["brikko-studio.trigger"] = evt.trigger;
        }
      };
      ...
      const paramsSummaryAttrs = (
        ...
        return {
          "brikko-studio.tool.params.kind": summary.kind,
          ...("length" in summary ? { "brikko-studio.tool.params.length": summary.length } : {}),
        };
      };
      ...
      const recordModelUsage = (
        ...
        const attrs = {
          "brikko-studio.channel": evt.channel ?? "unknown",
          "brikko-studio.agent": lowCardinalityAttr(evt.agentId),
          "brikko-studio.provider": evt.provider ?? "unknown",
          "brikko-studio.model": evt.model ?? "unknown",
        };
        ...
        if (usage.input) {
          tokensCounter.add(usage.input, { ...attrs, "brikko-studio.token": "input" });
        ...
        if (usage.output) {
          tokensCounter.add(usage.output, { ...attrs, "brikko-studio.token": "output" });
        ...
        if (usage.cacheRead) {
          tokensCounter.add(usage.cacheRead, { ...attrs, "brikko-studio.token": "cache_read" });
        }
        if (usage.cacheWrite) {
          tokensCounter.add(usage.cacheWrite, { ...attrs, "brikko-studio.token": "cache_write" });
        }
        if (usage.promptTokens) {
          tokensCounter.add(usage.promptTokens, { ...attrs, "brikko-studio.token": "prompt" });
        }
        if (usage.total) {
          tokensCounter.add(usage.total, { ...attrs, "brikko-studio.token": "total" });
        }
        ...
        if (evt.context?.limit) {
          contextHistogram.record(evt.context.limit, {
            ...attrs,
            "brikko-studio.context": "limit",
          });
        }
        if (evt.context?.used) {
          contextHistogram.record(evt.context.used, {
            ...attrs,
            "brikko-studio.context": "used",
          });
        }
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "brikko-studio.tokens.input": usage.input ?? 0,
          "brikko-studio.tokens.output": usage.output ?? 0,
          "brikko-studio.tokens.cache_read": usage.cacheRead ?? 0,
          "brikko-studio.tokens.cache_write": usage.cacheWrite ?? 0,
          "brikko-studio.tokens.total": usage.total ?? 0,
        };
        ...
        const span = spanWithDuration("brikko-studio.model.usage", spanAttrs, evt.durationMs, {
      ...
      const recordWebhookReceived = (
        ...
        const attrs = {
          "brikko-studio.channel": evt.channel ?? "unknown",
          "brikko-studio.webhook": evt.updateType ?? "unknown",
        };
      ...
      const recordWebhookProcessed = (
        ...
        if (evt.chatId !== undefined) {
          spanAttrs["brikko-studio.chatId"] = String(evt.chatId);
        }
        const span = spanWithDuration("brikko-studio.webhook.processed", spanAttrs, evt.durationMs);
        span.end();
      };
      ...
      const recordWebhookError = (
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "brikko-studio.error": redactedError,
        };
        if (evt.chatId !== undefined) {
          spanAttrs["brikko-studio.chatId"] = String(evt.chatId);
        }
        const span = tracer.startSpan("brikko-studio.webhook.error", {
      ...
      const recordMessageQueued = (
        ...
        const attrs = {
          "brikko-studio.channel": evt.channel ?? "unknown",
          "brikko-studio.source": evt.source ?? "unknown",
        };
      ...
      const recordMessageProcessed = (
        ...
        const attrs = {
          "brikko-studio.channel": evt.channel ?? "unknown",
          "brikko-studio.outcome": evt.outcome ?? "unknown",
        };
        ...
        if (evt.messageId !== undefined) {
          spanAttrs["brikko-studio.messageId"] = String(evt.messageId);
        }
        if (evt.reason) {
          spanAttrs["brikko-studio.reason"] = redactSensitiveText(evt.reason);
        }
        const span = spanWithDuration("brikko-studio.message.processed", spanAttrs, evt.durationMs);
      ...
      const messageDeliveryAttrs = (
        ...
        "brikko-studio.channel": evt.channel,
        "brikko-studio.delivery.kind": evt.deliveryKind,
      });
      ...
      const recordMessageDeliveryCompleted = (
        ...
        const span = spanWithDuration(
          ...
          {
            ...attrs,
            "brikko-studio.delivery.result_count": evt.resultCount,
          },
          evt.durationMs,
          { endTimeMs: evt.ts },
        );
      ...
      const recordMessageDeliveryError = (
        ...
        const attrs = {
          ...
          "brikko-studio.errorCategory": lowCardinalityAttr(evt.errorCategory, "other"),
        };
        messageDeliveryDurationHistogram.record(evt.durationMs, attrs);
        if (!tracesEnabled) {
          return;
        }
        const span = spanWithDuration("brikko-studio.message.delivery", attrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
        span.setStatus({
      ...
      const recordRunStarted = (
        ...
        const span = trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("brikko-studio.run", spanAttrs, undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordLaneEnqueue = (
        ...
        const attrs = { "brikko-studio.lane": evt.lane };
      ...
      const recordSessionState = (
        ...
        const attrs: Record<string, string> = { "brikko-studio.state": evt.state };
        if (evt.reason) {
          attrs["brikko-studio.reason"] = redactSensitiveText(evt.reason);
        }
      ...
      const recordSessionStuck = (
        ...
        const spanAttrs: Record<string, string | number> = { ...attrs };
        spanAttrs["brikko-studio.queueDepth"] = evt.queueDepth ?? 0;
        spanAttrs["brikko-studio.ageMs"] = evt.ageMs;
        const span = tracer.startSpan("brikko-studio.session.stuck", { attributes: spanAttrs });
        span.setStatus({ code: SpanStatusCode.ERROR, message: "session stuck" });
        span.end();
      };

      const recordRunAttempt = (evt: Extract<DiagnosticEventPayload, { type: "run.attempt" }>) => {
        runAttemptCounter.add(1, { "brikko-studio.attempt": evt.attempt });
      };

      const toolLoopAttrs = (
        evt: Extract<DiagnosticEventPayload, { type: "tool.loop" }>,
      ): Record<string, string | number> => ({
        "brikko-studio.toolName": lowCardinalityAttr(evt.toolName, "tool"),
        "brikko-studio.loop.level": evt.level,
        "brikko-studio.loop.action": evt.action,
        "brikko-studio.loop.detector": evt.detector,
        "brikko-studio.loop.count": evt.count,
        ...(evt.pairedToolName
          ? { "brikko-studio.loop.paired_tool": lowCardinalityAttr(evt.pairedToolName, "tool") }
          : {}),
      });
      ...
      const recordToolLoop = (evt: Extract<DiagnosticEventPayload, { type: "tool.loop" }>) => {
        ...
        const span = spanWithDuration("brikko-studio.tool.loop", attrs, 0, { endTimeMs: evt.ts });
      ...
      const recordMemoryPressure = (
        ...
        const attrs = {
          "brikko-studio.memory.level": evt.level,
          "brikko-studio.memory.reason": evt.reason,
        };
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...attrs,
          "brikko-studio.memory.rss_bytes": evt.memory.rssBytes,
          "brikko-studio.memory.heap_used_bytes": evt.memory.heapUsedBytes,
          "brikko-studio.memory.heap_total_bytes": evt.memory.heapTotalBytes,
          "brikko-studio.memory.external_bytes": evt.memory.externalBytes,
          "brikko-studio.memory.array_buffers_bytes": evt.memory.arrayBuffersBytes,
          ...(evt.thresholdBytes !== undefined
            ? { "brikko-studio.memory.threshold_bytes": evt.thresholdBytes }
            : {}),
          ...(evt.rssGrowthBytes !== undefined
            ? { "brikko-studio.memory.rss_growth_bytes": evt.rssGrowthBytes }
            : {}),
          ...(evt.windowMs !== undefined ? { "brikko-studio.memory.window_ms": evt.windowMs } : {}),
        };
        const span = spanWithDuration("brikko-studio.memory.pressure", spanAttrs, 0, {
          endTimeMs: evt.ts,
        });
      ...
      const recordRunCompleted = (
        ...
        const attrs: Record<string, string | number> = {
          "brikko-studio.outcome": evt.outcome,
          "brikko-studio.provider": evt.provider ?? "unknown",
          "brikko-studio.model": evt.model ?? "unknown",
        };
        if (evt.channel) {
          attrs["brikko-studio.channel"] = evt.channel;
        }
        durationHistogram.record(evt.durationMs, attrs);
        if (!tracesEnabled) {
          return;
        ...
        if (evt.errorCategory) {
          spanAttrs["brikko-studio.errorCategory"] = lowCardinalityAttr(evt.errorCategory, "other");
        }
        ...
        const span =
          trackedSpan ??
          spanWithDuration("brikko-studio.run", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
      ...
      const harnessRunMetricAttrs = (evt: HarnessRunDiagnosticEvent) => ({
        "brikko-studio.harness.id": lowCardinalityAttr(evt.harnessId, "unknown"),
        "brikko-studio.harness.plugin": lowCardinalityAttr(evt.pluginId),
        ...(evt.type === "harness.run.started"
          ? {}
          : {
              "brikko-studio.outcome": evt.type === "harness.run.error" ? "error" : evt.outcome,
            }),
        "brikko-studio.provider": lowCardinalityAttr(evt.provider, "unknown"),
        "brikko-studio.model": lowCardinalityAttr(evt.model, "unknown"),
        ...(evt.channel ? { "brikko-studio.channel": lowCardinalityAttr(evt.channel) } : {}),
      });
      ...
      const recordHarnessRunStarted = (
        ...
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("brikko-studio.harness.run", harnessRunMetricAttrs(evt), undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordHarnessRunCompleted = (
        ...
        if (evt.resultClassification) {
          spanAttrs["brikko-studio.harness.result_classification"] = lowCardinalityAttr(
            evt.resultClassification,
          );
        }
        if (typeof evt.yieldDetected === "boolean") {
          spanAttrs["brikko-studio.harness.yield_detected"] = evt.yieldDetected;
        }
        if (evt.itemLifecycle) {
          spanAttrs["brikko-studio.harness.items.started"] = evt.itemLifecycle.startedCount;
          spanAttrs["brikko-studio.harness.items.completed"] = evt.itemLifecycle.completedCount;
          spanAttrs["brikko-studio.harness.items.active"] = evt.itemLifecycle.activeCount;
        }
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("brikko-studio.harness.run", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordHarnessRunError = (
        ...
        const attrs = {
          ...harnessRunMetricAttrs(evt),
          "brikko-studio.harness.phase": evt.phase,
          "brikko-studio.errorCategory": errorType,
        };
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...
          ...(evt.cleanupFailed ? { "brikko-studio.harness.cleanup_failed": true } : {}),
        };
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("brikko-studio.harness.run", spanAttrs, evt.durationMs, {
      ...
      const recordContextAssembled = (
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          "brikko-studio.context.message_count": evt.messageCount,
          "brikko-studio.context.history_text_chars": evt.historyTextChars,
          "brikko-studio.context.history_image_blocks": evt.historyImageBlocks,
          "brikko-studio.context.max_message_text_chars": evt.maxMessageTextChars,
          "brikko-studio.context.system_prompt_chars": evt.systemPromptChars,
          "brikko-studio.context.prompt_chars": evt.promptChars,
          "brikko-studio.context.prompt_images": evt.promptImages,
        };
        addRunAttrs(spanAttrs, evt);
        if (evt.contextTokenBudget !== undefined) {
          spanAttrs["brikko-studio.context.token_budget"] = evt.contextTokenBudget;
        }
        if (evt.reserveTokens !== undefined) {
          spanAttrs["brikko-studio.context.reserve_tokens"] = evt.reserveTokens;
        }
        const span = spanWithDuration("brikko-studio.context.assembled", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      ...
      const modelCallMetricAttrs = (evt: ModelCallLifecycleDiagnosticEvent) => ({
        "brikko-studio.provider": evt.provider,
        "brikko-studio.model": evt.model,
        "brikko-studio.api": lowCardinalityAttr(evt.api),
        "brikko-studio.transport": lowCardinalityAttr(evt.transport),
      });
      ...
      const recordModelCallStarted = (
        ...
        if (evt.api) {
          spanAttrs["brikko-studio.api"] = evt.api;
        }
        if (evt.transport) {
          spanAttrs["brikko-studio.transport"] = evt.transport;
        }
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("brikko-studio.model.call", spanAttrs, undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordModelCallCompleted = (
        ...
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("brikko-studio.model.call", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordModelCallError = (
        ...
        const metricAttrs = {
          ...
          "brikko-studio.errorCategory": errorType,
          ...(evt.failureKind
            ? { "brikko-studio.failureKind": lowCardinalityAttr(evt.failureKind, "other") }
            : {}),
        };
        modelCallDurationHistogram.record(evt.durationMs, metricAttrs);
        recordModelCallSizeTimingMetrics(evt, metricAttrs);
        ...
        if (evt.failureKind) {
          spanAttrs["brikko-studio.failureKind"] = lowCardinalityAttr(evt.failureKind, "other");
        }
        assignGenAiModelCallAttrs(spanAttrs, evt);
        if (evt.api) {
          spanAttrs["brikko-studio.api"] = evt.api;
      ...
      const toolExecutionBaseAttrs = (
        ...
        "brikko-studio.toolName": evt.toolName,
      ...
      const recordToolExecutionStarted = (
        ...
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("brikko-studio.tool.execution", toolExecutionBaseAttrs(evt), undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordToolExecutionCompleted = (
        ...
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("brikko-studio.tool.execution", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordToolExecutionError = (
        ...
        addRunAttrs(spanAttrs, evt);
        if (evt.errorCode) {
          spanAttrs["brikko-studio.errorCode"] = lowCardinalityAttr(evt.errorCode, "other");
        }
        assignOtelToolContentAttributes(
          spanAttrs,
          evt as unknown as Record<string, unknown>,
      ...
      const recordToolExecutionBlocked = (
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...toolExecutionBaseAttrs(evt),
          "brikko-studio.outcome": "blocked",
          "brikko-studio.deniedReason": lowCardinalityAttr(evt.deniedReason, "other"),
        };
        addRunAttrs(spanAttrs, evt);
        const span = spanWithDuration("brikko-studio.tool.execution", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordExecProcessCompleted = (
        ...
        const attrs: Record<string, string | number> = {
          "brikko-studio.exec.target": evt.target,
          "brikko-studio.exec.mode": evt.mode,
          "brikko-studio.outcome": evt.outcome,
        };
        if (evt.failureKind) {
          attrs["brikko-studio.failureKind"] = evt.failureKind;
        }
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...attrs,
          "brikko-studio.exec.command_length": evt.commandLength,
        };
        if (typeof evt.exitCode === "number") {
          spanAttrs["brikko-studio.exec.exit_code"] = evt.exitCode;
        }
        if (evt.exitSignal) {
          spanAttrs["brikko-studio.exec.exit_signal"] = lowCardinalityAttr(evt.exitSignal, "other");
        }
        if (evt.timedOut !== undefined) {
          spanAttrs["brikko-studio.exec.timed_out"] = evt.timedOut;
        }

        const span = spanWithDuration("brikko-studio.exec", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
      ...
      const recordHeartbeat = (
        ...
        queueDepthHistogram.record(evt.queued, { "brikko-studio.channel": "heartbeat" });
      };
      ...
      const recordLivenessWarning = (
        ...
        const attrs = {
          "brikko-studio.liveness.reason": lowCardinalityAttr(reason, "unknown"),
        };
        livenessWarningCounter.add(1, attrs);
        queueDepthHistogram.record(evt.queued, { "brikko-studio.channel": "liveness" });
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "brikko-studio.liveness.active": evt.active,
          "brikko-studio.liveness.waiting": evt.waiting,
          "brikko-studio.liveness.queued": evt.queued,
          "brikko-studio.liveness.interval_ms": evt.intervalMs,
          ...(evt.eventLoopDelayP99Ms !== undefined
            ? { "brikko-studio.liveness.event_loop_delay_p99_ms": evt.eventLoopDelayP99Ms }
            : {}),
          ...(evt.eventLoopDelayMaxMs !== undefined
            ? { "brikko-studio.liveness.event_loop_delay_max_ms": evt.eventLoopDelayMaxMs }
            : {}),
          ...(evt.eventLoopUtilization !== undefined
            ? { "brikko-studio.liveness.event_loop_utilization": evt.eventLoopUtilization }
            : {}),
          ...(evt.cpuUserMs !== undefined
            ? { "brikko-studio.liveness.cpu_user_ms": evt.cpuUserMs }
            : {}),
          ...(evt.cpuSystemMs !== undefined
            ? { "brikko-studio.liveness.cpu_system_ms": evt.cpuSystemMs }
            : {}),
          ...(evt.cpuTotalMs !== undefined
            ? { "brikko-studio.liveness.cpu_total_ms": evt.cpuTotalMs }
            : {}),
          ...(evt.cpuCoreRatio !== undefined
            ? { "brikko-studio.liveness.cpu_core_ratio": evt.cpuCoreRatio }
            : {}),
        };
        const span = spanWithDuration("brikko-studio.liveness.warning", spanAttrs, 0, {
          endTimeMs: evt.ts,
        });
        span.setStatus({
          code: SpanStatusCode.ERROR,
      ...
      const recordTelemetryExporter = (
        ...
        telemetryExporterCounter.add(1, {
          "brikko-studio.exporter": lowCardinalityAttr(evt.exporter, "unknown"),
          "brikko-studio.signal": evt.signal,
          "brikko-studio.status": evt.status,
          ...(evt.reason ? { "brikko-studio.reason": evt.reason } : {}),
          ...(evt.errorCategory
            ? { "brikko-studio.errorCategory": lowCardinalityAttr(evt.errorCategory, "other") }
            : {}),
        });
