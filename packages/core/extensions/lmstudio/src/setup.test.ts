...
} from "brikko-studio/plugin-sdk/provider-setup";
import type { WizardPrompter } from "brikko-studio/plugin-sdk/setup";
...
vi.mock("brikko-studio/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("brikko-studio/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("brikko-studio/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("brikko-studio/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
describe("lmstudio setup", () => {
  ...
  it("interactive Docker setup defaults to the host LM Studio endpoint", async () => {
    vi.stubEnv("BRIKKO_STUDIO_DOCKER_SETUP", "1");
    const { prompter, text } = createQueuedWizardPrompterHarness([
