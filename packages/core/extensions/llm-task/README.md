...
`brikko-studio.invoke --each`) without adding new Brikko Studio code per workflow.
...
This extension depends on Brikko Studio internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Brikko Studio extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.brikko-studio/extensions` as a standalone plugin directory.
