...
export function createToolFactoryHarness(cfg: BrikkoStudioPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<BrikkoStudioPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
  ...
  return {
    api: api as BrikkoStudioPluginApi,
    resolveTool,
  };
}
