...
describe("qa multipass runtime", () => {
  ...
  it("rejects repo-local symlink output directories that escape the repo root", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-multipass-"));
  ...
  it("reuses suite scenario semantics and resolves mounted artifact paths", () => {
    ...
    expect(plan.guestOutputDir).toBe("/workspace/brikko-studio-host/.artifacts/qa-e2e/multipass-test");
  ...
  it("renders a guest script that runs the live qa suite by default", () => {
    ...
    expect(script).toContain("'pnpm' 'brikko-studio' 'qa' 'suite' '--transport' 'qa-channel'");
    ...
    expect(script).toContain("/workspace/brikko-studio-host/.artifacts/qa-e2e/multipass-test");
  });
  ...
  it("forwards live key list and numbered key env shapes", () => {
    vi.stubEnv("BRIKKO_STUDIO_LIVE_ANTHROPIC_KEYS", "anthropic-a anthropic-b");
    ...
    expect(plan.forwardedEnv.BRIKKO_STUDIO_LIVE_ANTHROPIC_KEYS).toBe("anthropic-a anthropic-b");
  ...
  it("skips stale CODEX_HOME values that do not exist on the host", () => {
    vi.stubEnv("CODEX_HOME", "/tmp/does-not-exist-brikko-studio-codex-home");
    const plan = createQaMultipassPlan({
      repoRoot: process.cwd(),
      outputDir: path.join(process.cwd(), ".artifacts", "qa-e2e", "multipass-live-test"),
      providerMode: "live-frontier",
  ...
  it("falls back to os.homedir() when HOME is unset for CODEX_HOME discovery", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-multipass-home-"));
    ...
    try {
      ...
      expect(plan.guestCodexHomePath).toBe("/workspace/brikko-studio-codex-home");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
