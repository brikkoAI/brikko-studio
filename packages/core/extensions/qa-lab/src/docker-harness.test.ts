...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    expect(result.files).toEqual(
      expect.arrayContaining([
        ...
        path.join(outputDir, "state", "brikko-studio.json"),
    ...
    expect(compose).toContain("image: brikko-studio:qa-local-prebaked");
    expect(compose).toContain("qa-mock-openai:");
    expect(compose).toContain("18889:18789");
    expect(compose).toContain('      - "43124:43123"');
    expect(compose).toContain(":/opt/brikko-studio-qa-lab-ui:ro");
    ...
    expect(compose).toContain('      - "http://brikko-studio-qa-gateway:18789/"');
    expect(compose).toContain("      - --send-kickoff-on-start");
    expect(compose).toContain("      - --ui-dist-dir");
    expect(compose).toContain('      - "/opt/brikko-studio-qa-lab-ui"');
    expect(compose).toContain(":/opt/brikko-studio-repo:ro");
    expect(compose).toContain("./state:/opt/brikko-studio-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/brikko-studio-scaffold/seed-workspace/. /tmp/brikko-studio/workspace/",
    );
    expect(compose).toContain("BRIKKO_STUDIO_CONFIG_PATH: /tmp/brikko-studio/brikko-studio.json");
    expect(compose).toContain("BRIKKO_STUDIO_STATE_DIR: /tmp/brikko-studio/state");
    expect(compose).toContain('BRIKKO_STUDIO_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("BRIKKO_STUDIO_GATEWAY_TOKEN=qa-token");
    ...
    const config = await readFile(path.join(outputDir, "state", "brikko-studio.json"), "utf8");
    ...
    expect(config).toContain('"/tmp/brikko-studio/workspace"');
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    const result = await buildQaDockerHarnessImage(
      {
        repoRoot: "/repo/brikko-studio",
        imageName: "brikko-studio:qa-local-prebaked",
      },
      {
    ...
    expect(result.imageName).toBe("brikko-studio:qa-local-prebaked");
    expect(calls).toEqual([
      expect.stringContaining(
        "docker build -t brikko-studio:qa-local-prebaked --build-arg BRIKKO_STUDIO_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/brikko-studio",
      ),
    ]);
  });
});
