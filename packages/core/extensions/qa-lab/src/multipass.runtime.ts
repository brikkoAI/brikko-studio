...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/brikko-studio-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/brikko-studio";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/brikko-studio-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `brikko-studio-qa-${createVmSuffix()}`;
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "BRIKKO_STUDIO_CONFIG_PATH" &&
          key !== "BRIKKO_STUDIO_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `BRIKKO_STUDIO_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `BRIKKO_STUDIO_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  await writeFile(
    plan.hostLogPath,
    `# Brikko Studio QA Multipass host log\nvmName=${plan.vmName}\noutputDir=${plan.outputDir}\n\n`,
    "utf8",
  );
  await writeFile(
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm brikko-studio qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredBrikko StudioTmpDir(), `${plan.vmName}-qa-suite-`),
  );
