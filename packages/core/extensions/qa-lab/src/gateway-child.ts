...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    BRIKKO_STUDIO_HOME: params.homeDir,
    BRIKKO_STUDIO_CONFIG_PATH: params.configPath,
    BRIKKO_STUDIO_STATE_DIR: params.stateDir,
    BRIKKO_STUDIO_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    BRIKKO_STUDIO_SKIP_STARTUP_MODEL_PREWARM: "1",
    BRIKKO_STUDIO_NO_RESPAWN: "1",
    BRIKKO_STUDIO_TEST_FAST: "1",
    BRIKKO_STUDIO_QA_PARENT_PID: String(process.pid),
    BRIKKO_STUDIO_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    BRIKKO_STUDIO_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { BRIKKO_STUDIO_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  const keepTemp = process.env.BRIKKO_STUDIO_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
