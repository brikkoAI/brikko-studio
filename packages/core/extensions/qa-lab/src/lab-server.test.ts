...
vi.mock("@brikko-studio/qa-channel/api.js", async () => await import("../../qa-channel/api.js"));
...
vi.mock("brikko-studio/plugin-sdk/proxy-capture", () => ({
  ...
  resolveDebugProxySettings: () => ({
    dbPath: process.env.BRIKKO_STUDIO_DEBUG_PROXY_DB_PATH ?? "",
    blobDir: process.env.BRIKKO_STUDIO_DEBUG_PROXY_BLOB_DIR ?? "",
    proxyUrl: process.env.BRIKKO_STUDIO_DEBUG_PROXY_URL ?? "",
...
describe("qa-lab server", () => {
  ...
  it("aborts an in-flight runner model catalog when the lab stops", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "  process.exit(0);",
        "});",
        `fs.writeFileSync(${JSON.stringify(markerPath)}, process.env.BRIKKO_STUDIO_CODEX_DISCOVERY_LIVE || "", "utf8");`,
        "setInterval(() => {}, 1000);",
      ].join("\n"),
      "utf8",
    );
  ...
  it("serves proxy capture sessions, events, and query rows", async () => {
    ...
    cleanups.push(async () => {
      delete process.env.BRIKKO_STUDIO_DEBUG_PROXY_DB_PATH;
      delete process.env.BRIKKO_STUDIO_DEBUG_PROXY_BLOB_DIR;
      await lab.stop();
    });
