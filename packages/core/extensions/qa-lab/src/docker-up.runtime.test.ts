...
function createHealthyDockerDeps(calls: string[]): QaDockerUpDeps {
  return {
    async runCommand(command, args, cwd) {
      ...
      if (args.join(" ").includes("ps --format json brikko-studio-qa-gateway")) {
...
describe("runQaDockerUp", () => {
  it("builds the QA UI, writes the harness, starts compose, and waits for health", async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        "pnpm qa:lab:build @/repo/brikko-studio",
        `docker compose -f ${outputDir}/docker-compose.qa.yml down --remove-orphans @/repo/brikko-studio`,
        expect.stringContaining(
          `docker compose -f ${outputDir}/docker-compose.qa.yml up --build -d @/repo/brikko-studio`,
        ),
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps --format json brikko-studio-qa-gateway @/repo/brikko-studio`,
      ]);
  ...
  it("skips UI build and compose --build for prebuilt images", async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        `docker compose -f ${outputDir}/docker-compose.qa.yml down --remove-orphans @/repo/brikko-studio`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml up -d @/repo/brikko-studio`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps --format json brikko-studio-qa-gateway @/repo/brikko-studio`,
      ]);
  ...
  it("uses a repo-root-relative default output dir when none is provided", async () => {
    ...
    try {
      ...
      expect(calls).toEqual([
        ...
        `docker compose -f ${path.join(repoRoot, ".artifacts/qa-docker/docker-compose.qa.yml")} ps --format json brikko-studio-qa-gateway @${repoRoot}`,
      ]);
    } finally {
  ...
  it("falls back to the container IP when the host gateway port is unreachable", async () => {
    ...
    try {
      const result = await runQaDockerUp(
        ...
        },
        {
          async runCommand(command, args, cwd) {
            ...
            if (joined.includes("ps --format json brikko-studio-qa-gateway")) {
              return { stdout: '{"Health":"healthy","State":"running"}\n', stderr: "" };
            }
            if (joined.includes("ps -q brikko-studio-qa-gateway")) {
      ...
      expect(calls).toEqual([
        ...
        `docker compose -f ${outputDir}/docker-compose.qa.yml up -d @/repo/brikko-studio`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps --format json brikko-studio-qa-gateway @/repo/brikko-studio`,
        `docker compose -f ${outputDir}/docker-compose.qa.yml ps -q brikko-studio-qa-gateway @/repo/brikko-studio`,
        "docker inspect --format {{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}} gateway-container @/repo/brikko-studio",
      ]);
      expect(fetchCalls).toEqual([
        "http://127.0.0.1:43124/healthz",
        "http://127.0.0.1:18889/healthz",
