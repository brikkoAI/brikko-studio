    ...
    try {
      ...
      try {
        ...
        const cliSas = parseMatrixQaCliSasText(
          sasOutput.text,
          "interactive brikko-studio matrix verify self",
        );
        ...
        return {
          ...
          details: [
            "Matrix CLI self-verification established full Matrix identity trust through interactive brikko-studio matrix verify self",
    ...
    async ({ client, driverUserId, roomId, roomKey }) => {
      ...
      return {
        ...
        details: [
          ...
          "observed-event artifacts redact body/formatted_body unless BRIKKO_STUDIO_QA_MATRIX_CAPTURE_CONTENT=1",
          `encrypted room id: ${result.roomId}`,
          `isolated driver user: ${driverUserId}`,
          ...buildMatrixReplyDetails("E2EE reply", result.reply),
        ].join("\n"),
