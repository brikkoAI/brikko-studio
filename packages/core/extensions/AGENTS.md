...
- Extension production code should import from `brikko-studio/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `brikko-studio.plugin.json` and the package
  `brikko-studio` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `brikko-studio/plugin-sdk/*`.
