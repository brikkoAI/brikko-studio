...
import { OPENAI_COMPATIBLE_REPLAY_HOOKS } from "brikko-studio/plugin-sdk/provider-model-shared";
import { defaultToolStreamExtraParams } from "brikko-studio/plugin-sdk/provider-stream-shared";
import { jsonResult, readProviderEnvValue } from "brikko-studio/plugin-sdk/provider-web-search";
    ...
    execute: async (toolCallId: string, args: Record<string, unknown>) => {
      ...
      if (!tool) {
        return jsonResult({
          ...
          docs: "https://docs.brikko-studio.ai/tools/code-execution",
        });
      }
