...
import { writeJsonFileAtomically } from "brikko-studio/plugin-sdk/json-store";
import { resolveRequiredHomeDir, resolveStateDir } from "brikko-studio/plugin-sdk/state-paths";

const MATRIX_MIGRATION_SNAPSHOT_DIRNAME = "brikko-studio-migrations";
...
export async function maybeCreateMatrixMigrationSnapshot(params: {
  ...
  outputDir?: string;
  createBackupArchive?: typeof import("brikko-studio/plugin-sdk/runtime").createBackupArchive;
  log?: { info?: (message: string) => void; warn?: (message: string) => void };
}): Promise<MatrixMigrationSnapshotResult> {
  const env = params.env ?? process.env;
  const createBackupArchive =
    params.createBackupArchive ?? (await import("brikko-studio/plugin-sdk/runtime")).createBackupArchive;
