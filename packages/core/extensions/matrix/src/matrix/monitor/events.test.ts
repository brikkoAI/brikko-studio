...
describe("registerMatrixMonitorEvents verification routing", () => {
  ...
  it("classifies repeated fresh post-healthy-sync decrypt failures separately", async () => {
    ...
    try {
      ...
      expect(logger.warn).toHaveBeenNthCalledWith(
        4,
        "matrix: repeated fresh encrypted messages are still failing to decrypt after Matrix resumed healthy sync. This device may still be missing new room keys. Check 'brikko-studio matrix verify status --verbose --account ops' and 'brikko-studio matrix devices list --account ops'.",
        expect.objectContaining({
          failureCount: 3,
          roomCount: 3,
          senderCount: 3,
