...
const TEST_UNDICI_RUNTIME_DEPS_KEY = "__BRIKKO_STUDIO_TEST_UNDICI_RUNTIME_DEPS__";
...
describe("MatrixClient request hardening", () => {
  ...
  it("wires the sync store into the SDK and flushes it on shutdown", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-matrix-sdk-store-"));
...
describe("MatrixClient crypto bootstrapping", () => {
  ...
  it("passes cryptoDatabasePrefix into initRustCrypto", async () => {
    ...
    const client = new MatrixClient("https://matrix.example.org", "token", {
      encryption: true,
      cryptoDatabasePrefix: "brikko-studio-matrix-test",
    });
  ...
  it("schedules periodic crypto snapshot persistence", async () => {
    ...
    const client = new MatrixClient("https://matrix.example.org", "token", {
      ...
      cryptoDatabasePrefix: "brikko-studio-matrix-interval",
    });
