...
- Repo: `https://github.com/brikko-studio/brikko-studio`
...
- Installers: sibling `../brikko-studio.ai`.
...
- Extensions cross into core only via `brikko-studio/plugin-sdk/*`, manifest metadata, injected runtime helpers, documented barrels (`api.ts`, `runtime-api.ts`).
...
- CLI: `pnpm brikko-studio ...` or `pnpm dev`; build: `pnpm build`.
...
- Vitest flags only; no Jest flags like `--runInBand`. For serial runs use `pnpm test:serial` or `BRIKKO_STUDIO_VITEST_MAX_WORKERS=1 pnpm test ...`.
...
- Testbox full-suite profile: `blacksmith testbox run --id <ID> "env NODE_OPTIONS=--max-old-space-size=4096 BRIKKO_STUDIO_TEST_PROJECTS_PARALLEL=6 BRIKKO_STUDIO_VITEST_MAX_WORKERS=1 pnpm test"`. For installable package proof, prefer the GitHub `Package Acceptance` workflow over ad hoc Testbox commands.
...
- Search/dedupe: prefer `gh search issues 'repo:brikko-studio/brikko-studio is:open <terms>' --json number,title,state,updatedAt --limit 20`.
...
- Wait matrix:
  ...
  - release/manual only: `Docker Release`, `Brikko Studio NPM Release`, `macOS Release`, `Brikko Studio Release Checks`, `Cross-OS Release Checks`, `NPM Telegram Beta E2E`.
...
- Naming: **Brikko Studio** product/docs; `brikko-studio` CLI/package/path/config.
...
- Prefer injection; if module mocking, mock narrow local `*.runtime.ts`, not broad barrels or `brikko-studio/plugin-sdk/*`.
- Share fixtures/builders; delete duplicate assertions; assert behavior that can regress here.
- Do not edit baseline/inventory/ignore/snapshot/expected-failure files to silence checks without explicit approval.
- Do not run multiple independent `pnpm test`/Vitest commands concurrently in the same worktree. They can race on `node_modules/.experimental-vitest-cache` and fail with `ENOTEMPTY`. Use one grouped `pnpm test ...` invocation, run targeted lanes sequentially, or set distinct `BRIKKO_STUDIO_VITEST_FS_MODULE_CACHE_PATH` values when true parallel Vitest processes are needed.
- Test workers max 16. Memory pressure: `BRIKKO_STUDIO_VITEST_MAX_WORKERS=1 pnpm test`.
- Live: `BRIKKO_STUDIO_LIVE_TEST=1 pnpm test:live`; verbose `BRIKKO_STUDIO_LIVE_TEST_QUIET=0`.
...
- PR/issue workflows: `$brikko-studio-pr-maintainer`. `/landpr`: `~/.codex/prompts/landpr.md`.

## Security / Release

- Never commit real phone numbers, videos, credentials, live config.
- Secrets: channel/provider creds in `~/.brikko-studio/credentials/`; model auth profiles in `~/.brikko-studio/agents/<agentId>/agent/auth-profiles.json`.
...
- Releases/publish/version bumps need explicit approval. Release docs: `docs/reference/RELEASING.md`; use `$brikko-studio-release-maintainer`.
- GHSA/advisories: `$brikko-studio-ghsa-maintainer`.
...
- Mac gateway: dev watch = `pnpm gateway:watch` (tmux `brikko-studio-gateway-watch-main`, auto-attach). Noninteractive: `BRIKKO_STUDIO_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch`; attach/stop: `tmux attach -t brikko-studio-gateway-watch-main` / `tmux kill-session -t brikko-studio-gateway-watch-main`. Managed installs: `brikko-studio gateway restart/status --deep`. No launchd/ad-hoc tmux. Logs: `./scripts/clawlog.sh`.
...
- Rebrand/migration/config warnings: run `brikko-studio doctor`.
