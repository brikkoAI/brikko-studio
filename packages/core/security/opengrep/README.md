...
`precise.yml` is Brikko Studio's shipped precise OpenGrep rulepack. Each rule is tied
...
```bash
# from the brikko-studio repo root
...
Some Brikko Studio paths are excluded by `.gitignore` for build reasons even though
