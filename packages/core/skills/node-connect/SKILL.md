---
name: node-connect
description: Diagnose Brikko Studio Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Brikko Studio is advertising that route, then fix pairing/auth.
...
- whether `brikko-studio devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `brikko-studio qr --json`. It uses the same setup-code payload Android scans.

```bash
brikko-studio config get gateway.mode
brikko-studio config get gateway.bind
brikko-studio config get gateway.tailscale.mode
brikko-studio config get gateway.remote.url
brikko-studio config get gateway.auth.mode
brikko-studio config get gateway.auth.allowTailscale
brikko-studio config get plugins.entries.device-pair.config.publicUrl
brikko-studio qr --json
brikko-studio devices list
brikko-studio nodes status
```

If this Brikko Studio instance is pointed at a remote gateway, also run:

```bash
brikko-studio qr --remote --json
```
...
`brikko-studio qr --json` success means:
...
If `brikko-studio qr --json` says `Gateway is only bound to loopback`:
...
- `brikko-studio devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run brikko-studio qr again, rescan, then approve the pending device pairing.`
