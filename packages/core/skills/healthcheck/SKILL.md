---
name: healthcheck
description: Audit and harden hosts running Brikko Studio for SSH, firewall, updates, exposure, cron checks, and risk posture.
---

# Brikko Studio Host Hardening

## Overview

Assess and harden the host running Brikko Studio, then align it to a user-defined risk tolerance without breaking access. Use Brikko Studio security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Brikko Studio changes the host firewall, SSH, or OS updates; it does not.
...
5. Brikko Studio gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Brikko Studio can access sensitive data.
...
### 2) Run Brikko Studio security audits (read-only)

As part of the default read-only checks, run `brikko-studio security audit --deep`. Only offer alternatives if the user requests them:

1. `brikko-studio security audit` (faster, non-probing)
2. `brikko-studio security audit --json` (structured output)

Offer to apply Brikko Studio safe defaults (numbered):

1. `brikko-studio security audit --fix`

Be explicit that `--fix` only tightens Brikko Studio defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Brikko Studio version/update status (read-only)

As part of the default read-only checks, run `brikko-studio update status`.
...
- Credential hygiene notes (location of Brikko Studio creds, prefer disk encryption)
...
- Brikko Studio security audit (re-run)
...
After Brikko Studio install or first hardening pass, run at least one baseline audit and version check:

- `brikko-studio security audit`
- `brikko-studio security audit --deep`
- `brikko-studio update status`

Ongoing monitoring is recommended. Use the Brikko Studio cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `brikko-studio cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `brikko-studio update status`
...
Before creating, `brikko-studio cron list` and match on exact `name`. If found, `brikko-studio cron edit <id> ...`.
If not found, `brikko-studio cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `brikko-studio update status` (preferred for source checkouts and channels)
2. `npm view brikko-studio version` (published npm version)

## Brikko Studio command accuracy

Use only supported commands and flags:

- `brikko-studio security audit [--deep] [--fix] [--json]`
- `brikko-studio status` / `brikko-studio status --deep`
- `brikko-studio health --json`
- `brikko-studio update status`
- `brikko-studio cron add|list|runs|run`

Do not invent CLI flags or imply Brikko Studio enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Brikko Studio compaction:
