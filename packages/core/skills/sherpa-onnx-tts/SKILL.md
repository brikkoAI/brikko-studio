...
1. Download the runtime for your OS (extracts into `$BRIKKO_STUDIO_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.brikko-studio/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$BRIKKO_STUDIO_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.brikko-studio/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Brikko Studio config file (`$BRIKKO_STUDIO_CONFIG_PATH`, default `~/.brikko-studio/brikko-studio.json`):
