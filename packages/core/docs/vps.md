---
summary: "Run Brikko Studio on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for Brikko Studio
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the Brikko Studio Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install Brikko Studio on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep Brikko Studio bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/brikko-studio-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `BRIKKO_STUDIO_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for a stable startup path:
  ...
  - `NODE_COMPILE_CACHE=/var/tmp/brikko-studio-compile-cache`
...
For the standard `brikko-studio onboard --install-daemon` path, edit the user unit:
...
`brikko-studio-gateway.service` via `sudo systemctl edit brikko-studio-gateway.service`.
