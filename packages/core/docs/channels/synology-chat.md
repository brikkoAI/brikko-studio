---
summary: "Synology Chat webhook setup and Brikko Studio config"
read_when:
  - Setting up Synology Chat with Brikko Studio
...
Synology Chat ships as a bundled plugin in current Brikko Studio releases, so normal
...
```bash
brikko-studio plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   ...
   - `brikko-studio onboard` now shows Synology Chat in the same channel setup list as `brikko-studio channels add`.
   - Non-interactive setup: `brikko-studio channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Brikko Studio gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Brikko Studio.
   - Guided: `brikko-studio onboard`
   - Direct: `brikko-studio channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- Brikko Studio accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
- Accepted header forms:
  - `x-synology-token`
  - `x-webhook-token`
  - `x-brikko-studio-token`
...
- Pairing approvals work with:
  - `brikko-studio pairing list synology-chat`
  - `brikko-studio pairing approve synology-chat <CODE>`
...
```bash
brikko-studio message send --channel synology-chat --target 123456 --text "Hello from Brikko Studio"
brikko-studio message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
brikko-studio message send --channel synology-chat --target synology:123456 --text "Short prefix"
```

Media sends are supported by URL-based file delivery.
Outbound file URLs must use `http` or `https`, and private or otherwise blocked network targets are rejected before Brikko Studio forwards the URL to the NAS webhook.
...
Give each enabled account a distinct `webhookPath`. Brikko Studio now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached Brikko Studio
- `Rate limit exceeded`:
