...
read_when:
  - You want to connect Brikko Studio to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to Brikko Studio via the LINE Messaging API. The plugin runs as a webhook
...
```bash
brikko-studio plugins install @brikko-studio/line
```

Local checkout (when running from a git repo):

```bash
brikko-studio plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Brikko Studio applies strict pre-auth body limits and timeout before verification.
- Brikko Studio processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
brikko-studio pairing list line
brikko-studio pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.brikko-studio/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. Brikko Studio validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
