...
read_when:
  - You want Brikko Studio to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Brikko Studio to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current Brikko Studio releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`brikko-studio onboard`) and `brikko-studio channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install the npm package directly.

```bash
brikko-studio plugins install @brikko-studio/nostr
```
...
```bash
brikko-studio plugins install --link <path-to-local-nostr-plugin>
```
...
brikko-studio channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "brikko-studio@example.com",
        lud16: "brikko-studio@example.com",
      },
    },
  },
}
