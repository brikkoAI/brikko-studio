...
```bash
brikko-studio pairing list imessage
brikko-studio pairing approve imessage <CODE>
```
  ...
  <Tab title="Remote Mac over SSH">
    Brikko Studio only requires a stdio-compatible `cliPath`, so you can point `cliPath` at a wrapper script that SSHes to a remote Mac and runs `imsg`.
...
```json5
{
  channels: {
    imessage: {
      enabled: true,
      cliPath: "~/.brikko-studio/scripts/imsg-ssh",
...
```
    ...
    If `remoteHost` is not set, Brikko Studio attempts to auto-detect it by parsing the SSH wrapper script.
    `remoteHost` must be `host` or `user@host` (no spaces or SSH options).
    Brikko Studio uses strict host-key checking for SCP, so the relay host key must already exist in `~/.ssh/known_hosts`.
...
- Full Disk Access is required for the process context running Brikko Studio/`imsg` (Messages DB access).
...
<Tabs>
  ...
  <Tab title="Sessions and deterministic replies">
    ...
    If that `chat_id` is explicitly configured under `channels.imessage.groups`, Brikko Studio treats it as group traffic (group gating + group session isolation).
...
<AccordionGroup>
  <Accordion title="Dedicated bot macOS user (separate iMessage identity)">
    ...
    4. Create SSH wrapper so Brikko Studio can run `imsg` in that user context.
...
```
  ...
  <Accordion title="DMs are ignored">
    ...
    - pairing approvals (`brikko-studio pairing list imessage`)
...
```
    ...
    Confirm Full Disk Access + Automation are granted for the process context that runs Brikko Studio/`imsg`.
