...
- bot user = the Brikko Studio Matrix account that sends the reply
...
If no pushers come back, fix normal Matrix push delivery for this account before continuing.
  ...
  <Step title="Install the override push rule">
    Brikko Studio marks finalized text-only preview edits with `content["com.brikko-studio.finalized_preview"] = true`. Install a rule that matches that marker plus the bot MXID as sender:

```bash
curl -sS -X PUT \
  "https://matrix.example.org/_matrix/client/v3/pushrules/global/override/brikko-studio-finalized-preview-botname" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  --data '{
    "conditions": [
      ...
      {
        "kind": "event_property_is",
        "key": "content.com\\.brikko-studio\\.finalized_preview",
        "value": true
      },
...
```
    ...
    - `brikko-studio-finalized-preview-botname`: a rule ID unique per bot per recipient (pattern: `brikko-studio-finalized-preview-<botname>`)
    - `@bot:example.org`: your Brikko Studio bot MXID, not the recipient's
...
Push rules are keyed by `ruleId`: re-running `PUT` against the same ID updates a single rule. For multiple Brikko Studio bots notifying the same recipient, create one rule per bot with a distinct sender match.
