...
read_when:
  - Setting up Twitch chat integration for Brikko Studio
title: "Twitch"
sidebarTitle: "Twitch"
---

Twitch chat support via IRC connection. Brikko Studio connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

<Note>
Twitch ships as a bundled plugin in current Brikko Studio releases, so normal packaged builds do not need a separate install.
</Note>
...
<Tabs>
  <Tab title="npm registry">
    ```bash
    brikko-studio plugins install @brikko-studio/twitch
    ```
  </Tab>
  <Tab title="Local checkout">
    ```bash
    brikko-studio plugins install ./path/to/local/twitch-plugin
    ```
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="Configure the token">
    - Env: `BRIKKO_STUDIO_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "brikko-studio", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use BRIKKO_STUDIO_TWITCH_ACCESS_TOKEN env var)
...
<Tabs>
  <Tab title="Env var (default account only)">
    ```bash
    BRIKKO_STUDIO_TWITCH_ACCESS_TOKEN=oauth:abc123...
    ```
  </Tab>
  <Tab title="Config">
    ```json5
