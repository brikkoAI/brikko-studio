...
Brikko Studio routes replies **back to the channel where a message came from**. The
...
Brikko Studio infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Brikko Studio still records inbound session metadata, but it
...
when a guarded path must not create a new Brikko Studio session. In that mode,
Brikko Studio may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Brikko Studio would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.brikko-studio/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.brikko-studio`):

- `~/.brikko-studio/agents/<agentId>/sessions/sessions.json`
