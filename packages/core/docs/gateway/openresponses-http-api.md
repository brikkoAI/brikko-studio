...
Brikko Studio’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`brikko-studio agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-brikko-studio-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-brikko-studio-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "brikko-studio"`, `model: "brikko-studio/default"`, `model: "brikko-studio/<agentId>"`, or `x-brikko-studio-agent-id`
- use `x-brikko-studio-model` when you want to override the selected agent's backend model
- use `x-brikko-studio-session-key` for explicit session routing
- use `x-brikko-studio-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: Brikko Studio reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
Brikko Studio normalizes common OpenAI-style aliases before those counters reach
...
curl -sS http://127.0.0.1:18789/v1/responses \
  ...
  -H 'x-brikko-studio-agent-id: main' \
  -d '{
    "model": "brikko-studio",
    "input": "hi"
  }'
