---
summary: "Run Brikko Studio on local LLMs (LM Studio, vLLM, LiteLLM, custom OpenAI endpoints)"
read_when:
...
| LiteLLM / OAI-proxy / custom OpenAI-compatible proxy | You front another model API and need Brikko Studio to treat it as OpenAI         |
...
If `api` is omitted on a custom provider with a `baseUrl`, Brikko Studio defaults to
...
- hidden Brikko Studio attribution headers (`originator`, `version`, `User-Agent`)
...
- Some local models emit standalone bracketed tool requests as text, such as
  `[tool_name]` followed by JSON and `[END_TOOL_REQUEST]`. Brikko Studio promotes
...
- If a model emits JSON, XML, or ReAct-style text that looks like a tool call
  but the provider did not emit a structured invocation, Brikko Studio leaves it as
...
- If tools appear as assistant text instead of running, for example raw JSON,
  ...
  It overrides Brikko Studio's default proxy value of `tool_choice: "auto"`.
  Replace `local/my-local-model` with the exact provider/model ref shown by
  `brikko-studio models list`.

  ```bash
  brikko-studio config set agents.defaults.models '{"local/my-local-model":{"params":{"extra_body":{"tool_choice":"required"}}}}' --strict-json --merge
  ```
...
1. **Confirm the local model itself responds.** No tools, no agent context:
   ...
   ```bash
   brikko-studio infer model run --local --model <provider/model> --prompt "Reply with exactly: pong" --json
   ```
...
2. **Confirm Gateway routing.** Sends only the supplied prompt — skips transcript, AGENTS bootstrap, context-engine assembly, tools, and bundled MCP servers, but still exercises Gateway routing, auth, and provider selection:
   ...
   ```bash
   brikko-studio infer model run --gateway --model <provider/model> --prompt "Reply with exactly: pong" --json
   ```
...
5. **Past that, the bottleneck is upstream.** If the backend still fails only on larger Brikko Studio runs after lean mode and `supportsTools: false`, the remaining issue is usually upstream model or server capacity — context window, GPU memory, kv-cache eviction, or a backend bug. It is not Brikko Studio's transport layer at that point.
...
- Local server says `terminated`, `ECONNRESET`, or closes the stream mid-turn?
  Brikko Studio records a low-cardinality `model.call.error.failureKind` plus the
  Brikko Studio process RSS/heap snapshot in diagnostics. For LM Studio/Ollama
