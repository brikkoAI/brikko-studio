...
Brikko Studio has three related (but different) controls:
...
Use the inspector to see what Brikko Studio is _actually_ doing:
...
- Brikko Studio validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:brikko-studio`: all built-in Brikko Studio tools (excludes provider plugins)
