---
summary: "Run multiple Brikko Studio Gateways on one host (isolation, ports, and profiles)"
read_when:
...
brikko-studio --profile rescue onboard
brikko-studio --profile rescue gateway install --port 19789
```

If your main bot is already running, that is usually all you need.

During `brikko-studio --profile rescue onboard`:
...
`brikko-studio --profile rescue onboard` uses the normal onboarding flow, but it
...
- config file
- state directory
- workspace (by default `~/.brikko-studio/workspace-rescue`)
...
# extra gateway
brikko-studio --profile ops setup
brikko-studio --profile ops gateway --port 19789
```

If you want both Gateways to use named profiles, that also works:

```bash
brikko-studio --profile main setup
brikko-studio --profile main gateway --port 18789
...
brikko-studio --profile ops gateway install --port 19789
```
...
- `BRIKKO_STUDIO_CONFIG_PATH` — per-instance config file
- `BRIKKO_STUDIO_STATE_DIR` — per-instance sessions, creds, caches
...
Base port = `gateway.port` (or `BRIKKO_STUDIO_GATEWAY_PORT` / `--port`).
...
```bash
BRIKKO_STUDIO_CONFIG_PATH=~/.brikko-studio/main.json \
BRIKKO_STUDIO_STATE_DIR=~/.brikko-studio \
brikko-studio gateway --port 18789

BRIKKO_STUDIO_CONFIG_PATH=~/.brikko-studio/rescue.json \
BRIKKO_STUDIO_STATE_DIR=~/.brikko-studio-rescue \
...
brikko-studio --profile rescue gateway status --deep
brikko-studio --profile rescue gateway probe
brikko-studio status
brikko-studio --profile rescue status
brikko-studio --profile rescue browser status
```
