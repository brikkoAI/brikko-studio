---
summary: "Use OpenShell as a managed sandbox backend for Brikko Studio agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend for Brikko Studio. Instead of running Docker
containers locally, Brikko Studio delegates sandbox lifecycle to the `openshell` CLI,
...
- Brikko Studio Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Brikko Studio creates an OpenShell
...
- You edit files locally outside Brikko Studio and want those changes visible in the
...
- When the sandbox is first created, Brikko Studio seeds the remote workspace from
...
- Brikko Studio does **not** sync remote changes back into the local workspace.
...
<Warning>
If you edit files on the host outside Brikko Studio after the initial seed, the remote sandbox does **not** see those changes. Use `brikko-studio sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"brikko-studio"`  | Sandbox source for first-time create                  |
...
1. Brikko Studio calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Brikko Studio calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `brikko-studio sandbox` commands
