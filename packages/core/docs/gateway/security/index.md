...
<Warning>
  **Personal assistant trust model.** This guidance assumes one trusted
  ...
  Brikko Studio is **not** a hostile multi-tenant security boundary for multiple
...
Brikko Studio security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `brikko-studio security audit`
...
```bash
brikko-studio security audit
brikko-studio security audit --deep
brikko-studio security audit --fix
brikko-studio security audit --json
```
...
Brikko Studio is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
Brikko Studio assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.brikko-studio`, including `brikko-studio.json`), treat them as a trusted operator.
...
- Brikko Studio's product default for trusted single-operator setups is that host exec on `gateway`/`node` is allowed without approval prompts (`security="full"`, `ask="off"` unless you tighten it). That default is intentional UX, not a vulnerability by itself.
...
Brikko Studio separates two concepts:
...
If you run `--deep`, Brikko Studio also attempts a best-effort live Gateway probe.
...
- **Codex runtime state**: `~/.brikko-studio/agents/<agentId>/agent/codex-home/`
...
`brikko-studio security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`brikko-studio security audit` raises `config.insecure_or_dangerous_flags` when
...
gateway:
  ...
  auth:
    mode: password
    password: ${BRIKKO_STUDIO_GATEWAY_PASSWORD}
```
...
- Brikko Studio gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from Brikko Studio responses.
...
Brikko Studio stores session transcripts on disk under `~/.brikko-studio/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.brikko-studio` (see the audit section below). If you need
...
- Approval mode binds exact request context and, when possible, one concrete local script/file operand. If Brikko Studio cannot identify exactly one direct local file for an interpreter/runtime command, approval-backed execution is denied rather than promising full semantic coverage.
...
Brikko Studio can refresh the skills list mid-session:
...
- If you install or update plugins (`brikko-studio plugins install <package>`, `brikko-studio plugins update <id>`), treat it like running untrusted code:
  - The install path is the per-plugin directory under the active plugin install root.
  - Brikko Studio runs a built-in dangerous-code scan before install/update. `critical` findings block by default.
  - npm and git plugin installs run package-manager dependency convergence only during the explicit install/update flow. Local paths and archives are treated as self-contained plugin packages; Brikko Studio copies/references them without running `npm install`.
  ...
  - Gateway-backed skill dependency installs follow the same dangerous/suspicious split: built-in `critical` findings block unless the caller explicitly sets `dangerouslyForceUnsafeInstall`, while suspicious findings still warn only. `brikko-studio skills install` remains the separate ClawHub skill download/install flow.
...
```bash
brikko-studio pairing list <channel>
brikko-studio pairing approve <channel> <code>
```
...
By default, Brikko Studio routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Brikko Studio has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.brikko-studio/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- “Paste the full contents of ~/.brikko-studio or your logs.”

## External content special-token sanitization

Brikko Studio strips common self-hosted LLM chat-template special-token literals from wrapped external content and metadata before they reach the model. Covered marker families include Qwen/ChatML, Llama, Gemma, Mistral, Phi, and GPT-OSS role/turn tokens.
...
Brikko Studio includes explicit bypass flags that disable external-content safety wrapping:
...
Brikko Studio strips common model-family special-token literals from wrapped
...
- `~/.brikko-studio/brikko-studio.json`: `600` (user read/write only)
- `~/.brikko-studio`: `700` (user only)

`brikko-studio doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `BRIKKO_STUDIO_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__brikko-studio__/canvas/` and `/__brikko-studio__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
If you run Brikko Studio with Docker on a VPS, remember that published container ports
...
The Gateway broadcasts its presence via mDNS (`_brikko-studio-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `BRIKKO_STUDIO_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `brikko-studio doctor --generate-gateway-token`.
...
paths, set `BRIKKO_STUDIO_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as
break-glass. This is intentionally process environment only, not an
`brikko-studio.json` config key.
...
- Brikko Studio also has a narrow backend/container-local self-connect path for
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `BRIKKO_STUDIO_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `BRIKKO_STUDIO_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Brikko Studio
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. Brikko Studio verifies the identity by resolving the
...
- On the OpenAI-compatible HTTP surface, shared-secret bearer auth restores the full default operator scopes (`operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`) and owner semantics for agent turns; narrower `x-brikko-studio-scopes` values do not reduce that shared-secret path.
...
- In those identity-bearing modes, omitting `x-brikko-studio-scopes` falls back to the normal operator default scope set; send the header explicitly when you want a narrower scope set.
...
- Brikko Studio will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.brikko-studio/` (or `$BRIKKO_STUDIO_STATE_DIR/`) may contain secrets or private data:

- `brikko-studio.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
Brikko Studio loads workspace-local `.env` files for agents and tools, but never lets those files silently override gateway runtime controls.

- Any key that starts with `BRIKKO_STUDIO_*` is blocked from untrusted workspace `.env` files.
...
Why: workspace `.env` files frequently live next to agent code, get committed by accident, or get written by tools. Blocking the whole `BRIKKO_STUDIO_*` prefix means adding a new `BRIKKO_STUDIO_*` flag later can never regress into silent inheritance from workspace state.
...
- When sharing diagnostics, prefer `brikko-studio status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@brikko-studio", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.brikko-studio`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.brikko-studio/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `brikko-studio` profile).
...
Brikko Studio’s browser navigation policy is strict by default: private/internal destinations stay blocked unless you explicitly opt in.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.brikko-studio/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.brikko-studio/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.brikko-studio/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "none",
        },
        // Session tools can reveal sensitive data from transcripts. By default Brikko Studio limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `brikko-studio gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `BRIKKO_STUDIO_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/brikko-studio/brikko-studio-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.brikko-studio/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `brikko-studio security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + Brikko Studio version
...
Found a vulnerability in Brikko Studio? Please report responsibly:

1. Email: [security@brikko-studio.ai](mailto:security@brikko-studio.ai)
