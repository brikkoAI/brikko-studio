...
- Legacy single-account Baileys auth dir is migrated by `brikko-studio doctor` into `whatsapp/default`.
...
- `apiRoot` is the Telegram Bot API root only. Use `https://api.telegram.org` or your self-hosted/proxy root, not `https://api.telegram.org/bot<TOKEN>`; `brikko-studio doctor --fix` removes an accidental trailing `/bot<TOKEN>` suffix.
- Optional `channels.telegram.defaultAccount` overrides default account selection when it matches a configured account id.
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `brikko-studio doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["brikko-studio-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-brikko-studio",
          requireMention: false,
...
- `channels.discord.voice.reconnectGraceMs` controls how long a disconnected voice session may take to enter reconnect signalling before Brikko Studio destroys it (`15000` by default).
- Brikko Studio additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a bundled plugin in current Brikko Studio releases. Older or
custom builds can install a current npm package with
`brikko-studio plugins install @brikko-studio/mattermost`. Check
[npmjs.com/package/@brikko-studio/mattermost](https://www.npmjs.com/package/@brikko-studio/mattermost)
...
- `commands.callbackUrl` must resolve to the Brikko Studio gateway endpoint and be reachable from the Mattermost server.
- Native slash callbacks are authenticated with the per-command tokens returned
  ...
  commands are activated, Brikko Studio rejects callbacks with
...
Brikko Studio spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `brikko-studio channels add` (or channel onboarding) while still on a single-account top-level channel config, Brikko Studio promotes account-scoped top-level single-account values into the channel account map first so the original account keeps working. Most channels move them into `channels.<channel>.accounts.default`; Matrix can preserve an existing matching named/default target instead.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `brikko-studio doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into the promoted account chosen for that channel. Most channels use `accounts.default`; Matrix can preserve an existing matching named/default target instead.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@brikko-studio", "brikko-studio"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@brikko-studio"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `brikko-studio.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
- `mcp: true` enables `/mcp` for Brikko Studio-managed MCP server config under `mcp.servers`.
