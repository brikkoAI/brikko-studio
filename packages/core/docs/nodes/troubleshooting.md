...
```bash
brikko-studio nodes describe --node <idOrNameOrIp>
brikko-studio nodes canvas snapshot --node <idOrNameOrIp>
brikko-studio logs --follow
```
...
brikko-studio nodes status
brikko-studio approvals get --node <idOrNameOrIp>
brikko-studio approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`brikko-studio approvals get --node ...`), not in the gateway pairing record.
