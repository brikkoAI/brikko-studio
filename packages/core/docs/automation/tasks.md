...
- `brikko-studio tasks list` shows all tasks; `brikko-studio tasks audit` surfaces issues.
...
| CLI operations         | `cli`        | `brikko-studio agent` commands that run through the gateway | `silent`              |
...
| `cancelled` | Stopped by the operator via `brikko-studio tasks cancel`                        |
...
- CLI tasks: isolated child-session tasks use the child session; chat-backed
  ...
  `brikko-studio agent` runs also finalize from their run result, so completed runs
  do not sit active until the sweeper marks them `lost`.

## Delivery and notifications

When a task reaches a terminal state, Brikko Studio notifies you. There are two delivery paths:

**Direct delivery** — if the task has a channel target (the `requesterOrigin`), the completion message goes straight to that channel (Telegram, Discord, Slack, etc.). For subagent completions, Brikko Studio also preserves bound thread/topic routing when available and can fill a missing `to` / account from the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) before giving up on direct delivery.
...
<AccordionGroup>
  <Accordion title="tasks list">
    ```bash
    brikko-studio tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
    ```
  ...
  <Accordion title="tasks audit">
    ```bash
    brikko-studio tasks audit [--json]
    ```

    Surfaces operational issues. Findings also appear in `brikko-studio status` when issues are detected.
  ...
  <Accordion title="tasks maintenance">
    ```bash
    brikko-studio tasks maintenance [--json]
  ...
  <Accordion title="tasks flow list | show | cancel">
    ```bash
    brikko-studio tasks flow list [--status <status>] [--json]
...
For the full operator ledger, use the CLI: `brikko-studio tasks list`.

## Status integration (task pressure)

`brikko-studio status` includes an at-a-glance task summary:
...
```
$BRIKKO_STUDIO_STATE_DIR/tasks/runs.sqlite
```
...
<AccordionGroup>
  <Accordion title="Tasks and Task Flow">
    [Task Flow](/automation/taskflow) is the flow orchestration layer above background tasks. A single flow may coordinate multiple tasks over its lifetime using managed or mirrored sync modes. Use `brikko-studio tasks` to inspect individual task records and `brikko-studio tasks flow` to inspect the orchestrating flow.

    See [Task Flow](/automation/taskflow) for details.

  </Accordion>
  <Accordion title="Tasks and cron">
    A cron job **definition** lives in `~/.brikko-studio/cron/jobs.json`; runtime execution state lives beside it in `~/.brikko-studio/cron/jobs-state.json`. **Every** cron execution creates a task record — both main-session and isolated. Main-session cron tasks default to `silent` notify policy so they track without generating notifications.
