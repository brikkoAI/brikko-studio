---
summary: "Use Xiaomi MiMo models with Brikko Studio"
read_when:
  - You want Xiaomi MiMo models in Brikko Studio
  - You need XIAOMI_API_KEY setup
title: "Xiaomi MiMo"
---

Xiaomi MiMo is the API platform for **MiMo** models. Brikko Studio uses the Xiaomi
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    brikko-studio onboard --auth-choice xiaomi-api-key
    ```

    Or pass the key directly:

    ```bash
    brikko-studio onboard --auth-choice xiaomi-api-key --xiaomi-api-key "$XIAOMI_API_KEY"
    ```

  </Step>
  <Step title="Verify the model is available">
    ```bash
    brikko-studio models list --provider xiaomi
    ```
  </Step>
</Steps>
...
targets such as Feishu and Telegram, Brikko Studio transcodes Xiaomi output to 48kHz
...
<AccordionGroup>
  ...
  <Accordion title="Troubleshooting">
    ...
    - When the Gateway runs as a daemon, ensure the key is available to that process (for example in `~/.brikko-studio/.env` or via `env.shellEnv`).

    <Warning>
    Keys set only in your interactive shell are not visible to daemon-managed gateway processes. Use `~/.brikko-studio/.env` or `env.shellEnv` config for persistent availability.
    </Warning>
