---
summary: "Use Kilo Gateway's unified API to access many models in Brikko Studio"
title: "Kilocode"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in Brikko Studio
---
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    brikko-studio onboard --auth-choice kilocode-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    brikko-studio models list --provider kilocode
    ```
  </Step>
</Steps>
...
<Note>
Brikko Studio treats `kilocode/kilo/auto` as the stable default ref, but does not
...
hard-coded in Brikko Studio.
</Note>

## Built-in catalog

Brikko Studio dynamically discovers available models from the Kilo Gateway at startup. Use
...
<Tip>
At startup, Brikko Studio queries `GET https://api.kilo.ai/api/gateway/models` and merges
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path, so Brikko Studio keeps
  ...
  <Accordion title="Troubleshooting">
    - If model discovery fails at startup, Brikko Studio falls back to the bundled static catalog containing `kilocode/kilo/auto`.
    - Confirm your API key is valid and that your Kilo account has the desired models enabled.
    - When the Gateway runs as a daemon, ensure `KILOCODE_API_KEY` is available to that process (for example in `~/.brikko-studio/.env` or via `env.shellEnv`).
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full Brikko Studio configuration reference.
  </Card>
