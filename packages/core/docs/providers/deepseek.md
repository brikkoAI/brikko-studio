...
title: "DeepSeek"
read_when:
  - You want to use DeepSeek with Brikko Studio
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    brikko-studio onboard --auth-choice deepseek-api-key
    ```
  ...
  <Step title="Verify models are available">
    ```bash
    brikko-studio models list --provider deepseek
    ```
    ...
    ```bash
    brikko-studio models list --all --provider deepseek
    ```
...
<Tip>
V4 models support DeepSeek's `thinking` control. Brikko Studio also replays
...
`reasoning_content` on follow-up requests. Brikko Studio handles this inside the
...
DeepSeek `reasoning_content`. Brikko Studio fills that missing field on replayed
assistant messages for DeepSeek V4 thinking requests so the provider can accept
the history without requiring `/new`.

When thinking is disabled in Brikko Studio (including the UI **None** selection),
Brikko Studio sends DeepSeek `thinking: { type: "disabled" }` and strips replayed
...
```bash
BRIKKO_STUDIO_LIVE_PROVIDERS=deepseek \
BRIKKO_STUDIO_LIVE_MODELS="deepseek/deepseek-v4-flash,deepseek/deepseek-v4-pro" \
