...
read_when:
  - You want to use Tencent Hy3 preview with Brikko Studio
  - You need the TokenHub API key setup
---

# Tencent Cloud TokenHub

Tencent Cloud ships as a **bundled provider plugin** in Brikko Studio. It gives access to Tencent Hy3 preview through the TokenHub endpoint (`tencent-tokenhub`).
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    brikko-studio onboard --auth-choice tokenhub-api-key
    ```
  </Step>
  <Step title="Verify the model">
    ```bash
    brikko-studio models list --provider tencent-tokenhub
    ```
  </Step>
</Steps>
...
<Tip>
The model id is `hy3-preview`. Do not confuse it with Tencent's `HY-3D-*` models, which are 3D generation APIs and are not the Brikko Studio chat model configured by this provider.
</Tip>

## Endpoint override

Brikko Studio defaults to Tencent Cloud's `https://tokenhub.tencentmaas.com/v1` endpoint. Tencent also documents an international TokenHub endpoint:

```bash
brikko-studio config set models.providers.tencent-tokenhub.baseUrl "https://tokenhub-intl.tencentmaas.com/v1"
```
...
- [Brikko Studio Configuration](/gateway/configuration)
