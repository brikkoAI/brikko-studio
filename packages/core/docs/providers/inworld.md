---
summary: "Inworld streaming text-to-speech for Brikko Studio replies"
read_when:
...
Inworld is a streaming text-to-speech (TTS) provider. In Brikko Studio it
...
Brikko Studio posts to Inworld's streaming TTS endpoint, concatenates the
...
<Steps>
  ...
  <Step title="Send a message">
    Send a reply through any connected channel. Brikko Studio synthesizes the
...
<AccordionGroup>
  ...
  <Accordion title="Audio outputs">
    ...
    Brikko Studio asks Inworld for `OGG_OPUS` so the audio plays as a native
