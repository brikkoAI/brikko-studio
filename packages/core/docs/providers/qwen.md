---
summary: "Use Qwen Cloud via Brikko Studio's bundled qwen provider"
read_when:
  - You want to use Qwen with Brikko Studio
...
See [Issue #49557](https://github.com/brikko-studio/brikko-studio/issues/49557) for
background.

</Warning>

Brikko Studio now treats Qwen as a first-class bundled provider with canonical id
    ...
    **Best for:** subscription-based access through the Qwen Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        brikko-studio onboard --auth-choice qwen-api-key
        ```

        For the **China** endpoint:

        ```bash
        brikko-studio onboard --auth-choice qwen-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        brikko-studio models list --provider qwen
        ```
      </Step>
    </Steps>
    ...
    **Best for:** pay-as-you-go access through the Standard Model Studio endpoint, including models like `qwen3.6-plus` that may not be available on the Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        brikko-studio onboard --auth-choice qwen-standard-api-key
        ```

        For the **China** endpoint:

        ```bash
        brikko-studio onboard --auth-choice qwen-standard-api-key-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
...
Brikko Studio currently ships this bundled Qwen catalog. The configured catalog is
...
For reasoning-enabled Qwen Cloud models, the bundled provider maps Brikko Studio
...
<AccordionGroup>
  ...
  <Accordion title="Qwen 3.6 Plus availability">
    ...
    Brikko Studio's bundled Qwen catalog does not advertise `qwen3.6-plus` on Coding
  ...
  <Accordion title="Video generation details">
    For video generation, Brikko Studio maps the configured Qwen region to the matching
