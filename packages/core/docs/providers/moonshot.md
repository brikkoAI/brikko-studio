    ...
    **Best for:** Kimi K2 models via the Moonshot Open Platform.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        brikko-studio onboard --auth-choice moonshot-api-key
        ```

        Or for the China endpoint:

        ```bash
        brikko-studio onboard --auth-choice moonshot-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        brikko-studio models list --provider moonshot
        ```
      </Step>
      <Step title="Run a live smoke test">
        ...
        ```bash
        BRIKKO_STUDIO_CONFIG_PATH=/tmp/brikko-studio-kimi/brikko-studio.json \
        BRIKKO_STUDIO_STATE_DIR=/tmp/brikko-studio-kimi \
        brikko-studio agent --local \
  ...
  <Tab title="Kimi Coding">
    **Best for:** code-focused tasks via the Kimi Coding endpoint.
    ...
    <Steps>
      <Step title="Run onboarding">
        ```bash
        brikko-studio onboard --auth-choice kimi-code-api-key
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
      ...
      <Step title="Verify the model is available">
        ```bash
        brikko-studio models list --provider kimi
        ```
      </Step>
    </Steps>
...
Brikko Studio also ships **Kimi** as a `web_search` provider, backed by Moonshot web
search.
...
<AccordionGroup>
  <Accordion title="Native thinking mode">
    ...
    Brikko Studio also maps runtime `/think` levels for Moonshot:
    ...
    <Warning>
    When Moonshot thinking is enabled, `tool_choice` must be `auto` or `none`. Brikko Studio normalizes incompatible `tool_choice` values to `auto` for compatibility.
    </Warning>
    ...
    default strategy. Brikko Studio only forwards `thinking.keep` for
  ...
  <Accordion title="Tool call id sanitization">
    Moonshot Kimi serves tool_call ids shaped like `functions.<name>:<index>`. Brikko Studio preserves them unchanged so multi-turn tool calls keep working.
  ...
  <Accordion title="Streaming usage compatibility">
    ...
    shared `openai-completions` transport. Brikko Studio keys that off endpoint
