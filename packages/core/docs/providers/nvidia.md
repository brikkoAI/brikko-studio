---
summary: "Use NVIDIA's OpenAI-compatible API in Brikko Studio"
read_when:
  - You want to use open models in Brikko Studio for free
...
<Steps>
  ...
  <Step title="Export the key and run onboarding">
    ...
    brikko-studio onboard --auth-choice nvidia-api-key
    ```
  </Step>
  <Step title="Set an NVIDIA model">
    ```bash
    brikko-studio models set nvidia/nvidia/nemotron-3-super-120b-a12b
    ```
  </Step>
</Steps>
...
```bash
brikko-studio onboard --auth-choice nvidia-api-key --nvidia-api-key "nvapi-..."
```
