---
summary: "Use OpenRouter's unified API to access many models in Brikko Studio"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in Brikko Studio
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    brikko-studio onboard --auth-choice openrouter-api-key
    ```
  </Step>
  <Step title="(Optional) Switch to a specific model">
    ...
    ```bash
    brikko-studio models set openrouter/<provider>/<model>
    ```
...
Brikko Studio submits text-to-video and image-to-video jobs to OpenRouter, polls
...
On real OpenRouter requests (`https://openrouter.ai/api/v1`), Brikko Studio also adds
...
<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, Brikko Studio
...
<AccordionGroup>
  <Accordion title="Anthropic cache markers">
    ...
    OpenRouter-specific Anthropic `cache_control` markers that Brikko Studio uses for
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, Brikko Studio maps the selected thinking level to
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: Brikko Studio keeps
    Gemini thought-signature sanitation there, but does not enable native Gemini
    replay validation or bootstrap rewrites.
  </Accordion>

  <Accordion title="Provider routing metadata">
    If you pass OpenRouter provider routing under model params, Brikko Studio forwards
