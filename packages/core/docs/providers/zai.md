---
summary: "Use Z.AI (GLM models) with Brikko Studio"
read_when:
  - You want Z.AI / GLM models in Brikko Studio
  - You need a simple ZAI_API_KEY setup
title: "Z.AI"
---

Z.AI is the API platform for **GLM** models. It provides REST APIs for GLM and uses API keys
for authentication. Create your API key in the Z.AI console. Brikko Studio uses the `zai` provider
with a Z.AI API key.
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. Brikko Studio detects the matching Z.AI endpoint from the key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        brikko-studio onboard --auth-choice zai-api-key
        ```
      </Step>
      ...
      <Step title="Verify the model is listed">
        ```bash
        brikko-studio models list --all --provider zai
        ```
      </Step>
    </Steps>
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        brikko-studio onboard --auth-choice zai-coding-global

        # Coding Plan CN (China region)
        brikko-studio onboard --auth-choice zai-coding-cn

        # General API
        brikko-studio onboard --auth-choice zai-global

        # General API CN (China region)
        brikko-studio onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
...
Brikko Studio ships the bundled `zai` provider catalog in the plugin manifest, so read-only
...
<AccordionGroup>
  ...
  <Accordion title="Thinking and preserved thinking">
    Z.AI thinking follows Brikko Studio's `/think` controls. With thinking off,
    Brikko Studio sends `thinking: { type: "disabled" }` to avoid responses that
    ...
    When enabled and thinking is on, Brikko Studio sends
