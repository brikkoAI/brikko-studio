---
summary: "Use Venice AI privacy-focused models in Brikko Studio"
read_when:
  - You want privacy-focused inference in Brikko Studio
...
<Steps>
  ...
  <Step title="Configure Brikko Studio">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        brikko-studio onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    brikko-studio agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, Brikko Studio shows all available Venice models. Pick based on your needs:
...
```bash
brikko-studio models set venice/kimi-k2-5
brikko-studio models set venice/claude-opus-4-6
```

List all available models:

```bash
brikko-studio models list --all --provider venice
```

You can also run `brikko-studio configure`, select **Model/auth**, and choose **Venice AI**.
...
`venice/deepseek-v4-flash`, Brikko Studio fills the required DeepSeek V4
...
Brikko Studio keeps that provider-specific replay fix separate from the native
...
Brikko Studio ships a manifest-backed Venice seed catalog for read-only model listing. Runtime refresh can still discover models from the Venice API, and falls back to the manifest catalog if the API is unreachable.
...
brikko-studio agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
brikko-studio agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
brikko-studio agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
brikko-studio agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
brikko-studio agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  <Accordion title="API key not recognized">
    ```bash
    echo $VENICE_API_KEY
    brikko-studio models list | grep venice
    ```
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `brikko-studio models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
