---
summary: "GLM model family overview + how to use it in Brikko Studio"
read_when:
  - You want GLM models in Brikko Studio
...
GLM is a **model family** (not a company) available through the Z.AI platform. In Brikko Studio, GLM
...
<Steps>
  ...
  <Step title="Set GLM as the default model">
    ```bash
    brikko-studio config set agents.defaults.model.primary "zai/glm-5.1"
    ```
  </Step>
  <Step title="Verify models are available">
    ```bash
    brikko-studio models list --provider zai
    ```
  </Step>
</Steps>
...
<Tip>
`zai-api-key` lets Brikko Studio detect the matching Z.AI endpoint from the key and
...
Brikko Studio currently seeds the bundled `zai` provider with these GLM refs:
...
<AccordionGroup>
  <Accordion title="Endpoint auto-detection">
    When you use the `zai-api-key` auth choice, Brikko Studio inspects the key format
