---
summary: "Use Synthetic's Anthropic-compatible API in Brikko Studio"
read_when:
...
Brikko Studio registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    brikko-studio onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
Brikko Studio's Anthropic client appends `/v1` to the base URL automatically, so use
...
`brikko-studio models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that Brikko Studio appends `/v1` automatically.
