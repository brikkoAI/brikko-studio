---
summary: "Run Brikko Studio with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Brikko Studio against a local SGLang server
  - You want OpenAI-compatible /v1 endpoints with your own models
title: "SGLang"
---

SGLang can serve open-source models via an **OpenAI-compatible** HTTP API.
Brikko Studio can connect to SGLang using the `openai-completions` API.

Brikko Studio can also **auto-discover** available models from SGLang when you opt
...
Brikko Studio treats `sglang` as a local OpenAI-compatible provider that supports
...
define `models.providers.sglang`, Brikko Studio will query:
