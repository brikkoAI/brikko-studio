...
read_when:
  - You need a beginner-friendly overview of Brikko Studio logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

Brikko Studio has two main log surfaces:
...
Brikko Studio keeps up to five numbered archives beside the active file, such as
`brikko-studio-YYYY-MM-DD.1.log`, and keeps writing to a fresh active log instead of
suppressing diagnostics.

You can override this in `~/.brikko-studio/brikko-studio.json`:

```json
{
  "logging": {
    "file": "/path/to/brikko-studio.log"
  }
}
```
...
or times out before `logs.tail` answers, `brikko-studio logs` falls back to the
...
```bash
brikko-studio channels logs --channel whatsapp
```
...
Brikko Studio preserves the original structured log arguments alongside these fields
...
`brikko-studio gateway` also has WebSocket protocol logging for RPC traffic:
...
All logging configuration lives under `logging` in `~/.brikko-studio/brikko-studio.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/brikko-studio/brikko-studio-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
You can override both via the **`BRIKKO_STUDIO_LOG_LEVEL`** environment variable (e.g. `BRIKKO_STUDIO_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `brikko-studio.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `brikko-studio --log-level debug gateway run`), which overrides the environment variable for that command.
...
Brikko Studio writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
Brikko Studio can redact sensitive tokens before they hit console output, file logs,
...
policy. Brikko Studio still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `BRIKKO_STUDIO_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `brikko-studio doctor` first.
