...
This hub links the core docs for how Brikko Studio connects, pairs, and secures
devices across localhost, LAN, and tailnet.

## Core model

Most operations flow through the Gateway (`brikko-studio gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Canvas host** is served on the same port as the Gateway (`/__brikko-studio__/canvas/`, `/__brikko-studio__/a2ui/`), protected by Gateway auth when bound beyond loopback.
