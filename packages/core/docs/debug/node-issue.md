...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Brikko Studio
...
Running Brikko Studio via Node with `tsx` fails at startup with:

```
[brikko-studio] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `brikko-studio status` and `gateway:watch` worked.
