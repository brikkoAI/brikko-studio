...
summary: "Specification for making the bundled Codex app-server harness honor Brikko Studio context-engine plugins"
read_when:
...
Make the bundled Codex app-server harness honor the same Brikko Studio context-engine
...
Brikko Studio-level compaction policy as far as the Codex app-server boundary allows.
...
That means the required hook point is in Brikko Studio-controlled code. The external
boundary is the Codex app-server protocol itself: Brikko Studio can control what it
...
For Codex harness turns, Brikko Studio should preserve this lifecycle:

1. Read the mirrored Brikko Studio session transcript.
...
8. Mirror the Codex result back into the Brikko Studio transcript.
...
11. Preserve Codex native compaction signals and Brikko Studio compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. Brikko Studio should
not try to mutate the app-server's internal history except through supported
protocol calls.

Brikko Studio's transcript mirror remains the source for Brikko Studio features:
...
The context-engine interface returns Brikko Studio `AgentMessage[]`, not a Codex
...
- Label it clearly as Brikko Studio assembled context.
...
```text
Brikko Studio assembled context for this turn:
...
inside Brikko Studio and preserves context-engine semantics.
...
Brikko Studio transcript mirror.
...
1. Brikko Studio context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit Brikko Studio compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
Brikko Studio compaction should prefer the context engine's `compact()` result for
the Brikko Studio transcript mirror and plugin state.
...
the Brikko Studio session file. Preserve that behavior.

Also ensure context-engine state cleanup continues to happen through existing
Brikko Studio session lifecycle paths. Do not add Codex-specific cleanup unless the
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole Brikko Studio compaction when the context engine is primary.
...
Avoid requiring lossless-claw in Brikko Studio core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for Brikko Studio/plugin state.
