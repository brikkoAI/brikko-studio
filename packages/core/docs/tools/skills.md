...
Brikko Studio uses **[AgentSkills](https://agentskills.io)-compatible** skill
folders to teach the agent how to use tools. Each skill is a directory
containing a `SKILL.md` with YAML frontmatter and instructions. Brikko Studio
...
Brikko Studio loads skills from these sources, **highest precedence first**:
...
| 4   | Managed/local skills  | `~/.brikko-studio/skills`             |
...
Codex CLI's native `$CODEX_HOME/skills` directory is not one of these Brikko Studio
...
Use `brikko-studio migrate codex --dry-run` to inventory them and
`brikko-studio migrate codex` to choose skill directories with an interactive
checkbox prompt before copying them into the current Brikko Studio agent workspace.
...
| Shared managed/local | `~/.brikko-studio/skills`                        | All agents on that machine  |
...
`brikko-studio.plugin.json` (paths relative to the plugin root). Plugin skills
...
`metadata.brikko-studio.requires.config` on the plugin's config entry.
...
[ClawHub](https://clawhub.ai) is the public skills registry for Brikko Studio.
Use native `brikko-studio skills` commands for discover/install/update, or the
separate `clawhub` CLI for publish/sync workflows. Full guide:
[ClawHub](/tools/clawhub).

| Action                             | Command                                |
| ---------------------------------- | -------------------------------------- |
| Install a skill into the workspace | `brikko-studio skills install <skill-slug>` |
| Update all installed skills        | `brikko-studio skills update --all`         |
| Sync (scan + publish updates)      | `clawhub sync --all`                   |

Native `brikko-studio skills install` installs into the active workspace
...
configured Brikko Studio workspace). Brikko Studio picks that up as
...
`brikko-studio skills install <slug>` remains only the install path; publishers
...
- `brikko-studio skills install <slug>` is different — it downloads a ClawHub skill folder into the workspace and does not use the installer-metadata path above.
...
Brikko Studio follows the AgentSkills spec for layout/intent. The parser used
...
<ParamField path="homepage" type="string">
  URL surfaced as "Website" in the macOS Skills UI. Also supported via `metadata.brikko-studio.homepage`.
</ParamField>
<ParamField path="user-invocable" type="boolean" default="true">
  When `true`, the skill is exposed as a user slash command.
</ParamField>
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, Brikko Studio keeps the skill's instructions out of the agent's normal
...
Brikko Studio filters skills at load time using `metadata` (single-line JSON):
...
Fields under `metadata.brikko-studio`:
...
<ParamField path="requires.config" type="string[]">
  List of `brikko-studio.json` paths that must be truthy.
</ParamField>
...
If no `metadata.brikko-studio` is present, the skill is always eligible (unless
disabled in config or blocked by `skills.allowBundled` for bundled skills).

<Note>
Legacy `metadata.clawdbot` blocks are still accepted when
`metadata.brikko-studio` is absent, so older installed skills keep their
dependency gates and installer hints. New and updated skills should use
`metadata.brikko-studio`.
</Note>
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, Brikko Studio lists each entry so you can see the available artifacts.
    - Installer specs can include `os: ["darwin"|"linux"|"win32"]` to filter options by platform.
    - Node installs honor `skills.install.nodeManager` in `brikko-studio.json` (default: npm; options: npm/pnpm/yarn/bun). This only affects skill installs; the Gateway runtime should still be Node — Bun is not recommended for WhatsApp/Telegram.
    - Gateway-backed installer selection is preference-driven: when install specs mix kinds, Brikko Studio prefers Homebrew when `skills.install.preferBrew` is enabled and `brew` exists, then `uv`, then the configured node manager, then other fallbacks like `go` or `download`.
    - If every install spec is `download`, Brikko Studio surfaces all download options instead of collapsing to one preferred installer.

  </Accordion>
  <Accordion title="Per-installer details">
    - **Go installs:** if `go` is missing and `brew` is available, the gateway installs Go via Homebrew first and sets `GOBIN` to Homebrew's `bin` when possible.
    - **Download installs:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`), `extract` (default: auto when archive detected), `stripComponents`, `targetDir` (default: `~/.brikko-studio/tools/<skillKey>`).
...
under `skills.entries` in `~/.brikko-studio/brikko-studio.json`:
...
<ParamField path="apiKey" type='string | { source, provider, id }'>
  Convenience for skills that declare `metadata.brikko-studio.primaryEnv`. Supports plaintext or SecretRef.
</ParamField>
...
defines `metadata.brikko-studio.skillKey`, use that key under `skills.entries`.

<Note>
For stock image generation/editing inside Brikko Studio, use the core
...
When an agent run starts, Brikko Studio:
...
For the bundled `claude-cli` backend, Brikko Studio also materializes the same
...
Brikko Studio still owns precedence, per-agent allowlists, gating, and
`skills.entries.*` env/API key injection. Other CLI backends use the
prompt catalog only.

## Snapshots and refresh

Brikko Studio snapshots the eligible skills **when a session starts** and
...
session, Brikko Studio refreshes the snapshot so visible skills stay aligned
with the current agent.

### Skills watcher

By default, Brikko Studio watches skill folders and bumps the skills snapshot
...
Brikko Studio can treat macOS-only skills as eligible when the required
...
probes, Brikko Studio clears its cached bin matches so agents no longer see
skills that cannot currently run there.

## Token impact

When skills are eligible, Brikko Studio injects a compact XML list of available
...
Brikko Studio ships a baseline set of skills as **bundled skills** with the
install (npm package or Brikko Studio.app). `~/.brikko-studio/skills` exists for
