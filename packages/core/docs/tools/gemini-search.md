...
Brikko Studio supports Gemini models with built-in
...
For a gateway install, put env keys in `~/.brikko-studio/.env`.
...
`pd`, `pw`, `pm`, and `py`. Brikko Studio converts these values, or an explicit
