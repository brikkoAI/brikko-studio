...
Brikko Studio supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Brikko Studio also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `brikko-studio configure --section web`. It stores the key in
`~/.brikko-studio/brikko-studio.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.brikko-studio/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
