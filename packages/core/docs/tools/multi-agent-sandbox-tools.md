...
<Note>
Legacy `agent.*` configs are migrated by `brikko-studio doctor`; prefer `agents.defaults` + `agents.list` going forward.
</Note>
...
<Steps>
  ...
  <Step title="Verify sandbox containers">
    ```bash
    docker ps --filter "name=brikko-studio-sbx-"
    ```
  </Step>
  ...
  <Step title="Monitor logs">
    ```bash
    tail -f "${BRIKKO_STUDIO_STATE_DIR:-$HOME/.brikko-studio}/logs/gateway.log" | grep -E "routing|sandbox|tools"
    ```
  </Step>
</Steps>
