...
read_when:
  - Running Brikko Studio Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, Brikko Studio Gateway runs inside WSL2, Chrome runs on Windows, and browser control must cross the WSL2 and Windows boundary. The layered failure pattern from [issue #39369](https://github.com/brikko-studio/brikko-studio/issues/39369) means several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Brikko Studio and Chrome are on the same machine
...
- Brikko Studio points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Brikko Studio is not the problem yet.
...
Fix that before touching Brikko Studio config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Brikko Studio at the address that is reachable from WSL2:
...
- use HTTP(S) when you want Brikko Studio to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting Brikko Studio to succeed
...
```bash
brikko-studio browser open https://example.com --browser-profile remote
brikko-studio browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `brikko-studio browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `brikko-studio browser stop --browser-profile remote`
...
3. Brikko Studio config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Brikko Studio config or Control UI auth
