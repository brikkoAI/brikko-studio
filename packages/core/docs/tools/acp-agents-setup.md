...
read_when:
  ...
  - Enabling the plugin-tools or Brikko Studio-tools MCP bridge
...
Codex has two Brikko Studio routes:
...
When Brikko Studio uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Brikko Studio `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by Brikko Studio before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, Brikko Studio/acpx cannot force a model selection.
...
Packaged installs use the official `@brikko-studio/acpx` runtime plugin for ACP.
...
```bash
brikko-studio plugins install ./path/to/local/acpx-plugin
```
...
live probe. Set `BRIKKO_STUDIO_ACPX_RUNTIME_STARTUP_PROBE=1` only when you need the
...
- `command` accepts an absolute path, relative path (resolved from the Brikko Studio workspace), or command name.
...
When you install Brikko Studio globally with `npm install -g brikko-studio`, the acpx
...
normally and reports the missing dependency through `brikko-studio acp doctor`.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose Brikko Studio plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
Brikko Studio plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
brikko-studio config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `brikko-studio-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled Brikko Studio
  plugins.
...
### Brikko Studio tools MCP bridge

By default, ACPX sessions also do **not** expose built-in Brikko Studio tools through
MCP. Enable the separate core-tools bridge when an ACP agent needs selected
built-in tools such as `cron`:

```bash
brikko-studio config set plugins.entries.acpx.config.openClawToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `brikko-studio-tools` into ACPX session
  bootstrap.
- Exposes selected built-in Brikko Studio tools. The initial server exposes `cron`.
...
```bash
brikko-studio config set plugins.entries.acpx.config.timeoutSeconds 180
```
...
```bash
brikko-studio config set plugins.entries.acpx.config.probeAgent claude
```
...
These ACPX harness permissions are separate from Brikko Studio exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.
...
```bash
brikko-studio config set plugins.entries.acpx.config.permissionMode approve-all
brikko-studio config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

<Warning>
Brikko Studio defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
