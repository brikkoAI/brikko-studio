---
summary: "Export redacted trajectory bundles for debugging an Brikko Studio agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an Brikko Studio session
...
Trajectory capture is Brikko Studio's per-session flight recorder. It records a
...
Brikko Studio writes the bundle under the workspace:

```text
.brikko-studio/trajectory-exports/brikko-studio-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.brikko-studio/trajectory-exports/`. Absolute
...
create the bundle; do not use allow-all. In group chats, Brikko Studio sends the
...
Trajectory capture is on by default for Brikko Studio agent runs.
...
```json
{
  "traceSchema": "brikko-studio-trajectory",
  "schemaVersion": 1
}
```
...
| `metadata.json`       | Brikko Studio version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
Brikko Studio also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `BRIKKO_STUDIO_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export BRIKKO_STUDIO_TRAJECTORY_DIR=/var/lib/brikko-studio/trajectories
```

When this variable is set, Brikko Studio writes one JSONL file per session id in that
directory.
...
Set `BRIKKO_STUDIO_TRAJECTORY=0` before starting Brikko Studio:

```bash
export BRIKKO_STUDIO_TRAJECTORY=0
```
...
Brikko Studio redacts sensitive values before writing export files:
...
- confirm Brikko Studio was started without `BRIKKO_STUDIO_TRAJECTORY=0`
- check whether `BRIKKO_STUDIO_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.brikko-studio/trajectory-exports/`
