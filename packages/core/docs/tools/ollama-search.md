...
Brikko Studio supports **Ollama Web Search** as a bundled `web_search` provider. It
...
- an Ollama host that is reachable from Brikko Studio
- `ollama signin`
...
If no explicit Ollama base URL is set, Brikko Studio uses `http://127.0.0.1:11434`.

If your Ollama host expects bearer auth, Brikko Studio reuses
...
- If the Ollama host is auth-protected, Brikko Studio reuses the normal Ollama
  provider API key when present.
- If `baseUrl` is `https://ollama.com`, Brikko Studio calls
...
- If the configured host does not expose web search and `OLLAMA_API_KEY` is set,
  Brikko Studio can fall back to `https://ollama.com/api/web_search` without sending
  that env key to the local host.
- Brikko Studio warns during setup if Ollama is unreachable or not signed in, but
