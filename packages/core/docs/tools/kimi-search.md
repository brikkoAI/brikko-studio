...
Brikko Studio supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), Brikko Studio reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.brikko-studio/.env`.

If you omit `baseUrl`, Brikko Studio defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, Brikko Studio defaults to `kimi-k2.6`.
...
Brikko Studio treats Kimi `web_search` as successful only after Moonshot returns
...
Brikko Studio returns a structured `kimi_web_search_ungrounded` error instead of
