...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.brikko-studio/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Brikko Studio picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    brikko-studio agent --message "give me a greeting"
    ```
...
| `metadata.brikko-studio.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.)                    |
| `metadata.brikko-studio.requires.bins`   | No       | Required binaries on PATH                                      |
| `metadata.brikko-studio.requires.config` | No       | Required config keys                                           |
...
- **Test locally** — use `brikko-studio agent --message "..."` to test before sharing
...
| `~/.brikko-studio/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Brikko Studio) | Low        | Global                |
