...
`~/.brikko-studio/brikko-studio.json`. Agent-specific skill visibility lives under
...
- Built-in skill roots always include `~/.brikko-studio/skills`, `~/.agents/skills`,
...
- `install.nodeManager`: node installer preference (`npm` | `pnpm` | `yarn` | `bun`, default: npm).
  ...
  - `brikko-studio setup --node-manager` is narrower and currently accepts `npm`,
...
- Keys under `entries` map to the skill name by default. If a skill defines
  `metadata.brikko-studio.skillKey`, use that key instead.
- Load precedence is `<workspace>/skills` → `<workspace>/.agents/skills` →
  `~/.agents/skills` → `~/.brikko-studio/skills` → bundled skills →
