...
read_when:
  - You are building an Brikko Studio plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page is for the **native Brikko Studio plugin manifest** only.
...
Brikko Studio auto-detects those bundle layouts too, but they are not validated
against the `brikko-studio.plugin.json` schema described here.

For compatible bundles, Brikko Studio currently reads bundle metadata plus declared
...
Brikko Studio runtime expectations.

Every native Brikko Studio plugin **must** ship a `brikko-studio.plugin.json` file in the
**plugin root**. Brikko Studio uses this manifest to validate configuration
...
`brikko-studio.plugin.json` is the metadata Brikko Studio reads **before it loads your
...
- QA runner metadata the shared `brikko-studio qa` host can inspect
...
| `providerAuthEnvVars`                | No       | `Record<string, string[]>`       | Deprecated compatibility env metadata for provider auth/status lookup. Prefer `setup.providers[].envVars` for new plugins; Brikko Studio still reads this during the deprecation window.                                                 |
| `providerAuthAliases`                | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                                          |
| `channelEnvVars`                     | No       | `Record<string, string[]>`       | Cheap channel env metadata that Brikko Studio can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                                            |
...
| `qaRunners`                          | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `brikko-studio qa` host before plugin runtime loads.                                                                                                                                      |
...
Brikko Studio reads these fields before provider runtime loads so core tools can
...
ownership. `toolMetadata` declares cheap availability evidence so Brikko Studio can
...
If a tool has no `toolMetadata`, Brikko Studio preserves the existing behavior and
...
Brikko Studio reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                        | User-facing label. If omitted, Brikko Studio falls back to `choiceId`.                                        |
...
mistakenly put in `plugins.allow` or try to run as a root CLI command. Brikko Studio
...
the shared `brikko-studio qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `brikko-studio qa`, for example `matrix`.    |
...
Brikko Studio also includes `setup.providers[].envVars` in generic provider auth and
...
Brikko Studio can also derive simple setup choices from `setup.providers[].authMethods`
...
setup surface. Brikko Studio treats explicit `false` as a descriptor-only contract
and will not execute `setup-api` or `brikko-studio.setupEntry` for setup lookup. If
a descriptor-only plugin still ships one of those setup runtime entries,
Brikko Studio reports an additive diagnostic and continues ignoring it. Omitted
...
Use `contracts` only for static capability ownership metadata that Brikko Studio can
...
| `migrationProviders`             | `string[]` | Import provider ids this plugin owns for `brikko-studio migrate`.          |
...
Brikko Studio reads manifest metadata to decide which plugin owns that configured
...
`channelConfigs` entries. Without them, Brikko Studio can still load the plugin, but
...
the same defaults through `package.json#brikko-studio.channel.commands` alongside
...
When `channels.chat` is configured, Brikko Studio considers both the channel id and
the preferred plugin id. If the lower-priority plugin was only selected because
it is bundled or enabled by default, Brikko Studio disables it in the effective
runtime config so one plugin owns the channel and its tools. Explicit user
selection still wins: if the user explicitly enables both plugins, Brikko Studio
...
Use `modelSupport` when Brikko Studio should infer your provider plugin from
...
Brikko Studio applies this precedence:
...
Use `modelCatalog` when Brikko Studio should know provider model metadata before
...
provider-filtered list uses an alias, Brikko Studio can read the owning manifest and
...
| `compat`        | `object`                                                       | Optional compatibility flags matching Brikko Studio model config compatibility.  |
...
refreshable rows are not authoritative by themselves. Use `runtime` when Brikko Studio
...
| Field                      | Type               | What it means                                                                                                        |
| -------------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `provider`                 | `string`           | External catalog provider id when it differs from the Brikko Studio provider id, for example `z-ai` for a `zai` provider. |
...
### Brikko Studio Provider Index

The Brikko Studio Provider Index is Brikko Studio-owned preview metadata for providers
...
4. Brikko Studio Provider Index preview rows.
...
Legacy top-level capability keys are deprecated. Use `brikko-studio doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `brikko-studio.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `brikko-studio` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Brikko Studio must know it before loading plugin code, put it in `brikko-studio.plugin.json`
...
`brikko-studio` block instead of `brikko-studio.plugin.json`.
`brikko-studio.bundle` and `brikko-studio.bundle.json` are not Brikko Studio plugin contracts;
native plugins must use `brikko-studio.plugin.json` plus the supported
`package.json#brikko-studio` fields below.

Important examples:

| Field                                                                                      | What it means                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `brikko-studio.extensions`                                                                      | Declares native plugin entrypoints. Must stay inside the plugin package directory.                                                                                                   |
| `brikko-studio.runtimeExtensions`                                                               | Declares built JavaScript runtime entrypoints for installed packages. Must stay inside the plugin package directory.                                                                 |
| `brikko-studio.setupEntry`                                                                      | Lightweight setup-only entrypoint used during onboarding, deferred channel startup, and read-only channel status/SecretRef discovery. Must stay inside the plugin package directory. |
| `brikko-studio.runtimeSetupEntry`                                                               | Declares the built JavaScript setup entrypoint for installed packages. Requires `setupEntry`, must exist, and must stay inside the plugin package directory.                         |
| `brikko-studio.channel`                                                                         | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                                                                 |
| `brikko-studio.channel.commands`                                                                | Static native command and native skill auto-default metadata used by config, audit, and command-list surfaces before channel runtime loads.                                          |
| `brikko-studio.channel.configuredState`                                                         | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime.                                         |
| `brikko-studio.channel.persistedAuthState`                                                      | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.                                               |
| `brikko-studio.install.clawhubSpec` / `brikko-studio.install.npmSpec` / `brikko-studio.install.localPath` | Install/update hints for bundled and externally published plugins.                                                                                                                   |
| `brikko-studio.install.defaultChoice`                                                           | Preferred install path when multiple install sources are available.                                                                                                                  |
| `brikko-studio.install.minHostVersion`                                                          | Minimum supported Brikko Studio host version, using a semver floor like `>=2026.3.22` or `>=2026.5.1-beta.1`.                                                                             |
| `brikko-studio.install.expectedIntegrity`                                                       | Expected npm dist integrity string such as `sha512-...`; install and update flows verify the fetched artifact against it.                                                            |
| `brikko-studio.install.allowInvalidConfigRecovery`                                              | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                                                                       |
| `brikko-studio.startup.deferConfiguredChannelFullLoadUntilAfterListen`                          | Lets setup-only channel surfaces load before the full channel plugin during startup.                                                                                                 |

Manifest metadata decides which provider/channel/setup choices appear in
onboarding before runtime loads. `package.json#brikko-studio.install` tells
onboarding how to fetch or enable that plugin when the user picks one of those
choices. Do not move install hints into `brikko-studio.plugin.json`.

`brikko-studio.install.minHostVersion` is enforced during install and manifest
...
`"npmSpec": "@wecom/wecom-brikko-studio-plugin@1.2.3"`. Official external catalog
...
Channel plugins should provide `brikko-studio.setupEntry` when status, channel list,
...
entrypoint fields. For example, `brikko-studio.runtimeExtensions` cannot make an
escaping `brikko-studio.extensions` path loadable.

`brikko-studio.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
to `brikko-studio doctor --fix`.

`brikko-studio.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`brikko-studio.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
Brikko Studio discovers plugins from several roots (bundled, global install, workspace, explicit config-selected paths). If two discoveries share the same `id`, only the **highest-precedence** manifest is kept; lower-precedence duplicates are dropped instead of loading beside it.

Precedence, highest to lowest:

1. **Config-selected** — a path explicitly pinned in `plugins.entries.<id>`
2. **Bundled** — plugins shipped with Brikko Studio
3. **Global install** — plugins installed into the global Brikko Studio plugin root
...
- When extending or forking a bundled plugin with new config keys, update that plugin's `brikko-studio.plugin.json` `configSchema` at the same time. Bundled plugin schemas are strict, so adding `plugins.entries.<id>.config.myNewKey` in user config without adding `myNewKey` to `configSchema.properties` will be rejected before the plugin runtime loads.
...
- The manifest is **required for native Brikko Studio plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
...
- Declare exclusive plugin kind in this manifest. Runtime-entry `Brikko StudioPluginDefinition.kind` is deprecated and remains only as a compatibility fallback for older plugins.
