---
summary: "Install and use Codex, Claude, and Cursor bundles as Brikko Studio plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Brikko Studio maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

Brikko Studio can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
Brikko Studio maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Brikko Studio plugins. Native plugins run
...
of requiring authors to rewrite them as native Brikko Studio plugins, Brikko Studio
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    brikko-studio plugins install ./my-bundle

    # Archive
    brikko-studio plugins install ./my-bundle.tgz
...
## What Brikko Studio maps from bundles

Not every bundle feature runs in Brikko Studio today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal Brikko Studio skills                                           | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                  | Claude, Cursor |
| Hook packs    | Brikko Studio-style `HOOK.md` + `handler.ts` layouts                                             | Codex          |
...
- bundle skill roots load as normal Brikko Studio skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal Brikko Studio skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal Brikko Studio hook-pack
...
- Brikko Studio merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- Brikko Studio exposes supported bundle MCP tools during embedded Pi agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, Brikko Studio uses `sse`
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in Brikko Studio config. `brikko-studio mcp set` and `brikko-studio doctor --fix` normalize the common alias.
...
Brikko Studio registers bundle MCP tools with provider-safe names in the form
...
- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- Brikko Studio sanitizes shell override keys before applying them
...
- Brikko Studio loads `.lsp.json` plus any manifest-declared `lspServers` paths
...
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `brikko-studio plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but Brikko Studio does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Brikko Studio best when they use skill roots and Brikko Studio-style
...
Brikko Studio checks for native plugin format first:

1. `brikko-studio.plugin.json` or valid `package.json` with `brikko-studio.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, Brikko Studio uses the native path. This prevents
...
- Third-party compatible bundles do not get startup `npm install` repair. They
  should be installed through `brikko-studio plugins install` and ship everything
  they need in the installed plugin directory.
- Brikko Studio-owned bundled plugins are either shipped lightweight in core or
  downloadable through the plugin installer. Gateway startup never runs a
  package manager for them.
- `brikko-studio doctor --fix` removes legacy staged dependency directories and can
...
- Brikko Studio does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `brikko-studio plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. Brikko Studio does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Brikko Studio hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
