...
Set `dimensions` for non-standard embedding models. Brikko Studio knows the
...
`memory_forget` query path, and `brikko-studio ltm search`. Auto-recall prefers the
...
```bash
brikko-studio ltm list
brikko-studio ltm search "project preferences"
brikko-studio ltm stats
```

The plugin also extends `brikko-studio memory` with a non-vector `query` subcommand
that runs against the LanceDB table directly:

```bash
brikko-studio memory query --cols id,text,createdAt --limit 20
brikko-studio memory query --filter "category = 'preference'" --order-by createdAt:desc
```
...
By default, LanceDB data lives under `~/.brikko-studio/memory/lancedb`. Override the
path with `dbPath`:
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "~/.brikko-studio/memory/lancedb",
          embedding: {
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "s3://memory-bucket/brikko-studio",
          storageOptions: {
...
Brikko Studio treats that package as part of the plugin package. Gateway startup
...
`@lancedb/lancedb` error during plugin load, upgrade Brikko Studio and restart the
Gateway.
...
```bash
brikko-studio ltm stats
brikko-studio ltm search "recent preference"
```
