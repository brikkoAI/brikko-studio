---
summary: "How Brikko Studio installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged Brikko Studio installs or bundled plugin manifests
...
Brikko Studio keeps plugin dependency work at install/update time. Runtime loading
does not run package managers, repair dependency trees, or mutate the Brikko Studio
package directory.
...
- SDK/core imports are peer or supplied Brikko Studio imports
- local development plugins bring their own already-installed dependencies
- npm and git plugins are installed into Brikko Studio-owned package roots

Brikko Studio owns only the plugin lifecycle:
...
Brikko Studio uses stable per-source roots:

- npm packages install under `~/.brikko-studio/npm`
- git packages clone under `~/.brikko-studio/git`
- local/path/archive installs are copied or referenced without dependency repair

npm installs run in the npm root with:

```bash
npm install --prefix ~/.brikko-studio/npm <spec> --omit=dev --ignore-scripts --no-audit --no-fund
```

npm may hoist transitive dependencies to `~/.brikko-studio/npm/node_modules` beside
the plugin package. Brikko Studio scans the managed npm root before trusting the
...
Local plugins are treated as developer-controlled directories. Brikko Studio does not
...
brikko-studio plugins install <source>
brikko-studio doctor --fix
```

`doctor --fix` can clean legacy Brikko Studio-generated dependency state and install
...
Lightweight and core-critical bundled plugins are shipped as part of Brikko Studio.
...
In source checkouts, Brikko Studio treats the repository as a pnpm monorepo. After
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g brikko-studio`        | Built runtime tree inside the package | Brikko Studio package and explicit plugin install/update/doctor flows     |
...
| `brikko-studio plugins install ...`   | Managed npm/git/ClawHub plugin root   | The plugin install/update flow                                       |

## Legacy cleanup

Older Brikko Studio versions generated bundled-plugin dependency roots at startup or
...
`.brikko-studio-runtime-deps*` manifests, generated plugin `node_modules`, install
