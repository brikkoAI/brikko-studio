---
summary: "Set up Codex Computer Use for Codex-mode Brikko Studio agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode Brikko Studio agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. Brikko Studio
...
Use this page when Brikko Studio is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## Brikko Studio.app and Peekaboo

Brikko Studio.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want Brikko Studio.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode Brikko Studio agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an Brikko Studio node and exposes mobile
...
Brikko Studio-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through Brikko Studio's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the Brikko Studio command:

```bash
cua-driver mcp-config --client brikko-studio
```

or register the stdio server yourself:

```bash
brikko-studio mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
```
...
available as a normal Brikko Studio MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. Brikko Studio
...
With this config, Brikko Studio checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, Brikko Studio asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, Brikko Studio also tries to
...
plugin command surface is available. These are Brikko Studio chat/runtime commands,
not `brikko-studio codex ...` CLI subcommands:
...
Brikko Studio uses the same app-server API that Codex itself exposes. The
...
During install, Brikko Studio polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, Brikko Studio prefers
...
`computer-use` is registered, Brikko Studio tries to add the standard bundled
...
## What Brikko Studio checks

Brikko Studio reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If Brikko Studio says Computer Use
...
Brikko Studio intentionally fails closed when `computerUse.enabled` is true. A
...
fresh Brikko Studio session.

**A Computer Use tool says `Native hook relay unavailable`.** The Codex-native
tool hook could not reach an active Brikko Studio relay through the local bridge or
Gateway fallback. Start a fresh Brikko Studio session with `/new` or `/reset`. If it
