...
Plugin hooks are in-process extension points for Brikko Studio plugins. Use them
...
current plugin options; Brikko Studio injects it per handler without mutating the
...
- Brikko Studio strips `toolResult.details` before provider replay and compaction
...
`before_agent_start` and `agent_end` include `event.runId` when Brikko Studio can
...
identifier when Brikko Studio can derive one from the session key or delivery
metadata.
...
Brikko Studio continues; it does not cancel plugin-owned network work unless the
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when Brikko Studio can derive a
...
Codex native `Stop` hooks are relayed into this hook as Brikko Studio
...
reach the next model turn exactly once. Brikko Studio drains queued injections before
...
context when syncing external wake schedulers, and keep Brikko Studio as the
