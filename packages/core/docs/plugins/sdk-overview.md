...
read_when:
  ...
  - You want a reference for all registration methods on Brikko StudioPluginApi
...
<Note>
  This page is for plugin authors using `brikko-studio/plugin-sdk/*` inside
  Brikko Studio. For external apps, scripts, dashboards, CI jobs, and IDE extensions
  that want to run agents through the Gateway, use the
  [Brikko Studio App SDK](/concepts/brikko-studio-sdk) and the `@brikko-studio/sdk` package
  instead.
</Note>
...
prefer `brikko-studio/plugin-sdk/channel-core`; keep `brikko-studio/plugin-sdk/core` for
...
`brikko-studio.plugin.json#channelConfigs`. The `plugin-sdk/channel-config-schema`
subpath is for shared schema primitives and the generic builder. Brikko Studio's
...
<Warning>
  ...
  `brikko-studio/plugin-sdk/slack`, `.../discord`, `.../signal`, `.../whatsapp`).
...
`brikko-studio/plugin-sdk/discord` and `brikko-studio/plugin-sdk/telegram-account` are
...
The `register(api)` callback receives an `Brikko StudioPluginApi` object with these
methods:
...
cannot register this middleware; keep normal Brikko Studio plugin hooks for work
...
Gateway on a local discovery transport such as mDNS/Bonjour. Brikko Studio calls the
...
- User config still wins. Brikko Studio merges `agents.defaults.cliBackends.<id>` over the
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `brikko-studio/plugin-sdk/memory-host-core` instead of reaching into a specific
...
- `cron_changed`: observe gateway-owned cron lifecycle changes. Use `event.job?.state?.nextRunAtMs` and `ctx.getCron?.()` when syncing external wake schedulers, and keep Brikko Studio as the source of truth for due checks and execution.
...
| `api.config`             | `Brikko StudioConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `brikko-studio/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when Brikko Studio is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through Brikko Studio's plugin
...
- **`@brikko-studio/openai-provider`**: `api.ts` exports provider builders,
  default-model helpers, and realtime provider builders.
- **`@brikko-studio/openrouter-provider`**: `api.ts` exports the provider builder
  plus onboarding/config helpers.

<Warning>
  Extension production code should also avoid `brikko-studio/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `brikko-studio/plugin-sdk/speech`, `.../provider-model-shared`, or another
