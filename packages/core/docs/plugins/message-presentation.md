...
Message presentation is Brikko Studio's shared contract for rich outbound chat UI.
...
```bash
brikko-studio message send --channel slack \
  --target channel:C123 \
...
```bash
brikko-studio message send --channel telegram \
...
Use helpers from `brikko-studio/plugin-sdk/interactive-runtime` when bridging older
code:
