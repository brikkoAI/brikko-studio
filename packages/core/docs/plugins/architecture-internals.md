...
At startup, Brikko Studio does roughly this:
...
The manifest is the control-plane source of truth. Brikko Studio uses it to:
...
channels, slots, and auto-enable rules. If that derived set is empty, Brikko Studio
...
Brikko Studio does not cache plugin discovery results or direct manifest registry
...
Brikko Studio still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Brikko Studio calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that Brikko Studio no longer calls, such as
...
| --  | _(built-in model lookup)_         | Brikko Studio tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
that still runs on Brikko Studio's normal inference loop.
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of Brikko Studio's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Brikko Studio adds those
...
- Brikko Studio only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Brikko Studio plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-brikko-studio-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-brikko-studio-scopes` only when the header is explicitly present
  - if `x-brikko-studio-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-brikko-studio-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `brikko-studio/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                             | Purpose                                            |
| ----------------------------------- | -------------------------------------------------- |
| `brikko-studio/plugin-sdk/plugin-entry`  | Plugin registration primitives                     |
| `brikko-studio/plugin-sdk/channel-core`  | Channel entry/build helpers                        |
| `brikko-studio/plugin-sdk/core`          | Generic shared helpers and umbrella contract       |
| `brikko-studio/plugin-sdk/config-schema` | Root `brikko-studio.json` Zod schema (`Brikko StudioSchema`) |
...
<Info>
`brikko-studio/plugin-sdk/channel-runtime`, `brikko-studio/plugin-sdk/config-runtime`,
and `brikko-studio/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `brikko-studio/plugin-sdk/*` subpaths. Never
...
`brikko-studio/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Brikko Studio writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Brikko Studio's
...
- if both `catalog` and `discovery` are registered, Brikko Studio uses `catalog`
...
- Read-only command paths such as `brikko-studio status`, `brikko-studio status --all`,
  `brikko-studio channels status`, `brikko-studio channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `brikko-studio.extensions`:
...
Security guardrail: every `brikko-studio.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `brikko-studio plugins install` installs plugin dependencies with a
...
Optional: `brikko-studio.setupEntry` can point at a lightweight setup-only module.
When Brikko Studio needs setup surfaces for a disabled channel plugin, or
...
Optional: `brikko-studio.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Brikko Studio load the
...
Channel plugins can advertise setup/discovery metadata via `brikko-studio.channel` and
install hints via `brikko-studio.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@brikko-studio/nextcloud-talk",
  "brikko-studio": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@brikko-studio/nextcloud-talk",
...
Useful `brikko-studio.channel` fields beyond the minimal example:
...
Brikko Studio can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.brikko-studio/mpm/plugins.json`
- `~/.brikko-studio/mpm/catalog.json`
- `~/.brikko-studio/plugins/catalog.json`

Or point `BRIKKO_STUDIO_PLUGIN_CATALOG_PATHS` (or `BRIKKO_STUDIO_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "brikko-studio": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `brikko-studio.install` block. The
...
```ts
import { buildMemorySystemPromptAddition } from "brikko-studio/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `Brikko StudioPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Brikko Studio stays opinionated without becoming hardcoded to one
