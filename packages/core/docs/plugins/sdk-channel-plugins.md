---
summary: "Step-by-step guide to building a messaging channel plugin for Brikko Studio"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Brikko Studio to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects Brikko Studio to a
...
<Info>
  If you have not built any Brikko Studio plugin before, read
...
Channel plugins do not need their own send/edit/react tools. Brikko Studio keeps one
...
Use `brikko-studio/plugin-sdk/channel-route` when plugin code needs to normalize
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `brikko-studio/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization plus transport/presentation facts. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, and `createApproverRestrictedNativeApprovalCapability` from `brikko-studio/plugin-sdk/approval-runtime`. Put the channel-specific facts behind `approvalCapability.nativeRuntime`, ideally via `createChannelApprovalNativeRuntimeAdapter(...)` or `createLazyChannelApprovalNativeRuntimeAdapter(...)`, so core can assemble the handler and own request filtering, routing, dedupe, expiry, gateway subscription, and routed-elsewhere notices. `nativeRuntime` is split into a few smaller seams:
...
- `brikko-studio/plugin-sdk/approval-auth-runtime`
- `brikko-studio/plugin-sdk/approval-client-runtime`
- `brikko-studio/plugin-sdk/approval-delivery-runtime`
- `brikko-studio/plugin-sdk/approval-gateway-runtime`
- `brikko-studio/plugin-sdk/approval-handler-adapter-runtime`
- `brikko-studio/plugin-sdk/approval-handler-runtime`
- `brikko-studio/plugin-sdk/approval-native-runtime`
- `brikko-studio/plugin-sdk/approval-reply-runtime`
- `brikko-studio/plugin-sdk/channel-runtime-context`

Likewise, prefer `brikko-studio/plugin-sdk/setup-runtime`,
`brikko-studio/plugin-sdk/setup-adapter-runtime`,
`brikko-studio/plugin-sdk/reply-runtime`,
`brikko-studio/plugin-sdk/reply-dispatch-runtime`,
`brikko-studio/plugin-sdk/reply-reference`, and
`brikko-studio/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `brikko-studio/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `brikko-studio/plugin-sdk/setup-adapter-runtime` is the narrow env-aware adapter
  seam for `createEnvPatchedAccountSetupAdapter`
- `brikko-studio/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `brikko-studio.setupEntry` in
...
- use the broader `brikko-studio/plugin-sdk/setup` seam only when you also need the
...
- `brikko-studio/plugin-sdk/account-core`,
  `brikko-studio/plugin-sdk/account-id`,
  `brikko-studio/plugin-sdk/account-resolution`, and
  `brikko-studio/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `brikko-studio/plugin-sdk/inbound-envelope` and
  `brikko-studio/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `brikko-studio/plugin-sdk/messaging-targets` for target parsing/matching
- `brikko-studio/plugin-sdk/outbound-media` and
  `brikko-studio/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `brikko-studio/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `brikko-studio/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `brikko-studio/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `brikko-studio/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `brikko-studio/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `brikko-studio/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
`brikko-studio/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.

The older `resolveMentionGating*` helpers remain on
`brikko-studio/plugin-sdk/channel-inbound` as compatibility exports only. New code
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#brikko-studio-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/brikko-studio-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "brikko-studio": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Brikko Studio to Acme Chat."
        }
      }
    }
    ```

    ```json brikko-studio.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { Brikko StudioConfig } from "brikko-studio/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "brikko-studio/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Brikko Studio
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "brikko-studio/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    Brikko Studio loads this instead of the full entry when the channel is disabled
    ...
    `brikko-studio/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    Brikko Studio. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Brikko Studio.
...
├── package.json              # brikko-studio.channel metadata
├── brikko-studio.plugin.json      # Manifest with config schema
