---
summary: "Frequently asked questions about Brikko Studio setup, configuration, and usage"
read_when:
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/brikko-studio/brikko-studio-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   brikko-studio health --json
   brikko-studio health --verbose   # shows the target URL + config path on errors
   ```
...
<AccordionGroup>
  ...
  <Accordion title="What are the top five everyday use cases for Brikko Studio?">
  ...
  <Accordion title="Can Brikko Studio help with lead gen, outreach, ads, and blogs for a SaaS?">
    Yes for **research, qualification, and drafting**. It can scan sites, build shortlists,
    summarize prospects, and write outreach or ad copy drafts.

    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and
    platform policies, and review anything before it is sent. The safest pattern is to let
    Brikko Studio draft and you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    Brikko Studio is a **personal assistant** and coordination layer, not an IDE replacement. Use
    Claude Code or Codex for the fastest direct coding loop inside a repo. Use Brikko Studio when you
    ...
    Showcase: [https://brikko-studio.ai/showcase](https://brikko-studio.ai/showcase)
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put your changes in `~/.brikko-studio/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.brikko-studio/brikko-studio.json`). Precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.brikko-studio/skills` → bundled → `skills.load.extraDirs`, so managed overrides still win over bundled skills without touching git. If you need the skill installed globally but only visible to some agents, keep the shared copy in `~/.brikko-studio/skills` and control visibility with `agents.defaults.skills` and `agents.list[].skills`. Only upstream-worthy edits should live in the repo and go out as PRs.
  </Accordion>

  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.brikko-studio/brikko-studio.json` (lowest precedence). Default precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.brikko-studio/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Brikko Studio treats as `<workspace>/skills` on the next session. If the skill should only be visible to certain agents, pair that with `agents.defaults.skills` or `agents.list[].skills`.
  </Accordion>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, Brikko Studio falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is the exact silent token `NO_REPLY` / `no_reply`, or exactly `ANNOUNCE_SKIP`, Brikko Studio intentionally suppresses the announce instead of posting stale earlier progress.
    ...
    ```bash
    brikko-studio tasks show <runId-or-sessionKey>
    ```
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    - Confirm cron is enabled (`cron.enabled`) and `BRIKKO_STUDIO_SKIP_CRON` is not set.
    ...
    ```bash
    brikko-studio cron run <jobId>
    brikko-studio cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `brikko-studio skills` commands or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
    Browse skills at [https://clawhub.ai](https://clawhub.ai).

    ```bash
    brikko-studio skills search "calendar"
    brikko-studio skills search --limit 20
    brikko-studio skills install <skill-slug>
    brikko-studio skills install <skill-slug> --version <version>
    brikko-studio skills install <skill-slug> --force
    brikko-studio skills update --all
    brikko-studio skills list --eligible
    brikko-studio skills check
    ```

    Native `brikko-studio skills install` writes into the active workspace `skills/`
    ...
    `~/.brikko-studio/skills` and use `agents.defaults.skills` or
    `agents.list[].skills` if you want to narrow which agents can see it.

  </Accordion>

  <Accordion title="Can Brikko Studio run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.brikko-studio.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option B - use a macOS node (no SSH).**
    Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. Brikko Studio can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
    ...
    **Option C - proxy macOS binaries over SSH (advanced).**
    ...
    3. Override the skill metadata (workspace or `~/.brikko-studio/skills`) to allow Linux:

       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "brikko-studio": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.brikko-studio/skills/<name>/SKILL.md`. If only some agents should see a shared install, configure `agents.defaults.skills` or `agents.list[].skills`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills), [Skills config](/tools/skills-config), and [ClawHub](/tools/clawhub).

  </Accordion>

  <Accordion title="How do I use my existing signed-in Chrome with Brikko Studio?">
    ...
    brikko-studio browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Docker feels limited - how do I enable full features?">
    ...
    - Persist `/home/node` with `BRIKKO_STUDIO_HOME_VOLUME` so caches survive.
    - Bake system deps into the image with `BRIKKO_STUDIO_DOCKER_APT_PACKAGES`.
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    Brikko Studio validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor. That means symlink-parent escapes still fail closed even when the last path segment does not exist yet, and allowed-root checks still apply after symlink resolution.
  ...
  <Accordion title="How does memory work?">
    Brikko Studio memory is just Markdown files in the agent workspace:

    - Daily notes in `memory/YYYY-MM-DD.md`
    - Curated long-term notes in `MEMORY.md` (main/private sessions only)

    Brikko Studio also runs a **silent pre-compaction memory flush** to remind the model
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    If you don't set a provider explicitly, Brikko Studio auto-selects a provider when it
    ...
    configured and present, Brikko Studio
...
<AccordionGroup>
  <Accordion title="Is all data used with Brikko Studio saved locally?">
    No - **Brikko Studio's state is local**, but **external services still see what you send them**.

    - **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
      (`~/.brikko-studio` + your workspace directory).
  ...
  <Accordion title="Where does Brikko Studio store its data?">
    Everything lives under `$BRIKKO_STUDIO_STATE_DIR` (default: `~/.brikko-studio`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$BRIKKO_STUDIO_STATE_DIR/brikko-studio.json`                             | Main config (JSON5)                                                |
    | `$BRIKKO_STUDIO_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
    | `$BRIKKO_STUDIO_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
    | `$BRIKKO_STUDIO_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
    | `$BRIKKO_STUDIO_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
    | `$BRIKKO_STUDIO_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)            |
    | `$BRIKKO_STUDIO_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                              |
    | `$BRIKKO_STUDIO_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                           |
    | `$BRIKKO_STUDIO_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                       |

    Legacy single-agent path: `~/.brikko-studio/agent/*` (migrated by `brikko-studio doctor`).

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.brikko-studio/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These files live in the **agent workspace**, not `~/.brikko-studio`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      `MEMORY.md`, `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
      Lowercase root `memory.md` is legacy repair input only; `brikko-studio doctor --fix`
      can merge it into `MEMORY.md` when both files exist.
    - **State dir (`~/.brikko-studio`)**: config, channel/provider state, auth profiles, sessions, logs,
      and shared skills (`~/.brikko-studio/skills`).

    Default workspace is `~/.brikko-studio/workspace`, configurable via:
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.brikko-studio` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="How do I completely uninstall Brikko Studio?">
  ...
  <Accordion title="Can agents work outside the workspace?">
    ...
    `workspace` to the repo root. The Brikko Studio repo is just source code; keep the
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    Brikko Studio reads an optional **JSON5** config from `$BRIKKO_STUDIO_CONFIG_PATH` (default: `~/.brikko-studio/brikko-studio.json`):

    ```
    $BRIKKO_STUDIO_CONFIG_PATH
    ```

    If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.brikko-studio/workspace`).
  ...
  <Accordion title='I set gateway.bind: "lan" (or "tailnet") and now nothing listens / the UI says unauthorized'>
    ...
    - For password auth, set `gateway.auth.mode: "password"` plus `gateway.auth.password` (or `BRIKKO_STUDIO_GATEWAY_PASSWORD`) instead.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one Brikko Studio.` every time.
    - `random`: rotating funny/seasonal taglines (default behavior).
    - If you want no banner at all, set env `BRIKKO_STUDIO_HIDE_BANNER=1`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `brikko-studio configure --section web` and choose a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, Brikko Studio auto-detects the first ready fetch fallback provider from available credentials. Today the bundled provider is Firecrawl.
    - Daemons read env vars from `~/.brikko-studio/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    Current Brikko Studio protects many accidental clobbers:

    - Brikko Studio-owned config writes validate the full post-change config before writing.
    - Invalid or destructive Brikko Studio-owned writes are rejected and saved as `brikko-studio.json.rejected.*`.
    - If a direct edit breaks startup or hot reload, the Gateway restores the last-known-good config and saves the rejected file as `brikko-studio.json.clobbered.*`.
    - The main agent receives a boot warning after recovery so it does not blindly write the bad config again.

    Recover:

    - Check `brikko-studio logs --follow` for `Config auto-restored from last-known-good`, `Config write rejected:`, or `config reload restored last-known-good config`.
    - Inspect the newest `brikko-studio.json.clobbered.*` or `brikko-studio.json.rejected.*` beside the active config.
    - Keep the active restored config if it works, then copy only the intended keys back with `brikko-studio config set` or `config.patch`.
    - Run `brikko-studio config validate` and `brikko-studio doctor`.
    - If you have no last-known-good or rejected payload, restore from backup, or re-run `brikko-studio doctor` and reconfigure channels/models.
    - If this was unexpected, file a bug and include your last known config or any backup.
    - A local coding agent can often reconstruct a working config from logs or history.

    Avoid it:

    - Use `brikko-studio config set` for small changes.
    - Use `brikko-studio configure` for interactive edits.
  ...
  <Accordion title="Can the Brikko Studio browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="Tailscale is connected but I get no replies. What now?">
    ...
    - Gateway is running: `brikko-studio gateway status`
    - Gateway health: `brikko-studio status`
    - Channel health: `brikko-studio channels status`
  ...
  <Accordion title="Can two Brikko Studio instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    `brikko-studio agent --message ... --deliver`, targeting a chat where the other bot
    ...
    ```bash
    brikko-studio agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
  ...
  <Accordion title="How do I set up Tailscale on a VPS and connect from my Mac?">
    ...
    ```bash
    brikko-studio gateway --tailscale serve
    ```
...
<AccordionGroup>
  <Accordion title="How does Brikko Studio load environment variables?">
    Brikko Studio reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory
    - a global fallback `.env` from `~/.brikko-studio/.env` (aka `$BRIKKO_STUDIO_STATE_DIR/.env`)
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.brikko-studio/.env` so they're picked up even when the service doesn't inherit your shell env.
    ...
    `BRIKKO_STUDIO_LOAD_SHELL_ENV=1`, `BRIKKO_STUDIO_SHELL_ENV_TIMEOUT_MS=15000`.

  </Accordion>

  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `brikko-studio models status` reports whether **shell env import** is enabled. "Shell env: off"
    does **not** mean your env vars are missing - it just means Brikko Studio won't load
    your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
    environment. Fix by doing one of these:

    1. Put the token in `~/.brikko-studio/.env`:
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of Brikko Studio instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset Brikko Studio but keep it installed?">
    ...
    ```bash
    brikko-studio reset --scope full --yes --non-interactive
    ```
    ...
    - If you used profiles (`--profile` / `BRIKKO_STUDIO_PROFILE`), reset each state dir (defaults are `~/.brikko-studio-<profile>`).
    - Dev reset: `brikko-studio gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    headers like `# Heading`), Brikko Studio skips the heartbeat run to save API calls.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. Brikko Studio runs on **your own account**, so if you're in the group, Brikko Studio can see it.
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    brikko-studio logs --follow --json
    ```
    ...
    ```bash
    brikko-studio directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does Brikko Studio not reply in a group?">
    Two common causes:
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth:** sessions + transcripts live under `~/.brikko-studio/agents/<agentId>/sessions/`.
    ...
    - Use `brikko-studio doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title="What port does the Gateway use?">
    ...
    ```
    --port > BRIKKO_STUDIO_GATEWAY_PORT > gateway.port > default 18789
    ```

  </Accordion>

  <Accordion title='Why does brikko-studio gateway status say "Runtime: running" but "Connectivity probe: failed"?'>
    ...
    Use `brikko-studio gateway status` and trust these lines:
  ...
  <Accordion title='Why does brikko-studio gateway status show "Config (cli)" and "Config (service)" different?'>
    You're editing one config file while the service is running another (often a `--profile` / `BRIKKO_STUDIO_STATE_DIR` mismatch).

    Fix:

    ```bash
    brikko-studio gateway install --force
    ```
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    Brikko Studio enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

    Fix: stop the other instance, free the port, or run with `brikko-studio gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run Brikko Studio in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `brikko-studio gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `brikko-studio dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
    - If you don't have a token yet: `brikko-studio doctor --generate-gateway-token`.
    - If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
    - Shared-secret mode: set `gateway.auth.token` / `BRIKKO_STUDIO_GATEWAY_TOKEN` or `gateway.auth.password` / `BRIKKO_STUDIO_GATEWAY_PASSWORD`, then paste the matching secret in Control UI settings.
    ...
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `brikko-studio devices list`
      - `brikko-studio devices rotate --device <id> --role operator`
    ...
    - Still stuck? Run `brikko-studio status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    - `BRIKKO_STUDIO_CONFIG_PATH` (per-instance config)
    - `BRIKKO_STUDIO_STATE_DIR` (per-instance state)
    - `agents.defaults.workspace` (workspace isolation)
    - `gateway.port` (unique ports)

    Quick setup (recommended):

    - Use `brikko-studio --profile <name> ...` per instance (auto-creates `~/.brikko-studio-<name>`).
    - Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
    - Install a per-profile service: `brikko-studio --profile <name> gateway install`.

    Profiles also suffix service names (`ai.brikko-studio.<profile>`; legacy `com.brikko-studio.*`, `brikko-studio-gateway-<profile>.service`, `Brikko Studio Gateway (<profile>)`).
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```
    brikko-studio tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    ```
    /tmp/brikko-studio/brikko-studio-YYYY-MM-DD.log
    ```
    ...
    - macOS: `$BRIKKO_STUDIO_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.brikko-studio/logs/...`; profiles use `~/.brikko-studio-<profile>/logs/...`)
    - Linux: `journalctl --user -u brikko-studio-gateway[-<profile>].service -n 200 --no-pager`
    - Windows: `schtasks /Query /TN "Brikko Studio Gateway (<profile>)" /V /FO LIST`
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `brikko-studio gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart Brikko Studio?">
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    ...
    1. Is the Gateway running? `brikko-studio gateway status`
    2. Is the Gateway healthy? `brikko-studio status`
    3. Does the UI have the right token? `brikko-studio dashboard`
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    ```bash
    brikko-studio channels status
    brikko-studio channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Brikko Studio already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="ELI5: brikko-studio gateway restart vs brikko-studio gateway">
    - `brikko-studio gateway restart`: restarts the **background service** (launchd/systemd).
    - `brikko-studio gateway`: runs the gateway **in the foreground** for this terminal session.

    If you installed the service, use the gateway commands. Use `brikko-studio gateway` when
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Brikko Studio assistant setup](/start/brikko-studio) and [Agent send](/tools/agent-send).

    CLI sending:

    ```bash
    brikko-studio message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose Brikko Studio to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `brikko-studio pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `brikko-studio pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `brikko-studio doctor` to surface risky DM policies.
  ...
  <Accordion title="I ran /start in Telegram but did not get a pairing code">
    ...
    ```bash
    brikko-studio pairing list telegram
    ```
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Brikko Studio only replies to chats it receives or to explicit sends you trigger.

    Approve pairing with:

    ```bash
    brikko-studio pairing approve whatsapp <code>
    ```

    List pending requests:

    ```bash
    brikko-studio pairing list whatsapp
    ```
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    Brikko Studio blocks **cross-provider** messaging by default. If a tool call is bound
