---
summary: "Symptom first troubleshooting hub for Brikko Studio"
read_when:
  - Brikko Studio is not working and you need the fastest path to a fix
...
- `brikko-studio status` → shows configured channels and no obvious auth errors.
- `brikko-studio status --all` → full report is present and shareable.
- `brikko-studio gateway probe` → expected gateway target is reachable (`Reachable: yes`). `Capability: ...` tells you what auth level the probe could prove, and `Read probe: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `brikko-studio gateway status` → `Runtime: running`, `Connectivity probe: ok`, and a plausible `Capability: ...` line. Use `--require-rpc` if you need read-scope RPC proof too.
- `brikko-studio doctor` → no blocking config/service errors.
- `brikko-studio channels status --probe` → reachable gateway returns live per-account
...
- `brikko-studio logs --follow` → steady activity, no repeating fatal errors.
...
## Local OpenAI-compatible backend works directly but fails in Brikko Studio

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `brikko-studio infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on Brikko Studio agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger Brikko Studio prompts crash the
...
## Plugin install fails with missing brikko-studio extensions

If install fails with `package.json missing brikko-studio.extensions`, the plugin package
is using an old shape that Brikko Studio no longer accepts.

Fix in the plugin package:

1. Add `brikko-studio.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `brikko-studio plugins install <package>` again.

Example:

```json
{
  "name": "@brikko-studio/my-plugin",
  "version": "1.2.3",
  "brikko-studio": {
...
```mermaid
flowchart TD
  A[Brikko Studio is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `brikko-studio gateway status`
    - `Connectivity probe: ok`
    - `Capability: read-only`, `write-capable`, or `admin-capable`
  ...
  <Accordion title="Exec suddenly asks for approval">
    ```bash
    brikko-studio config get tools.exec.host
    brikko-studio config get tools.exec.security
    brikko-studio config get tools.exec.ask
    ...
    ```bash
    brikko-studio config set tools.exec.host gateway
    brikko-studio config set tools.exec.security full
    brikko-studio config set tools.exec.ask off
    brikko-studio gateway restart
    ```
  ...
  <Accordion title="Browser tool fails">
    ...
    - `brikko-studio` starts, or `user` can see local Chrome tabs.
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `brikko-studio browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
