...
The macOS app surfaces Brikko Studio skills via the gateway; it does not parse skills locally.

## Data source

- `skills.status` (gateway) returns all skills plus eligibility and missing requirements
  (including allowlist blocks for bundled skills).
- Requirements are derived from `metadata.brikko-studio.requires` in each `SKILL.md`.

## Install actions

- `metadata.brikko-studio.install` defines install options (brew/node/go/uv).
...
- The app stores keys in `~/.brikko-studio/brikko-studio.json` under `skills.entries.<skillKey>`.
