...
read_when:
  - Hosting PeekabooBridge in Brikko Studio.app
...
Brikko Studio can host **PeekabooBridge** as a local, permission‑aware UI automation
broker. This lets the `peekaboo` CLI drive UI automation while reusing the
macOS app’s TCC permissions.

## What this is (and is not)

- **Host**: Brikko Studio.app can act as a PeekabooBridge host.
- **Client**: use the `peekaboo` CLI (no separate `brikko-studio ui ...` surface).
- **UI**: visual overlays stay in Peekaboo.app; Brikko Studio is a thin broker host.

## Relationship to Computer Use

Brikko Studio has three desktop-control paths, and they intentionally stay separate:

- **PeekabooBridge host**: Brikko Studio.app can host the local PeekabooBridge socket.
  The `peekaboo` CLI remains the client and uses Brikko Studio.app's macOS
...
- **Codex Computer Use**: the bundled `codex` plugin prepares Codex app-server,
  ...
  Codex own native desktop-control tool calls during Codex-mode turns. Brikko Studio
  does not proxy those actions through PeekabooBridge.
- **Direct `cua-driver` MCP**: Brikko Studio can register TryCua's upstream
...
Use Peekaboo when you want the broad macOS automation surface and Brikko Studio.app's
...
when you want the CUA driver exposed to any Brikko Studio-managed runtime as a normal
MCP server.
...
When enabled, Brikko Studio starts a local UNIX socket server. If disabled, the host
...
3. Brikko Studio.app (thin broker)
...
- The bridge validates **caller code signatures**; an allowlist of TeamIDs is
  enforced (Peekaboo host TeamID + Brikko Studio app TeamID).
...
- If `peekaboo` reports “bridge client is not authorized”, ensure the client is
  ...
  in **debug** mode only.
- If no hosts are found, open one of the host apps (Peekaboo.app or Brikko Studio.app)
