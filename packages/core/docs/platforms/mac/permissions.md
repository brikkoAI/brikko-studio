...
- Same path: run the app from a fixed location (for Brikko Studio, `dist/Brikko Studio.app`).
...
```bash
sudo tccutil reset Accessibility ai.brikko-studio.mac
sudo tccutil reset ScreenCapture ai.brikko-studio.mac
...
Workaround: move files into the Brikko Studio workspace (`~/.brikko-studio/workspace`) if you want to avoid per-folder grants.
