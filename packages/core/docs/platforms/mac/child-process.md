...
service via the external `brikko-studio` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.brikko-studio.gateway`
  (or `ai.brikko-studio.<profile>` when using `--profile`/`BRIKKO_STUDIO_PROFILE`; legacy `com.brikko-studio.*` is supported).
