---
summary: "Brikko Studio on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Brikko Studio on DigitalOcean
  - Looking for cheap VPS hosting for Brikko Studio
title: "DigitalOcean (platform)"
---

# Brikko Studio on DigitalOcean

## Goal

Run a persistent Brikko Studio Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
```bash
brikko-studio channels login whatsapp
# Scan QR code
```
...
- `~/.brikko-studio/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl --user -u brikko-studio-gateway.service --no-pager -n 50
```
