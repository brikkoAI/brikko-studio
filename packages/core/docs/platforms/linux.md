...
2. `npm i -g brikko-studio@latest`
3. `brikko-studio onboard --install-daemon`
...
Brikko Studio installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `brikko-studio gateway install` and
`brikko-studio onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/brikko-studio-gateway[-<profile>].service`:

```
[Unit]
Description=Brikko Studio Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. Brikko Studio therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, Brikko Studio starts the child through a short
...
- Brikko Studio-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `BRIKKO_STUDIO_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
