...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `brikko-studio.internal.`), **or**
...
Run `brikko-studio devices list` again before approval.
...
brikko-studio gateway call node.list --params "{}"
```
...
- `BRIKKO_STUDIO_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Brikko Studio iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Brikko Studio builds.
...
```bash
export BRIKKO_STUDIO_APNS_TEAM_ID="TEAMID"
export BRIKKO_STUDIO_APNS_KEY_ID="KEYID"
export BRIKKO_STUDIO_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.brikko-studio/credentials/apns
chmod 700 ~/.brikko-studio/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.brikko-studio/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.brikko-studio/credentials/apns/AuthKey_KEYID.p8
export BRIKKO_STUDIO_APNS_PRIVATE_KEY_PATH="$HOME/.brikko-studio/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_brikko-studio-gw._tcp` on `local.` and, when configured, the same
...
`brikko-studio.internal.`) and Tailscale split DNS.
...
```bash
brikko-studio nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__brikko-studio__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__brikko-studio__/canvas/` and `/__brikko-studio__/a2ui/`.
...
tools; the iOS app exposes iPhone capabilities through Brikko Studio node commands
such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through Brikko Studio by invoking node
...
```bash
brikko-studio nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__brikko-studio; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
brikko-studio nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `brikko-studio devices list` and approve manually.
