...
CLI onboarding is the **recommended** way to set up Brikko Studio on macOS,
...
`brikko-studio dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`brikko-studio configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.brikko-studio/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `brikko-studio doctor` first.
</Note>
...
- CLI command reference: [`brikko-studio onboard`](/cli/onboard)
