---
summary: "Get Brikko Studio installed and run your first chat in minutes."
read_when:
...
Install Brikko Studio, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.brikko-studio/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.brikko-studio/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run Brikko Studio as a service account or want custom paths:

- `BRIKKO_STUDIO_HOME` — home directory for internal path resolution
- `BRIKKO_STUDIO_STATE_DIR` — override the state directory
- `BRIKKO_STUDIO_CONFIG_PATH` — override the config file path
