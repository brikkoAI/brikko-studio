...
`node-llama-cpp` runtime package next to Brikko Studio, then point `local.modelPath`
at a GGUF file:
...
Brikko Studio indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.brikko-studio/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `brikko-studio memory index --force`
...
**Memory search disabled?** Check `brikko-studio memory status`. If no provider is
...
**Stale results?** Run `brikko-studio memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** Brikko Studio falls back to in-process cosine similarity
automatically. `brikko-studio memory status --deep` reports the local vector store
