...
Brikko Studio gives agents tools to work across sessions, inspect status, and
...
After the target responds, Brikko Studio can run a **reply-back loop** where the
...
the caller's current session; visible client labels such as `brikko-studio-tui` are
not session keys.
...
`subagents` is the control-plane helper for already spawned Brikko Studio
...
the completion origin only identifies a channel Brikko Studio can still reuse the
