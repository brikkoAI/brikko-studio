...
- `/usage cost` in chats: local cost summary aggregated from Brikko Studio session logs.
- CLI: `brikko-studio status --usage` prints a full per-provider breakdown.
- CLI: `brikko-studio channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Brikko Studio treats
  ...
  quota, so Brikko Studio inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Brikko Studio prefers the
...
can supply plugin-specific usage auth logic; otherwise Brikko Studio falls back to
