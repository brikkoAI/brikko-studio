...
Brikko Studio selects models in this order:
...
<AccordionGroup>
  <Accordion title="Related model surfaces">
    - `agents.defaults.models` is the allowlist/catalog of models Brikko Studio can use (plus aliases).
...
- User session selections are exact. `/model`, the model picker, `session_status(model=...)`, and `sessions.patch` store `modelOverrideSource: "user"`; if that selected provider/model is unreachable, Brikko Studio fails visibly instead of falling through to another configured model.
...
<AccordionGroup>
  <Accordion title="Clobber protection rules">
    `brikko-studio config set` protects model/provider maps from accidental clobbers. A plain object assignment to `agents.defaults.models`, `models.providers`, or `models.providers.<id>.models` is rejected when it would remove existing entries. Use `--merge` for additive changes; use `--replace` only when the provided value should become the complete target value.

    Interactive provider setup and `brikko-studio configure --section model` also merge provider-scoped selections into the existing allowlist, so adding Codex, Ollama, or another provider does not drop unrelated model entries. Configure preserves an existing `agents.defaults.model.primary` when provider auth is re-applied. Explicit default-setting commands such as `brikko-studio models auth login --provider <id> --set-default` and `brikko-studio models set <model>` still replace `agents.defaults.model.primary`.
...
If `agents.defaults.models` is set, it becomes the **allowlist** for `/model` and for session overrides. When a user selects a model that isn't in that allowlist, Brikko Studio returns:
...
exact provider/model shown by `brikko-studio models list --provider <provider>`.
...
<AccordionGroup>
  <Accordion title="Picker behavior">
    ...
    - On Telegram, `/models` picker selections are session-scoped; they do not change the agent's persistent default in `brikko-studio.json`.
  ...
  <Accordion title="Ref parsing">
    ...
    - If you omit the provider, Brikko Studio resolves the input in this order:
      1. alias match
      2. unique configured-provider match for that exact unprefixed model id
      3. deprecated fallback to the configured default provider — if that provider no longer exposes the configured default model, Brikko Studio instead falls back to the first configured provider/model to avoid surfacing a stale removed-provider default.
  </Accordion>
</AccordionGroup>
...
brikko-studio models set <provider/model>
brikko-studio models set-image <provider/model>

brikko-studio models aliases list
brikko-studio models aliases add <alias> <provider/model>
brikko-studio models aliases remove <alias>

brikko-studio models fallbacks list
brikko-studio models fallbacks add <provider/model>
brikko-studio models fallbacks remove <provider/model>
brikko-studio models fallbacks clear

brikko-studio models image-fallbacks list
brikko-studio models image-fallbacks add <provider/model>
brikko-studio models image-fallbacks remove <provider/model>
brikko-studio models image-fallbacks clear
```

`brikko-studio models` (no subcommand) is a shortcut for `models status`.
...
`brikko-studio models scan` inspects OpenRouter's **free model catalog** and can optionally probe models for tool and image support.
...
When live probes run in a TTY, you can select fallbacks interactively. In non-interactive mode, pass `--yes` to accept defaults. Metadata-only results are informational; `--set-default` and `--set-image` require live probes so Brikko Studio does not configure an unusable keyless OpenRouter model.

## Models registry (`models.json`)

Custom providers in `models.providers` are written into `models.json` under the agent directory (default `~/.brikko-studio/agents/<agentId>/agent/models.json`). This file is merged by default unless `models.mode` is set to `replace`.
...
<Note>
Marker persistence is source-authoritative: Brikko Studio writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values. This applies whenever Brikko Studio regenerates `models.json`, including command-driven paths like `brikko-studio agent`.
</Note>
