---
summary: "How Brikko Studio separates model providers, models, channels, and agent runtimes"
...
the finished turn to Brikko Studio.
...
| ------------- | ------------------------------------- | ------------------------------------------------------------------- |
| Provider      | `openai`, `anthropic`, `openai-codex` | How Brikko Studio authenticates, discovers models, and names model refs. |
| Model         | `gpt-5.5`, `claude-opus-4-6`          | The model selected for the agent turn.                              |
| Agent runtime | `pi`, `codex`, `claude-cli`           | The low level loop or backend that executes the prepared turn.      |
| Channel       | Telegram, Discord, Slack, WhatsApp    | Where messages enter and leave Brikko Studio.                            |
...
implements the `codex` runtime. Public config uses `agentRuntime.id`; `brikko-studio
doctor --fix` rewrites older runtime-policy keys to that shape.

There are two runtime families:

- **Embedded harnesses** run inside Brikko Studio's prepared agent loop. Today this
...
| Surface                                              | Brikko Studio name/config                       | What it does                                                                                               |
| ---------------------------------------------------- | ------------------------------------------ | ---------------------------------------------------------------------------------------------------------- |
...
| Codex OAuth provider route                           | `openai-codex/*` model refs                | Uses ChatGPT/Codex subscription OAuth through the normal Brikko Studio PI runner.                               |
...
That means Brikko Studio selects an OpenAI model ref, then asks the Codex app-server
runtime to run the embedded agent turn. It does not mean "use API billing," and
it does not mean the channel, model provider catalog, or Brikko Studio session store
becomes Codex.
...
3. If the user asks for **Codex OAuth/subscription auth on the normal Brikko Studio
...
| Surface                     | Brikko Studio PI embedded                    | Codex app-server                                                            |
| --------------------------- | --------------------------------------- | --------------------------------------------------------------------------- |
| Model loop owner            | Brikko Studio through the PI embedded runner | Codex app-server                                                            |
| Canonical thread state      | Brikko Studio transcript                     | Codex thread, plus Brikko Studio transcript mirror                               |
| Brikko Studio dynamic tools      | Native Brikko Studio tool loop               | Bridged through the Codex adapter                                           |
| Native shell and file tools | PI/Brikko Studio path                        | Codex-native tools, bridged through native hooks where supported            |
| Context engine              | Native Brikko Studio context assembly        | Brikko Studio projects assembled context into the Codex turn                     |
| Compaction                  | Brikko Studio or selected context engine     | Codex-native compaction, with Brikko Studio notifications and mirror maintenance |
| Channel delivery            | Brikko Studio                                | Brikko Studio                                                                    |

This ownership split is the main design rule:

- If Brikko Studio owns the surface, Brikko Studio can provide normal plugin hook behavior.
- If the native runtime owns the surface, Brikko Studio needs runtime events or native hooks.
- If the native runtime owns canonical thread state, Brikko Studio should mirror and project context, not rewrite unsupported internals.

## Runtime selection

Brikko Studio chooses an embedded runtime after provider and model resolution:
...
2. `BRIKKO_STUDIO_AGENT_RUNTIME=<id>` forces that runtime for new or reset sessions.
...
4. In `auto` mode, registered plugin runtimes can claim supported provider/model
   pairs.
5. If no runtime claims a turn in `auto` mode, Brikko Studio uses PI as the
...
If `brikko-studio doctor` warns that the `codex` plugin is enabled while
...
When a runtime is not PI, it should document what Brikko Studio surfaces it supports.
...
| Who owns canonical thread history?     | Determines whether Brikko Studio can edit history or only mirror it.                                   |
| Do Brikko Studio dynamic tools work?        | Messaging, sessions, cron, and Brikko Studio-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around Brikko Studio-owned tools. |
