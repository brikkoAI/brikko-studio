...
When a message arrives while a session run is already streaming, Brikko Studio can
...
internal steering queue. Brikko Studio adapts the same modes there:
...
- `queue` keeps the legacy serialized shape by sending separate `turn/steer`
  requests.
- `followup`, `collect`, `steer-backlog`, and `interrupt` stay Brikko Studio-owned
  queue behavior around the active Codex turn.

Codex review and manual compaction turns reject same-turn steering. When a
runtime cannot accept steering, Brikko Studio falls back to the followup queue where
that mode allows it.
...
- `collect`: Brikko Studio waits until the active run ends, then creates a followup
...
Use `collect` when you want Brikko Studio to build a later followup turn that can
...
Codex harness, Brikko Studio uses the same debounce value as the quiet window before
