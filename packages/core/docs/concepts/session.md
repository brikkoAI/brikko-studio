---
summary: "How Brikko Studio manages conversation sessions"
read_when:
...
Brikko Studio organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `brikko-studio security audit`.
...
Brikko Studio automatically bounds session storage over time. By default, it runs
...
Preview with `brikko-studio sessions cleanup --dry-run`.

## Inspecting sessions

- `brikko-studio status` -- session store path and recent activity.
- `brikko-studio sessions --json` -- all sessions (filter with `--active <minutes>`).
