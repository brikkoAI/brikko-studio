...
[Honcho](https://honcho.dev) adds AI-native memory to Brikko Studio. It persists
...
```bash
brikko-studio plugins install @honcho-ai/brikko-studio-honcho
brikko-studio honcho setup
...
Settings live under `plugins.entries["brikko-studio-honcho"].config`:

```json5
{
  plugins: {
    entries: {
      "brikko-studio-honcho": {
        config: {
          apiKey: "your-api-key", // omit for self-hosted
          workspaceId: "brikko-studio", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `brikko-studio honcho setup` detects and
...
```bash
brikko-studio honcho setup                        # Configure API key and migrate files
brikko-studio honcho status                       # Check connection status
brikko-studio honcho ask <question>               # Query Honcho about the user
brikko-studio honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/brikko-studio-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Brikko Studio integration guide](https://docs.honcho.dev/v3/guides/integrations/brikko-studio)
- [Memory](/concepts/memory) -- Brikko Studio memory overview
