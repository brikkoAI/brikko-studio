...
Brikko Studio features that can generate provider usage or paid API calls.
...
- Gemini CLI note: when the CLI returns JSON output, Brikko Studio reads usage from
  `stats`, normalizes `stats.cached` into `cacheRead`, and derives input tokens
  from `stats.input_tokens - stats.cached` when needed.

Anthropic note: Anthropic staff told us Brikko Studio-style Claude CLI usage is
...
Anthropic still does not expose a per-message dollar estimate that Brikko Studio can
show in `/usage full`.

**CLI usage windows (provider quotas)**

- `brikko-studio status --usage` and `brikko-studio channels list` show provider **usage windows**
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so Brikko Studio inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, Brikko Studio prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise Brikko Studio falls back to matching OAuth/API-key
...
Brikko Studio can pick up credentials from:
...
Brikko Studio's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's Brikko Studio Claude-login path with **Extra Usage** enabled.
...
- `brikko-studio models status --json`
...
`brikko-studio models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
