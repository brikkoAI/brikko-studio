...
Brikko Studio integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Brikko Studio owns lifecycle when `channels.signal.autoStart=true`.
...
- Brikko Studio spawns `imsg rpc` as a child process (legacy iMessage integration).
