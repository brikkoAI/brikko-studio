---
summary: "How Brikko Studio builds prompt context and reports token usage + costs"
read_when:
...
Brikko Studio tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Brikko Studio assembles its own system prompt on every run. It includes:
...
For images, Brikko Studio downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Brikko Studio session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `brikko-studio status --usage` and `brikko-studio channels list` show
...
available; otherwise Brikko Studio falls back to matching OAuth/API-key credentials
...
Brikko Studio keeps provider usage accounting separate from the current context
...
`cacheWrite`. If pricing is missing, Brikko Studio shows tokens only. OAuth tokens
never show dollar cost.

After sidecars and channels reach the Gateway ready path, Brikko Studio starts an
...
Provider prompt caching only applies within the cache TTL window. Brikko Studio can
...
Anthropic's 1M context window is currently beta-gated. Brikko Studio can inject the
...
Brikko Studio skips the `context-1m-*` beta header because Anthropic currently
