---
summary: "CLI reference for `brikko-studio channels` (accounts, status, login/logout, logs)"
read_when:
...
```bash
brikko-studio channels list
brikko-studio channels status
brikko-studio channels capabilities
brikko-studio channels capabilities --channel discord --target channel:123
brikko-studio channels resolve --channel slack "#general" "@jane"
brikko-studio channels logs --channel all
```
...
Do not use `brikko-studio sessions`, Gateway `sessions.list`, or the agent
...
```bash
brikko-studio channels add --channel telegram --token <bot-token>
brikko-studio channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
brikko-studio channels remove --channel telegram --delete
```

<Tip>
`brikko-studio channels add --help` shows per-channel flags (token, private key, app token, signal-cli paths, etc).
</Tip>
...
If a channel plugin needs to be installed during a flag-driven add command, Brikko Studio uses the channel's default install source without opening the interactive plugin install prompt.

When you run `brikko-studio channels add` without flags, the interactive wizard can prompt:
...
You can also manage the same routing rules later with `brikko-studio agents bindings`, `brikko-studio agents bind`, and `brikko-studio agents unbind` (see [agents](/cli/agents)).

When you add a non-default account to a channel that is still using single-account top-level settings, Brikko Studio promotes account-scoped top-level values into the channel's account map before writing the new account. Most channels land those values in `channels.<channel>.accounts.default`, but bundled channels can preserve an existing matching promoted account instead. Matrix is the current example: if one named account already exists, or `defaultAccount` points at an existing named account, promotion preserves that account instead of creating a new `accounts.default`.
...
- Run `brikko-studio status --deep` for a broad probe.
- Use `brikko-studio doctor` for guided fixes.
- `brikko-studio channels list` prints `Claude: HTTP 403 ... user:profile` → usage snapshot needs the `user:profile` scope. Use `--no-usage`, or provide a claude.ai session key (`CLAUDE_WEB_SESSION_KEY` / `CLAUDE_WEB_COOKIE`), or re-auth via Claude CLI.
- `brikko-studio channels status` falls back to config-only summaries when the gateway is unreachable. If a supported channel credential is configured via SecretRef but unavailable in the current command path, it reports that account as configured with degraded notes instead of showing it as not configured.
...
```bash
brikko-studio channels resolve --channel slack "#general" "@jane"
brikko-studio channels resolve --channel discord "My Server/#support" "@someone"
brikko-studio channels resolve --channel matrix "Project Room"
```
