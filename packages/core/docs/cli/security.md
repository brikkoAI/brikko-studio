---
summary: "CLI reference for `brikko-studio security` (audit and fix common security footguns)"
read_when:
...
brikko-studio security audit --deep --password <password>
brikko-studio security audit --deep --token <token>
...
It also emits `security.trust_model.multi_user_heuristic` when config suggests likely shared-user ingress (for example open DM/group policy, configured group targets, or wildcard sender rules), and reminds you that Brikko Studio is a personal-assistant trust model by default.
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `brikko-studio.browserConfigEpoch`) and recommends `brikko-studio sandbox recreate --browser --all`.
...
```bash
brikko-studio security audit --json | jq '.summary'
brikko-studio security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
brikko-studio security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
...
- tightens permissions for state/config and common sensitive files
  ...
  `*.jsonl`)
- also tightens config include files referenced from `brikko-studio.json`
