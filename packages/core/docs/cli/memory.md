---
summary: "CLI reference for `brikko-studio memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
brikko-studio memory status
brikko-studio memory status --deep
brikko-studio memory status --fix
brikko-studio memory index --force
brikko-studio memory search "meeting notes"
brikko-studio memory search --query "deployment" --max-results 20
brikko-studio memory promote --limit 10 --min-score 0.75
brikko-studio memory promote --apply
brikko-studio memory promote --json --min-recall-count 0 --min-unique-queries 0
brikko-studio memory promote-explain "router vlan"
brikko-studio memory promote-explain "router vlan" --json
brikko-studio memory rem-harness
brikko-studio memory rem-harness --json
brikko-studio memory status --json
brikko-studio memory status --deep --index
brikko-studio memory status --deep --index --verbose
brikko-studio memory status --agent main
brikko-studio memory index --agent main --verbose
```
...
```bash
brikko-studio memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `brikko-studio cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
brikko-studio memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
brikko-studio memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
