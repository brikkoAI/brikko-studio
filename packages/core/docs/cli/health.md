---
summary: "CLI reference for `brikko-studio health` (gateway health snapshot via RPC)"
read_when:
...
brikko-studio health --timeout 2500
brikko-studio health --verbose
brikko-studio health --debug
```

Notes:

- Default `brikko-studio health` asks the running gateway for its health snapshot. When the
