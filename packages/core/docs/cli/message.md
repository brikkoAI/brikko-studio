---
summary: "CLI reference for `brikko-studio message` (send + channel actions)"
read_when:
...
```
brikko-studio message <subcommand> [flags]
```
...
- `brikko-studio message` resolves the selected channel to its owning plugin when `--channel` or a channel-prefixed target is present; otherwise it loads configured channel plugins for default-channel inference.
...
- On cache miss, Brikko Studio will attempt a live directory lookup when the provider supports it.
...
- `brikko-studio message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Brikko Studio does not force a `default` account SecretRef scope
...
```
brikko-studio message send --channel discord \
...
```bash
brikko-studio message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
brikko-studio message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
brikko-studio message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
brikko-studio message poll --channel msteams \
...
```
brikko-studio message react --channel slack \
...
```
brikko-studio message react --channel signal \
...
```
brikko-studio message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
brikko-studio message send --channel telegram --target @mychat \
