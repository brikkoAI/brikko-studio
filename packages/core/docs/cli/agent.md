---
summary: "CLI reference for `brikko-studio agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
brikko-studio agent --to +15555550123 --message "status update" --deliver
brikko-studio agent --agent ops --message "Summarize logs"
brikko-studio agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
brikko-studio agent --session-id 1234 --message "Summarize inbox" --thinking medium
brikko-studio agent --to +15555550123 --message "Trace logs" --verbose on --json
brikko-studio agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
brikko-studio agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: Brikko Studio persists markers from the active source config snapshot, not from resolved runtime secret values.
