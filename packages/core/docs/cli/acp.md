...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Brikko Studio Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`brikko-studio acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Brikko Studio channel
conversations instead of hosting an ACP harness session, use
[`brikko-studio mcp serve`](/cli/mcp) instead.

## What this is not

This page is often confused with ACP harness sessions.

`brikko-studio acp` means:

- Brikko Studio acts as an ACP server
- an IDE or ACP client connects to Brikko Studio
- Brikko Studio forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Brikko Studio runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Brikko Studio: use `brikko-studio acp`
- Brikko Studio should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Brikko Studio gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
brikko-studio acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
brikko-studio acp --url wss://gateway-host:18789 --token-file ~/.brikko-studio/gateway.token

# Attach to an existing session key
brikko-studio acp --session agent:main:main

# Attach by label (must already exist)
brikko-studio acp --session-label "support inbox"

# Reset the session key before the first prompt
brikko-studio acp --session agent:main:main --reset-session
```
...
```bash
brikko-studio acp client

# Point the spawned bridge at a remote Gateway
brikko-studio acp client --server-args --url wss://gateway-host:18789 --token-file ~/.brikko-studio/gateway.token

# Override the server command (default: brikko-studio)
brikko-studio acp client --server "node" --server-args brikko-studio.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Brikko Studio through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an Brikko Studio Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `brikko-studio acp` over stdio.

Example config (persisted):

```bash
brikko-studio config set gateway.remote.url wss://gateway-host:18789
brikko-studio config set gateway.remote.token <token>
```
...
```bash
brikko-studio acp --session agent:main:main
brikko-studio acp --session agent:design:main
brikko-studio acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Brikko Studio plugin tools or selected
...
[Brikko Studio tools MCP bridge](/tools/acp-agents-setup#brikko-studio-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
Brikko Studio bot over ACP, use `acpx` with its built-in `brikko-studio` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx brikko-studio` at `brikko-studio acp`.
3. Target the Brikko Studio session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Brikko Studio ACP session
acpx brikko-studio exec "Summarize the active Brikko Studio session state."

# Persistent named session for follow-up turns
acpx brikko-studio sessions ensure --name codex-bridge
acpx brikko-studio -s codex-bridge --cwd /path/to/repo \
  "Ask my Brikko Studio work agent for recent context relevant to this repo."
```

If you want `acpx brikko-studio` to target a specific Gateway and session key every
time, override the `brikko-studio` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "brikko-studio": {
      "command": "env BRIKKO_STUDIO_HIDE_BANNER=1 BRIKKO_STUDIO_SUPPRESS_NOTES=1 brikko-studio acp --url ws://127.0.0.1:18789 --token-file ~/.brikko-studio/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Brikko Studio checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env BRIKKO_STUDIO_HIDE_BANNER=1 BRIKKO_STUDIO_SUPPRESS_NOTES=1 node brikko-studio.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Brikko Studio agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Brikko Studio ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`BRIKKO_STUDIO_GATEWAY_TOKEN`, `BRIKKO_STUDIO_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`BRIKKO_STUDIO_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `BRIKKO_STUDIO_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `brikko-studio acp client` sets `BRIKKO_STUDIO_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `brikko-studio`).
