---
summary: "CLI reference for `brikko-studio qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
brikko-studio qr
brikko-studio qr --setup-code-only
brikko-studio qr --json
brikko-studio qr --remote
brikko-studio qr --url wss://gateway.example/ws
```
...
- With `--remote`, Brikko Studio requires either `gateway.remote.url` or
