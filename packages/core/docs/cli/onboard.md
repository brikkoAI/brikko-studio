---
summary: "CLI reference for `brikko-studio onboard` (interactive onboarding)"
read_when:
...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding overview" href="/start/onboarding-overview" icon="map">
    How Brikko Studio onboarding fits together.
  </Card>
...
brikko-studio onboard --flow quickstart
brikko-studio onboard --flow manual
brikko-studio onboard --flow import
brikko-studio onboard --import-from hermes --import-source ~/.hermes
brikko-studio onboard --skip-bootstrap
brikko-studio onboard --mode remote --remote-url wss://gateway-host:18789
```

`--flow import` uses plugin-owned migration providers such as Hermes. It only runs against a fresh Brikko Studio setup; if existing config, credentials, sessions, or workspace memory/identity files are present, reset or choose a fresh setup before importing.

`--modern` starts the Crestodian conversational onboarding preview. Without
`--modern`, `brikko-studio onboard` keeps the classic onboarding flow.

For plaintext private-network `ws://` targets (trusted networks only), set
`BRIKKO_STUDIO_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
There is no `brikko-studio.json` equivalent for this client-side transport
break-glass.
...
Brikko Studio marks common vision model IDs as image-capable automatically. Pass `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `brikko-studio gateway run`.
...
<AccordionGroup>
  ...
  <Accordion title="Other behaviors">
    ...
    - Fastest first chat: `brikko-studio dashboard` (Control UI, no channel setup).
