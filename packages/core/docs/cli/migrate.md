---
summary: "CLI reference for `brikko-studio migrate` (import state from another agent system)"
read_when:
  - You want to migrate from Hermes or another agent system into Brikko Studio
  - You are adding a plugin-owned migration provider
title: "Migrate"
---

# `brikko-studio migrate`
...
```bash
brikko-studio migrate list
brikko-studio migrate claude --dry-run
brikko-studio migrate codex --dry-run
brikko-studio migrate codex --skill gog-vault77-google-workspace
brikko-studio migrate hermes --dry-run
brikko-studio migrate hermes
brikko-studio migrate apply codex --yes --skill gog-vault77-google-workspace
brikko-studio migrate apply codex --yes
brikko-studio migrate apply claude --yes
brikko-studio migrate apply hermes --yes
brikko-studio migrate apply hermes --include-secrets --yes
brikko-studio onboard --flow import
brikko-studio onboard --import-from claude --import-source ~/.claude
brikko-studio onboard --import-from hermes --import-source ~/.hermes
```

<ParamField path="<provider>" type="string">
  Name of a registered migration provider, for example `hermes`. Run `brikko-studio migrate list` to see installed providers.
</ParamField>
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local Brikko Studio state exists.
</ParamField>
...
`brikko-studio migrate` is preview-first.
...
<AccordionGroup>
  <Accordion title="Preview before apply">
    ...
    `brikko-studio migrate apply <provider>` previews the plan and prompts before changing state unless `--yes` is set. In non-interactive mode, apply requires `--yes`.

  </Accordion>
  <Accordion title="Backups">
    Apply creates and verifies an Brikko Studio backup before applying the migration. If no local Brikko Studio state exists yet, the backup step is skipped and the migration can continue. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the Brikko Studio agent workspace.
...
- Claude command Markdown files converted into Brikko Studio skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, local memory, path-scoped rules, subagents, caches, plans, and project history are preserved in the migration report or reported as manual-review items. Brikko Studio does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the Brikko Studio Codex harness and you want to
...
Running `brikko-studio migrate codex` in an interactive terminal previews the full
...
```bash
brikko-studio migrate codex --dry-run --skill gog-vault77-google-workspace
brikko-studio migrate apply codex --yes --skill gog-vault77-google-workspace
```
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current
  Brikko Studio agent workspace when you want per-agent ownership.
...
them into Brikko Studio. Config and hook files are copied into the migration report
for manual review.
...
- `SOUL.md` and `AGENTS.md` into the Brikko Studio agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for Brikko Studio file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
Hermes state that Brikko Studio cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live Brikko Studio config or credentials. This preserves opaque or unsafe state without pretending Brikko Studio can execute or trust it automatically:
...
Migration sources are plugins. A plugin declares its provider ids in `brikko-studio.plugin.json`:
...
Provider plugins can use `brikko-studio/plugin-sdk/migration` for item construction and summary counts, plus `brikko-studio/plugin-sdk/migration-runtime` for conflict-aware file copies, archive-only report copies, cached config-runtime wrappers, and migration reports.

## Onboarding integration

Onboarding can offer migration when a provider detects a known source. Both `brikko-studio onboard --flow import` and `brikko-studio setup --wizard --import-from hermes` use the same plugin migration provider and still show a preview before applying.

<Note>
Onboarding imports require a fresh Brikko Studio setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move Brikko Studio to a new machine.
