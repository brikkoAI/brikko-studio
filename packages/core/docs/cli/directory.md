---
summary: "CLI reference for `brikko-studio directory` (self, peers, groups)"
read_when:
...
- `directory` is meant to help you find IDs you can paste into other commands (especially `brikko-studio message send --target ...`).
...
```bash
brikko-studio directory peers list --channel slack --query "U0"
brikko-studio message send --channel slack --target user:U012ABCDEF --message "hello"
```
...
```bash
brikko-studio directory peers list --channel zalouser
brikko-studio directory peers list --channel zalouser --query "name"
brikko-studio directory peers list --channel zalouser --limit 50
```

## Groups

```bash
brikko-studio directory groups list --channel zalouser
