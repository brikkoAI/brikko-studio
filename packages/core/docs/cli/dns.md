---
summary: "CLI reference for `brikko-studio dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You’re setting up split DNS for a custom discovery domain (example: brikko-studio.internal)
title: "DNS"
---
...
```bash
brikko-studio dns setup
brikko-studio dns setup --domain brikko-studio.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `brikko-studio.internal`)
...
- zone file path
- current tailnet IPs
- recommended `brikko-studio.json` discovery config
...
- If `--domain` is omitted, Brikko Studio uses `discovery.wideArea.domain` from config.
