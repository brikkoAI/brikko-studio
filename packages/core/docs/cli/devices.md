---
summary: "CLI reference for `brikko-studio devices` (device pairing + token rotation/revocation)"
read_when:
...
```
brikko-studio devices list
brikko-studio devices list --json
```
...
### `brikko-studio devices remove <deviceId>`
...
```
brikko-studio devices remove <deviceId>
brikko-studio devices remove <deviceId> --json
```

### `brikko-studio devices clear --yes [--pending]`

Clear paired devices in bulk.

```
brikko-studio devices clear --yes
brikko-studio devices clear --yes --pending
brikko-studio devices clear --yes --pending --json
```

### `brikko-studio devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, Brikko Studio only prints the selected pending
request and exits; rerun approval with the exact request ID after verifying
the details.

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), Brikko Studio supersedes the previous pending entry and issues a new `requestId`. Run `brikko-studio devices list` right before approval to use the current ID.
</Note>

If the device is already paired and asks for broader scopes or a broader role,
Brikko Studio keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `brikko-studio devices list`
or use `brikko-studio devices approve --latest` to preview the exact upgrade before
approving it.
...
```
brikko-studio devices approve
brikko-studio devices approve <requestId>
brikko-studio devices approve --latest
```
...
### `brikko-studio devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
brikko-studio devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `brikko-studio devices revoke --device <id> --role <role>`
...
```
brikko-studio devices revoke --device <deviceId> --role node
```
...
```bash
brikko-studio devices rotate --device <deviceId> --role operator
```
