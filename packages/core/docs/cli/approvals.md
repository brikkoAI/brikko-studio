---
summary: "CLI reference for `brikko-studio approvals` and `brikko-studio exec-policy`"
read_when:
...
# `brikko-studio approvals`

Manage exec approvals for the **local host**, **gateway host**, or a **node host**.
By default, commands target the local approvals file on disk. Use `--gateway` to target the gateway, or `--node` to target a specific node.

Alias: `brikko-studio exec-approvals`
...
## `brikko-studio exec-policy`

`brikko-studio exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.brikko-studio/exec-approvals.json`

Examples:

```bash
brikko-studio exec-policy show
brikko-studio exec-policy show --json

brikko-studio exec-policy preset yolo
brikko-studio exec-policy preset cautious --json

brikko-studio exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `brikko-studio exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `brikko-studio approvals set --gateway`
or `brikko-studio approvals set --node <id|name|ip>`.

## Common commands

```bash
brikko-studio approvals get
brikko-studio approvals get --node <id|name|ip>
brikko-studio approvals get --gateway
```

`brikko-studio approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
brikko-studio approvals set --file ./exec-approvals.json
brikko-studio approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
brikko-studio approvals set --node <id|name|ip> --file ./exec-approvals.json
brikko-studio approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested Brikko Studio policy aligned, also set:
...
```bash
brikko-studio approvals allowlist add "~/Projects/**/bin/rg"
brikko-studio approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
brikko-studio approvals allowlist add --agent "*" "/usr/bin/uname"

brikko-studio approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `brikko-studio nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.brikko-studio/exec-approvals.json`.
