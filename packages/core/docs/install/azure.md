---
summary: "Run Brikko Studio Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Brikko Studio running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Brikko Studio Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

# Brikko Studio on Azure Linux VM

This guide sets up an Azure Linux VM with the Azure CLI, applies Network Security Group (NSG) hardening, configures Azure Bastion for SSH access, and installs Brikko Studio.
...
- Install Brikko Studio with the installer script
- Verify the Gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    RG="rg-brikko-studio"
    LOCATION="westus2"
    VNET_NAME="vnet-brikko-studio"
    VNET_PREFIX="10.40.0.0/16"
    VM_SUBNET_NAME="snet-brikko-studio-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-brikko-studio-vm"
    VM_NAME="vm-brikko-studio"
    ADMIN_USERNAME="brikko-studio"
    BASTION_NAME="bas-brikko-studio"
    BASTION_PIP_NAME="pip-brikko-studio-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install Brikko Studio (in the VM shell)">
    ```bash
    curl -fsSL https://brikko-studio.ai/install.sh -o /tmp/install.sh
    bash /tmp/install.sh
    rm -f /tmp/install.sh
    ```

    The installer installs Node LTS and dependencies if not already present, installs Brikko Studio, and launches the onboarding wizard. See [Install](/install) for details.
  ...
  <Step title="Verify the Gateway">
    ...
    Most enterprise Azure teams already have GitHub Copilot licenses. If that is your case, we recommend choosing the GitHub Copilot provider in the Brikko Studio onboarding wizard. See [GitHub Copilot provider](/providers/github-copilot).
...
- **Deallocate the VM** when not in use (stops compute billing; disk charges remain). The Brikko Studio Gateway will not be reachable while the VM is deallocated — restart it when you need it live again:
...
- For more details on Brikko Studio Azure deployment with the GitHub Copilot model provider: [Brikko Studio on Azure with GitHub Copilot](https://github.com/johnsonshi/brikko-studio-azure-github-copilot)
