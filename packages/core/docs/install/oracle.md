---
summary: "Host Brikko Studio on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Brikko Studio on Oracle Cloud
  - Looking for free VPS hosting for Brikko Studio
  - Want 24/7 Brikko Studio on a small server
title: "Oracle Cloud"
---

Run a persistent Brikko Studio Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@brikko-studio`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=brikko-studio --reset` to re-authenticate.
