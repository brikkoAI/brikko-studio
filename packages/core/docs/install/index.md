---
summary: "Install Brikko Studio — installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Brikko Studio, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://brikko-studio.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want Brikko Studio and Node kept under a local prefix such as
`~/.brikko-studio`, without depending on a system-wide Node install:

```bash
curl -fsSL https://brikko-studio.ai/install-cli.sh | bash
```
...
`brikko-studio update --channel dev` and `brikko-studio update --channel stable`. See
...
<Tabs>
  <Tab title="npm">
    ```bash
    npm install -g brikko-studio@latest
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g brikko-studio@latest
```
...
Or skip the link and use `pnpm brikko-studio ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:brikko-studio/brikko-studio#main
```
...
```bash
brikko-studio --version      # confirm the CLI is available
brikko-studio doctor         # check for config issues
brikko-studio gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `brikko-studio onboard --install-daemon` or `brikko-studio gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Brikko Studio on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove Brikko Studio completely.
  </Card>
</CardGroup>

## Troubleshooting: `brikko-studio` not found

If the install succeeded but `brikko-studio` is not found in your terminal:
