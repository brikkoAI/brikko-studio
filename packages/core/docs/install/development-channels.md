...
Brikko Studio ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/brikko-studio`, override with
  `BRIKKO_STUDIO_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
brikko-studio update --tag 2026.4.1-beta.1
...
brikko-studio update --tag brikko-studio@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `brikko-studio update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Brikko Studio prompts for confirmation (skip with `--yes`).
...
Preview what `brikko-studio update` would do without making changes:

```bash
brikko-studio update --dry-run
brikko-studio update --channel beta --dry-run
brikko-studio update --tag 2026.4.1-beta.1 --dry-run
brikko-studio update --dry-run --json
```
...
When you switch channels with `brikko-studio update`, Brikko Studio also syncs plugin
sources:
