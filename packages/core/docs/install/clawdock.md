---
summary: "ClawDock shell helpers for Docker-based Brikko Studio installs"
read_when:
  - You run Brikko Studio with Docker often and want shorter day-to-day commands
  - You want a helper layer for dashboard, logs, token setup, and pairing flows
title: "ClawDock"
---

ClawDock is a small shell-helper layer for Docker-based Brikko Studio installs.
...
| `clawdock-cli <command>`  | Run Brikko Studio CLI commands in Docker           |
...
| `clawdock-config`      | Open `~/.brikko-studio`                      |
...
- `~/.brikko-studio/.env` for env-backed provider keys and bot tokens
- `~/.brikko-studio/agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `~/.brikko-studio/brikko-studio.json` for behavior config

Use `clawdock-show-config` when you want to inspect the `.env` files and `brikko-studio.json` quickly. It redacts `.env` values in its printed output.
