---
summary: "Run Brikko Studio Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Brikko Studio running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# Brikko Studio on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent Brikko Studio Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Brikko Studio 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Brikko Studio Gateway in Docker
- Persist `~/.brikko-studio` + `~/.brikko-studio/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.brikko-studio` state includes `brikko-studio.json`, per-agent
...
3. SSH into the VM
4. Install Docker
5. Clone Brikko Studio repository
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-brikko-studio-project --name="Brikko Studio Gateway"
    gcloud config set project my-brikko-studio-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create brikko-studio-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh brikko-studio-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the Brikko Studio repository">
    ```bash
  ...
  <Step title="Create persistent host directories">
    ...
    ```bash
    mkdir -p ~/.brikko-studio
    mkdir -p ~/.brikko-studio/workspace
    ```
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    BRIKKO_STUDIO_IMAGE=brikko-studio:latest
    BRIKKO_STUDIO_GATEWAY_TOKEN=
    BRIKKO_STUDIO_GATEWAY_BIND=lan
    BRIKKO_STUDIO_GATEWAY_PORT=18789

    BRIKKO_STUDIO_CONFIG_DIR=/home/$USER/.brikko-studio
    BRIKKO_STUDIO_WORKSPACE_DIR=/home/$USER/.brikko-studio/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.brikko-studio
    ```

    Leave `BRIKKO_STUDIO_GATEWAY_TOKEN` blank unless you explicitly want to
    manage it through `.env`; Brikko Studio writes a random gateway token to
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `BRIKKO_STUDIO_GATEWAY_TOKEN`.
    ...
    services:
      brikko-studio-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - BRIKKO_STUDIO_GATEWAY_BIND=${BRIKKO_STUDIO_GATEWAY_BIND}
          - BRIKKO_STUDIO_GATEWAY_PORT=${BRIKKO_STUDIO_GATEWAY_PORT}
          - BRIKKO_STUDIO_GATEWAY_TOKEN=${BRIKKO_STUDIO_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${BRIKKO_STUDIO_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`BRIKKO_STUDIO_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm brikko-studio-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh brikko-studio-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop brikko-studio-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type brikko-studio-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start brikko-studio-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create brikko-studio-deploy \
     --display-name="Brikko Studio Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-brikko-studio-project \
     --member="serviceAccount:brikko-studio-deploy@my-brikko-studio-project.iam.gserviceaccount.com" \
