...
import "@a2ui/lit/ui";
import { themeContext } from "@brikko-studio/a2ui-theme-context";
...
class Brikko StudioA2UIHost extends LitElement {
  ...
  themeProvider = new ContextProvider(this, {
    ...
    initialValue: brikko-studioTheme,
  });
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--brikko-studio-a2ui-inset-top, 0px)
        var(--brikko-studio-a2ui-inset-right, 0px)
        var(--brikko-studio-a2ui-inset-bottom, 0px)
        var(--brikko-studio-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--brikko-studio-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--brikko-studio-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--brikko-studio-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--brikko-studio-a2ui-empty-top, var(--brikko-studio-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.brikko-studioA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["brikko-studio:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__brikko-studioLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.brikko-studioCanvasA2UIAction ??
      globalThis.brikko-studioCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.brikko-studioCanvasA2UIAction) {
