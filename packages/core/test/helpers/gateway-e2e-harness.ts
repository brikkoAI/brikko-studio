...
import { createBrikko StudioTestInstance, type Brikko StudioTestInstance } from "./brikko-studio-test-instance.js";
...
export type GatewayInstance = Brikko StudioTestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createBrikko StudioTestInstance({ name });
  try {
