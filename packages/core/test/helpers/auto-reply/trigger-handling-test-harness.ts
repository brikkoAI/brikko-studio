...
const piEmbeddedMocks = getSharedMocks("brikko-studio.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("brikko-studio.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("brikko-studio.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("brikko-studio.trigger-handling.web-session-mocks", () => ({
...
type TempHomeEnvSnapshot = {
  ...
  brikko-studioHome: string | undefined;
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    brikko-studioHome: process.env.BRIKKO_STUDIO_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("BRIKKO_STUDIO_HOME", snapshot.brikko-studioHome);
  restoreKey("BRIKKO_STUDIO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.BRIKKO_STUDIO_STATE_DIR = join(home, ".brikko-studio");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "brikko-studio-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".brikko-studio", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
  ...
  return withFastReplyConfig({
    agents: {
      defaults: {
        ...
        workspace: join(home, "brikko-studio"),
