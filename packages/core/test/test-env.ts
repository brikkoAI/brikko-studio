...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.BRIKKO_STUDIO_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "BRIKKO_STUDIO_TEST_FAST", value: process.env.BRIKKO_STUDIO_TEST_FAST },
    {
      key: "BRIKKO_STUDIO_STRICT_FAST_REPLY_CONFIG",
      value: process.env.BRIKKO_STUDIO_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "BRIKKO_STUDIO_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.BRIKKO_STUDIO_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "BRIKKO_STUDIO_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.BRIKKO_STUDIO_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "BRIKKO_STUDIO_STATE_DIR", value: process.env.BRIKKO_STUDIO_STATE_DIR },
    { key: "BRIKKO_STUDIO_CONFIG_PATH", value: process.env.BRIKKO_STUDIO_CONFIG_PATH },
    { key: "BRIKKO_STUDIO_GATEWAY_PORT", value: process.env.BRIKKO_STUDIO_GATEWAY_PORT },
    { key: "BRIKKO_STUDIO_BRIDGE_ENABLED", value: process.env.BRIKKO_STUDIO_BRIDGE_ENABLED },
    { key: "BRIKKO_STUDIO_BRIDGE_HOST", value: process.env.BRIKKO_STUDIO_BRIDGE_HOST },
    { key: "BRIKKO_STUDIO_BRIDGE_PORT", value: process.env.BRIKKO_STUDIO_BRIDGE_PORT },
    { key: "BRIKKO_STUDIO_CANVAS_HOST_PORT", value: process.env.BRIKKO_STUDIO_CANVAS_HOST_PORT },
    { key: "BRIKKO_STUDIO_TEST_HOME", value: process.env.BRIKKO_STUDIO_TEST_HOME },
    { key: "BRIKKO_STUDIO_AGENT_DIR", value: process.env.BRIKKO_STUDIO_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.BRIKKO_STUDIO_TEST_HOME = tempHome;
  process.env.BRIKKO_STUDIO_TEST_FAST = "1";
  process.env.BRIKKO_STUDIO_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.BRIKKO_STUDIO_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.BRIKKO_STUDIO_BRIDGE_ENABLED;
  delete process.env.BRIKKO_STUDIO_BRIDGE_HOST;
  delete process.env.BRIKKO_STUDIO_BRIDGE_PORT;
  delete process.env.BRIKKO_STUDIO_CANVAS_HOST_PORT;
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.BRIKKO_STUDIO_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.BRIKKO_STUDIO_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".brikko-studio");
  const priorIsolatedHome = params.env.BRIKKO_STUDIO_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".brikko-studio")
  ) {
    realStateDir = path.join(params.realHome, ".brikko-studio");
  }
  const tempStateDir = path.join(params.tempHome, ".brikko-studio");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.BRIKKO_STUDIO_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.BRIKKO_STUDIO_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "brikko-studio.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "brikko-studio.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.BRIKKO_STUDIO_LIVE_TEST === "1" ||
    process.env.BRIKKO_STUDIO_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.BRIKKO_STUDIO_LIVE_USE_REAL_HOME);
