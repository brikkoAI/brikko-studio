...
} from "../scripts/brikko-studio-npm-postpublish-verify.ts";
...
describe("buildPublishedInstallScenarios", () => {
  it("uses a single fresh scenario for plain stable releases", () => {
    expect(buildPublishedInstallScenarios("2026.3.23")).toEqual([
      {
        name: "fresh-exact",
        installSpecs: ["brikko-studio@2026.3.23"],
  ...
  it("adds a stable-to-correction upgrade scenario for correction releases", () => {
    expect(buildPublishedInstallScenarios("2026.3.23-2")).toEqual([
      {
        name: "fresh-exact",
        installSpecs: ["brikko-studio@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
      {
        name: "upgrade-from-base-stable",
        installSpecs: ["brikko-studio@2026.3.23", "brikko-studio@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
    ]);
  });
...
describe("buildPublishedInstallCommandArgs", () => {
  it("runs lifecycle scripts for published install verification", () => {
    const args = buildPublishedInstallCommandArgs("/tmp/brikko-studio-prefix", "brikko-studio@2026.4.10");

    expect(args).toEqual([
      "install",
      "-g",
      "--prefix",
      "/tmp/brikko-studio-prefix",
      "brikko-studio@2026.4.10",
      "--no-fund",
      "--no-audit",
    ]);
...
describe("collectInstalledPackageErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "brikko-studio-postpublish-package-"));
  }
  ...
  it("flags version mismatches", () => {
    const errors = collectInstalledPackageErrors({
      ...
      packageRoot: "/tmp/empty-brikko-studio",
    });
...
describe("collectInstalledContextEngineRuntimeErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "brikko-studio-postpublish-context-engine-"));
  }
...
describe("resolveInstalledBinaryPath", () => {
  it("uses the Unix global bin path on non-Windows platforms", () => {
    expect(resolveInstalledBinaryPath("/tmp/brikko-studio-prefix", "darwin")).toBe(
      "/tmp/brikko-studio-prefix/bin/brikko-studio",
    );
  });

  it("uses the Windows npm shim path on win32", () => {
    expect(resolveInstalledBinaryPath("C:/brikko-studio-prefix", "win32")).toBe(
      "C:/brikko-studio-prefix/brikko-studio.cmd",
    );
  });
});
...
describe("collectInstalledRootDependencyManifestErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "brikko-studio-postpublish-root-deps-"));
  }
