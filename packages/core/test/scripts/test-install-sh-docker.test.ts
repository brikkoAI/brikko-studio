...
const RELEASE_CHECKS_WORKFLOW_PATH = ".github/workflows/brikko-studio-release-checks.yml";
const LIVE_E2E_WORKFLOW_PATH = ".github/workflows/brikko-studio-live-and-e2e-checks-reusable.yml";
...
describe("test-install-sh-docker", () => {
  ...
  it("supports npm update package specs without a separate expected-version env", () => {
    ...
    expect(script).toContain(
      'UPDATE_EXPECT_VERSION="${BRIKKO_STUDIO_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"',
    );
  ...
  it("uses npm latest as the update baseline and resolves it to the concrete packed version", () => {
    ...
    expect(script).toContain(
      'UPDATE_BASELINE_VERSION="${BRIKKO_STUDIO_INSTALL_SMOKE_UPDATE_BASELINE:-latest}"',
    );
    ...
    expect(runner).toContain(
      'UPDATE_BASELINE_VERSION="${BRIKKO_STUDIO_INSTALL_UPDATE_BASELINE:-latest}"',
    );
    ...
    expect(workflow).toContain(
      "BRIKKO_STUDIO_INSTALL_SMOKE_UPDATE_BASELINE: ${{ inputs.update_baseline_version || 'latest' }}",
    );
  });
  ...
  it("can reuse dist from the already-built root Docker smoke image", () => {
    ...
    expect(script).toContain('UPDATE_DIST_IMAGE="${BRIKKO_STUDIO_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"');
  ...
  it("writes the package dist inventory before packing ignore-scripts tarballs", () => {
    ...
    expect(script).toContain("node scripts/check-brikko-studio-package-tarball.mjs");
  });
});
...
describe("install-sh smoke runner", () => {
  it("wraps long npm/update operations with heartbeat and install-size audits", () => {
    ...
    expect(script).toContain(
      'HEARTBEAT_INTERVAL="${BRIKKO_STUDIO_INSTALL_SMOKE_HEARTBEAT_INTERVAL:-60}"',
    );
    expect(script).toContain(
      'INSTALL_COMMAND_TIMEOUT="${BRIKKO_STUDIO_INSTALL_SMOKE_COMMAND_TIMEOUT:-900}"',
    );
    ...
    expect(script).toContain("brikko-studio update --tag");
  ...
  it("covers plain npm global installs and npm-driven updates", () => {
    ...
    expect(script).toContain('SKIP_NPM_GLOBAL="${BRIKKO_STUDIO_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"');
    expect(script).toContain('NPM_CACHE_DIR="${BRIKKO_STUDIO_INSTALL_SMOKE_NPM_CACHE_DIR:-}"');
    ...
    expect(script).toContain("BRIKKO_STUDIO_INSTALL_SMOKE_MODE=npm-global");
...
describe("bun global install smoke", () => {
  it("packs the current tree and verifies image-provider discovery through Bun", () => {
    ...
    expect(script).toContain("BRIKKO_STUDIO_BUN_GLOBAL_SMOKE_DIST_IMAGE");
  });
  ...
  it("gates workflow Bun install smoke to scheduled and release-check runs", () => {
    ...
    expect(workflow).toContain(
      "BRIKKO_STUDIO_BUN_GLOBAL_SMOKE_DIST_IMAGE: ${{ needs.root_dockerfile_image.outputs.image_ref }}",
    );
    expect(workflow).toContain(
    ...
    expect(workflow).toContain("BRIKKO_STUDIO_CI_WORKFLOW_BUN_GLOBAL_INSTALL_SMOKE");
    ...
    expect(workflow).toContain('BRIKKO_STUDIO_INSTALL_SMOKE_SKIP_NPM_GLOBAL: "1"');
