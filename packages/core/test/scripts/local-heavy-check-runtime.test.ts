...
function makeEnv(overrides: Record<string, string | undefined> = {}) {
  const env = {
    ...process.env,
    BRIKKO_STUDIO_LOCAL_CHECK: "1",
    ...overrides,
  };
  if (!Object.hasOwn(overrides, "BRIKKO_STUDIO_LOCAL_CHECK_MODE")) {
    delete env.BRIKKO_STUDIO_LOCAL_CHECK_MODE;
  }
  return env;
}
...
describe("local-heavy-check-runtime", () => {
  it("reenables local heavy-check policy for local wrapper entrypoints", () => {
    expect(resolveLocalHeavyCheckEnv({ BRIKKO_STUDIO_LOCAL_CHECK: "0", PATH: "/usr/bin" })).toEqual({
      BRIKKO_STUDIO_LOCAL_CHECK: "1",
      PATH: "/usr/bin",
    });
    expect(resolveLocalHeavyCheckEnv({ BRIKKO_STUDIO_LOCAL_CHECK: "false", PATH: "/usr/bin" })).toEqual({
      BRIKKO_STUDIO_LOCAL_CHECK: "1",
      PATH: "/usr/bin",
    });
  });
  ...
  it("skips declaration transforms for no-emit tsgo checks", () => {
    const { args } = applyLocalTsgoPolicy([], makeEnv({ BRIKKO_STUDIO_LOCAL_CHECK: "0" }), ROOMY_HOST);
  ...
  it("keeps explicit tsgo flags and Go env overrides intact when throttled", () => {
    const { args, env } = applyLocalTsgoPolicy(
      ...
      makeEnv({
        GOGC: "80",
        GOMEMLIMIT: "5GiB",
        BRIKKO_STUDIO_TSGO_PPROF_DIR: "/tmp/profile",
      }),
      CONSTRAINED_HOST,
    );
  ...
  it("keeps explicit tsgo declaration flags intact", () => {
    const env = makeEnv({ BRIKKO_STUDIO_LOCAL_CHECK_MODE: "full" });
  ...
  it("uses the configured local tsgo build info file", () => {
    const { args } = applyLocalTsgoPolicy(
      [],
      makeEnv({
        BRIKKO_STUDIO_LOCAL_CHECK_MODE: "full",
        BRIKKO_STUDIO_TSGO_BUILD_INFO_FILE: ".artifacts/custom/tsgo.tsbuildinfo",
      }),
      ROOMY_HOST,
    );
  ...
  it("avoids incremental cache reuse for ad hoc tsgo runs", () => {
    const { args } = applyLocalTsgoPolicy(
      ["--extendedDiagnostics"],
      makeEnv({ BRIKKO_STUDIO_LOCAL_CHECK_MODE: "full" }),
      ROOMY_HOST,
    );
  ...
  it("allows forcing the throttled tsgo policy on roomy hosts", () => {
    const { args, env } = applyLocalTsgoPolicy(
      [],
      makeEnv({
        BRIKKO_STUDIO_LOCAL_CHECK_MODE: "throttled",
      }),
      ROOMY_HOST,
    );
  ...
  it("allows forcing the tsgo lock back on", () => {
    expect(
      shouldAcquireLocalHeavyCheckLockForTsgo(
        ["--help"],
        makeEnv({ BRIKKO_STUDIO_TSGO_FORCE_LOCK: "1" }),
      ),
    ).toBe(true);
  });
  ...
  it("skips the heavy-check lock for explicit oxlint file targets", () => {
    const cwd = createTempDir("brikko-studio-oxlint-lock-skip-");
  ...
  it("keeps the heavy-check lock for directory targets and broad oxlint runs", () => {
    const cwd = createTempDir("brikko-studio-oxlint-lock-keep-");
  ...
  it("allows forcing the oxlint lock back on", () => {
    const cwd = createTempDir("brikko-studio-oxlint-lock-force-");
    ...
    expect(
      shouldAcquireLocalHeavyCheckLockForOxlint(["--type-aware", "--", "sample.ts"], {
        cwd,
        env: makeEnv({ BRIKKO_STUDIO_OXLINT_FORCE_LOCK: "1" }),
      }),
    ).toBe(true);
  });

  it("reclaims stale local heavy-check locks from dead pids", () => {
    const cwd = createTempDir("brikko-studio-local-heavy-check-");
    const commonDir = path.join(cwd, ".git");
    const lockDir = path.join(commonDir, "brikko-studio-local-checks", "heavy-check.lock");
  ...
  it("cleans up stale legacy test locks when acquiring the shared heavy-check lock", () => {
    const cwd = createTempDir("brikko-studio-local-heavy-check-legacy-");
    const commonDir = path.join(cwd, ".git");
    const locksDir = path.join(commonDir, "brikko-studio-local-checks");
