...
function hasWorkerOverride(env: Record<string, string | undefined>): boolean {
  return Boolean((env.BRIKKO_STUDIO_VITEST_MAX_WORKERS ?? env.BRIKKO_STUDIO_TEST_WORKERS)?.trim());
}
...
export const sharedVitestConfig = {
  ...
  resolve: {
    alias: [
      {
        find: "brikko-studio/extension-api",
        replacement: path.join(repoRoot, "src", "extensionAPI.ts"),
      },
      {
        find: "@brikko-studio/qa-channel/api.js",
        replacement: path.join(repoRoot, "extensions", "qa-channel", "api.ts"),
      },
      {
        find: "@brikko-studio/discord/api.js",
