...
export function loadIncludePatternsFromEnv(
  ...
  return loadPatternListFromEnv("BRIKKO_STUDIO_VITEST_INCLUDE_FILE", env);
}

export function loadExtraExcludePatternsFromEnv(
  env: Record<string, string | undefined> = process.env,
): string[] {
  return loadPatternListFromEnv("BRIKKO_STUDIO_VITEST_EXTRA_EXCLUDE_FILE", env) ?? [];
}
...
export function createUnitVitestConfigWithOptions(
  ...
  return defineConfig({
    ...
    test: {
      ...
      setupFiles: [
        ...new Set(
          [...(sharedTest.setupFiles ?? []), "test/setup-brikko-studio-runtime.ts"].map(
            resolveRepoRootPath,
          ),
        ),
      ],
