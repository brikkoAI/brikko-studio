...
export function loadVitestExperimentalConfig(
  ...
  if (!windowsEnv && !isDisabled(env.BRIKKO_STUDIO_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (windowsEnv && isEnabled(env.BRIKKO_STUDIO_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (experimental.fsModuleCache && env.BRIKKO_STUDIO_VITEST_FS_MODULE_CACHE_PATH?.trim()) {
    experimental.fsModuleCachePath = env.BRIKKO_STUDIO_VITEST_FS_MODULE_CACHE_PATH.trim();
  }
  if (isEnabled(env.BRIKKO_STUDIO_VITEST_IMPORT_DURATIONS)) {
    experimental.importDurations = { print: true };
  }
  if (isEnabled(env.BRIKKO_STUDIO_VITEST_PRINT_IMPORT_BREAKDOWN)) {
