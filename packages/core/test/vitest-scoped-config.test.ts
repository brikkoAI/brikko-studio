...
import { BUNDLED_PLUGIN_TEST_GLOB, bundledPluginFile } from "brikko-studio/plugin-sdk/test-fixtures";
...
describe("resolveVitestIsolation", () => {
      ...
      expect.arrayContaining(
        PRIVATE_PLUGIN_SDK_SUBPATHS.map((subpath) =>
          expect.objectContaining({
            find: `brikko-studio/plugin-sdk/${subpath}`,
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ BRIKKO_STUDIO_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ BRIKKO_STUDIO_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ BRIKKO_STUDIO_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
...
describe("createScopedVitestConfig", () => {
  it("applies the non-isolated runner by default", () => {
    ...
    expect(normalizeConfigPaths(config.test?.setupFiles)).toEqual([
      "test/setup.ts",
      "test/setup-brikko-studio-runtime.ts",
    ]);
  });
  ...
  it("loads scoped include overrides from BRIKKO_STUDIO_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-vitest-scoped-"));
    try {
...
describe("scoped vitest configs", () => {
  ...
  it("keeps the process lane off the brikko-studio runtime setup", () => {
  ...
  it("keeps selected plugin-sdk and commands light lanes off the brikko-studio runtime setup", () => {
  ...
  it("keeps the ui lane off both the brikko-studio runtime setup and unit-fast excludes", () => {
  ...
  it("loads channel include overrides from BRIKKO_STUDIO_VITEST_INCLUDE_FILE", () => {
    const tempDirs: string[] = [];
    const tempDir = makeTempDir(tempDirs, "brikko-studio-vitest-channels-");
    try {
      const includeFile = path.join(tempDir, "include.json");
      fs.writeFileSync(
        includeFile,
