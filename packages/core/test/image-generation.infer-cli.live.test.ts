...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.BRIKKO_STUDIO_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through brikko-studio infer", () => {
    const outputBase = path.join(os.tmpdir(), `brikko-studio-infer-image-${process.pid}.png`);
