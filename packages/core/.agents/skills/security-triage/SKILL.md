...
description: Triage Brikko Studio security advisories, drafts, and GHSA reports with shipped-tag and trust-model proof.
---

# Security Triage

Use when reviewing Brikko Studio security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/brikko-studio/brikko-studio/security-advisories/<GHSA>`.
3. Inspect the exact implicated code paths.
4. Verify shipped state:
   - `git tag --sort=-creatordate | head`
   - `npm view brikko-studio version --userconfig "$(mktemp)"`
...
```bash
gh api /repos/brikko-studio/brikko-studio/security-advisories/<GHSA>
gh api /repos/brikko-studio/brikko-studio/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view brikko-studio version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo brikko-studio/brikko-studio --match title,body,comments -- "<terms>"
gh search prs --repo brikko-studio/brikko-studio --match title,body,comments -- "<terms>"
```
