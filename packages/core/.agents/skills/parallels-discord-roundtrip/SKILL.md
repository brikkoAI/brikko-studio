...
```bash
export BRIKKO_STUDIO_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.brikko-studio/brikko-studio.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env BRIKKO_STUDIO_PARALLELS_DISCORD_TOKEN \
...
- Use the `brikko-studio` wrapper for guest `message send/read`; `node brikko-studio.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/brikko-studio-parallels-smoke.*`
...
- host inbound nonce appears in `brikko-studio message read` output
