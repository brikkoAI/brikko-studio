...
function ensureBrikko StudioExtensionAlias(params) {
  const pluginSdkDir = path.join(params.repoRoot, "dist", "plugin-sdk");
  if (!fs.existsSync(pluginSdkDir)) {
    return;
  }

  const aliasDir = path.join(params.distExtensionsRoot, "node_modules", "brikko-studio");
...
function shouldCopyRuntimeFile(relativePath) {
  return (
    ...
    isPathOrNestedPath(relativePath, "brikko-studio.plugin.json") ||
...
export function stageBundledPluginRuntime(params = {}) {
  ...
  ensureBrikko StudioExtensionAlias({ repoRoot, distExtensionsRoot });
