...
const ROOT_DIR = path.resolve(process.env.BRIKKO_STUDIO_DOCKER_E2E_REPO_ROOT || SCRIPT_ROOT_DIR);
...
const DEFAULT_GITHUB_WORKFLOW = "brikko-studio-live-and-e2e-checks-reusable.yml";
...
function resourceLimitEnvName(resource) {
  return `BRIKKO_STUDIO_DOCKER_ALL_${resource.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_LIMIT`;
}
...
function parseSchedulerOptions(env, parallelism) {
  const weightLimit = parsePositiveInt(
    env.BRIKKO_STUDIO_DOCKER_ALL_WEIGHT_LIMIT,
    parallelism,
    "BRIKKO_STUDIO_DOCKER_ALL_WEIGHT_LIMIT",
  );
...
function appendExtension(env, extension) {
  const current = env.BRIKKO_STUDIO_DOCKER_BUILD_EXTENSIONS ?? env.BRIKKO_STUDIO_EXTENSIONS ?? "";
  const tokens = current.split(/\s+/).filter(Boolean);
  if (!tokens.includes(extension)) {
    tokens.push(extension);
  }
  env.BRIKKO_STUDIO_DOCKER_BUILD_EXTENSIONS = tokens.join(" ");
}
...
function githubWorkflowRef() {
  const explicit = process.env.BRIKKO_STUDIO_DOCKER_E2E_WORKFLOW_REF;
  if (explicit) {
    return explicit;
  }
...
function githubWorkflowRerunCommand(laneNames, ref) {
  ...
  const releasePath = process.env.BRIKKO_STUDIO_DOCKER_ALL_PROFILE === RELEASE_PATH_PROFILE;
  const fields = [
    "gh workflow run",
    shellQuote(process.env.BRIKKO_STUDIO_DOCKER_E2E_WORKFLOW || DEFAULT_GITHUB_WORKFLOW),
  ...
  if (process.env.GITHUB_RUN_ID) {
    ...
    fields.push(
      ...
      `package_artifact_name=${shellQuote(
        process.env.BRIKKO_STUDIO_DOCKER_E2E_PACKAGE_ARTIFACT_NAME || "docker-e2e-package",
      )}`,
    );
  }
  if (process.env.BRIKKO_STUDIO_UPGRADE_SURVIVOR_BASELINE_SPEC) {
    fields.push(
      "-f",
      `published_upgrade_survivor_baseline=${shellQuote(process.env.BRIKKO_STUDIO_UPGRADE_SURVIVOR_BASELINE_SPEC)}`,
    );
  }
  if (process.env.BRIKKO_STUDIO_UPGRADE_SURVIVOR_BASELINE_SPECS) {
    fields.push(
      "-f",
      `published_upgrade_survivor_baselines=${shellQuote(process.env.BRIKKO_STUDIO_UPGRADE_SURVIVOR_BASELINE_SPECS)}`,
    );
  }
  if (process.env.BRIKKO_STUDIO_UPGRADE_SURVIVOR_SCENARIOS) {
    fields.push(
      "-f",
      `published_upgrade_survivor_scenarios=${shellQuote(process.env.BRIKKO_STUDIO_UPGRADE_SURVIVOR_SCENARIOS)}`,
    );
  }
  if (process.env.BRIKKO_STUDIO_DOCKER_E2E_BARE_IMAGE) {
    fields.push(
      "-f",
      `docker_e2e_bare_image=${shellQuote(process.env.BRIKKO_STUDIO_DOCKER_E2E_BARE_IMAGE)}`,
    );
  }
  if (process.env.BRIKKO_STUDIO_DOCKER_E2E_FUNCTIONAL_IMAGE) {
    fields.push(
      "-f",
      `docker_e2e_functional_image=${shellQuote(process.env.BRIKKO_STUDIO_DOCKER_E2E_FUNCTIONAL_IMAGE)}`,
    );
  }
  return fields.join(" ");
}
...
function buildLaneRerunCommand(name, baseEnv) {
  ...
  const image = poolLane ? e2eImageForLane(poolLane, baseEnv) : baseEnv.BRIKKO_STUDIO_DOCKER_E2E_IMAGE;
  const env = [
    ["BRIKKO_STUDIO_DOCKER_ALL_LANES", name],
    ["BRIKKO_STUDIO_DOCKER_ALL_BUILD", build],
    ["BRIKKO_STUDIO_DOCKER_ALL_PREFLIGHT", "0"],
    ["BRIKKO_STUDIO_SKIP_DOCKER_BUILD", "1"],
    ["BRIKKO_STUDIO_DOCKER_E2E_IMAGE", image || DEFAULT_E2E_IMAGE],
    ["BRIKKO_STUDIO_DOCKER_E2E_BARE_IMAGE", baseEnv.BRIKKO_STUDIO_DOCKER_E2E_BARE_IMAGE],
    ["BRIKKO_STUDIO_DOCKER_E2E_FUNCTIONAL_IMAGE", baseEnv.BRIKKO_STUDIO_DOCKER_E2E_FUNCTIONAL_IMAGE],
    ["BRIKKO_STUDIO_CURRENT_PACKAGE_TGZ", baseEnv.BRIKKO_STUDIO_CURRENT_PACKAGE_TGZ],
    ["BRIKKO_STUDIO_UPGRADE_SURVIVOR_BASELINE_SPEC", baseEnv.BRIKKO_STUDIO_UPGRADE_SURVIVOR_BASELINE_SPEC],
    ["BRIKKO_STUDIO_UPGRADE_SURVIVOR_BASELINE_SPECS", baseEnv.BRIKKO_STUDIO_UPGRADE_SURVIVOR_BASELINE_SPECS],
    ["BRIKKO_STUDIO_UPGRADE_SURVIVOR_SCENARIOS", baseEnv.BRIKKO_STUDIO_UPGRADE_SURVIVOR_SCENARIOS],
  ];
  if (baseEnv.BRIKKO_STUDIO_DOCKER_ALL_PNPM_COMMAND) {
    env.push(["BRIKKO_STUDIO_DOCKER_ALL_PNPM_COMMAND", baseEnv.BRIKKO_STUDIO_DOCKER_ALL_PNPM_COMMAND]);
  }
  return `${env
...
function withResolvedPnpmCommand(command, env) {
  const pnpmCommand = env.BRIKKO_STUDIO_DOCKER_ALL_PNPM_COMMAND?.trim();
  if (!pnpmCommand) {
    return command;
  }
...
function liveDockerHarnessScriptCommand(script) {
  return `bash -c 'harness="\${BRIKKO_STUDIO_DOCKER_E2E_TRUSTED_HARNESS_DIR:-}"; if [ -z "$harness" ]; then if [ -d .release-harness/scripts ]; then harness=.release-harness; else harness=.; fi; fi; BRIKKO_STUDIO_LIVE_DOCKER_REPO_ROOT="\${BRIKKO_STUDIO_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/${script}"'`;
}
...
async function writeRunSummary(logDir, summary) {
  ...
  const payload = {
    ...summary,
    packageArtifactName: process.env.BRIKKO_STUDIO_DOCKER_E2E_PACKAGE_ARTIFACT_NAME || undefined,
    ...
    github: {
      ...
      runUrl:
        process.env.GITHUB_SERVER_URL && process.env.GITHUB_REPOSITORY && process.env.GITHUB_RUN_ID
          ...
          : undefined,
      selectedSha: process.env.BRIKKO_STUDIO_DOCKER_E2E_SELECTED_SHA || undefined,
...
async function writeFailureIndex(logDir, summary) {
  const ref =
    ...
    process.env.BRIKKO_STUDIO_DOCKER_E2E_SELECTED_SHA ||
    summary.github?.sha ||
    summary.github?.ref ||
  ...
  const failureIndex = {
    ...
    version: 1,
    workflow: process.env.BRIKKO_STUDIO_DOCKER_E2E_WORKFLOW || DEFAULT_GITHUB_WORKFLOW,
  };
...
function dockerPreflightContainerNames(raw) {
  return raw
    ...
    .filter((name) =>
      /^(?:brikko-studio-(?:gateway-e2e|openwebui|openwebui-gateway|config-reload-e2e)-)/.test(name),
    );
}
...
async function prepareBrikko StudioPackage(baseEnv, logDir) {
  ...
  const packDir = path.join(logDir, "brikko-studio-package");
...
function e2eImageForLane(poolLane, baseEnv) {
  if (poolLane.e2eImageKind === "bare") {
    return baseEnv.BRIKKO_STUDIO_DOCKER_E2E_BARE_IMAGE;
  }
  if (poolLane.e2eImageKind === "functional") {
    return baseEnv.BRIKKO_STUDIO_DOCKER_E2E_FUNCTIONAL_IMAGE;
  }
  return undefined;
}
...
function laneEnv(poolLane, baseEnv, logDir, cacheKey) {
  ...
  env.BRIKKO_STUDIO_DOCKER_ALL_LANE_NAME = name;
  const image = e2eImageForLane(poolLane, baseEnv);
  if (image) {
    env.BRIKKO_STUDIO_DOCKER_E2E_IMAGE = image;
  }
  if (poolLane.e2eImageKind) {
    env.BRIKKO_STUDIO_DOCKER_E2E_IMAGE_KIND = poolLane.e2eImageKind;
  }
  const cacheName = cacheKey || name;
  if (!process.env.BRIKKO_STUDIO_DOCKER_CLI_TOOLS_DIR) {
    env.BRIKKO_STUDIO_DOCKER_CLI_TOOLS_DIR = path.join(logDir, `${cacheName}-cli-tools`);
  }
  if (!process.env.BRIKKO_STUDIO_DOCKER_CACHE_HOME_DIR) {
    env.BRIKKO_STUDIO_DOCKER_CACHE_HOME_DIR = path.join(logDir, `${cacheName}-cache`);
  }
  return env;
}
...
async function runLane(lane, baseEnv, logDir, fallbackTimeoutMs) {
  ...
  await mkdir(env.BRIKKO_STUDIO_DOCKER_CLI_TOOLS_DIR, { recursive: true });
  await mkdir(env.BRIKKO_STUDIO_DOCKER_CACHE_HOME_DIR, { recursive: true });
  await fs.promises.writeFile(
    logFile,
    [
      `==> [${name}] cli tools dir: ${env.BRIKKO_STUDIO_DOCKER_CLI_TOOLS_DIR}`,
      `==> [${name}] cache dir: ${env.BRIKKO_STUDIO_DOCKER_CACHE_HOME_DIR}`,
      ...
      `==> [${name}] e2e image: ${env.BRIKKO_STUDIO_DOCKER_E2E_IMAGE ?? ""}`,
      "",
    ].join("\n"),
  );
  ...
  return {
    ...
    attempts,
    finishedAt: new Date().toISOString(),
    image: env.BRIKKO_STUDIO_DOCKER_E2E_IMAGE,
      ...
      if (running.size === 0) {
        ...
        throw new Error(
          `No Docker lanes fit scheduler limits (${describeDockerSchedulerLimits(
            parallelism,
            options,
          )}): ${blocked}. Tune BRIKKO_STUDIO_DOCKER_ALL_PARALLELISM, BRIKKO_STUDIO_DOCKER_ALL_WEIGHT_LIMIT, or BRIKKO_STUDIO_DOCKER_ALL_<RESOURCE>_LIMIT.`,
        );
      }
...
async function main() {
  ...
  const parallelism = parsePositiveInt(
    process.env.BRIKKO_STUDIO_DOCKER_ALL_PARALLELISM,
    DEFAULT_PARALLELISM,
    "BRIKKO_STUDIO_DOCKER_ALL_PARALLELISM",
  );
  const tailParallelism = parsePositiveInt(
    process.env.BRIKKO_STUDIO_DOCKER_ALL_TAIL_PARALLELISM,
    Math.min(parallelism, DEFAULT_TAIL_PARALLELISM),
    "BRIKKO_STUDIO_DOCKER_ALL_TAIL_PARALLELISM",
  );
  const tailLines = parsePositiveInt(
    process.env.BRIKKO_STUDIO_DOCKER_ALL_FAILURE_TAIL_LINES,
    DEFAULT_FAILURE_TAIL_LINES,
    "BRIKKO_STUDIO_DOCKER_ALL_FAILURE_TAIL_LINES",
  );
  const laneTimeoutMs = parsePositiveInt(
    process.env.BRIKKO_STUDIO_DOCKER_ALL_LANE_TIMEOUT_MS,
    DEFAULT_LANE_TIMEOUT_MS,
    "BRIKKO_STUDIO_DOCKER_ALL_LANE_TIMEOUT_MS",
  );
  const laneStartStaggerMs = parseNonNegativeInt(
    process.env.BRIKKO_STUDIO_DOCKER_ALL_START_STAGGER_MS,
    DEFAULT_LANE_START_STAGGER_MS,
    "BRIKKO_STUDIO_DOCKER_ALL_START_STAGGER_MS",
  );
  const statusIntervalMs = parseNonNegativeInt(
    process.env.BRIKKO_STUDIO_DOCKER_ALL_STATUS_INTERVAL_MS,
    DEFAULT_STATUS_INTERVAL_MS,
    "BRIKKO_STUDIO_DOCKER_ALL_STATUS_INTERVAL_MS",
  );
  const preflightRunTimeoutMs = parsePositiveInt(
    process.env.BRIKKO_STUDIO_DOCKER_ALL_PREFLIGHT_RUN_TIMEOUT_MS,
    DEFAULT_PREFLIGHT_RUN_TIMEOUT_MS,
    "BRIKKO_STUDIO_DOCKER_ALL_PREFLIGHT_RUN_TIMEOUT_MS",
  );
  const failFast = parseBool(process.env.BRIKKO_STUDIO_DOCKER_ALL_FAIL_FAST, true);
  const dryRun = parseBool(process.env.BRIKKO_STUDIO_DOCKER_ALL_DRY_RUN, false);
  const preflightEnabled = parseBool(process.env.BRIKKO_STUDIO_DOCKER_ALL_PREFLIGHT, true);
  const preflightCleanup = parseBool(process.env.BRIKKO_STUDIO_DOCKER_ALL_PREFLIGHT_CLEANUP, true);
  const timingsEnabled = parseBool(process.env.BRIKKO_STUDIO_DOCKER_ALL_TIMINGS, true);
  const buildEnabled = parseBool(process.env.BRIKKO_STUDIO_DOCKER_ALL_BUILD, true);
  const planJson =
    cliArgs.has("--plan-json") || parseBool(process.env.BRIKKO_STUDIO_DOCKER_ALL_PLAN_JSON, false);
  const planReleaseAll = parseBool(process.env.BRIKKO_STUDIO_DOCKER_ALL_PLAN_RELEASE_ALL, false);
  const profile = parseProfile(process.env.BRIKKO_STUDIO_DOCKER_ALL_PROFILE);
  const releaseChunk = process.env.BRIKKO_STUDIO_DOCKER_ALL_CHUNK || process.env.DOCKER_E2E_CHUNK || "";
  const includeOpenWebUI = parseBool(
    process.env.BRIKKO_STUDIO_DOCKER_ALL_INCLUDE_OPENWEBUI ?? process.env.INCLUDE_OPENWEBUI,
    true,
  );
  const selectedLaneNamesRaw =
    process.env.BRIKKO_STUDIO_DOCKER_ALL_LANES || process.env.DOCKER_E2E_LANES || "";
  const selectedLaneNames = parseLaneSelection(selectedLaneNamesRaw);
  if (selectedLaneNamesRaw && selectedLaneNames.length === 0) {
    throw new Error("BRIKKO_STUDIO_DOCKER_ALL_LANES must include at least one lane name");
  }
  const liveMode = parseLiveMode(process.env.BRIKKO_STUDIO_DOCKER_ALL_LIVE_MODE);
  const liveRetries = parseNonNegativeInt(
    process.env.BRIKKO_STUDIO_DOCKER_ALL_LIVE_RETRIES,
    DEFAULT_LIVE_RETRIES,
    "BRIKKO_STUDIO_DOCKER_ALL_LIVE_RETRIES",
  );
  const timingsFile = path.resolve(
    process.env.BRIKKO_STUDIO_DOCKER_ALL_TIMINGS_FILE || DEFAULT_TIMINGS_FILE,
  );
  const runId = process.env.BRIKKO_STUDIO_DOCKER_ALL_RUN_ID || utcStampForPath();
  const logDir = path.resolve(
    process.env.BRIKKO_STUDIO_DOCKER_ALL_LOG_DIR ||
      path.join(ROOT_DIR, ".artifacts/docker-tests", runId),
  );

  const baseEnv = commandEnv({
    BRIKKO_STUDIO_DOCKER_E2E_BARE_IMAGE:
      process.env.BRIKKO_STUDIO_DOCKER_E2E_BARE_IMAGE ||
      process.env.BRIKKO_STUDIO_DOCKER_E2E_IMAGE ||
      DEFAULT_E2E_BARE_IMAGE,
    BRIKKO_STUDIO_DOCKER_E2E_FUNCTIONAL_IMAGE:
      process.env.BRIKKO_STUDIO_DOCKER_E2E_FUNCTIONAL_IMAGE ||
      process.env.BRIKKO_STUDIO_DOCKER_E2E_IMAGE ||
      DEFAULT_E2E_FUNCTIONAL_IMAGE,
  });
  baseEnv.BRIKKO_STUDIO_DOCKER_E2E_IMAGE =
    process.env.BRIKKO_STUDIO_DOCKER_E2E_IMAGE || baseEnv.BRIKKO_STUDIO_DOCKER_E2E_FUNCTIONAL_IMAGE;
  ...
  const { orderedLanes, orderedTailLanes, plan, scheduledLanes } = resolveDockerE2ePlan({
    ...
    selectedLaneNames,
    timingStore,
    upgradeSurvivorBaselines: process.env.BRIKKO_STUDIO_UPGRADE_SURVIVOR_BASELINE_SPECS,
    upgradeSurvivorScenarios: process.env.BRIKKO_STUDIO_UPGRADE_SURVIVOR_SCENARIOS,
  });
  ...
  console.log(`==> Docker E2E bare image: ${baseEnv.BRIKKO_STUDIO_DOCKER_E2E_BARE_IMAGE}`);
  console.log(`==> Docker E2E functional image: ${baseEnv.BRIKKO_STUDIO_DOCKER_E2E_FUNCTIONAL_IMAGE}`);
  ...
  console.log(`==> Live-test bundled plugins: ${baseEnv.BRIKKO_STUDIO_DOCKER_BUILD_EXTENSIONS}`);
  ...
  if (buildEnabled) {
    ...
    if (lanesNeedE2eImageKind(scheduledLanes, "bare")) {
      buildEntries.push({
        ...
        env: {
          BRIKKO_STUDIO_DOCKER_E2E_IMAGE: baseEnv.BRIKKO_STUDIO_DOCKER_E2E_BARE_IMAGE,
          BRIKKO_STUDIO_DOCKER_E2E_TARGET: "bare",
        },
        label: `shared bare Docker E2E image once: ${baseEnv.BRIKKO_STUDIO_DOCKER_E2E_BARE_IMAGE}`,
        phaseDetails: { image: baseEnv.BRIKKO_STUDIO_DOCKER_E2E_BARE_IMAGE, imageKind: "bare" },
        phases,
      });
    }
    if (lanesNeedE2eImageKind(scheduledLanes, "functional")) {
      buildEntries.push({
        command: "pnpm test:docker:e2e-build",
        env: {
          BRIKKO_STUDIO_DOCKER_E2E_IMAGE: baseEnv.BRIKKO_STUDIO_DOCKER_E2E_FUNCTIONAL_IMAGE,
          BRIKKO_STUDIO_DOCKER_E2E_TARGET: "functional",
        },
        label: `shared functional Docker E2E image once: ${baseEnv.BRIKKO_STUDIO_DOCKER_E2E_FUNCTIONAL_IMAGE}`,
        phaseDetails: {
          image: baseEnv.BRIKKO_STUDIO_DOCKER_E2E_FUNCTIONAL_IMAGE,
  ...
  if (failFast && failures.length > 0) {
    await writeRunSummary(logDir, {
      ...
      failures,
      image: baseEnv.BRIKKO_STUDIO_DOCKER_E2E_IMAGE,
      images: {
        bare: baseEnv.BRIKKO_STUDIO_DOCKER_E2E_BARE_IMAGE,
        functional: baseEnv.BRIKKO_STUDIO_DOCKER_E2E_FUNCTIONAL_IMAGE,
      },
      lanes: allResults,
      phases,
      profile,
