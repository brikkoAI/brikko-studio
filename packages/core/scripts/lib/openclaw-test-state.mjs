...
function usage() {
  return `Usage:
  node scripts/lib/brikko-studio-test-state.mjs -- create [--label <name>] [--scenario <name>] [--env-file <path>] [--json]
  node scripts/lib/brikko-studio-test-state.mjs shell [--label <name>] [--scenario <name>]
  node scripts/lib/brikko-studio-test-state.mjs shell-function
...
function scenarioConfig(scenario, options = {}) {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      skills: {
        allowBundled: ["memory", "brikko-studio-testing"],
        limits: {
          maxSkillsInPrompt: 8,
  ...
  if (scenario === "gateway-loopback") {
    return {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.token || "brikko-studio-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function renderConfigWrite(configPathExpression, config) {
  ...
  return [
    `cat > ${configPathExpression} <<'BRIKKO_STUDIO_TEST_STATE_JSON'`,
    json,
    "BRIKKO_STUDIO_TEST_STATE_JSON",
  ].join("\n");
}
...
function buildCreatePlan(options = {}) {
  ...
  const env = {
    ...
    BRIKKO_STUDIO_CONFIG_PATH: configPath,
...
export async function createState(options = {}) {
  const label = normalizeLabel(options.label);
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `brikko-studio-${label}-`));
...
export function renderShellSnippet(options = {}) {
  ...
  const homeTemplate = `brikko-studio-${label}-${scenario}-home.XXXXXX`;
  const lines = [
    'BRIKKO_STUDIO_TEST_STATE_TMP_ROOT="${BRIKKO_STUDIO_TEST_STATE_TMPDIR:-${TMPDIR:-/tmp}}"',
    "export BRIKKO_STUDIO_TEST_STATE_TMP_ROOT",
    'mkdir -p "$BRIKKO_STUDIO_TEST_STATE_TMP_ROOT"',
    `BRIKKO_STUDIO_TEST_STATE_HOME="$(mktemp -d "$BRIKKO_STUDIO_TEST_STATE_TMP_ROOT/${homeTemplate}")"`,
    'export HOME="$BRIKKO_STUDIO_TEST_STATE_HOME"',
    'export USERPROFILE="$BRIKKO_STUDIO_TEST_STATE_HOME"',
    'export BRIKKO_STUDIO_HOME="$BRIKKO_STUDIO_TEST_STATE_HOME"',
    'export BRIKKO_STUDIO_STATE_DIR="$BRIKKO_STUDIO_TEST_STATE_HOME/.brikko-studio"',
    'export BRIKKO_STUDIO_CONFIG_PATH="$BRIKKO_STUDIO_STATE_DIR/brikko-studio.json"',
    'export BRIKKO_STUDIO_TEST_WORKSPACE_DIR="$BRIKKO_STUDIO_TEST_STATE_HOME/workspace"',
    'mkdir -p "$BRIKKO_STUDIO_STATE_DIR" "$BRIKKO_STUDIO_TEST_WORKSPACE_DIR"',
  ];
  for (const [key, value] of Object.entries(env)) {
    lines.push(`export ${key}=${shellQuote(value)}`);
  }
  const configWrite = renderConfigWrite('"$BRIKKO_STUDIO_CONFIG_PATH"', config);
  if (configWrite) {
...
export function renderShellFunction() {
  return `brikko-studio_test_state_create() {
  ...
  case "$raw_label" in
    /*)
      BRIKKO_STUDIO_TEST_STATE_HOME="$raw_label"
      mkdir -p "$BRIKKO_STUDIO_TEST_STATE_HOME"
      ;;
    *)
      label="$(printf "%s" "$label" | tr -cs "A-Za-z0-9_.-" "-" | sed -e "s/^-*//" -e "s/-*$//")"
      [ -n "$label" ] || label="state"
      local tmp_root="\${BRIKKO_STUDIO_TEST_STATE_TMPDIR:-\${TMPDIR:-/tmp}}"
      mkdir -p "$tmp_root"
      BRIKKO_STUDIO_TEST_STATE_HOME="$(mktemp -d "$tmp_root/brikko-studio-$label-$scenario-home.XXXXXX")"
      ;;
  esac
  export HOME="$BRIKKO_STUDIO_TEST_STATE_HOME"
  export USERPROFILE="$BRIKKO_STUDIO_TEST_STATE_HOME"
  ...
  export BRIKKO_STUDIO_CONFIG_PATH="$BRIKKO_STUDIO_STATE_DIR/brikko-studio.json"
  export BRIKKO_STUDIO_TEST_WORKSPACE_DIR="$BRIKKO_STUDIO_TEST_STATE_HOME/workspace"
  unset BRIKKO_STUDIO_AGENT_DIR
  unset PI_CODING_AGENT_DIR
  unset BRIKKO_STUDIO_SERVICE_REPAIR_POLICY
  mkdir -p "$BRIKKO_STUDIO_STATE_DIR" "$BRIKKO_STUDIO_TEST_WORKSPACE_DIR"
  case "$scenario" in
    minimal)
      cat > "$BRIKKO_STUDIO_CONFIG_PATH" <<'BRIKKO_STUDIO_TEST_STATE_JSON'
{}
BRIKKO_STUDIO_TEST_STATE_JSON
      ;;
    update-stable)
      ...
      cat > "$BRIKKO_STUDIO_CONFIG_PATH" <<'BRIKKO_STUDIO_TEST_STATE_JSON'
{
  "gateway": {
    ...
    "auth": {
      "mode": "token",
      "token": "brikko-studio-test-token"
    },
    "controlUi": {
      "enabled": false
    }
...
BRIKKO_STUDIO_TEST_STATE_JSON
      ;;
    external-service)
      export BRIKKO_STUDIO_SERVICE_REPAIR_POLICY="external"
      cat > "$BRIKKO_STUDIO_CONFIG_PATH" <<'BRIKKO_STUDIO_TEST_STATE_JSON'
{}
BRIKKO_STUDIO_TEST_STATE_JSON
      ;;
