...
function isSystemThrottleDisabled(env) {
  const normalized = env.BRIKKO_STUDIO_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestEnv(env = process.env) {
  const normalizedLocalCheck = env.BRIKKO_STUDIO_LOCAL_CHECK?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.BRIKKO_STUDIO_VITEST_MAX_WORKERS ?? env.BRIKKO_STUDIO_TEST_WORKERS);
  if (override !== null) {
