...
const BRIKKO_STUDIO_PACKAGE_SPEC_RE =
  /^brikko-studio@(main|alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;
...
export function createHarnessEnv(params: {
  ...
  return {
    ...params.baseEnv,
    BRIKKO_STUDIO_NPM_TELEGRAM_PACKAGE_SPEC: params.spec,
    ...(params.packageTgz ? { BRIKKO_STUDIO_NPM_TELEGRAM_PACKAGE_TGZ: params.packageTgz } : {}),
    BRIKKO_STUDIO_NPM_TELEGRAM_PACKAGE_LABEL: `${params.spec} (${params.version})`,
    BRIKKO_STUDIO_NPM_TELEGRAM_PROVIDER_MODE: params.providerMode,
    BRIKKO_STUDIO_NPM_TELEGRAM_SCENARIOS: params.scenarios.join(","),
    BRIKKO_STUDIO_NPM_TELEGRAM_OUTPUT_DIR: params.rawOutputDir,
    BRIKKO_STUDIO_NPM_TELEGRAM_FAST: params.baseEnv.BRIKKO_STUDIO_NPM_TELEGRAM_FAST ?? "1",
    BRIKKO_STUDIO_NPM_TELEGRAM_WARM_SAMPLES: String(params.samples),
    BRIKKO_STUDIO_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS: String(params.sampleTimeoutMs),
    BRIKKO_STUDIO_QA_TELEGRAM_CANARY_TIMEOUT_MS: String(params.timeoutMs),
    BRIKKO_STUDIO_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: String(params.timeoutMs),
  };
}
