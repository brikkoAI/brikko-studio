...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "brikko-studio-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "brikko-studio.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".brikko-studio", "workspace");
