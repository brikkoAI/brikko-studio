...
export const CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS = [
  "brikko-studio/plugin-sdk/agent-runtime-test-contracts",
  "brikko-studio/plugin-sdk/plugin-test-contracts",
  "brikko-studio/plugin-sdk/provider-test-contracts",
] as const;
...
export function writePackedBundledPluginActivationConfig(homeDir: string): void {
  const configPath = join(homeDir, ".brikko-studio", "brikko-studio.json");
  mkdirSync(join(homeDir, ".brikko-studio"), { recursive: true });
  writeFileSync(
    configPath,
    `${JSON.stringify(
      {
        ...
        models: {
          providers: {
            openai: {
              apiKey: "sk-brikko-studio-release-check",
...
function runPackedBundledPluginActivationSmoke(packageRoot: string, tmpRoot: string): void {
  ...
  const env = createPackedCliSmokeEnv(process.env, {
    HOME: homeDir,
    OPENAI_API_KEY: "sk-brikko-studio-release-check",
  });

  writePackedBundledPluginActivationConfig(homeDir);
  execFileSync(
    process.execPath,
    [join(packageRoot, "brikko-studio.mjs"), ...PACKED_BUNDLED_RUNTIME_DEPS_REPAIR_ARGS],
    {
      cwd: packageRoot,
      stdio: "inherit",
      env,
  ...
  execFileSync(process.execPath, [join(packageRoot, "brikko-studio.mjs"), "plugins", "doctor"], {
    cwd: packageRoot,
    stdio: "inherit",
    env,
  });
...
function runPackedBundledChannelEntrySmoke(): void {
  const tmpRoot = mkdtempSync(join(tmpdir(), "brikko-studio-release-pack-smoke-"));
  try {
    ...
    const packageRoot = join(resolveGlobalRoot(prefixDir, tmpRoot), "brikko-studio");
    ...
    execFileSync(
      process.execPath,
      [join(packageRoot, "brikko-studio.mjs"), ...PACKED_COMPLETION_SMOKE_ARGS],
      {
        cwd: packageRoot,
        stdio: "inherit",
  ...
  return [...paths]
    .filter(
      (path) =>
        ...
        /(^|\/)\.brikko-studio-runtime-deps-[^/]+(\/|$)/u.test(path) ||
        path.endsWith("/.brikko-studio-runtime-deps-stamp.json") ||
...
// Critical functions that channel extension plugins import from brikko-studio/plugin-sdk.
...
export function collectCriticalPluginSdkEntrypointSizeErrors(rootDir = process.cwd()): string[] {
  ...
  for (const specifier of CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS) {
    const subpath = specifier.slice("brikko-studio/plugin-sdk/".length);
