...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const brikko-studioMeta =
    source.packageJson &&
    typeof source.packageJson === "object" &&
    !Array.isArray(source.packageJson) &&
    "brikko-studio" in source.packageJson
      ? (source.packageJson.brikko-studio as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    brikko-studioMeta &&
    typeof brikko-studioMeta.channel === "object" &&
    brikko-studioMeta.channel &&
    !Array.isArray(brikko-studioMeta.channel)
      ? (brikko-studioMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
