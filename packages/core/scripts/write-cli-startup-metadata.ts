...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.brikko-studio?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.brikko-studio?.channel?.order;
      const labelRaw = parsed.brikko-studio?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".brikko-studio-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "brikko-studio-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "brikko-studio-build",
...
function renderSourceBrowserHelpText(
  ...
  const inlineModule = [
    ...
    `registerBrowserCli(program, ["node", "brikko-studio", "browser", "--help"]);`,
  ...
  const result = spawnSync(
    ...
    ["--import", "tsx", "--input-type=module", "--eval", inlineModule],
    {
      ...
      env: {
        ...renderContext.env,
        BRIKKO_STUDIO_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",
      },
