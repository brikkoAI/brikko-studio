...
if (options.format === "shell") {
  process.stdout.write(
    [
      `BRIKKO_STUDIO_IOS_VERSION=${version.canonicalVersion}`,
      `BRIKKO_STUDIO_MARKETING_VERSION=${version.marketingVersion}`,
      `BRIKKO_STUDIO_BUILD_VERSION=${version.buildVersion}`,
    ].join("\n") + "\n",
  );
} else {
