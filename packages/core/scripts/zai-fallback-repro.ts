...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "brikko-studio.json");
  ...
  const sessionId = process.env.BRIKKO_STUDIO_ZAI_FALLBACK_SESSION_ID ?? randomUUID();
  ...
  const run1 = await runCommand(
    "run1",
    ["brikko-studio", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["brikko-studio", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
