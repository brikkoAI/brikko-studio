...
type DriverMode = "token" | "webhook" | "brikko-studio";
...
function resolveStateDir(): string {
  const override = process.env.BRIKKO_STUDIO_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.BRIKKO_STUDIO_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".brikko-studio");
}
...
function usage(): string {
  return (
    ...
    "  --driver <token|webhook|brikko-studio> Driver transport mode (default: token)\n" +
    ...
    "  BRIKKO_STUDIO_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  BRIKKO_STUDIO_DISCORD_SMOKE_DRIVER\n" +
    "  BRIKKO_STUDIO_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  BRIKKO_STUDIO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  BRIKKO_STUDIO_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  BRIKKO_STUDIO_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  BRIKKO_STUDIO_DISCORD_SMOKE_AGENT\n" +
    "  BRIKKO_STUDIO_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  BRIKKO_STUDIO_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  BRIKKO_STUDIO_DISCORD_SMOKE_POLL_MS\n" +
    "  BRIKKO_STUDIO_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  BRIKKO_STUDIO_DISCORD_SMOKE_BRIKKO_STUDIO_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.BRIKKO_STUDIO_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.BRIKKO_STUDIO_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "brikko-studio"
        ? "brikko-studio"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.BRIKKO_STUDIO_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.BRIKKO_STUDIO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.BRIKKO_STUDIO_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.BRIKKO_STUDIO_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.BRIKKO_STUDIO_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.BRIKKO_STUDIO_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.BRIKKO_STUDIO_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.BRIKKO_STUDIO_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.BRIKKO_STUDIO_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.BRIKKO_STUDIO_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const brikko-studioBin =
    resolveArg("--brikko-studio-bin") || process.env.BRIKKO_STUDIO_DISCORD_SMOKE_BRIKKO_STUDIO_BIN || "brikko-studio";
...
async function brikko-studioCliJson<T>(params: { brikko-studioBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.brikko-studioBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`brikko-studio ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await brikko-studioCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    brikko-studioBin: params.brikko-studioBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "brikko-studio") {
    return await readMessagesWithOpenclaw({
      brikko-studioBin: params.args.brikko-studioBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `brikko-studio-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await brikko-studioCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        brikko-studioBin: args.brikko-studioBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("brikko-studio message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
