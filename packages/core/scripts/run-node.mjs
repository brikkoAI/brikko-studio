...
const extensionRestartMetadataFiles = new Set(["brikko-studio.plugin.json", "package.json"]);
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.BRIKKO_STUDIO_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.BRIKKO_STUDIO_BUILD_PRIVATE_QA === "1" &&
...
export const resolveRuntimePostBuildRequirement = (deps) => {
  if (deps.env.BRIKKO_STUDIO_FORCE_RUNTIME_POSTBUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by BRIKKO_STUDIO_FORCE_BUILD",
...
const RUNTIME_POSTBUILD_REASON_LABELS = {
  force_runtime_postbuild: "forced by BRIKKO_STUDIO_FORCE_RUNTIME_POSTBUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "BRIKKO_STUDIO_RUN_NODE_OUTPUT_LOG";
const RUN_NODE_CPU_PROF_DIR_ENV = "BRIKKO_STUDIO_RUN_NODE_CPU_PROF_DIR";
const RUN_NODE_BUILD_LOCK_TIMEOUT_ENV = "BRIKKO_STUDIO_RUN_NODE_BUILD_LOCK_TIMEOUT_MS";
const RUN_NODE_BUILD_LOCK_POLL_ENV = "BRIKKO_STUDIO_RUN_NODE_BUILD_LOCK_POLL_MS";
const RUN_NODE_BUILD_LOCK_STALE_ENV = "BRIKKO_STUDIO_RUN_NODE_BUILD_LOCK_STALE_MS";
...
const logRunner = (message, deps) => {
  if (deps.env.BRIKKO_STUDIO_RUNNER_LOG === "0") {
    return;
  }
  const line = `[brikko-studio] ${message}\n`;
...
const resolveRunNodeCpuProfileArgs = (deps) => {
  ...
  const profileName = `brikko-studio-${commandName}-${pid}-${timestamp}.cpuprofile`;
...
const closeRunNodeOutputTee = async (deps, exitCode) => {
  ...
  try {
    ...
    deps.stderr.write(
      `[brikko-studio] Failed to write output log: ${error?.message ?? "unknown error"}\n`,
    );
...
const shouldSkipWatchRuntimeSync = (deps, requirement) =>
  deps.env.BRIKKO_STUDIO_WATCH_MODE === "1" &&
...
const shouldUseExistingDistForGatewayClient = (deps, buildRequirement) =>
  ...
  deps.env.BRIKKO_STUDIO_FORCE_BUILD !== "1" &&
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.BRIKKO_STUDIO_BUILD_PRIVATE_QA = "1";
    deps.env.BRIKKO_STUDIO_ENABLE_PRIVATE_QA_CLI = "1";
  }
