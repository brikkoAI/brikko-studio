...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.brikko-studio) ? packageJson.brikko-studio : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.brikko-studio?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.brikko-studio?.channel?.id) || left.name;
    const rightId = trimString(right.brikko-studio?.channel?.id) || right.name;
