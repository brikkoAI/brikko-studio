...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@brikko-studio/build-smoke-plugin",
      type: "module",
      brikko-studio: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  path.join(distPluginDir, "brikko-studio.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'brikko-studio/plugin-sdk';",
...
const registry = loadBrikko StudioPlugins({
  ...
  env: {
    ...process.env,
    BRIKKO_STUDIO_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "dist-runtime", "extensions"),
  },
  config: {
    plugins: {
      enabled: true,
