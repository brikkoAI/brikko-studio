...
const APP_ID_ENV = "BRIKKO_STUDIO_GH_READ_APP_ID";
const KEY_FILE_ENV = "BRIKKO_STUDIO_GH_READ_PRIVATE_KEY_FILE";
const INSTALLATION_ID_ENV = "BRIKKO_STUDIO_GH_READ_INSTALLATION_ID";
const PERMISSIONS_ENV = "BRIKKO_STUDIO_GH_READ_PERMISSIONS";
...
async function githubJson<T>(
  ...
  const response = await fetch(`https://api.github.com${path}`, {
    ...
    headers: {
      ...
      "User-Agent": "brikko-studio-gh-read",
...
async function main() {
  if (process.argv.length <= 2) {
    fail(
      "usage: scripts/gh-read <gh args...>\nset BRIKKO_STUDIO_GH_READ_APP_ID and BRIKKO_STUDIO_GH_READ_PRIVATE_KEY_FILE first",
    );
  }
