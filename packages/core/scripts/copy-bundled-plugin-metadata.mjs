...
function shouldCopyBundledPluginMetadata(id, env) {
  ...
  return env.BRIKKO_STUDIO_BUILD_PRIVATE_QA === "1";
}
...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@brikko-studio/${params.dirName}`) {
    return false;
  }
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "brikko-studio.plugin.json");
    ...
    if (packageJson.brikko-studio && "extensions" in packageJson.brikko-studio) {
      packageJson.brikko-studio = {
        ...packageJson.brikko-studio,
        extensions: rewritePackageExtensions(packageJson.brikko-studio.extensions),
        ...(typeof packageJson.brikko-studio.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.brikko-studio.setupEntry) }
          : {}),
      };
    }
