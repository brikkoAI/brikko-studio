...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.BRIKKO_STUDIO_VITEST_MAX_WORKERS || env.BRIKKO_STUDIO_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      BRIKKO_STUDIO_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
...
async function main() {
  ...
  const isExplicitParallelMultiConfigRun =
    Boolean(baseEnv.BRIKKO_STUDIO_TEST_PROJECTS_PARALLEL) &&
    runSpecs.length > 1 &&
    !runSpecs.some((spec) => spec.watchMode);
  ...
  if (isParallelShardRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        !isCiLikeEnv(baseEnv) &&
        !baseEnv.BRIKKO_STUDIO_TEST_PROJECTS_PARALLEL &&
        !baseEnv.BRIKKO_STUDIO_VITEST_MAX_WORKERS &&
        !baseEnv.BRIKKO_STUDIO_TEST_WORKERS &&
