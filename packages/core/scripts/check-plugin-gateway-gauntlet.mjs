...
function parseArgs(argv) {
  const options = {
    ...
    pluginIds: [],
    shardTotal: readOptionalPositiveIntEnv("BRIKKO_STUDIO_PLUGIN_GATEWAY_GAUNTLET_TOTAL") ?? 1,
    shardIndex: readOptionalNonNegativeIntEnv("BRIKKO_STUDIO_PLUGIN_GATEWAY_GAUNTLET_INDEX") ?? 0,
    limit: undefined,
    skipPrebuild: false,
    skipLifecycle: false,
    skipQa: false,
  ...
  const envIds = normalizeCsv(process.env.BRIKKO_STUDIO_PLUGIN_GATEWAY_GAUNTLET_IDS);
...
function brikko-studioCommand(repoRoot, args) {
  return {
...
function createIsolatedEnv(repoRoot, runRoot) {
  ...
  return {
    ...
    BRIKKO_STUDIO_LOG_DIR: path.join(runRoot, "logs"),
    BRIKKO_STUDIO_QA_SUITE_PROGRESS: process.env.BRIKKO_STUDIO_QA_SUITE_PROGRESS ?? "1",
    ...
    for (const { phase, args } of commands) {
      ...
      params.rows.push(
        runMeasuredCommand({
          ...
          ...brikko-studioCommand(params.repoRoot, ["plugins", ...args]),
    ...
    for (const alias of plugin.cliCommandAliases) {
      ...
      params.rows.push(
        runMeasuredCommand({
          ...
          ...brikko-studioCommand(params.repoRoot, [command, "--help"]),
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "brikko-studio-plugin-gauntlet-"));
