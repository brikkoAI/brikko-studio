...
const a2uiAppDir = path.join(rootDir, "apps", "shared", "Brikko StudioKit", "Tools", "CanvasA2UI");
...
export function getBundleHashRepoInputPaths(repoRoot = rootDir) {
  return [
    ...
    path.join(repoRoot, "apps", "shared", "Brikko StudioKit", "Tools", "CanvasA2UI"),
  ];
}
...
async function main() {
  ...
  if (!hasRendererDir || !hasAppDir) {
    ...
    if (process.env.BRIKKO_STUDIO_SPARSE_PROFILE || process.env.BRIKKO_STUDIO_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI sources missing; skipping bundle because BRIKKO_STUDIO_A2UI_SKIP_MISSING=1 or BRIKKO_STUDIO_SPARSE_PROFILE is set.",
      );
      return;
    }
