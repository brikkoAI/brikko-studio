...
const TMUX_CWD_ENV_KEY = "BRIKKO_STUDIO_GATEWAY_WATCH_CWD";
const TMUX_CWD_OPTION_KEY = "@brikko-studio.gateway_watch.cwd";
...
export const resolveGatewayWatchTmuxSessionName = ({ args = [], env = process.env } = {}) => {
  const profile =
    env.BRIKKO_STUDIO_PROFILE ||
    readArgValue(args, "--profile") ||
    (args.includes("--dev") ? "dev" : null);
  const port = env.BRIKKO_STUDIO_GATEWAY_PORT || readArgValue(args, "--port");
  const parts = [
    "brikko-studio",
    "gateway",
    "watch",
...
export const buildGatewayWatchTmuxCommand = ({
  ...
  const childEnv = [
    "env",
    ...colorEnv.options,
    `BRIKKO_STUDIO_GATEWAY_WATCH_TMUX_CHILD=1`,
    `BRIKKO_STUDIO_GATEWAY_WATCH_SESSION=${sessionName}`,
...
const log = (stderr, message) => {
  stderr.write(`[brikko-studio] ${message}\n`);
};
...
const shouldAttachTmux = ({ env, stdinIsTTY, stdoutIsTTY }) => {
  const raw = String(env.BRIKKO_STUDIO_GATEWAY_WATCH_ATTACH ?? "").toLowerCase();
...
export const runGatewayWatchTmuxMain = (params = {}) => {
  ...
  if (TMUX_DISABLE_VALUES.has((deps.env.BRIKKO_STUDIO_GATEWAY_WATCH_TMUX ?? "").toLowerCase())) {
  ...
  if (deps.env.BRIKKO_STUDIO_GATEWAY_WATCH_TMUX_CHILD === "1") {
    return runForegroundWatcher({
      args: deps.args,
      cwd: deps.cwd,
      env: deps.env,
