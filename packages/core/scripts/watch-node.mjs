...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.BRIKKO_STUDIO_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[brikko-studio] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[brikko-studio] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[brikko-studio] This usually means a file in node_modules is empty or truncated.");
  console.error("[brikko-studio] Recommended recovery:");
  console.error("[brikko-studio]   rm -rf node_modules");
  console.error("[brikko-studio]   pnpm store prune");
  console.error("[brikko-studio]   pnpm install");
  console.error("");
  console.error("[brikko-studio] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.BRIKKO_STUDIO_WATCH_MODE = "1";
  childEnv.BRIKKO_STUDIO_WATCH_SESSION = watchSession;
  ...
  childEnv.BRIKKO_STUDIO_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.BRIKKO_STUDIO_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `brikko-studio doctor --fix --non-interactive` once.",
        deps,
      );
