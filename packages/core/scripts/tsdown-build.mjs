...
const logLevel = process.env.BRIKKO_STUDIO_BUILD_VERBOSE ? "info" : "warn";
...
export async function runTsdownBuildInvocation(invocation, params = {}) {
  ...
  const timeoutMs = parsePositiveInteger(env.BRIKKO_STUDIO_TSDOWN_TIMEOUT_MS);
  const heartbeatMs =
    parseNonNegativeInteger(env.BRIKKO_STUDIO_TSDOWN_HEARTBEAT_MS) ?? DEFAULT_HEARTBEAT_MS;
  let timedOut = false;
  let settled = false;
