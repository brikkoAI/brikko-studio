 ...
 * `brikko-studio update`. Use it to verify the completion installation flow
...
\`brikko-studio update\`, \`brikko-studio doctor\`, and \`brikko-studio onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Brikko Studio Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`brikko-studio completion --install\` later to enable.`));
    return;
  }
