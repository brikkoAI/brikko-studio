...
function scanLogs() {
  const roots = ["/tmp", path.join(process.env.HOME, ".brikko-studio")];
  const files = [];
  const visit = (entry) => {
    ...
    if (/\.(?:log|jsonl)$/u.test(entry) || /brikko-studio-kitchen-sink-/u.test(path.basename(entry))) {
...
function readConfig() {
  const configPath = path.join(process.env.HOME, ".brikko-studio", "brikko-studio.json");
  return {
    configPath,
...
function assertClawHubExternalInstallContract(installPath) {
  const brikko-studioPeerPath = path.join(installPath, "node_modules", "brikko-studio");
  if (!fs.existsSync(brikko-studioPeerPath)) {
    throw new Error(`missing kitchen-sink brikko-studio peer symlink: ${brikko-studioPeerPath}`);
  }
  if (!fs.lstatSync(brikko-studioPeerPath).isSymbolicLink()) {
    throw new Error(`kitchen-sink brikko-studio peer is not a symlink: ${brikko-studioPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(brikko-studioPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected kitchen-sink brikko-studio peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertCutoverPreinstalled() {
  ...
  const indexPath = path.join(process.env.HOME, ".brikko-studio", "plugins", "installs.json");
