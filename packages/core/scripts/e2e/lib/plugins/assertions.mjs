...
function getInstallRecords() {
  ...
  const allowLegacyCompat = process.env.BRIKKO_STUDIO_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function readBrikko StudioConfig() {
  const configPath = path.join(process.env.HOME, ".brikko-studio", "brikko-studio.json");
...
function assertPluginRemoved(params) {
  ...
  const config = readBrikko StudioConfig();
...
function recordFixturePluginTrust() {
  ...
  const ledgerPath = path.join(process.env.HOME, ".brikko-studio", "plugins", "installs.json");
  const ledger = fs.existsSync(ledgerPath)
    ? readJson(ledgerPath)
    : {
        version: 1,
        warning:
          "DO NOT EDIT. This file is generated by Brikko Studio plugin install/update/uninstall commands. Use `brikko-studio plugins install/update/uninstall` instead.",
        records: {},
      };
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".brikko-studio", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(brikko-studioPeerPath)) {
    throw new Error(`missing ClawHub brikko-studio peer symlink: ${brikko-studioPeerPath}`);
  }
  if (!fs.lstatSync(brikko-studioPeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub brikko-studio peer is not a symlink: ${brikko-studioPeerPath}`);
  }
  ...
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub brikko-studio peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@brikko-studio/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@brikko-studio/demo-plugin-npm") {
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.BRIKKO_STUDIO_CLAWHUB_URL ||
    process.env.CLAWHUB_URL ||
    "https://clawhub.ai"
  ).replace(/\/+$/, "");
  const token =
    process.env.BRIKKO_STUDIO_CLAWHUB_TOKEN ||
...
function assertClawHubInstalled() {
  ...
  const installPath = record.installPath.replace(/^~(?=$|\/)/u, process.env.HOME);
  const extensionsRoot = path.join(process.env.HOME, ".brikko-studio", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".brikko-studio", "brikko-studio.json");
