...
const defaultOptions = (): WindowsOptions => ({
  ...
  installUrl: "https://brikko-studio.ai/install.ps1",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
  ...
  snapshotHint: "pre-brikko-studio-native-e2e-2026-03-12",
  targetPackageSpec: "",
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-brikko-studio-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --provider <openai|anthropic|minimax>
  --model <provider/model>    Override the model used for the agent-turn smoke.
  --api-key-env <var>        Host env var name for provider API key.
  --openai-api-key-env <var> Alias for --api-key-env (backward compatible)
  --install-url <url>        Installer URL for latest release. Default: https://brikko-studio.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run brikko-studio update --channel dev.
...
class WindowsSmoke {
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("brikko-studio-parallels-windows.");
    this.phases = new PhaseRunner(this.runDir);
    this.guest = new WindowsGuest(this.options.vmName, this.phases);
    this.tgzDir = await makeTempDir("brikko-studio-parallels-windows-tgz.");
    try {
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () => this.installMain("brikko-studio-main-fresh.tgz"));
    ...
    await this.phase(
      ...
      Number(process.env.BRIKKO_STUDIO_PARALLELS_WINDOWS_AGENT_TIMEOUT_S || 2700),
      () => this.verifyTurn(),
    );
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    await this.phase(
      "upgrade.update-dev",
      Number(process.env.BRIKKO_STUDIO_PARALLELS_WINDOWS_UPDATE_TIMEOUT_S || 1200),
...
if ($LASTEXITCODE -ne 0) { throw "brikko-studio onboard failed with exit code $LASTEXITCODE" }`,
  ...
  private async guestPowerShellBackground(
    ...
    const fileBase = `brikko-studio-parallels-${nonce}`;
...
if (Test-Path $donePath) {
  ...
  exit 0
      ...
      const offsetMatch = result.stdout.match(/__BRIKKO_STUDIO_LOG_OFFSET__:(\d+)/);
      if (offsetMatch) {
        lastLogOffset = Number(offsetMatch[1]);
      }
      if (result.stdout.includes("__BRIKKO_STUDIO_BACKGROUND_DONE__")) {
        const exitMatch = result.stdout.match(/__BRIKKO_STUDIO_BACKGROUND_EXIT__:(\S+)/);
...
if ($LASTEXITCODE -ne 0) { throw "gateway ${action} failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyGatewayReachable(): void {
    ...
    const recoveryAfter =
      Number(process.env.BRIKKO_STUDIO_PARALLELS_WINDOWS_GATEWAY_RECOVERY_AFTER_S || 180) * 1000;
...
if (-not $agentOk) { throw 'brikko-studio agent finished without OK response' }`,
      Number(process.env.BRIKKO_STUDIO_PARALLELS_WINDOWS_AGENT_TIMEOUT_S || 2700) * 1000,
    );
  }
