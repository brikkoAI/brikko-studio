...
class LinuxSmoke {
  ...
  private disableBonjour = parseBoolEnv(process.env.BRIKKO_STUDIO_PARALLELS_LINUX_DISABLE_BONJOUR);
  private hostIp = "";
  private hostPort = 0;
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("brikko-studio-parallels-linux.");
    this.phases = new PhaseRunner(this.runDir);
    this.tgzDir = await makeTempDir("brikko-studio-parallels-linux-tgz.");
    try {
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () =>
      this.installMainTgz("brikko-studio-main-fresh.tgz"),
    );
    ...
    await this.phase(
      ...
      Number(process.env.BRIKKO_STUDIO_PARALLELS_LINUX_AGENT_TIMEOUT_S || 1500),
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    await this.phase("upgrade.install-main", 420, () =>
      this.installMainTgz("brikko-studio-main-upgrade.tgz"),
    );
  ...
  private installLatestRelease(): void {
    this.guestExec(["curl", "-fsSL", this.options.installUrl, "-o", "/tmp/brikko-studio-install.sh"]);
    if (this.options.installVersion) {
      this.guestExec([
        "/usr/bin/env",
        "BRIKKO_STUDIO_NO_ONBOARD=1",
        "bash",
        "/tmp/brikko-studio-install.sh",
        "--version",
    ...
    this.guestExec(["brikko-studio", "--version"]);
  }
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec(["brikko-studio", "--version"]);
...
plugin_dir=/root/.brikko-studio/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{"name":"@brikko-studio/test-bad-plugin","version":"1.0.0","brikko-studio":{"extensions":["./index.cjs"],"setupEntry":"./setup-entry.cjs"}}
JSON
cat >"$plugin_dir/brikko-studio.plugin.json" <<'JSON'
...
config_path = Path("/root/.brikko-studio/brikko-studio.json")
...
PY`);
  ...
  private startGatewayBackground(): void {
    const bonjourEnv = this.disableBonjour ? " BRIKKO_STUDIO_DISABLE_BONJOUR=1" : "";
    this.guestBash(
      String.raw`pkill -f "brikko-studio gateway run" >/dev/null 2>&1 || true
rm -f /tmp/brikko-studio-parallels-linux-gateway.log
setsid sh -lc ` +
        shellQuote(
          `exec env BRIKKO_STUDIO_HOME=/root BRIKKO_STUDIO_STATE_DIR=/root/.brikko-studio BRIKKO_STUDIO_CONFIG_PATH=/root/.brikko-studio/brikko-studio.json${bonjourEnv} ${this.auth.apiKeyEnv}=${shellQuote(
  ...
  private showGatewayStatusCompat(check = true): boolean {
    const help = this.guestExec(["brikko-studio", "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["brikko-studio", "gateway", "status", "--deep", "--require-rpc"]
...
if isinstance(paths, list):
    load["paths"] = [path for path in paths if path != "/root/.brikko-studio/test-bad-plugin"]
allow = plugins.get("allow")
if isinstance(allow, list):
    plugins["allow"] = [plugin_id for plugin_id in allow if plugin_id != "test-bad-plugin"]
config_path.write_text(json.dumps(config, indent=2) + "\n")
PY
rm -rf /root/.brikko-studio/test-bad-plugin`);
  }

  private verifyLocalTurn(): void {
    this.guestExec(["brikko-studio", "models", "set", this.auth.modelId]);
...
JSON
brikko-studio config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    ...
    this.guestExec(["brikko-studio", "config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} brikko-studio agent --local --agent main --session-id "$session_id" --message ${shellQuote(
