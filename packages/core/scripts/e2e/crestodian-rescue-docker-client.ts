...
async function main() {
  const stateDir =
    ...
    (await fs.mkdtemp(path.join(os.tmpdir(), "brikko-studio-crestodian-")));
  const configPath = process.env.BRIKKO_STUDIO_CONFIG_PATH ?? path.join(stateDir, "brikko-studio.json");
  ...
  const refPlan = await invoke(
    "/crestodian config set-ref gateway.auth.token env BRIKKO_STUDIO_GATEWAY_TOKEN",
    cfg,
  );
  assert(
  ...
  const agentPlan = await invoke("/crestodian create agent work workspace /tmp/brikko-studio-work", cfg);
  assert(
  ...
  const setupPlan = await invoke(
    "/crestodian setup workspace /tmp/brikko-studio-setup model openai/gpt-5.2",
    cfg,
  );
  ...
  assert(
    updatedConfig.gateway?.auth?.token &&
      ...
      updatedConfig.gateway.auth.token.id === "BRIKKO_STUDIO_GATEWAY_TOKEN",
    "SecretRef set did not update gateway.auth.token",
  );
  assert(
    updatedConfig.agents?.defaults?.workspace === "/tmp/brikko-studio-setup",
    "setup did not update default workspace",
  );
  assert(
    updatedConfig.agents?.list?.some(
      (agent) => agent.id === "work" && agent.workspace === "/tmp/brikko-studio-work",
    ),
